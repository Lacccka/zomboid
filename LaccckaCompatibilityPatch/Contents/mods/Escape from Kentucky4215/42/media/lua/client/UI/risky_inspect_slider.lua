...
function riskyUI_slider:callback(value, slider)
    ...
    local list = {formatValue(self.slider1.currentValue), formatValue(self.slider2.currentValue),
                  formatValue(self.slider3.currentValue)}

    self.itempartoffsetment:getOffset():set(list[1], list[2], list[3])
    ...
    if not string.find(self.worldmodel, "nil") then
        local vector = self.parenta.scene.javaObject:fromLua4("setObjectPosition", self.worldmodel, list[1], list[2],
            list[3])
    end
end
