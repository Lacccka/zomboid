...
function loadPersistedWindowState(playerNum)
    ...
    return { open = data.open == true, collapsed = data.collapsed == true, expandedY = tonumber(data.expandedY), itemId = tostring(data.itemId or ""), uuid = tostring(data.uuid or "") }
end
...
function NMBoomboxWindow.queuePersistedRestore(playerNum)
...
function NMBoomboxWindow.restorePersistedStateForPlayer(playerNum)
    if not isFancyUIEnabled() then
    ...
    local win = NMBoomboxWindow.openForItem(playerNum, item)
...
function NMBoomboxWindow.tickPersistedRestore()
    if not isFancyUIEnabled() then
        return
    end
    ...
    for key, entry in pairs(pendingRestoreByPlayer) do
        ...
        if NMBoomboxWindow.restorePersistedStateForPlayer(playerNum) == true then
