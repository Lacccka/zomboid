...
-- Note to self -- Modify?? If waterStation not full AND Water tank not empty ....., litres (without barrelStart) -> add = litres - self.PreviousLitres (0 at start) -> setWaterAmount = waterStation:getFluidAmount + add
function ISPumpWaterToBarrel:update() 
	...
	if self.waterStation:getFluidAmount() ~= self.barrelTarget and self.part:getContainerContentAmount() > 0 then
...
function ISPumpWaterToBarrel:start()
	...
	self.barrelStart = self.waterStation:getFluidAmount();
	local barrelLitresFree = self.waterStation:getFluidCapacity() - self.barrelStart
	...
	--[[
	local barrelCap = self.waterStation:getFluidCapacity()
	local barrelAmount = self.waterStation:getFluidAmount()

	-----------taint water if source is tainted -------------
	if barrelAmount < barrelCap and self.partData.tainted == 1 and self.waterStation:getProperties():Val("CustomName") ~= "Sink" and self.filter == nil then
		--self.waterStation:setTaintedWater(true)
		self.waterStation:getFluidContainer():addFluid("WaterTainted", 1)
		self.waterStation:transmitModData()
	end
	
	if not barrelFull and self.filter ~= nil and self.waterStation:getProperties():Val("CustomName") ~= "Sink" then	-- tainting functions if Filter installed - Tread
...
function ISPumpWaterToBarrel:stop()
	...
	if barrelAmount < barrelCap and self.partData.tainted == 1 and self.waterStation:getProperties():Val("CustomName") ~= "Sink" and self.filter == nil then
		--self.waterStation:setTaintedWater(true)
		--self.waterStation:getFluidContainer():addFluid("WaterTainted", 1)
		self.waterStation:getFluidContainer():Empty()
		self.waterStation:getFluidContainer():addFluid("TaintedWater", barrelAmount)
		self.waterStation:transmitModData()
	end
