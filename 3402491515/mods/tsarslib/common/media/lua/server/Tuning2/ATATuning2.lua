    ...
    if part:getInventoryItem() then
        if wPart1:getInventoryItem() then
            if wPart1:getContainerContentAmount() ~= part:getContainerContentAmount() then
                ...
                -- TODO: sync inflation
                if isBaseVehicle(vehicle) then vehicle:setTireInflation(wheelIndex, wPart1:getContainerContentAmount() / wPart1:getContainerCapacity()) end
        ...
        if wPart2:getInventoryItem() then
            if wPart2:getContainerContentAmount() ~= part:getContainerContentAmount() then
                ...
                -- TODO: sync inflation
                if isBaseVehicle(vehicle) then vehicle:setTireInflation(wheelIndex, wPart2:getContainerContentAmount() / wPart2:getContainerCapacity()) end
                vehicle:transmitPartModData(part)
            end
...
function ATATuning2.ContainerAccess.MotoBags(vehicle, part, chr)
    ...
    return (isBaseVehicle(vehicle) and vehicle:isInArea(part:getArea(), chr))
end
...
function ATATuning2Utils.ModelByModData(vehicle, part, item)
    ...
    if isBaseVehicle(vehicle) then vehicle:doDamageOverlay() end
end
...
function ATATuning2.Update.Protection(vehicle, part, elapsedMinutes)
    ...
    local item = part:getInventoryItem();
    if not (item and isBaseVehicle(vehicle)) then return; end
...
function ATATuning2.Init.TireNotAllModelsVisible(vehicle, part)
    if not isBaseVehicle(vehicle) then return end--TODO recheck
