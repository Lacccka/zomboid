...
local function isBaseVehicle(vehicle)
    return vehicle.isGoodCar--VirtualVehicle does not implement that, see ;)
end

local function updateVehicleSound(vehicle, soundStop, soundPlay)
    if isBaseVehicle(vehicle) then
        if soundStop then
            vehicle:getEmitter():stopSoundByName(soundStop)
        end
        if soundPlay then
            vehicle:getEmitter():playSound(soundPlay)
        end
    end
end
...
function CommonTemplates.Update.Freezer(vehicle, part, elapsedMinutes)--VirtualVehicle Compatible
...
function CommonTemplates.Update.Oven(vehicle, part, elapsedMinutes)
    local md = part:getModData().tsarslib
    -- print("CommonTemplates.Update.Oven")
    local currentTemp = part:getItemContainer():getTemprature()
    -- print(currentTemp)
    local minTemp = 1.0
    local maxTemp = (md.maxTemperature + 100) / 100
    ...
    if part:getInventoryItem() and part:getItemContainer() then
        if part:getItemContainer():isActive() and vehicle:getBatteryCharge() > 0.00200 then
            ...
            if md.timer > 0 then
                if md.timePassed < md.timer then
                    md.timePassed = md.timePassed + 1
                else 
                    updateVehicleSound(vehicle, nil, "StoveTimerExpired")
                    md.timer = 0
                    md.timePassed = 0
                end
            end
...
function CommonTemplates.Update.Microwave(vehicle, part, elapsedMinutes)--VirtualVehicle compatible, except the sound
    ...
    if part:getInventoryItem() and part:getItemContainer() then
        if part:getItemContainer():isActive() and vehicle:getBatteryCharge() > 0.00200 then
            ...
            if md.timer > 0 and md.timePassed < md.timer then
                md.timePassed = md.timePassed + 0.3--Tch that static 0.3 is fishy
                stopTimer = false
            else
                if md.timePassed > 0 then
                    updateVehicleSound(vehicle, "MicrowaveRunning", "MicrowaveTimerExpired")
                end
                part:getItemContainer():setActive(false)
                md.timer = 0
                md.timePassed = 0
        ...
        if isBaseVehicle(vehicle) then vehicle:updateParts(); end
        if isServer() then
            vehicle:transmitPartModData(part)
        end
    end
...
function CommonTemplates.Update.LightApi(vehicle, part, elapsedMinutes)--compatible Virtual Vehicle
...
function CommonTemplates.Update.BatteryHeater(vehicle, part, elapsedMinutes)
    if not isBaseVehicle(vehicle) then return end--not compatible VirtualVehicle: getPartById
...
function CommonTemplates.Update.BatteryCharger(trailer, part, elapsedMinutes)--compatible VirtualVehicle
...
function CommonTemplates.Update.Freeplace(vehicle, part, elapsedMinutes)--compatible VirtualVehicle
    local invItem = part:getInventoryItem()
    if not invItem then return end
    local invItemName = invItem:getType()
    if invItemName == "TransportFreezer" then
