...
BanditCompatibility.StartMuzzleFlash = function(shooter)
    if getGameVersion() >= 42 then
        local patched = shooter.isBanditMuzzleFlashPatch
        if patched then
            ...
            local lightSource = IsoLightSource.new(square:getX(), square:getY(), square:getZ(), 0.8, 0.8, 0.7, 18, 1)
...
BanditCompatibility.SurpressZombieSounds = function(bandit)
    if getGameVersion() >= 42 then
        ...
        local tset  = desc:getVoicePrefix()
        desc:setVoicePrefix("NotAZombie")

        -- voice prefix usually works, but in same cases it dont so we need to do this too:
...
BanditCompatibility.GetBodyLocations = function(weapon)
    ...
    if getGameVersion() >= 42 then
        bodyLocations = {
            ...
            Bags = {"FannyPackFront", "FannyPackBack", "Webbing", "Back"},
