...
-- NightScale5755: Overwriting these 5 functions to implement new features.

local old_gas_tank = Vehicles.Update.GasTank
function Vehicles.Update.GasTank(vehicle, part, elapsedMinutes)
	old_gas_tank(vehicle, part, elapsedMinutes)
	...
	if elapsedMinutes > 0 and amount > 0 and vehicle:isEngineRunning() then
		...
		local modData = vehicle:getModData().tuning2
		if modData == nil then
			vehicle:getModData().tuning2 = {}
		end
		if modData and modData.gasUsage == nil then
			vehicle:getModData().tuning2.gasUsage = {}
		end

		local gasUsage = 0
		if modData and modData.gasUsage then
			local gasModData = modData.gasUsage
			for i, v in pairs(gasModData) do
				if v and v > 0 then
					gasUsage = gasUsage + v
				end
			end
		...
		local newAmount = (speedMultiplier / gasMultiplier) * gasUsage;
...
function ATATuning2.InstallComplete.Tuning(vehicle, part)
    ...
    if part:getModData().tuning2 and part:getModData().tuning2.model then
        ...
        if ATA2TuningTable[vehicleName] 
			...
			-- NightScale5755: Implement a GasUsage stat.
			if modelInfo.gasUsage then
				local modData = vehicle:getModData().tuning2
				if modData then
					modData.gasUsage[partName] = modelInfo.gasUsage
				end
        ...
        if part:getModData().tuning2.protection then
            for _, protectionPartName in ipairs(part:getModData().tuning2.protection) do
                if protectionPartName ~= "Engine" then -- защита кода от "защиты двигателя"
					...
					local modData = vehicle:getModData().tuning2
					if modData and modData.gasUsage[partName] then
						vehicle:getModData().tuning2.gasUsage[partName] = nil
						vehicle:transmitModData()
					end
                end
            end
