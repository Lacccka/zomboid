...
local function stashState(e)
    ...
    -- Cab locker (storage in the cab). Only the TILE is ours to persist: the items
    -- themselves ride in a real world container that the chunk saves at full fidelity,
    -- so all a reload needs is where to go looking. See RR_CabLocker's header for why
    -- the coordinates must survive even when the box's chunk is unloaded -- without
    -- them a reacquire cannot tell "not streamed in yet" from "gone" and would build
    -- a second locker.
    if e.box then
        md.rrBoxX = e.box.x
        md.rrBoxY = e.box.y
...
local function spawnEntityOnBuiltRoute(route, ox, oy, oz, label, routeId, opts)
    ...
    if RR.CabLocker then RR.CabLocker.onSpawn(rec) end        -- storage in the cab (no coords: builds one)
    -- She is being FOUND for the first time, so she is owed her operating manual
    -- (Task 2.I). This is the only place that arms it: re-adopting a locomotive on
    -- load must never hand one to a cab the player has already been through.
    if RR.Licence then pcall(function() RR.Licence.armCabSeed(rec) end) end
    return rec
end
...
local function adoptAnimal(animal)
    ...
    -- A locomotive from a save that predates the cab locker is owed her operating manual
    -- as well, ONCE -- she has never had a container for it to sit in, so her first locker
    -- is a first opening rather than a restock (Licence.armLegacyCabSeed carries the full
    -- argument). Must run BEFORE CabLocker.onSpawn: the reconcile that builds the locker
    -- reads the pending flag, and a debt armed after it would wait a whole extra load.
    -- Already-seeded locomotives are refused there, so this is a no-op on every load but
    -- the first.
    if RR.Licence then pcall(function() RR.Licence.armLegacyCabSeed(rec) end) end
    -- Cab locker: hand back the remembered tile, NOT a fresh build. If that chunk is
    -- still unloaded the reconcile waits instead of creating a second locker -- which
    -- is the whole reason the coordinates are persisted (RR_Locker.plan).
    if RR.CabLocker then
        RR.CabLocker.onSpawn(rec, tonumber(md.rrBoxX), tonumber(md.rrBoxY), tonumber(md.rrBoxZ))
    end
...
local function tickDerailed(e, dt, dtGame)
    ...
    -- The locker too, and on REAL dt: it is world-object churn, not simulation, so it
    -- must not reconcile forty times faster under fast-forward. A wreck is standing
    -- still, so this settles to a no-op after the first pass -- but it has to run at
    -- least once, or a loco that derailed before its first reconcile would never get
    -- one and the driver's stores would be unreachable.
    if RR.CabLocker then RR.CabLocker.update(e, dt) end
    if RR.LockerAccess then RR.LockerAccess.stamp() end   -- after pinRider; see the driving path
        ...
        elseif e.animal and not e.animal:isDead() then
            ...
            if eng then
                if eng.running then
                    ...
                    -- Ask RR_Ride rather than reading e.licensed raw: the cache is CLEARED
                    -- (set to nil) when the driver finishes an operating manual in the seat,
                    -- and a raw `~= false` read would score that nil as "licensed" for the
                    -- frames before something else repopulated it. isLicensed re-derives.
                    local env = e.rider and
                        { licensed = (not (RR.Ride and RR.Ride.isLicensed)) or RR.Ride.isLicensed(e) }
                        or nil
            ...
            -- Cab locker: REAL dt, unlike the climate above. The water temperature is
            -- chemistry and belongs on game time; carrying a world object to the next
            -- tile is neither, and reconciling it at the fast-forwarded rate would just
            -- multiply AddTileObject/RecalcAllWithNeighbours for nothing.
            if RR.CabLocker then RR.CabLocker.update(e, dt) end
            -- LAST, and that is the point: the loot panel rebuilds itself whenever the
            -- player's square or FACING changes (ISInventoryPage:update:500-507), and
            -- pinRider above has just turned the driver. Stamping from RR_LockerAccess's
            -- own OnTick handler ran BEFORE this one (handlers fire in registration =
            -- file-load order, and RR_LockerAccess loads first), so it recorded the
            -- heading from before the turn while the UI read the one after -- and the
            -- panel rebuilt every frame through every curve. Same ordering trap as the
            -- old rider-camera jitter, same fix: do it here, after the pose.
            if RR.LockerAccess then RR.LockerAccess.stamp() end
        end
    end
end
...
function TrainEntity.clear()
    for _, e in ipairs(TrainEntity.active) do
        ...
        if RR.CabLocker then RR.CabLocker.onRemove(e) end        -- take the locker with it (never orphan a
                                                                 -- container full of loot on the rails)
        if e.animal then
