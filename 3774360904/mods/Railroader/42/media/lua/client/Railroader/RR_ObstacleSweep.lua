...
local function blocksMovement(o)
    -- Our own cab locker rides on the seat tile, i.e. permanently inside the габарит.
    -- Its sprite is a rail FLOOR tile and carries no blocking flag, so this guard is
    -- belt and braces -- but the failure it prevents is the loco hard-stopping against
    -- its own storage, which would read as a phantom railcar and be very hard to place.
    -- Identity, not a modData read: this runs per object per swept square per tick, and
    -- IsoObject.getModData() allocates the table when there isn't one.
    if RR.CabLocker and RR.CabLocker.isOurs(o) then return false end
