...
--
-- GATED on RR.Debug like every other dev key: in a shipping build the loco is walked
-- to.
--
-- A TEMP_O_TELEPORT flag ungating O alone has now existed and been deleted THREE
-- times (added 2026-08-07 for the 1.1 sessions, deleted before that upload in
-- 653ee0f; re-added 2026-08-08 and deleted 2026-08-09; re-added and deleted again
-- the same day, after the cab-locker and legacy-journal test pass). It is useful and
-- it will be wanted a fourth time -- so the rule is written down rather than
-- re-argued: arm the whole dev set from the console with RR.Debug.set(true), and if
-- a flag really must come back, DELETE it before the upload rather than set it to
-- false. pack_workshop.ps1 enforces that: it refuses to stage while a live
-- (non-comment) TEMP_O_TELEPORT line exists anywhere in 42/, with no override.
--------------------------------------------------------------------------
