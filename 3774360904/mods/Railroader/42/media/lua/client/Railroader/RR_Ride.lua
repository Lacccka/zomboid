...
--------------------------------------------------------------------------
-- startEnv: the weather input to Engine.startChance -- COLD, and only cold. Pulled
-- from the live ClimateManager; missing data => ideal (no penalty). getTemperature()
-- is °C (verified in decompiled B42).
-- RAIN IS DELIBERATELY NOT READ (owner 2026-08-08, conductor ferreirajc87's report): a
-- diesel has no ignition system to soak, so damp weather cannot make it harder to start.
-- The isRaining() call that used to fill env.wet is gone; don't put it back.
--------------------------------------------------------------------------
...
local function startEnv(e)
    -- `pro` is the PROFESSION, not the licence: a self-taught driver who read the
    -- journal may start her, he is simply worse at it (RR_Engine START_BASE_SELF and
    -- the full drain per attempt). The two are separate fields on purpose -- a book
    -- must never quietly buy the professional's edge at the starter.
    local env = { licensed = Ride.isLicensed(e), pro = Ride.isPro(e),
                  externalPower = hasExternalPower(e) }
    pcall(function()
...
--------------------------------------------------------------------------
-- drainEnv(e): the ONE field prime/crankTick read -- who is at the starter. The
-- table is reused rather than built per call, because these two run every tick for
-- the whole three-second start sequence and a fresh table each frame is exactly the
-- kind of per-frame allocation the v1.0.1 pass went looking for. Ride.isPro caches
-- the trait query on the record, so this costs one field read.
--------------------------------------------------------------------------
local DRAIN_ENV = { pro = false }
local function drainEnv(e)
    DRAIN_ENV.pro = Ride.isPro(e)
    return DRAIN_ENV
end
...
--------------------------------------------------------------------------
-- isLicensed(e): may the character in this cab work the loco? The ONE place the
-- engine-side query is read from; the pure model gets it as a boolean (RR_Engine
-- env.licensed).
--
-- IT IS NO LONGER JUST THE PROFESSION. Since the training system (RR_Training) the
-- answer is "the Railroader profession, OR the operating journal for THIS model,
-- read in her cab" -- see docs/TRAINING_DESIGN.md. Still binary: there is no
-- driving skill and no partial competence, only qualified or not.
--
-- Cached on the record, because it is asked every tick and the query walks the
-- character's knownRecipes. Two things clear that cache: boarding (mountRecord) and
-- finishing a book while already seated (RR_Licence.grant sets e.licensed = nil) --
-- which is exactly the case that must not need climbing out and back in.
--
-- An EMPTY cab (debug G cruise, no driver) is unrestricted: nobody to card.
--------------------------------------------------------------------------
function Ride.isLicensed(e)
    if not e then return true end
    if e.licensed == nil then
        local L = RR and RR.Licence
        if L then
            e.licensed = L.mayOperate(getPlayer(), L.modelOf(e)) == true
        else
            -- RR_Licence missing (a load failure): fall back to the old profession-only
            -- rule rather than locking every cab. Fail open, loudly enough to log.
...
--------------------------------------------------------------------------
-- isPro(e): does the driver hold the PROFESSION (not merely a book)? This is the
-- one thing the books cannot buy -- it feeds RR_Engine env.pro, i.e. the better
-- start base and the halved battery drain per attempt. Cached beside the licence
-- and cleared with it.
--------------------------------------------------------------------------
function Ride.isPro(e)
    if not e then return false end
    if e.pro == nil then
        local L = RR and RR.Licence
        local P = RR and RR.Profession
        if L then
            e.pro = L.isPro(getPlayer()) == true
        else
            e.pro = (P ~= nil) and P.hasLicense(getPlayer()) == true or false
        end
    end
    return e.pro == true
end
    ...
    if not quiet then
        if not licensed then
            ...
            print("[Railroader] IN THE CAB -- but you don't know how to run a locomotive. "
                  .. "Read the operating manual for this model, in this cab -- or take the "
                  .. "Railroader profession.")
        else
...
function Ride.dismount()
    ...
    if e then
        ...
        e.pro        = nil       -- ...and with it the profession flag the starter reads
        e._licGate   = nil
        if e.animal then
...
function Ride.pinRider(e)
    ...
    if RR.CabClimate then pcall(function() RR.CabClimate.assertShelter(p) end) end
    -- ...and charge a claustrophobic driver for the steel box he is sitting in, exactly
    -- as vanilla charges him for a car (RR_CabClimate channel 3). Per frame on purpose:
    -- vanilla's dose is scaled by getThirtyFPSMultiplier(), not by game seconds. Guarded
    -- with pcall(f, arg) rather than pcall(function() ... end): passing the function by
    -- reference allocates nothing, which is what the v1.0.1 rule about per-frame closures
    -- was really about -- the closure was the cost, not the pcall.
    if RR.CabClimate and RR.CabClimate.claustro then pcall(RR.CabClimate.claustro, p) end
end
...
--     Engine.startChance (battery/cold/condition/fuel quality). Success -> warm-up; failure
...
local function updateEngineControls(e, dt)
    ...
    if not Ride.isLicensed(e) then
        if e._licGate ~= true then
            sayBad("IGUI_RR_NoLicense", "You don't know how to run a locomotive")
            print("[Railroader] you don't know how to operate this locomotive -- read her "
                  .. "operating manual sitting in this cab, or take the Railroader profession.")
            e._licGate = true
        end
        return
    end
    ...
    if not Engine.primeReady(eng) then
        Engine.primeTick(eng, dt, drainEnv(e))
    ...
    Engine.crankTick(eng, dt, drainEnv(e))
