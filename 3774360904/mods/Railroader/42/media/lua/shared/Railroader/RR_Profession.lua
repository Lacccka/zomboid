...
--------------------------------------------------------------------------
-- grantTrainingRecipes(): hang the training system's permits (RR_Training) on the
-- LICENCE TRAIT, so a Railroader holds them the way a reader does -- as real
-- entries in his own knownRecipes.
--
-- WHY THIS EXISTS AT ALL. The profession never needed the permits to DRIVE: the
-- gate short-circuits on the trait (RR_Training.mayOperate). But the permits are
-- now listed on the character sheet (the Recipes tab, built from item scripts that
-- carry LearnedRecipes), and without this a Railroader -- the one character who is
-- qualified on everything -- read as knowing NEITHER of his own capabilities.
-- Owner caught it 2026-08-09.
--
-- WHY THE TRAIT AND NOT THE PROFESSION. Both CharacterProfessionDefinition and
-- CharacterTraitDefinition have addGrantedRecipe (:131 / :175) and both are applied
-- at character creation. The trait is the single thing the whole licence already
-- hangs on, so hanging the permits there keeps one answer to "is he qualified?"
-- instead of two that can drift.
--
-- VERIFIED IT LANDS IN SINGLEPLAYER, which is the only mode we support: the two
-- creation paths that matter are IsoWorld:2244-2257 and LuaManager:6414-6425, and
-- BOTH walk IsoWorld.getLuaTraits() adding each trait definition's granted recipes
-- -- before OnNewGame fires. Our licence trait is profession-GRANTED, and the
-- creation screen puts a granted trait through addLuaTrait like any other, so it is
-- in that list. (IsoGameCharacter.applyCharacterTraitsRecipes:11526 is a third path
-- and is MP-only, CreatePlayerPacket:294.)
--
-- It also buys the book's "you already know this" line for free: Literature:170-193
-- checks trait.isGrantedRecipe on every LearnedRecipes entry and adds
-- Tooltip_literature_AlreadyKnown when it matches. A Railroader who picks up the
-- manual is told he does not need it.
--
-- Characters rolled BEFORE this existed are not reached here -- the grant only
-- happens at creation. RR_Licence backfills them on load.
--------------------------------------------------------------------------
local function grantTrainingRecipes()
    if not (CharacterTraitDefinition and Prof.trait) then return end
    local ok = pcall(function() require("Railroader/RR_Training") end)
    local T = ok and RR and RR.Training
    if not (T and T.allRecipes) then
        print("[Railroader] profession: RR_Training unavailable -- the licence grants no recipe entries.")
        return
    end
    pcall(function()
        local def = CharacterTraitDefinition.getCharacterTraitDefinition(Prof.trait)
        if not def then return end
        for _, recipe in ipairs(T.allRecipes()) do
            -- Guarded: addGrantedRecipe appends to a plain list, so a second pass
            -- over this file would list every permit twice.
            if not def:isGrantedRecipe(recipe) then def:addGrantedRecipe(recipe) end
        end
    end)
end
