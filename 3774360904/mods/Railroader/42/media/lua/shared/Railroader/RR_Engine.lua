...
--   * warm/healthy        (cold, a worn engine and stale fuel each lower the
--                          catch chance -> may need several cranks. RAIN DOES
--                          NOT: a diesel has nothing to soak -- see C.)
...
local C = {
    ...
    -- THE BASE IS THE HUMAN, and it is the only place the driver himself enters the
    -- start model (training system, 2026-08-08). A certified engineer has done this a
    -- thousand times; a survivor who learned it out of the manual in the cab is doing
    -- it from a page. Everything else in the product below is the MACHINE and applies
    -- to both alike -- a flat battery does not care who is holding the switch, and no
    -- amount of profession restores it.
    START_BASE_SELF = 0.88,    -- self-taught (read the operating journal)
    START_BASE_PRO  = 1.00,    -- Railroader profession
    -- The professional's second, larger edge: what a failed attempt COSTS. Battery
    -- charge is itself a factor in the next attempt's chance, so the cost of missing
    -- compounds -- the more you crank blindly, the worse your odds get. The EMD manual
    -- caps a press of the starter at fifteen seconds; an experienced man does not sit
    -- on it. Applied to the fuel-prime AND crank drains alike:
    --   self-taught  ~0.095 charge for the first attempt, ~0.08 for each repeat
    --                (a repeat keeps the prime, so it is crank-only)
    --   professional ~0.0475 / ~0.04
    -- The design note's table rounds those to 0.05/0.05; the halving is the rule, and
    -- the arithmetic falls where it falls. See docs/TRAINING_DESIGN.md section 4.
    PRO_DRAIN_FAC   = 0.50,
    ...
    -- NO WET/RAIN TERM. There used to be WET_FAC = 0.85 while it rained; it was wrong on the
    -- merits and is gone (owner 2026-08-08, on the report of the conductor ferreirajc87): a
    -- diesel has no ignition system to soak -- no coil, no points, no plug leads -- so damp
    -- weather makes no difference to whether it lights off. That is a petrol-car problem.
    -- Rain now enters the start model NOWHERE; do not re-add the factor. Cold is the real
    -- weather failure, and the truthful version of it is the frozen cooling water (Task 2.E),
    -- not a shaved probability. See docs/ENGINE_START_DESIGN.md "Field feedback".
}
Engine.C = C
...
--------------------------------------------------------------------------
-- startBase(env) / drainFactor(env): the DRIVER's two contributions, both keyed on
-- one plain boolean, env.pro -- "holds the Railroader profession".
--
-- env.pro is deliberately NOT the same field as env.licensed. Since the training
-- system a survivor can be licensed to drive (he read the model's operating journal
-- in her cab) without being a professional, and the difference between the two has
-- to live somewhere the books cannot reach. This is it: a better base chance, and
-- half the charge burnt on an attempt that misses.
--
-- ABSENT env.pro => self-taught. That is the safe default for a caller that has not
-- been taught to pass it: it under-states the driver rather than over-stating him.
--------------------------------------------------------------------------
function Engine.startBase(env)
    if env and env.pro == true then return C.START_BASE_PRO end
    return C.START_BASE_SELF
end

function Engine.drainFactor(env)
    if env and env.pro == true then return C.PRO_DRAIN_FAC end
    return 1.0
end
...
-- quality, cold. env = { tempC=, externalPower= } (all optional).
-- Fuel PRESENCE is a hard gate handled in tryStart, not a probability here.
-- RAIN IS NOT A FACTOR (see C above): env.wet is accepted and IGNORED, so an old
-- caller passing it changes nothing.
--------------------------------------------------------------------------
function Engine.startChance(s, env)
    ...
    local chance = Engine.startBase(env) * batt * cond * fq * cold
...
--------------------------------------------------------------------------
-- env (optional) is only read for env.pro -- see drainFactor.
function Engine.primeTick(s, dt, env)
    dt = dt or 0
    if s.running then return end
    s.phase     = "priming"
    s.primeTime = (s.primeTime or 0) + dt
    s.battery   = clamp((s.battery or 0) - C.PRIME_DRAIN * Engine.drainFactor(env) * dt,
                        0, C.BATTERY_FULL)
end
...
--------------------------------------------------------------------------
-- env (optional) is only read for env.pro -- see drainFactor.
function Engine.crankTick(s, dt, env)
    dt = dt or 0
    if s.running then return end
    s.phase     = "cranking"
    s.crankTime = (s.crankTime or 0) + dt
    s.battery   = clamp((s.battery or 0) - C.CRANK_DRAIN * Engine.drainFactor(env) * dt,
                        0, C.BATTERY_FULL)
end
