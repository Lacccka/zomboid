...
--[[ SECTION 1 : QUESTS
	...
	awardsitem = (OPTIONAL) One or more item types that will be given to the player when the quest is completed, followed by the quantity and separated by a semicolon. These are displayed in an NPC's dialogue for accepting the quest (See more on that in the dialogue section). It accepts an item "Base.Burger", item and quantity "Base.Burger;2" or multiple items and their quantities "Base.Fries;2;Base.Burger;1".
	awardslore = (OPTIONAL) The quest will receive one more text page in its lore UI, it accepts a string containing a translation entry. If the quest had no text pages and could not be clicked it will be clickable when this new page is added.
	awardsrep = (OPTIONAL) Reputation points that will be given when the quest is completed. Accepts factioncode (see the faction section) plus reputation amount, for example: "TestFaction;50".
	awardstask = (OPTIONAL) Unlocks another quest when this quest is completed, it accepts a string that must be a valid, unique quest guid (see guid in this list).
	awardsworld = (OPTIONAL) Unlocks an NPC dialogue when this quest is completed, It accepts a string that must be a valid, unique dialoguecode (see the dialogue section).
	completesound = (OPTIONAL) Links to a sound ID that will be played when the quest is completed, I use the old level up sound "levelup" for my quests.
	dailycode = (OPTIONAL) A string used to identify quests from the same daily pool. When it is time to re-roll and give a new quest from a pool the system checks if the character already has one.
	guid = (REQUIRED) This is a unique string used to identify your quest among all quests added by all mods, so make it really unique like "YourModName_UniqueQuestName".
	lore = (OPTIONAL) This is a table with translation entries, these entries should contain text that players can read if they click the quest, if no lore table is included clicking on the quest will not open the lore UI.
	needsitem = (OPTIONAL) Adding this means your quest needs one or more item to progress, it can be an item ID, or an item tag, followed by the quantity and separated by a semicolon. Examples: "Base.Nails;5", or one of the tag options:
	...
	objectives = (OPTIONAL) A table containing one table for each objective. Objectives display their own text and can have their own separate progress status. It is usually required to complete all objectives before being able to complete a quest but not necessarily in the order they are presented. Example: A quest could require talking to 4 other NPCs.
	ondone = (OPTIONAL)  A string containing a list of commands what will be executed when the quest or objective is marked as done (see the commands section).
	onobtained = (OPTIONAL) A string containing a list of commands what will be executed when the quest or objective is marked as obtained (see the commands section).
	text = (REQUIRED) The text that will be displayed in the list of quests, should be short and descriptive.
	texture = (OPTIONAL) The icon that will be displayed in the list of quests. If using an item icon from the game you must add the "Item_" preffix, if it is a modded icon the complete path and file must be provived. Example: "Item_Nails", "media/textures/Item_MyOwnIcon.png".
	...
	unlockedsound = (OPTIONAL) Links to a sound ID that will be played when the quest is given to a character, I provide a new sound "QuestUnlocked" that I use for my quests.
	unlocks - (OPTIONAL) A string containing a list of commands that will be executed when the player receives this quest (see the commands section).
	updates = (OPTIONAL) It means this quest is actually an update for another active quest and will not be added to the active list. It must be a valid, unique quest guid and that quest must be in the list of active quests at the momment of unlocking this update. Most values listed here can be used to change that quest for the character. Examples: texture, text, title.
...
--[[ SECTION 2 : COMMANDS
	...
	actionevent - Adds a specific condition that will be checked every 10 minutes, when the condition is met a list of commands will be given. Parameters: Condition and list of commands.
					Possible conditions: "ënteroom", followed by the coordinates for a square in that room. "killzombies", followed by the quantity, separated by : "killzombies:25".
					Commands: Any commands and their parameters, but with : instead of ;
	addmannequin - Adds a mannequin to the world. Keep in mind that the mannequin must have been inserted into the list of mannequins for this to work. Parameter: A squaretag (string composed of the square's x, y and z).
	additem - Adds an item to the player's inventory. Parameters: Item ID followed by quantity.
	clickevent - Adds a right click event at a specific square (see right click event section). Parameters: A "squaretag" and a unique string for the click event separated by : (Example: "12000x7500x0:MyUniqueClickEvent", values for the timed action separated by : (example: "time:50:anim:loot"), and a list of commands that will run when the action is performed. Each parameter separated by a semicolon ;
	...
	updatequeststatus - Updates the quest status, can use a variety of strings such as completed, done, obtained, . Parameters: A quest guid followed by a status.
	updateobjective - Updates a quest's objective status, much like the previous command. Parameters: Quest guid, objective's index and status.
	updateobjectivetext - Changes the text displayed for a certain objective. Paramters: Quest guid, objective's index and a translation entry.
	unlockquest - Unlocks a quest, only acceptable parameter is a quest's guid.
	unlockroom - Enables the character to receive a list of commands when entering a certain room in the map. Parameters: A squaretag followed by a commands list (with : instead of ;).
...
SFQuest_Database.MannequinPool["8000x5000x0"] = {sprite = "location_shop_mall_01_68", direction = "S", beard = "", hair = "Bald", outfit = "Farmer"};
	...
	sprite = (REQUIRED) A string with one of the available mannequin's sprites, location_shop_mall_01_65 to 70 and from 73 to 78.
	direction = A string with the direction this mannequin will face, must be one of these: "N", "W", "E" or "S".
	beard = (OPTIONAL) A string with the beard model's ID, if not included will use a random model according to outfit definitions. "" means no beard.
