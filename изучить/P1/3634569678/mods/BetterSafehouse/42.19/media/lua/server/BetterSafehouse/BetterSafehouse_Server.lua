...
local function handleAdminAdd(playerObj, args)
    ...
    -- A player who just joined starts as a plain member, never as a sub-owner.
    if BetterSafehouse.SubOwner and BetterSafehouse.SubOwner.onMemberJoined then
        pcall(function() BetterSafehouse.SubOwner.onMemberJoined(sh, username) end)
    end
...
local function playerIsSubOwner(playerObj, sh)
    local SO = BetterSafehouse.SubOwner
    if not SO or not SO.serverIsSubOwner or not playerObj or not sh then return false end
    if not playerObj.getUsername then return false end
    local ok, v = pcall(function() return SO.serverIsSubOwner(sh, playerObj:getUsername()) end)
...
local function handleRequestInvite(playerObj, args)
    ...
    local isSubOwner = playerIsSubOwner(playerObj, sh)

    if not (playerIsSafehouseOwnerOrAdmin(playerObj, sh) or isSubOwner) then
        sendInviteResult(playerObj, false, "IGUI_BetterSafehouse_Invite_NoPermission")
        return
    end

    -- This custom invite path serves EnhancedInvites and sub-owners. A sub-owner must use
    -- it even when EnhancedInvites is disabled because vanilla rejects non-owner invites.
    local enhanced = true
    if BetterSafehouse.getSandboxBool then
        enhanced = BetterSafehouse.getSandboxBool("EnhancedInvites", true) == true
    end
    if not enhanced and not isSubOwner then
    ...
    -- With EnhancedInvites disabled, sub-owners retain the vanilla restriction: the
    -- target cannot already own or belong to another safehouse.
    if not enhanced and SafeHouse and SafeHouse.hasSafehouse then
        local okH, other = pcall(function() return SafeHouse.hasSafehouse(target) end)
        if okH and other then
            sendInviteResult(playerObj, false, "IGUI_BetterSafehouse_Invite_TargetHasSafehouse")
            return
        end
    end
...
local function handleAcceptInvite(playerObj, args)
    ...
    -- Rejoining always starts at the plain-member rank.
    if BetterSafehouse.SubOwner and BetterSafehouse.SubOwner.onMemberJoined then
        pcall(function() BetterSafehouse.SubOwner.onMemberJoined(sh, username) end)
    end
