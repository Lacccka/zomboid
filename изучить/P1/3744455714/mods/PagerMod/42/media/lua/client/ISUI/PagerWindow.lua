...
function PagerWindow:prerender()
    ...
    if self.mode == MODE_COMPOSE then
        ...
        -- Illiterate senders can only page digits (letters are stripped on send),
        -- so say so here instead of letting the text silently disappear.
        if PagerMod.illiteracyGarbles() then
            self:drawText(getText("IGUI_PagerMod_IlliterateWrite"), pad, self.locTick.y + 22,
                0.9, 0.7, 0.2, 1, FONT_SMALL)
        end
    end
end
...
function PagerWindow.drawInboxItem(self, y, item, alt)
    ...
    -- Sender names and bodies are written words: readableText garbles them to "?"
    -- for an illiterate character. The 7-digit number stays legible.
    local fromLabel = msg.fromName and msg.fromName ~= "" and PagerMod.readableText(msg.fromName)
        or PagerMod.formatNumber(msg.from)
    ...
    self:drawText(truncate(PagerMod.readableText(msg.text or ""), self:getWidth() - 12, self.font), 6, y + 22, 0.85, 0.85, 0.85, 1, self.font)
...
function PagerWindow:onOpenMessage()
    ...
    local from = (msg.fromName and msg.fromName ~= "" and PagerMod.readableText(msg.fromName))
        or PagerMod.formatNumber(msg.from)
    local when = PagerMod.stampToClock(msg.stamp) .. " " .. PagerMod.stampToDate(msg.stamp)
    local parts = {}
    ...
    table.insert(parts, " ")
    table.insert(parts, tostring(PagerMod.readableText(msg.text or "")))
...
function PagerMod.OpenPagerUI(item)
    -- Illiteracy gate ("Cannot use a pager" mode). Here rather than only in the
    -- context menu so every open path is covered: the hotkey, quick-page, the
    -- compose shortcuts and the "opened after activation" path.
    if PagerMod.illiteracyBlocksUse() then
        PagerMod.halo(getText("IGUI_PagerMod_Illiterate"), { r = 0.9, g = 0.7, b = 0.2 })
        return
    end
...
function PagerMod.OpenTowerUI(number, name, powered)
    -- A tower console is a screen with words on it too.
    if PagerMod.illiteracyBlocksUse() then
        PagerMod.halo(getText("IGUI_PagerMod_Illiterate"), { r = 0.9, g = 0.7, b = 0.2 })
        return
    end
