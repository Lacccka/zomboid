...
-- ── Illiteracy (the Illiterate trait) ──────────────────────
-- B42 only: this build's Illiterate trait is exposed as CharacterTrait.ILLITERATE.
PagerMod.Illiteracy = {
    OFF     = 1, -- the trait has no effect on pagers (default; previous behaviour)
    NO_READ = 2, -- illiterate characters see page text as "?" and can only send digits
    NO_USE  = 3, -- illiterate characters cannot open a pager or a tower console at all
}
...
PagerMod.Config = {
    ...
    illiteracy       = PagerMod.Illiteracy.OFF, -- what the Illiterate trait does to a pager
...
function PagerMod.refreshConfig()
    ...
    if sv.Illiteracy       ~= nil then c.illiteracy       = sv.Illiteracy end
...
-- ── Illiteracy gates ───────────────────────────────────────
-- These are client-side questions about the LOCAL character ("can *I* read this
-- page?"), so they deliberately fail closed to "literate" wherever there is no
-- local player: on a dedicated server getPlayer() is nil, and the server must
-- never rewrite text on behalf of somebody else (a co-op host runs the server
-- logic in its own process, so a trait check there would leak onto everyone).
local traitCacheFor, traitCacheVal = nil, false

-- Does this character have the Illiterate trait? Cached per character object
-- (traits don't change mid-life) because the inbox renderer asks every frame.
local function hasIlliterateTrait(player)
    if player == traitCacheFor then return traitCacheVal end
    local has = false
    if CharacterTrait and CharacterTrait.ILLITERATE then
        local ok, v = pcall(function() return player:hasTrait(CharacterTrait.ILLITERATE) end)
        if ok then has = (v == true) end
    end
    -- Fallback for the string-keyed form of the API.
    if not has then
        local ok, v = pcall(function() return player:HasTrait("Illiterate") end)
        if ok then has = (v == true) end
    end
    traitCacheFor, traitCacheVal = player, has
    return has
end

-- True when the local character is illiterate AND the sandbox option is on.
function PagerMod.isIlliterate()
    PagerMod.refreshConfig()
    if (PagerMod.Config.illiteracy or PagerMod.Illiteracy.OFF) == PagerMod.Illiteracy.OFF then
        return false
    end
    local player = getPlayer and getPlayer()
    if not player then return false end
    return hasIlliterateTrait(player)
end

-- Mode 3: the pager (and a tower console) can't be operated at all.
function PagerMod.illiteracyBlocksUse()
    return PagerMod.Config.illiteracy == PagerMod.Illiteracy.NO_USE and PagerMod.isIlliterate()
end

-- Modes 2 and 3: written words are meaningless to this character.
function PagerMod.illiteracyGarbles()
    return PagerMod.isIlliterate()
end

-- The characters an illiterate character can still make sense of: digits, plain
-- whitespace and dashes, so a numeric page ("07 1234-5") still means something.
-- Spelled out rather than using %d/%s because those classes are locale/encoding
-- dependent and were matching stray bytes inside Cyrillic/Korean text, which then
-- survived into the output as broken glyphs.
local KEEP = "0-9 	%-"

-- Replace everything that has to be *read* with "?". Counts UTF-8 lead bytes (or
-- whole characters, when the runtime hands us a character-based string) so one
-- multi-byte glyph becomes one "?" rather than two or three.
function PagerMod.garbleText(text)
    text = tostring(text or "")
    return (text:gsub("[^" .. KEEP .. "]+", function(run)
        local n = 0
        for i = 1, #run do
            local b = run:byte(i)
...
-- Text as the local character can actually read it (garbled when illiterate).
function PagerMod.readableText(text)
    if PagerMod.illiteracyGarbles() then return PagerMod.garbleText(text) end
    return text
end

-- An illiterate character can't write words either, so their outgoing pages are
-- reduced to digits — the same rule the NumericOnly option applies to everyone.
function PagerMod.illiterateStrip(text)
    if not PagerMod.illiteracyGarbles() then return text end
    return (tostring(text or ""):gsub("[^" .. KEEP .. "]", ""))
end
