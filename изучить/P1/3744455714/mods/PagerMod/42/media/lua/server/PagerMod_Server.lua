...
-- The deployed tower object on a square (by modData tag, or its unique custom
-- sprite name as a fallback when modData didn't sync), or nil.
...
-- Create the deployed tower's world object ON THE SERVER so it syncs to every
-- client. The tower is a dropped PagerTower item (an IsoWorldInventoryObject)
-- rendering its WorldStaticModel — the proven B42 way to show a 3D model in the
-- world, same as the B41 tower. This MUST run server-side: AddWorldInventoryItem
-- only transmits to clients when GameServer.server is true (transmit=true), so a
-- client-placed object is local-only — that was the invisible-in-MP bug. Mirror
-- of removeTowerObjectAt above (which removes via transmitRemoveItemFromSquare).
local function spawnTowerObject(x, y, z, dir)
    local cell = getCell and getCell()
    if not cell then return end
    local sq = cell:getGridSquare(x, y, z)
    if not sq then return end
    pcall(function()
        -- instanceItem() is the engine's global item factory (InventoryItemFactory
        -- is NOT exposed to Lua — indexing it throws "CreateItem of non-table").
        local item = instanceItem(PagerMod.TOWER)
        if not item then return end
        -- Drop it as a world object; transmit=true syncs it to every client
        -- (only fires because we're on the server). It renders WorldStaticModel.
        sq:AddWorldInventoryItem(item, 0.5, 0.5, 0.0, true)
        local obj = item.getWorldItem and item:getWorldItem() or nil
        if obj then
            local omd = obj:getModData()
            omd.pagerTower = true
            omd.towerDir = dir or 0
        end
...
local function onDeployTower(player, args)
    ...
    -- If we reject the deploy, hand the carried item back to the placer. (The
    -- client no longer places a world object up front — the server spawns it on
    -- success below — so TOWER_REFUND now only needs to restore the item; its
    -- object-removal step is a harmless no-op.)
    ...
    -- Spawn the world object server-side so every player sees it and can operate
    -- it (the client that placed it no longer creates it locally).
    spawnTowerObject(x, y, z, args.dir)
