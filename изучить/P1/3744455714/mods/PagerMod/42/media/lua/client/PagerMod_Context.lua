...
-- "Cannot use a pager" illiteracy mode: show a single greyed-out entry with a
-- tooltip, the way vanilla handles books and notes for an illiterate character,
-- instead of silently offering options that would all be refused.
local function addIlliterateNotice(context, label)
    local opt = context:addOption(label, nil, nil)
    opt.notAvailable = true
    local tip = ISToolTip:new()
    tip:initialise()
    tip:setName(label)
    tip.description = getText("IGUI_PagerMod_Illiterate")
    opt.toolTip = tip
    return opt
end
...
local function fillActivatedMenu(context, item)
    ...
    -- Quick page (sending only — hidden on receive-only pagers). The canned
    -- phrases are words, so an illiterate character gets the greyed-out notice
    -- instead of phrases that would be stripped to nothing on send.
    if canSend then
        if PagerMod.illiteracyGarbles() then
            addIlliterateNotice(context, getText("IGUI_PagerMod_QuickPage"))
        else
    ...
    for _, entry in ipairs(items) do
        ...
        if item and item:getFullType() == PagerMod.ITEM then
            if PagerMod.illiteracyBlocksUse() then
                addIlliterateNotice(context, getText("IGUI_PagerMod_Open"))
            elseif item:getModData().pagerNumber then
...
local function onFillWorldMenu(playerNum, context, worldobjects)
    ...
    if not player then return end
    local towerObj = nil
    -- 1. a tower object directly under the cursor
    for _, obj in ipairs(worldobjects or {}) do
        if obj and instanceof(obj, "IsoObject") and PagerMod.isTowerObject(obj) then
            towerObj = obj
            break
        end
    end
    ...
    -- Illiterate ("cannot use") characters can still put a tower up or pack it
    -- away — that's physical work — but not operate its console or name it.
    if PagerMod.illiteracyBlocksUse() then
        addIlliterateNotice(context, getText("IGUI_PagerMod_UseTower"))
