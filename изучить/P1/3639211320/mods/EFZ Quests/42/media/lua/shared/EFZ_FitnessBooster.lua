...
local function levelUpFitnessOnce(playerObj)
    if not playerObj or not Perks or not Perks.Fitness then
        return false
    end

    return PerkXP.levelUpPerkKeepingProgress(playerObj, Perks.Fitness)
end

-- Called by item script: `OnEat = Use_FitnessBooster`
function Use_FitnessBooster(item, player, percent)
    ...
    levelUpFitnessOnce(targetPlayer)
end
