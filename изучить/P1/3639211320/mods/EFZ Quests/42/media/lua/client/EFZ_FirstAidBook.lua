...
-- b41: First Aid perk is `Perks.Doctor`
local function levelUpFirstAidOnce(playerObj)
    if not playerObj then
        return false
    end

    if not Perks or not Perks.Doctor then
        return false
    end

    local perk = Perks.Doctor
...
function Use_FirstAidBook(item, player, percent)
    ...
    if modData and modData.EFZ_FirstAidBookUsed == true then
        return
    end

    local before = getPerkLevelSafe(targetPlayer, Perks and Perks.Doctor) or 0
    if before >= 10 then
        return
    end
...
local function onFillInventoryObjectContextMenu(playerNum, context, items)
    ...
    for _, item in ipairs(actualItems) do
        if item and item.getFullType and item:getFullType() == "EFZ.FirstAidBook" then
            context:addOption(getText("ContextMenu_Use_FirstAidBook"), item, function(it)
                Use_FirstAidBook(it, playerObj, 1.0)
            end)
            break
        end
    end
