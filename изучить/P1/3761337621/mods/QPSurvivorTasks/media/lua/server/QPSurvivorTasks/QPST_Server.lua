...
-- QPST_WEEKLY_MONEY_B41_COMPAT_V070_TC24
local function qpstAddMoneyToInventory(playerObj, amount)
    ...
    -- Build 41 dedicated-server compatibility.
    --
    -- B41 does not expose the B42 sendAddItemToContainer() helper used by
    -- TC2.1. Vanilla B41 server code grants inventory items to a connected
    -- player through IsoPlayer:sendObjectChange("addItemOfType", ...).
    --
    -- Do not pre-add the items to the server-side inventory on this path:
    -- sendObjectChange is the canonical B41 server -> player inventory grant.
    if multiplayerServer
        and not sendAddItemToContainer then

        if not playerObj.sendObjectChange then
            return false
        end

        local ok, err = pcall(function()
            playerObj:sendObjectChange(
                "addItemOfType",
                {
                    type = "Base.Money",
                    count = amount
        ...
        if not ok then
            print(
                "[QPST-DEV] B41 Weekly money payout failed: "
                .. tostring(err)
            )
            return false
        end
    ...
    -- Build 42 dedicated server / local fallback.
