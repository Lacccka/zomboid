...
local explode = function(x, y, z)
    ...
    for dx = -9, 9 do
        for dy = -9, 9 do
            if isInCircle(x + dx, y + dy, x, y, 6) then
                for dz = 0, 10 do
                    local squareF = cell:getGridSquare(x + dx, y + dy, dz + z)
                    if squareF then
                        if dz >= 1 then
                            local objects = squareF:getObjects()
                            for i=objects:size()-1, 0, -1 do
                                ...
                                if object then
                                    if isClient() then
                                        ...
                                        squareF:transmitRemoveItemFromSquare(object)
                                    end
                                end
                            end
    ...
    for dy=-2, 2 do
        for dx = -2, 2 do
            ...
            if vsquare then
                ...
                -- street destruction
                if true or BanditUtils.HasZoneType(vsquare:getX(), vsquare:getY(), vsquare:getZ(), "Nav") then
    ...
    emitter:playSound("BWOBombExplode")
