...
function BanditCreationMain:updateHairCombo()
    ...
    local hairStyles = Bandit.maleHairStyles
    if self.model:isFemale() then
        hairStyles = Bandit.femaleHairStyles
    end

    for i, hairStyle in pairs(hairStyles) do
        local label = hairStyle
        if label == "" then
            label = getText("IGUI_Hair_Bald")
        else
            label = getText("IGUI_Hair_" .. label);
        end
        self.hairTypeCombo:addOptionWithData(label, hairStyle)
    end
    ...
    if self.model:isFemale() then
        ...
        for i, beardStyle in pairs(Bandit.beardStyles) do
            local label = beardStyle
            if label == "" then
                label = getText("IGUI_Beard_None")
            else
                label = getText("IGUI_Beard_" .. label);
            end
            self.beardTypeCombo:addOptionWithData(label, beardStyle)
        end
        --[[
