...
local function getVehicleType(v)
    if instanceof(v, "Item") then
        local item = v:InstanceItem(nil);
        return item:getMechanicType();
    end
    return 0;
end
...
function command_1:execute(sender)
    ...
    if item[1] then
        ...
        local vehicleType = getVehicleType(item[1]);
        sender.stock[#sender.stock+1] = { name = vehicleType > 0 and (getText("IGUI_ItemNameMechanicalType", item[1]:getDisplayName(), getText("IGUI_VehicleType_"..tostring(vehicleType)))) or tostring(item[1]:getDisplayName()), type = item[1]:getFullName(), amount = item[2], pack_size = item[1]:getCount(), texture = QItemFactory.getTextureFromItem(item[1]), w_texture = QItemFactory.getWorldSpriteFromItem(item[1]), default = item[2], value = value, unique = self.args[3] == "true", flag = self.args[4] ~= nil and tostring(self.args[4]) };
    else
...
SaveManager.load = function(progress)
    ...
    if progress[dataType] then
        for a=1, #progress[dataType] do
            if type(progress[dataType][a]) == "table" then
