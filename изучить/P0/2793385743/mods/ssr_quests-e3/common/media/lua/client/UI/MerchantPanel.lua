...
local label_worth_s = string.gsub(getTextOrNull("UI_QSystem_Trade_Single") or "\"This one is worth %d.\"", "%%d", "%d");
local label_worth_m = string.gsub(getTextOrNull("UI_QSystem_Trade_Multiple") or "\"Each of these is worth %d.\"", "%%d", "%d");
...
function MerchantPanel:drawStock(y, entry, alt)
	...
	self:repaintStencilRect(0, 0, self.width - offset, self.height);
...
local function isInsideBag(item)
	local container = item:getContainer();
	local bag = container and container:getContainingItem();
	if bag and (MerchantPanel.instance.player:isEquipped(bag) or MerchantPanel.instance.player:isEquippedClothing(bag)) then
		return true;
	end
	return false;
end
...
local function validate(item)
	if not MerchantPanel.instance.inventory:contains(item) and not isInsideBag(item) then
		return false;
...
local function purge() -- checks if offerred items are still following rulesets
	if MerchantPanel.instance then
		local update = false;
		...
		for i=#MerchantPanel.instance.offerings.items, 1, -1 do
			for j=#MerchantPanel.instance.offerings.items[i].item.items, 1, -1 do
				local k = MerchantPanel.instance.offerings.items[i].r;
				if MerchantManager.instance.items[MerchantPanel.instance.index].demands[k].item then
					if MerchantManager.instance.items[MerchantPanel.instance.index].demands[k].ruleset and not ItemFetcher.validate(MerchantPanel.instance.offerings.items[i].item.items[j], MerchantManager.instance.items[MerchantPanel.instance.index].demands[k].ruleset) then
						remove(i, j);
					end
				elseif not ItemFetcher.validate(MerchantPanel.instance.offerings.items[i].item.items[j], MerchantManager.instance.items[MerchantPanel.instance.index].demands[k].ruleset) then
					remove(i, j);
				end
			end
		end
...
function MerchantPanel:addToOfferings(item)
	if not validate(item) then return end
	...
	self.offerings.items[#self.offerings.items].r = id;
	self:updateButtons();
end
...
local function addToDemands(self)
	for i=1, #self.demands.items do
		if self.demands.items[i].item.type == self.stock.items[self.stock.selected].item.type then return; end
	end
...
function MerchantPanel:confirmBarter()
	...
	if self.barter.enable then purge() end
	if self.barter.enable then
		...
		for i=1, #self.offerings.items do
			...
			for j=1, #self.offerings.items[i].item.items do
				if isClient() then
					ids[#ids+1] = self.offerings.items[i].item.items[j]:getID();
				else -- single player
					local container = self.offerings.items[i].item.items[j]:getContainer();
					container:Remove(self.offerings.items[i].item.items[j]);
					sendRemoveItemFromContainer(container, self.offerings.items[i].item.items[j]);
				end
			end
			self.total = self.total - (self.offerings.items[i].item.value * self.offerings.items[i].item.amount);
		...
		if ids[1] then
			SyncClient.request('QSystem', 'removeItem', ids, string.format("barter, %s", MerchantManager.instance.items[self.index].id), callback);
		else
			callback();
		end
	end
...
function MerchantPanel:createModal(mode)
	...
	local amount = math.floor(self.stock.items[self.stock.selected].item.amount / self.stock.items[self.stock.selected].item.pack_size);
	local pending = self.stock.items[self.stock.selected].item.pending;
	self.modal = entryBox:new(self.width / 2 - 100*SSRLoader.scale, self.height / 2 - 50*SSRLoader.scale, mode == 1 and (amount - pending) or pending, mode, close)
...
function MerchantPanel:onButtonPressed(button)
	...
	elseif button.internal == 2 then -- add to demands
		if math.floor(self.stock.items[self.stock.selected].item.amount / self.stock.items[self.stock.selected].item.pack_size) - self.stock.items[self.stock.selected].item.pending > 9 then
			self:createModal(1);
		else
			self:addToDemands(1);
		end
...
function MerchantPanel:new(index, onConfirm, onCancel)
	...
	o.scales_tex = { getTexture("media/ui/QSystem/scales_good.png"), getTexture("media/ui/QSystem/scales_normal.png"), getTexture("media/ui/QSystem/scales_bad.png") }
	o.modal = nil;
	if MerchantPanel.instance then
