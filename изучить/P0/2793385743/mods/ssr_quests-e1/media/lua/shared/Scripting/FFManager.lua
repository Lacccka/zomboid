        ...
        for i=1, FFManager.instance.items_size do
            if FFManager.instance.items[i].instance then
                ...
                if distance > 35 and FFManager.instance.items[i].instance.javaObject then
                    if CharacterManager.instance.items[FFManager.instance.items[i].character_id].spawned then
                        FFManager.instance.items[i].instance:despawn();
                        if CharacterManager.instance.items[FFManager.instance.items[i].character_id].npc and CharacterManager.instance.items[FFManager.instance.items[i].character_id].npc[1] ~= FFManager.instance.index then
                            FFManager.instance.items[i].instance = nil;
                        end
                    end
                elseif distance <= 30 and not FFManager.instance.items[i].instance.javaObject then
                    if not CharacterManager.instance.items[FFManager.instance.items[i].character_id].spawned then
        ...
        if name == self.items[i].name then
            if self.items[i].instance then
                if self.items[i].instance.x ~= x or self.items[i].instance.y ~= y or self.items[i].instance.z ~= z or self.items[i].instance.sprite ~= sprite then
                    ...
                    self.items[i].instance = FNPC:new(name, sprite, x, y, z, self.items[i].character_id);
                end
            else
                self.items[i].instance = FNPC:new(name, sprite, x, y, z, self.items[i].character_id);
            end
            CharacterManager.instance.items[self.items[i].character_id]:register(FFManager.instance.index, i); -- change active NPC plugin for character
...
if QSystem.validate("ssr-quests-e1") then
    ...
    Events.OnTeleport.Add(function () if FFManager.instance then FFManager.instance:removeAll() end end)
    Events.OnCreatePlayer.Add(function () if FFManager.instance then FFManager.instance:removeAll() end end);
    Events.OnPlayerDeath.Add(function () if FFManager.instance then FFManager.instance:removeAll() end end);
