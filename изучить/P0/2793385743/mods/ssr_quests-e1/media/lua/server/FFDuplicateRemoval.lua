...
local function removeDupesFromSquare(square)
    if square then
        ...
        print("[QSystem] FurnitureFolks: Failed to check square for duped NPCs because it's NULL");
    end
end
...
function FFDuplicateRemoval.clearSquare(square)
    if FFDuplicateRemoval.enabled then
        ...
        if type(m.FNPC) == 'table' then
            local x, y, z = square:getX(), square:getY(), square:getZ();
            for i=#m.FNPC, 1, -1 do
                if x == m.FNPC[i].x and y == m.FNPC[i].y and z == m.FNPC[i].z and removeDupesFromSquare(square) then
