...
LuaEventManager.AddEvent("OnTeleport");

QSystem = {}
QSystem.initialised = false;
QSystem.network = isClient() or false;
local npc_plugin = {};
local jm_required = 1.05;

QSystem.preview = false; -- enables bleeding edge features, that may be undocumented, unstable and a subject to change in future.
...
QSystem.init = function()
	if isServer() then
		if JM.require(jm_required) then
			...
			if npc_plugin[1] then
				print("[QSystem] NPC Plugin(s) - "..table.concat(npc_plugin, ", "));
			else
...
QSystem.validate = function(plugin_id)
	for i=1, #npc_plugin do
		if npc_plugin[i] == plugin_id then
			return true;
		end
	end

	npc_plugin[#npc_plugin+1] = plugin_id;
	return true;
end
...
QSystem.load = function(args)
	...
	if isClient() then
		...
		if not (getPlayer() and getPlayer():getRole():hasAdminTool()) then
...
QSystem.OnClientCommand = function(module, command, player, args)
	...
	if QSystem.initialised then
		...
		elseif command == 'saveData' then
			if args[3] then
				if not player:getRole():hasAdminTool() then
		...
		elseif command == 'requestItem' then
			if type(args[2]) == 'table' then
				print("[QSystem] Received addItem request from client '"..tostring(username).."'. Info: "..tostring(args[1]));
		...
		elseif command == 'removeItem' then
			if type(args[2]) == 'table' then
				print("[QSystem] Received removeItem request from client '"..tostring(username).."'. Info: "..tostring(args[1]));
				if QItemFactory.remove(player, args[2]) then
					if #args == 3 then
						sendServerCommand(player, 'QSystem', 'callback', {args[3]});
					end
				end
		...
		elseif command == 'addXP' then
			if type(args[2]) == 'table' then
				print("[QSystem] Received EXP request from client '"..tostring(username).."'. Info: "..tostring(args[1]));
				if Kamisama.sendXP(player, username, args[2]) then
					if #args == 3 then
						sendServerCommand(player, 'QSystem', 'callback', {args[3]});
					end
				end
		...
		elseif command == 'spawnVehicle' then
			if type(args[2]) == 'table' then
				print("[QSystem] Received spawnVehicle request from client '"..tostring(username).."'. Info: "..tostring(args[1]));
				if Kamisama.spawnVehicle(player, args[2][1], args[2][2], args[2][3], args[2][4], args[2][5], args[2][6], args[2][7]) then
					if #args == 3 then
						sendServerCommand(player, 'QSystem', 'callback', {args[3]});
					end
				else
					print("[QSystem] Can't proceed spawnVehicle request from client '"..tostring(username).."'. Info: "..tostring(args[1]));
				end
			end
		elseif command == 'verify' then
			...
			if not player:getRole():hasAdminTool() then
				...
				print("[QSystem] Access level: "..tostring(player:getRole():getName())..". All good!");
			end
		end
	else
