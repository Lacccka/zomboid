...
function GetTextByTag(args)
    ...
    elseif size == 2 then
        ...
        elseif args[1] == "math" then
            local formula = args[2]:replace("screen_width", tostring(SRN.canvas[1])):replace("screen_height", tostring(SRN.canvas[2]));
            if string.match(formula, "^[%d+-/*^() ]+$") then
                local result = loadstring("return "..tostring(formula))();
                if result then
                    return tostring(result);
                end
            end
            return string.format("{incorrect formula '%s'}", tostring(formula));
        end
    elseif size == 1 then
...
local function message(self, sender)
    self:debug();
    local size = #self.args
    local speech = false;
    if size == 1 then
        sender.message.queue = self:queue(self.args[1], sender);
        if sender.append == false then
            sender:clearAvatar();
        end
    else
        if size == 3 then
            ...
            elseif result == 3 then
                return "Unsupported avatar type";
            end
        else
            sender:clearAvatar();
        end
        sender.message.queue = self:queue(" <RGB:1.0,0.9,0.7> ["..self.args[1].."] <LINE> <RGB:1,1,1> "..self.args[size], sender);
        speech = true;
        sender.append = false;
    end
...
function m:queue(text, sender)
    local function format(args)
        if #args == 2 then
            if args[1] == "wait" then
                local status, time = pcall(tonumber, args[2])
                if status and time then
                    local function callback()
                        SSRTimer.add_s(function () sender.message.queue[1][3] = true; end, time, false)
                    end
                    return callback;
                end
            elseif args[1] == "wait_ms" then
                local status, time = pcall(tonumber, args[2])
                if status and time then
                    local function callback()
                        SSRTimer.add_ms(function () sender.message.queue[1][3] = true; end, time, false)
                    end
                    return callback;
                end
            elseif args[1] == "bgm" then
                local file = tostring(args[2]);
                local function callback()
                    if AudioManager.playBGM(file, true, false) then
                        QuestLogger.print("[QSystem*] ${bgm_start}: Playing BGM - "..file);
                    end
                    sender.message.queue[1][3] = true;
                end
                return callback;
            elseif args[1] == "sfx" then
                local file = tostring(args[2]);
                local function callback()
                    if AudioManager.playSound(file) then
                        QuestLogger.print("[QSystem*] ${sfx}: Playing SFX - "..file);
                    end
                    sender.message.queue[1][3] = true;
                end
                return callback;
            elseif args[1] == "voice" then
                local file = tostring(args[2]);
                local function callback()
                    if AudioManager.playVoice(file) then
                        QuestLogger.print("[QSystem*] ${voice}: Playing Voice - "..file);
                        sender.voice = file;
                        sender.replay:setVisible(sender.voice ~= nil);
                    end
                    sender.message.queue[1][3] = true;
                end
                return callback;
            elseif args[1] == "stop" then
                local channel = (args[2] == "sfx" and 0) or (args[2] == "bgm" and 1) or (args[2] == "voice" and 2);
                if channel then
                    local function callback()
    ...
    while string.len(text) > 0 do
        ...
        if tag_start and tag_end then
            strings[p] = { p, string.sub(text, 1, tag_start-1) };
            p = p + 1;
            ...
            if another_tag and another_tag < tag_end then
                strings[p] = { p, string.sub(text, 1, another_tag-1) };
                text = string.sub(text, another_tag);
            else
                local tag = string.sub(text, tag_start, tag_end);
                local result = format(tag:ssplit(','));
                if result then
                    strings[p] = { p, result };
                else
                    strings[p] = { p, string.sub(text, 1, tag_end+1) };
                end
...
function m:execute(sender)
    local r = message(self, sender);
    if type(r) == "string" then return r; end
    sender.message.queue.speech = r;
    if sender.append == false then
        sender.message.text = "";
        sender.message:paginate(false, false);
    end
    if sender.script.index < #sender.script.lines and sender.script.layer < GetBlockLayer(sender.script.lines[sender.script.index+1]) then
        sender.message.callback = function ()
...
-- (up to six choices)
local lock_icon = getTexture("media/ui/QSystem/lock_icon.png");
...
function choice:execute(sender)
    ...
    for i=1, #choices do
        if i < 7 then
            if choices[i].hidden and choices[i].locked then
                hidden = hidden + 1;
            else
                ...
                if choices[i].locked then -- set lock on choice if flag is not raised
                    ...
                    if not sender.buttons[i].borderColorEnabled then
                        ...
                        sender.buttons[i].backgroundColorEnabled = { r = sender.buttons[i].backgroundColor.r, g = sender.buttons[i].backgroundColor.g, b = sender.buttons[i].backgroundColor.b, a = sender.buttons[i].backgroundColor.a };
                    end
                    sender.buttons[i]:setTextureRGBA(0.5, 0.5, 0.5, 0.7)
                    sender.buttons[i]:setBorderRGBA(0.5, 0.5, 0.5, 0.7)
                    sender.buttons[i]:setBackgroundRGBA(0, 0, 0, 1)
                else
        ...
        if task then
            ...
            for group_id=1, #groups do
                ...
                for i=1, #flags do
                    ...
                    elseif flags[i] == "locked" then
                        if quest.unlocked and task.unlocked then
                            group_status = false;
                            break;
                        end
                    elseif flags[i] == "unlocked" then
                        if not quest.unlocked or not task.unlocked then
                            group_status = false;
                            break;
                        end
                    else
...
function add_item:execute(sender)
    SaveManager.save(false, false, true);
    self:debug();
    local items = {}
...
function remove_item:execute(sender)
    ...
    for i=1,#self.args do
        ...
        local ids = {}
        local items = inventory:getSomeType(self.args[i][1], self.args[i][2]);
        if items then
            ...
            if ids[1] then
                ...
                local info = string.format("remove_item, %s, %s, line %d", sender.script.mod, sender.script.file, sender.script.index);
                SyncClient.request('QSystem', 'removeItem', ids, info, callback);
                return -2;
            end
        end
    end
...
function set_bg:execute(sender)
    ...
    if status and texture then
        VFX.SpriteRenderer.setBackground(texture);
    else
...
function set_fg:execute(sender)
    ...
    useTextureFiltering(false)
    if status and texture then
        VFX.SpriteRenderer.setForeground(texture);
    else
        return "Unable to load image";
    end
end
...
-- show_on_map characterA,1|characterB,0
local show_on_map = Command:derive("show_on_map")
function show_on_map:execute(sender)
    self:debug();
    for i=1, #self.args do
        local arg = self.args[i]:ssplit(',');
        local character_id = CharacterManager.instance:indexOf(arg[1]);
        if character_id then
			if arg[2] == 'true'  then
				CharacterManager.instance.items[character_id]:setShowOnMap(true)
			elseif arg[2] == 'false' then
				CharacterManager.instance.items[character_id]:setShowOnMap(false)
			else
				return "Invalid syntax";
			end
		else
...
CommandList_a[#CommandList_a+1] = show_on_map:new("show_on_map", 1, nil, type_dialogue);
...
function fade_out:execute(sender)
    ...
    if self.args[3] == "true" or self.args[3] == "1" then
        VFX.Dissolve.setFadeOut(nil, color, speed);
    else
        if VFX.Dissolve.setFadeOut(callback, color, speed) then
...
function fade_in:execute(sender)
    ...
    if self.args[2] == "true" or self.args[2] == "1" then
        VFX.Dissolve.setFadeIn(nil, speed);
    else
        if VFX.Dissolve.setFadeIn(callback, speed) then
            return -2; -- wait for callback
        end
    end
end
...
function look_at:execute(sender)
    ...
    if square then
        if self.args[2] == "true" then
            getPlayer():faceLocationF(coords[1], coords[2]);
...
function apply_stat:execute(sender)
    ...
    for i=1, #self.args do
        ...
        local stat = CharacterStat.getById(args[1]);
        if not stat then
            return "Unknown stat '"..tostring(args[1]).."'";
        end
...
function is_trait:execute(sender)
    ...
    for i=1, #self.args do
        local arg = self.args[i]:ssplit(',');
        local trait = Kamisama.getTraitById(arg[1]);
        if trait then
            local status = getPlayer():hasTrait(trait:getType());
            if arg[2] == 'true' then
                arg[2] = true;
            elseif arg[2] == 'false' then
                arg[2] = false;
...
-- create_vehicle type|x,y,z|d
-- create_vehicle type|x,y,z|d|skin
-- create_vehicle type|x,y,z|d|skin|keyName
local create_vehicle = Command:derive("create_vehicle")
function create_vehicle:execute(sender)
    self:debug();
    local v_type = tostring(self.args[1]);
    local script = getScriptManager():getVehicle(v_type)
    if script then
        ...
        if not Kamisama.getIsoDirectionFromString(self.args[3]) then
            return string.format("Invalid direction '%s'", self.args[3]);
        end

        local skin, keyName = -1, nil;
        if self.args[4] then
            if self.args[4] ~= "-1" then
                local status, _skin  = pcall(tonumber, self.args[4]);
                if status and skin then
                    skin = (_skin >= 0 and _skin <= script:getSkinCount()) and _skin or -1;
                else
        ...
        local info = string.format("create_vehicle, %s, %s, line %d", sender.script.mod, sender.script.file, sender.script.index);
        Kamisama.requestVehicle(info, v_type, coords[1], coords[2], coords[3], self.args[3], skin, keyName or false, callback);
    else
        return string.format("Invalid vehicle type '%s'", v_type);
    end

    return -2;
end

CommandList_a[#CommandList_a+1] = create_vehicle:new("create_vehicle", 3, 5, type_dialogue);
...
-- wait_ms millis
local wait_ms = Command:derive("wait_ms")
function wait_ms:execute(sender)
    self:debug();
    local status, time = pcall(tonumber, self.args[1])
    if status and time then
        local function callback()
        ...
        SSRTimer.add_ms(callback, time, false);
        return -2;
    else
        return "Invalid time";
    end
...
CommandList_a[#CommandList_a+1] = wait_ms:new("wait_ms", 1, nil, type_dialogue);
...
CommandList_a[#CommandList_a+1] = is_lua:new("is_lua", 2, nil, {type_dialogue, type_character_desc});
...
local function list_rewards(args, reward_select)
    ...
    for i=1, #args do
        ...
        if entry[1] == "EXP" and (size > 1 and size < 5) then
            local perk = Perks.FromString(entry[2]);
            if perk and perk:toString() ~= "MAX" then
        ...
        elseif size == 1 or size == 2 then
            if Kamisama.getTraitById(entry[1]) then
                if reward_select then
...
function reward_select:execute(sender)
    ...
    for i=1, #traits do
        lots[#lots+1] = Reward.Card:createTrait(Kamisama.getTraitById(traits[i].name));
            ...
            if task then
                if task.type == "Deliver" then
                    if task.unlocked and not task.pending and not task.completed then
                        ...
                        sender:setVisible(false);
                        local fetcher = ItemFetcher.UI:new(quest.index, task.index, onConfirm, onCancel)
                        fetcher:initialise();
