    ...
    if progress[1] then
        ...
        for character_id=1, CharacterManager.instance.items_size do
            for i=1, #progress[1] do
                if CharacterManager.instance.items[character_id].file == progress[1][i].file and (CharacterManager.instance.items[character_id].mod == progress[1][i].mod or (not progress[1][i].mod:find("\\") and CharacterManager.instance.items[character_id].mod:ends_with(progress[1][i].mod))) and CharacterManager.instance.items[character_id].name == progress[1][i].name then
                    -- stats
                    ...
                    -- showOnMap
                    if progress[1][i].showOnMap == 1 then
                        CharacterManager.instance.items[character_id].showOnMap = true;
                    elseif progress[1][i].showOnMap == 0 then
                        CharacterManager.instance.items[character_id].showOnMap = false;
                    end
    ...
    if progress[2] then
        ...
        for quest_id=1, QuestManager.instance.quests_size do
            ...
            for b=1, #progress[2] do
                if progress[2][b].file == QuestManager.instance.quests[quest_id].file and (progress[2][b].mod == QuestManager.instance.quests[quest_id].mod or (not progress[2][b].mod:find("\\") and QuestManager.instance.quests[quest_id].mod:ends_with(progress[2][b].mod))) then
...
SaveManager.save = function (forced, debug, undelayed)
    if forced then
        for i=1, #SaveManager.flags do
    ...
    if SaveManager.busy then
        if undelayed then
            SSRTimer.run(SaveManager.busy);
        end
    else
        local function save()
        ...
        if debug or undelayed then
            save();
        else
            SaveManager.busy = SSRTimer.add_s(save, 1, false);
        end
    end
end
...
SaveManager.data = function(debug) -- returns serialized player data
    ...
    if SaveManager.flags[1] then
        ...
        for i=1, CharacterManager.instance.items_size do
            ...
            char.showOnMap = CharacterManager.instance.items[i].showOnMap and 1 or 0;
            char.stats = CharacterManager.instance.items[i].stats;
            progress[i] = char;
        end
        data[1] = serpent.dump(progress):replace("\\", "\\\\"); -- FIXME: temp solution for mods not loading due to missing slash after deserialization
    ...
    if SaveManager.flags[2] then
        ...
        data[2] = serpent.dump(progress):replace("\\", "\\\\"); -- FIXME: temp solution for mods not loading due to missing slash after deserialization
