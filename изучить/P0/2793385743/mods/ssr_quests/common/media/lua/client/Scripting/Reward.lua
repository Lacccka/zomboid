...
function Reward.Card:prerender()
	...
	self:drawText(self.name, 16*SSRLoader.scale, 17*SSRLoader.scale, 1.0, 1.0, 1.0, self.alpha, UIFont.Medium);
...
function Reward.Card:createItem(item, amount, grade)
	...
	if grade == 4 then -- unique
		o.body = getTexture("media/ui/QSystem/loot_unique.png");
    elseif grade == 3 then -- special
		o.body = getTexture("media/ui/QSystem/loot_special.png");
	elseif grade == 2 then -- rare
		o.body = getTexture("media/ui/QSystem/loot_rare.png");
	else -- common
		o.body = getTexture("media/ui/QSystem/loot_common.png");
	end
...
function Reward.Card:createEXP(perk, exp)
	...
	o.amount = exp;
	o.texture = getTexture("media/ui/QSystem/perk_icon.png");
...
function Reward.Notification:new(cards, callback)
	...
	if cards[1].type == 2 then
		path = "media/ui/QSystem/labels/label_reward_exp_";
	else
		path = "media/ui/QSystem/labels/label_reward_"..(cards[1].type == 0 and "item" or "trait")..(#cards > 1 and "s" or "").."_";
	end
...
function Reward.Selection:new(rewards, callback)
	...
	local language, status = tostring(Translator.getLanguage());
	status, o.tex_title = pcall(getTexture, "media/ui/QSystem/labels/label_select_reward_"..language..".png");
	if not status or not o.tex_title then
		o.tex_title = getTexture("media/ui/QSystem/labels/label_select_reward_en.png");
	end
	o.tex_title_w, o.tex_title_h = o.tex_title:getWidth()*SSRLoader.scale, o.tex_title:getHeight()*SSRLoader.scale;

	status, o.tex_selected = pcall(getTexture, "media/ui/QSystem/labels/label_selected_"..language..".png");
	if not status or not o.tex_selected then
		o.tex_selected = getTexture("media/ui/QSystem/labels/label_selected_en.png");
	end
	o.tex_selected_w, o.tex_selected_h = o.tex_selected:getWidth()*SSRLoader.scale, o.tex_selected:getHeight()*SSRLoader.scale;

	status, o.tex_bonus = pcall(getTexture, "media/ui/QSystem/labels/label_bonus_"..language..".png");
	if not status or not o.tex_bonus then
		o.tex_bonus = getTexture("media/ui/QSystem/labels/label_bonus_en.png");
	end
