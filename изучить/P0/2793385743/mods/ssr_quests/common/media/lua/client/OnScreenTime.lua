...
function DrawQSystemDebugInfo()
    ...
    if player then
        local text = string.format("SSR: Quest System (%s Build)\r\nX=%i Y=%i Z=%i\r\n", QSystem.preview and "Test" or "Stable", math.floor(player:getX()), math.floor(player:getY()),  math.floor(player:getZ()));
...
Events.OnQSystemInit.Add(function ()
    ...
    if isClient() then
        if getPlayer() and getPlayer():getRole():hasAdminTool() then
