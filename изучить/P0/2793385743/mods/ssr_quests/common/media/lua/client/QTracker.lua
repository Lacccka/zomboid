...
local font_height_m = getTextManager():getFontHeight(UIFont.Medium)
local font_height_s = getTextManager():getFontHeight(UIFont.NewSmall)
...
local min_width = 220 * SSRLoader.scale;
...
QTracker.update = function()
	if QTracker.ActiveQuest then
		if quest_title then -- quest name render
			local title_x = (math.floor(UIManager.getSpeedControls():getX()) - QTracker.offset - 20);
				...
				if task_width > QTracker.offset then -- if task name is too long, word wrap
					if task_lines then
						for i=1, #task_lines do
							...
							T_MANAGER:DrawString(UIFont.NewSmall, task_x, font_height_m + (5 + ((i-1) * 15))*SSRLoader.scale, task_lines[i], 1, 1, 1, 1);
						end
					else
					...
					T_MANAGER:DrawString(UIFont.NewSmall, task_x, font_height_m + 5*SSRLoader.scale, task_name, 1, 1, 1, 1);
				end
