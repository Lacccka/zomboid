...
function QuestDebugger:onTickLoadProgress(index, selected)
    if index == 1 then
        self.checkbox_load.selected[1] = selected;
    end
end
...
local function reimport()
    ...
    QImport.reimport(QuestDebugger.instance.checkbox_load.selected[1] and loadState or QSystem.resume);
end
...
function QuestDebugger:createChildren()
    ...
    self.checkbox_save = ISTickBox:new(10*SSRLoader.scale, offset_y + 24*SSRLoader.scale, 200*SSRLoader.scale, 20*SSRLoader.scale, "", self, QuestDebugger.onTickSaveProgress);
    ...
    self.checkbox_load = ISTickBox:new(10*SSRLoader.scale, offset_y + 24*SSRLoader.scale*2, 200*SSRLoader.scale, 20*SSRLoader.scale, "", self, QuestDebugger.onTickLoadProgress);
	self.checkbox_load:initialise()
    self:addChild(self.checkbox_load)
    self.checkbox_load:addOption(getTextOrNull("UI_QDebugger_Checkbox_LoadProgress") or "AutoLoad");
    self.checkbox_load.selected[1] = false;
    self.checkbox_load.tooltip = getTextOrNull("UI_QDebugger_Tooltip_LoadProgress") or "Load state after reimport";
    ...
    self.bForceUpload = createButton(offset_x, offset_y, 105*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_ForceUpload") or "Force Upload", forceUpload)
    ...
    self.bStartScript = createButton(offset_x, offset_y+25*SSRLoader.scale, 222.5*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_StartDialogue") or "Start dialogue", createDialoguePanel)
	self:addChild(self.bStartScript);
    offset_x = offset_x + self.bForceUpload.width + 11*SSRLoader.scale;

    self.bSaveState = createButton(offset_x, offset_y, 105*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_SaveState") or "Save state", saveState)
    ...
    self.bResetScript = createButton(offset_x, offset_y+25*SSRLoader.scale, 222.5*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_EndDialogue") or "End dialogue", closeDialoguePanel)
	self:addChild(self.bResetScript);

    self.bLoadState = createButton(offset_x, offset_y, 105*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_LoadState") or "Load state", loadState)
    ...
    self.bReloadAll = createButton(offset_x, offset_y, 105*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_Reimport") or "Reimport", reimport)
	...
	offset_y=offset_y+self.checkbox_verbose:getHeight()+self.checkbox_save:getHeight()+self.checkbox_load:getHeight()+15*SSRLoader.scale;
    ...
    self.questList = createScrollingListBox(self.charList.x + self.charList.width + 5*SSRLoader.scale, offset_y, 145*SSRLoader.scale, 200*SSRLoader.scale);
    ...
    self.taskList = createScrollingListBox(self.questList.x + self.questList.width + 5*SSRLoader.scale, offset_y, 145*SSRLoader.scale, 200*SSRLoader.scale);
    ...
    self.actionList = createScrollingListBox(self.taskList.x + self.taskList.width + 5*SSRLoader.scale, offset_y, 155*SSRLoader.scale, 200*SSRLoader.scale);
    ...
    self.defaultPanel.flags = createScrollingListBox(10*SSRLoader.scale, 10*SSRLoader.scale, 220*SSRLoader.scale, self.height - y - 60*SSRLoader.scale);
    self.defaultPanel.flags.doDrawItem = drawDatas;
	self.defaultPanel:addChild(self.defaultPanel.flags);

    self.defaultPanel.deleteFlag = ISButton:new(10*SSRLoader.scale, self.defaultPanel.flags.height + 20*SSRLoader.scale, 220*SSRLoader.scale, 20*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_RemoveFlag") or "Remove Flag", self, removeFlag);
    ...
    self.defaultPanel.info = ISRichTextPanel:new(240*SSRLoader.scale, 10*SSRLoader.scale, 200*SSRLoader.scale, 35*SSRLoader.scale);
    ...
    self.defaultPanel.events = createScrollingListBox(240*SSRLoader.scale, 50*SSRLoader.scale, 200*SSRLoader.scale, self.height - y - 100*SSRLoader.scale);
    ...
    self.defaultPanel.deleteEvent = ISButton:new(240*SSRLoader.scale, self.defaultPanel.events.y + self.defaultPanel.events.height + 10*SSRLoader.scale, 200*SSRLoader.scale, 20*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_RemoveEvent") or "Remove Event", self, removeEvent);
    ...
    self.characterPanel.cb = ISTickBox:new(10*SSRLoader.scale, self.characterPanel.info.y+self.characterPanel.info.height+5*SSRLoader.scale, 210*SSRLoader.scale, 16*SSRLoader.scale, "", self, QuestDebugger.onTickCharProps);
	self.characterPanel.cb:initialise()
	self.characterPanel:addChild(self.characterPanel.cb)
	self.characterPanel.cb:addOption("Alive");
	self.characterPanel.cb:addOption("Revealed");

    self.characterPanel.stats = createScrollingListBox(240*SSRLoader.scale, 10*SSRLoader.scale, 200*SSRLoader.scale, self.height - y - 60*SSRLoader.scale);
    ...
    self.characterPanel.decreaseStat = ISButton:new(240*SSRLoader.scale, stat_button_y, 95*SSRLoader.scale, 20*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_DecreaseStat") or "Decrease", self, decreaseStat);
    ...
    self.characterPanel.increaseStat = ISButton:new(345*SSRLoader.scale, stat_button_y, 95*SSRLoader.scale, 20*SSRLoader.scale, getTextOrNull("UI_QDebugger_Button_IncreaseStat") or "Increase", self, increaseStat);
    ...
    self.questPanel.flags = ISTickBox:new(355*SSRLoader.scale, 10*SSRLoader.scale, 130*SSRLoader.scale, 20*SSRLoader.scale, "", self, QuestDebugger.onQuestFlagsChange);
    ...
    self.taskPanel.flags = ISTickBox:new(355*SSRLoader.scale, 10*SSRLoader.scale, 130*SSRLoader.scale, 20*SSRLoader.scale, "", self, QuestDebugger.onTaskFlagsChange);
    ...
    self.actionPanel.flags = ISTickBox:new(355*SSRLoader.scale, 10*SSRLoader.scale, 130*SSRLoader.scale, 20*SSRLoader.scale, "", self, QuestDebugger.onActionFlagsChange);
...
function QuestDebugger:new()
    local w, h = 620*SSRLoader.scale, 520*SSRLoader.scale;
