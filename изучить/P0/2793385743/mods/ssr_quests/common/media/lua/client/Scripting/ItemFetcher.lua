...
ItemFetcher.getNumberOfItem = function(item_id, maximum, ruleset_id) -- used by #add_item and #remove_item commands
...
local function isInsideBag(item)
	...
	if bag and (ItemFetcher.UI.instance.player:isEquipped(bag) or ItemFetcher.UI.instance.player:isEquippedClothing(bag)) then
		return true;
	end
	return false;
end
...
local function validate(item)
	if not ItemFetcher.UI.instance.inventory:contains(item) and not isInsideBag(item) then
		return false;
	elseif ItemFetcher.UI.instance.player:isEquipped(item) or ItemFetcher.UI.instance.player:isEquippedClothing(item) then
		return false;
...
local function purge() -- checks if offerred items are still following rulesets
	if ItemFetcher.UI.instance then
		for i=#ItemFetcher.UI.instance.offerings.items, 1, -1 do
			local j = ItemFetcher.UI.instance.offerings.items[i].r;
			if ItemFetcher.UI.instance.requirements.items[j].item.item then
				if ItemFetcher.UI.instance.requirements.items[j].item.ruleset and not ItemFetcher.validate(ItemFetcher.UI.instance.offerings.items[i].item, ItemFetcher.UI.instance.requirements.items[j].item.ruleset) then
					ItemFetcher.UI.instance:removeItem(i);
				end
			elseif not ItemFetcher.validate(ItemFetcher.UI.instance.offerings.items[i].item, ItemFetcher.UI.instance.requirements.items[j].item.ruleset) then
				ItemFetcher.UI.instance:removeItem(i);
			end
		end
	end
...
function ItemFetcher.UI:populateList()
	...
	for item_id=1, #self.task.items do
		...
		self.requirements.items[#self.requirements.items].collected = self.task.extdata[item_id];
	end
end
...
function ItemFetcher.UI:onButtonPressed(button)
	...
	elseif button.internal == 1 then -- confirm
		...
		if button.enable then purge() end
		if button.enable then
			if QuestManager.instance.quests[self.quest_id] and QuestManager.instance.quests[self.quest_id].tasks[self.task_id] then
				local ids = {};
				for i=1, #self.offerings.items do
					if isClient() then
						ids[#ids+1] = self.offerings.items[i].item:getID();
					else
						local container = self.offerings.items[i].item:getContainer();
						container:Remove(self.offerings.items[i].item);
						sendRemoveItemFromContainer(container, self.offerings.items[i].item);
					end
				end
				...
				local function callback()
					for i=1, #self.requirements.items do
						QuestManager.instance.quests[self.quest_id].tasks[self.task_id]:count(i, self.requirements.items[i].item, self.requirements.items[i].collected);
					end
					if QuestManager.instance.quests[self.quest_id].tasks[self.task_id]:check() then
				...
				if ids[1] then
					SyncClient.request('QSystem', 'removeItem', ids, string.format("deliver, %s, %s, %s, %s", self.quest.mod, self.quest.file, self.quest.internal, self.task.internal), callback);
				else
					callback();
				end
			else
				print("[QSystem] (Error) Unexpected error in ItemFetcher UI");
			end
		end
...
function ItemFetcher.UI:updateButtons()
	...
	self.confirm:setEnable(self.offerings.items[1] ~= nil);
end
...
function ItemFetcher.UI:removeItem(index)
	...
	self.requirements.items[self.offerings.items[index].r].collected = self.requirements.items[self.offerings.items[index].r].collected - 1;
...
function ItemFetcher.UI:new(quest_id, task_id, onConfirm, onCancel)
	...
	o.quest_id = quest_id; o.task_id = task_id;
	o.quest = QuestManager.instance.quests[quest_id];
	o.task = QuestManager.instance.quests[quest_id].tasks[task_id];
