...
RTPN = ISPanel:derive("RTPN");

RTPN.drawMargins = false;


function RTPN:initialise()
	ISPanel.initialise(self);
end

function RTPN:setText(text)
	self.text = text or ""
end

function RTPN:processCommand(command, x, y, lineImageHeight, lineHeight)
    ...
    elseif command == "TEXT" then
        ...
        self.fonts[self.currentLine] = self.font;
    elseif command == "CENTRE" then
        self.orient[self.currentLine] = "centre";
    elseif command == "LEFT" then
        self.orient[self.currentLine] = "left";
    elseif command == "RIGHT" then
        self.orient[self.currentLine] = "right";
    elseif string.find(command, "PUSHRGB:") then
...
--~         print(size);
		...
		self.fonts[self.currentLine] = self.font;
	elseif string.find(command, "INDENT:") then
        self.indent = tonumber(string.sub(command, 8))
    elseif string.find(command, "SETX:") then
        x = tonumber(string.sub(command, 6))
    elseif string.find(command, "SPACE") then
        if x > 0 then
            x = x + getTextManager():MeasureStringX(self.font, " ") + 2
        end
    --[[elseif string.find(command, "NAME:") then
		local name = string.sub(command,6, string.len(command));
		self.lines[1] = name;
		self.rgb[1] = {r=1, g=0.9, b=0.7};
		y = y + lineHeight + lineHeight;--]]
	end
...
function RTPN:replaceKeyName(text, offset)
...
function RTPN:replaceKeyNames(text)
	local offset = 1
	while true do
...
function RTPN:onMouseWheel(del)
	self:setYScroll(self:getYScroll() - (del*18));
    return true;
end


function RTPN:paginate(speech, append)
	local lines = 1;
	...
	if not self.lines or not append then
		self.lines = {};
		self.line = 1;
		self.char = 1;
	end
...
--			cur = cur - 1
			...
			if string.find(token, "<") and string.find(token, ">") then
				...
				local chunkText = self.buffer[lines] or ''
				...
				if chunkX + pixLen > maxLineWidth then
					if self.buffer[lines] and self.buffer[lines] ~= '' then
						lines = lines + 1;
					end
					local lineHeight = getTextManager():getFontFromEnum(self.font):getLineHeight();
					if lineHeight < lineImageHeight then
					...
					y = y + lineHeight;
					x = 0;
					self.buffer[lines] = string.trim(token)
					if self.buffer[lines] ~= "" then
						x = self.indent
					end
					self.lineX[lines] = x
					self.lineY[lines] = y
					x = x + getTextManager():MeasureStringX(self.font, self.buffer[lines])
				else
					if not self.buffer[lines] then
						self.buffer[lines] = ''
						self.lineX[lines] = x
					...
					self.buffer[lines] = chunkText
					if self.lineX[lines] == 0 and self.buffer[lines] ~= "" then
			...
			if string.trim(leftText) ~= '' then
				...
				self.buffer[lines] = string.trim(str);
				if x == 0 and self.buffer[lines] ~= "" then
					x = self.indent
				end
				self.lineX[lines] = x;
				self.lineY[lines] = y;
				local lineHeight = getTextManager():getFontFromEnum(self.font):getLineHeight();
				y = y + lineHeight
			elseif self.buffer[lines] and self.buffer[lines] ~= '' then
				local lineHeight = getTextManager():getFontFromEnum(self.font):getLineHeight();
				if lineHeight < lineImageHeight then
					lineHeight = lineImageHeight;
				end
	...
	if self.speech then
		self.lines[1] = self.buffer[1];
		self.line = self.line + 1;
	end
end

function RTPN:setContentTransparency(alpha)
    self.contentTransparency = alpha;
end

function RTPN:render()
...
function RTPN:onResize()
	--  ISUIElement.onResize(self);
	if self.width ~= self:getWidth() or self.height ~= self:getHeight() then
...
function RTPN:setMargins(left, top, right, bottom)
...
function RTPN:skipAnimation()
	if self.buffer then
		local size = #self.buffer
		if size ~= 0 then
			for i=1, size do
				self.lines[i] = self.buffer[i];
			end
			self.line = size;
			self.char = string.len(self.lines[self.line]);
		end
		self.buffer = nil;
	end
end

function RTPN:prerender()
	if self.buffer then
		self.lines[self.line] = self.buffer[self.line]:sub(1, self.char);
		if self.lines[self.line] == self.buffer[self.line] then
			if self.line < #self.buffer then
				self.line = self.line + 1;
				self.char = 1;
			else
				self.buffer = nil;
			...
			self.char = self.char + 5;
		end
	end
end

function RTPN:update()
	if self.queue[1] then
		if self.queue[1][1] then
			if type(self.queue[1][2]) == "function" then
				DialoguePanel.instance.input.enable = false;
				self.queue[1][2]();
			elseif type(self.queue[1][2]) == "string" then
				local append = DialoguePanel.instance.append or self.queue[1][1] ~= 1;
				self.text = append and self.text..self.queue[1][2] or self.queue[1][2];
				self:paginate(self.queue[1][1] == 1 and self.queue.speech, append);
				self.queue[1][3] = true; -- remove entry asap
				DialoguePanel.instance.input.enable = true;
			end
			self.queue[1][1] = false;
		elseif self.queue[1][3] and not self.buffer then
			table.remove(self.queue, 1);
		end
	elseif self.callback and not self.buffer then
		local callback = self.callback;
		SSRTimer.add_ms(callback, 0, false);
		self.callback = nil;
	end
end


function RTPN:new (x, y, width, height)
	local o = {}
