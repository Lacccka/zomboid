...
local alpha = 0;
local reverse = false;
function DialoguePanel:render()
	ISPanel.render(self);
	if self:isVisible() and not self.message.buffer and self.input.enable then
		if reverse then
			alpha = alpha - 0.03 * UIManager.getMillisSinceLastRender() / 33.3;
			if alpha < 0.1 then
				alpha = 0.1;
				reverse = false;
			end
		...
		self:drawTexture(self.next_tex,self.width - 32, self.height - 32, alpha, 1, 1, 1);
	end
end

function DialoguePanel:onClick()
	if self.execute or self.message.queue[1] then
		if self.message.buffer then
			self.message:skipAnimation();
...
function DialoguePanel:showError(err_msg)
	...
	self.message:skipAnimation();
	print(err_msg);
	SSRTimer.add_ms(function () self.input.enable = true; end, 250, false);
end
	...
	if self.active then
		...
		while self.execute do
			...
			elseif result == -2 then -- wait for callback
				...
				SaveManager.save();
				return;
			elseif result == -3 then -- wait for callback (#m)
				break;
...
function DialoguePanel:setSprite(sprite)
	sprite = sprite:starts_with("media/ui/") and sprite or "media/ui/"..sprite;
	useTextureFiltering(true);
	local status, tex = pcall(getTexture, sprite);
...
function DialoguePanel:setAvatar(obj)
	if self.sprite == obj then
		...
		if obj:starts_with("3D:") then
			...
			return self:setSprite(tostring(obj)) or 1;
		end
	end
end
...
function DialoguePanel:createChildren()
	...
	self.message = RTPN:new(0, 0, self.window:getWidth(), 80*SSRLoader.scale);
	...
	self.input.backgroundColorMouseOver = {r=0, g=0, b=0, a=0.0};
	...
	self.replay.normal = getTexture("media/ui/QSystem/voice_btn_normal.png");
	self.replay.hover = getTexture("media/ui/QSystem/voice_btn_hover.png");
...
function DialoguePanel:close()
	...
	getCore():setOptionUIRenderFPS(UIRenderFPS);
...
function DialoguePanel:new()
	...
	o.next_tex = getTexture("media/ui/QSystem/wait_for_click.png");
...
local function create_panel()
	UIRenderFPS = getCore():getOptionUIRenderFPS(); getCore():setOptionUIRenderFPS(30);
