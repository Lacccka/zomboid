...
-- ContextAction|Title|x,y,z|time,anim,sprite
...
function Task_ContextAction.create(internal, args)
	...
	if #args == 2 then
        ...
        if args[3][3] then
            local sprite;
            status, sprite = pcall(getSprite, args[3][3]);
            if not status or not sprite then return "Invalid sprite"; end
        end
		if #args == 5 then
			args[4] = args[4]:ssplit(',');
			local script, label = args[4][1], args[4][2];
			return Task_ContextAction:new(internal, args[1], coord[1], coord[2], coord[3], args[3][1], args[3][2] ~= "false" and args[3][2], args[3][3], script, label, args[5]);
		else
			return Task_ContextAction:new(internal, args[1], coord[1], coord[2], coord[3], args[3][1], args[3][2] ~= "false" and args[3][2], args[3][3]);
		end
	end
end
...
Task_ContextAction.onQSystemUpdate = function(code)
	if code == 4 then
		for i=1, #marker_ids do
			if marker_ids[i][2] then
				local marker = getIsoMarkers():getIsoMarker(marker_ids[i][1])
				if getIsoMarkers():removeIsoMarker(marker_ids[i][1]) then
					marker:removeTempSquareObjects()
					getWorldMarkers():removeHomingPoint(marker_ids[i][2]);
					QuestLogger.print("[QSystem*] Removed world marker with id '"..tostring(marker_ids[i]).."'. Task=ContextAction")
				end
			else
				if getWorldMarkers():removeGridSquareMarker(marker_ids[i][1]) then
					QuestLogger.print("[QSystem*] Removed world marker with id '"..tostring(marker_ids[i]).."'. Task=ContextAction")
				end
			end
		end
...
function Task_ContextAction:removeMarker()
	for i=#marker_ids, 1, -1 do
		if marker_ids[i][1] == self.marker:getID() then
			table.remove(marker_ids, i);
			break;
		end
	end
	if self.sprite then
		getIsoMarkers():removeIsoMarker(self.marker);
		self.marker:removeTempSquareObjects();
		self.marker = nil;
		self.marker_id = nil;
		getWorldMarkers():removeHomingPoint(self.arrow);
		self.arrow = nil;
		self.arrow_id = nil;
	else
		...
		if getTimeInMillis() > self.timer then
			if Task_ContextAction.isValidMarker(self.marker_id) then
				if self.sprite then
					...
					if square and IsoUtils.DistanceTo(square:getX(), square:getY(), self.x, self.y) < 3 and square:getZ() == self.z then
				...
				if square then
					if self.sprite then
						self.marker = getIsoMarkers():addIsoMarker({self.sprite}, {}, square, 1, 1, 1, true, true);
						self.marker_id = self.marker:getID();
						self.arrow = getWorldMarkers():addPlayerHomingPoint(getPlayer(), self.x - self.z*3, self.y - self.z*3, "arrow_triangle", 1, 1, 1, 0.6, true, 20); -- FIXME: решить проблему тайлов на верхних этажах
						self.arrow_id = self.arrow:getID();
						self.arrow:setActive(false);
						table.insert(marker_ids, {self.marker_id, self.arrow_id});
					else
					...
					for i=0,square:getObjects():size()-1 do
						...
						if object:getProperties() and object:getProperties():Is(IsoFlagType.canBeRemoved) then
							...
							self.ghostSprite:LoadFramesNoDirPageSimple("media/ui/QSystem/shiny.png");
							break;
						end
					end
				end
...
function Task_ContextAction:new(internal, text, x, y, z, time, animation, sprite, script, label, flag)
	local o = {}
	o = Task:new(internal);
	setmetatable(o, self)
	self.__index = self
...
local function getContainer()
	...
	if loot and loot.inventoryPane.inventory then
		local parent = loot.inventoryPane.inventory:getParent()
		if parent then
			return loot, parent:getContainer();
		end
	end
end
...
function Task_OpenContainer:update()
	if not self.pending then
		local loot, container = getContainer()
		if container then
...
function Task_LootContainer.create(internal, args)
	...
	if #coord == 3 then
		...
		return Task_LootContainer:new(internal, args[1], coord[1], coord[2], coord[3], items, tostring(QuestManager.instance.creator.quest.internal)..'_'..tostring(internal));
	else
		return "Invalid coordinates specified";
	end
end
...
function Task_LootContainer:trim()
	local m = getPlayer():getModData();
	if type(m.ispwn) ~= "table" then
		m.ispwn = {};
	end
	if type(m.ispwn[self.key]) == "table" then
		if self.extdata[1] and #self.extdata > #m.ispwn[self.key] then
			m.ispwn[self.key] = self.extdata;
		end
	elseif self.extdata[1] then
		m.ispwn[self.key] = self.extdata;
	else
		m.ispwn[self.key] = {};
		return self.list;
	...
	local function has_key(k)
		for i=1, #m.ispwn[self.key] do
			if k == m.ispwn[self.key][i] then return true end
		end
	end

	local list = {}
	for i=#self.list, 1, -1 do
		if not has_key(i) then
			list[#list+1] = self.list[i];
		end
	end
	return list;
end
		...
		if not self.spawned then
			...
			if container then
				if container:getType() == self.container then
					local square = container:getParent():getSquare()
					if square:getX() == self.x and square:getY() == self.y and square:getZ() == self.z then
						self.items = QItemSpawner.init(self, self:trim(), loot)
						self.spawned = true;
					end
				end
			end
...
function Task_LootContainer:new(internal, container, x, y, z, items, key)
	local o = {};
	o = Task:new(internal);
	setmetatable(o, self);
	self.__index = self;
...
function Task_Deliver:reload()
	if self.extdata and #self.extdata ~= self.items_size then
		self.extdata = nil;
	end
	Task.reload(self);
end

function Task_Deliver:reset()
	self.extdata = nil;
	Task.reset(self);
end

function Task_Deliver:lock()
	self.extdata = nil;
	Task.lock(self);
end

function Task_Deliver:count(index, item, collected)
	if self.extdata then
		if #self.extdata == self.items_size and self.items[index] == item then
			self.extdata[index] = collected;
			SaveManager.onQuestDataChange();
		else
			self.extdata = nil;
		end
...
function Task_Deliver:check()
	if self.extdata then
		if #self.extdata == self.items_size then
			for i=1, self.items_size do
				if self.extdata[i] < self.items[i].amount then
					return;
				end
			end
			self:setPending(true);
