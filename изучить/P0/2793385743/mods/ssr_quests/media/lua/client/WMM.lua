...
WMM.tex_target = getTexture("media/ui/QSystem/map/target.png");
useTextureFiltering(false);

if not WMM.tex_target then return end

function WMM.render(self, _x, _y, _tex)
    ...
    if api then
        ...
        self:drawTextureScaled(_tex or WMM.tex_target, x - _w, y - _w, w, w, 1, 1, 1, 1);
        self:clearStencilRect();
    end
end

function WMM.renderDot(self, _x, _y, r, g, b, a)
    local api = self.javaObject:getAPI();
    if api then
        local x = api:worldToUIX(_x, _y);
        local y = api:worldToUIY(_x, _y);
        local scale = 3;
        local w, _w = 20/scale, 20/scale;
        self:setStencilRect(0, 0, self.width, self.height)
        self:drawRect(x - _w, y - _w, w, w, a, r, g, b)
        self:clearStencilRect();
    end
end
