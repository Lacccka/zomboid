...
function Task:reset()
    ...
    QuestLogger.onStateChanged("reset", self.parent, self.index);
end
...
function Task:unlock(extdata)
    if not self.unlocked then
        ...
        set_active(self.quest_id, self.index); -- update the list of active tasks
        QuestLogger.onStateChanged("unlocked", self.parent, self.index);
    end
end
...
function Task:lock()
    if self.unlocked then
        ...
        triggerEvent("OnQSystemUpdate", 2);
        set_inactive(self.quest_id, self.index); -- update the list of active tasks
        QuestLogger.onStateChanged("locked", self.parent, self.index);
    end
end
...
function Task:complete()
    if not self.completed then
        ...
        set_inactive(self.quest_id, self.index); -- update the list of active tasks
        QuestLogger.onStateChanged("completed", self.parent, self.index);
    end
end
...
function Task:setPending(value)
    if self.pending ~= value then
        ...
        triggerEvent("OnQSystemUpdate", 2);
        SaveManager.save();
        QuestLogger.onStateChanged("set pending", self.parent, self.index);
    end
end
...
function Task:new(internal)
    ...
    o.parent = QuestManager.instance.creator.quest.index;
