...
-- All rights reserved
require "Scripting/AsyncTaskManager"
...
function Task_Import:update()
	...
	elseif self.step == 3 then
		if self.data_id > self.quests then
			...
			AsyncTaskManager.add(Task_LoadQuests:new(nil, QImport.scripts[self.entry_id].quest_data[self.data_id], QImport.scripts[self.entry_id].mod, QImport.scripts[self.entry_id].language))
		end
	end
...
QImport.reimport = function (callback) -- reloads all the scripts from disk
	...
	AsyncTaskManager.add(Task_Import:new(function ()
