...
function QCharacter:setShowOnMap(value)
    if (self.showOnMap and not value) or (not self.showOnMap and value) then
        self.showOnMap = value;
        SaveManager.onCharacterDataChange();
        triggerEvent("OnQSystemUpdate", 0);
    end
end
...
function QCharacter:register(plugin_id, npc_id)
    if (self.npc and (self.npc[1] ~= plugin_id or self.npc[2] ~= npc_id)) or not self.npc then
        self.npc = { plugin_id, npc_id };
    end
end
...
function CharacterManager:register(plugin)
    local index = self.plugins_size + 1;
    self.plugins[index] = plugin;
    self.plugins_size = index;
    return index;
end
