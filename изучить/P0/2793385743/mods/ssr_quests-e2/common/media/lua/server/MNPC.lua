...
if QSystem.validate("ssr-quests-e2") and not isServer() then
    ...
    function ISMoveableCursor:isValid(square)
        if ISMoveableCursor.mode[self.player] == "place" and MFManager.instance and square then
    ...
    function ISPlace3DItemCursor:isValid(square)
        if MFManager.instance and square then
            local x, y, z = square:getX(), square:getY(), square:getZ();
            for i=1, MFManager.instance.items_size do
                if MFManager.instance.items[i].instance and MFManager.instance.items[i].instance.javaObject and MFManager.instance.items[i].instance.x == x and MFManager.instance.items[i].instance.y == y and MFManager.instance.items[i].instance.z == z then
                    return false;
...
local function createSurvivorDesc(name, mannequin, template, isFemale)
    ...
    for i=1, #template.clothes do
        if type(template.clothes[i]) == "string" then
            local item = instanceItem(template.clothes[i])
            if item and (instanceof(item, "Clothing") or instanceof(item, "InventoryContainer")) then
                local locationId = item:getBodyLocation();
                locationId = not locationId and item:canBeEquipped() or locationId;
                if locationId and locationId ~= "" then
                    desc:setWornItem(locationId, item);
                    --QuestLogger.print("[QSystem*] MannequinFolks: Added clothing item - "..tostring(template.clothes[i]));
                else
                    print("[QSystem] (Error) MannequinFolks: Item cannot be equipped - "..tostring(template.clothes[i]));
                end
            else
                print("[QSystem] (Error) MannequinFolks: Invalid item - "..tostring(template.clothes[i]));
            end
        elseif type(template.clothes[i]) == "table" then
            local item = instanceItem(template.clothes[i][1])
            if item and item:getVisual() and (instanceof(item, "Clothing") or instanceof(item, "InventoryContainer")) then
                local locationId = item:getBodyLocation();
                locationId = not locationId and item:canBeEquipped() or locationId;
                if locationId and locationId ~= "" then
                    ...
                    print("[QSystem] (Error) MannequinFolks: Item cannot be equipped - "..tostring(template.clothes[i][1]));
                end
            else
                print("[QSystem] (Error) MannequinFolks: Invalid item - "..tostring(template.clothes[i][1]));
            end
        end
    end
    template.desc = desc;
...
function MNPC:despawn()
    if self.javaObject then
        ...
        if self.sq then
            ...
            self.javaObject:invalidateRenderChunkLevel(128);
            self.javaObject = nil;
            ...
            if self.sq == square then MFDuplicateRemoval.clearSquare(self.sq); end
...
function MNPC:new(name, template, state, x, y, z, direction, character_id)
    local o = {};
    setmetatable(o, self);
    self.__index = self;
    o:init();
