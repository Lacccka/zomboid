...
for i=1, #CommandList_a do
    if CommandList_a[i].command == "is_flag" or CommandList_a[i].command == "set_flag" or CommandList_a[i].command == "jump" or CommandList_a[i].command == "is_quest" or CommandList_a[i].command == "is_task" or CommandList_a[i].command == "is_event" or CommandList_a[i].command == "is_stat" or CommandList_a[i].command == "is_time" or CommandList_a[i].command == "is_alive" or CommandList_a[i].command == "is_lua" or CommandList_a[i].command == "exit" then
...
function npc_remove:validate(sender)
    if #self.args == 0 then
        return true;
    else
        local index = CharacterManager.instance:indexOf(self.args[1]);
        if not index or (index and not CharacterManager.instance.items[index].npc) then-- pass if NPC has never been spawned or specified id is wrong
            return true;
        elseif CharacterManager.instance.plugins[CharacterManager.instance.items[index].npc[1]].Type == "MFManager" then
            return true;
        end
    end
end
...
function npc_remove:execute(sender)
    self:debug();
    if #self.args == 0 then
        for i=1, CharacterManager.instance.plugins_size do
            CharacterManager.instance.plugins[i]:removeAll(true);
        end
...
function npc_update:validate(sender)
    local index = CharacterManager.instance:indexOf(self.args[1]);
    if not index or (index and not CharacterManager.instance.items[index].npc) then-- pass if NPC has never been spawned or specified id is wrong
        return true;
    elseif CharacterManager.instance.plugins[CharacterManager.instance.items[index].npc[1]].Type == "MFManager" then
