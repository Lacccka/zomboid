        ...
        for i=1, MFManager.instance.items_size do
            if MFManager.instance.items[i].instance then
                ...
                if distance > 35 and MFManager.instance.items[i].instance.javaObject then
                    if CharacterManager.instance.items[MFManager.instance.items[i].character_id].spawned then
                        MFManager.instance.items[i].instance:despawn();
                        if CharacterManager.instance.items[MFManager.instance.items[i].character_id].npc and CharacterManager.instance.items[MFManager.instance.items[i].character_id].npc[1] ~= MFManager.instance.index then
                            MFManager.instance.items[i].instance = nil;
                        end
                    end
                elseif distance <= 30 and not MFManager.instance.items[i].instance.javaObject then
                    if not CharacterManager.instance.items[MFManager.instance.items[i].character_id].spawned then
        ...
        if name == self.items[i].name then
            if self.items[i].instance then
                if self.items[i].instance.x ~= x or self.items[i].instance.y ~= y or self.items[i].instance.z ~= z or self.items[i].instance.template.name ~= template.name or self.items[i].instance.script:getAnimState() ~= state then
                    ...
                    self.items[i].instance = MNPC:new(name, template, state, x, y, z, direction, self.items[i].character_id);
            ...
            CharacterManager.instance.items[self.items[i].character_id]:register(MFManager.instance.index, i); -- change active NPC plugin for character
...
function MFManager:new()
    ...
    setmetatable(o, self);
    self.__index = self;
    o.index = CharacterManager.instance:register(o); -- register NPC plugin in Character Manager
    return o;
end
...
if QSystem.validate("ssr-quests-e2") then
    ...
    Events.OnTeleport.Add(function () if MFManager.instance then MFManager.instance:removeAll() end end)
    Events.OnCreatePlayer.Add(function () if MFManager.instance then MFManager.instance:removeAll() end end);
    Events.OnPlayerDeath.Add(function () if MFManager.instance then MFManager.instance:removeAll() end end);
