...
function BanditUtils.GetClosestPlayerLocation(character, config)
    ...
    if not result.id then
        ...
        if brain and brain.lastPost then
            local dist = BanditUtils.DistTo(cx, cy, brain.lastPost.x, brain.lastPost.y)
            if dist > 4 then
                result.dist = dist
                result.x = brain.lastPost.x
                result.y = brain.lastPost.y
                result.z = brain.lastPost.z
                result.d = brain.lastPost.d
                result.id = brain.lastPost.id
                result.player = true
            end
        end
    end
...
function BanditUtils.GetProjectileCount(reloadType)
    local projectiles = 1
    if reloadType == WeaponReloadType.SHOTGUN or reloadType == WeaponReloadType.DOUBLE_BARREL_SHOTGUN or reloadType == WeaponReloadType.DOUBLE_BARREL_SHOTGUN_SAWN then
        projectiles = 5
    end
    return projectiles
end
