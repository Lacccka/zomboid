...
function pzkCarUtils.ContainerAccess.TruckCabCompartment(vehicle, part, chr)
	...
	local doorPartL = vehicle:getPartById("DoorFrontLeft")
	local doorPartR = vehicle:getPartById("DoorFrontRight")
	
		if (doorPartL and doorPartL:getDoor() and not doorPartL:getDoor():isOpen()) or (doorPartR and doorPartR:getDoor() and not doorPartR:getDoor():isOpen() ) then
			return false
		end
