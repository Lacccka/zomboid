...
local function getMoveableDisplayName(obj)
    ...
    if type(obj.getSprite) ~= "function" then return nil end

    -- Very important: pass the function + self separately!
    local ok, sprite = pcall(obj.getSprite, obj)
    if not ok or not sprite then
        return nil
    end

    if type(sprite.getProperties) ~= "function" then
        return nil
    end

    local props = sprite:getProperties()
    if not props or not props:has("CustomName") then
        return nil
    end
				...
				if #fillableBottles > 0 then
					...
					--	slice:addSlice((getText("ContextMenu_Fill") .. " " .. item:getName()), item:getTex(), ISVehiclePartMenu.onTakeWater, playerObj, part, item) -- For reference, grabbing items sprite - Tread
						...
						for i = 1, #fillableBottles do
							...
							--if not item:isEmptyOfFluid() then -- workaround to get capacity of empty containers (their filled equivalents really) - Tread
							--if not item:isFullOfFluid() then -- workaround to get capacity of empty containers (their filled equivalents really) - Tread
								--local newItemType = item:getReplaceOnUseOn()
								local newItemType = item
...
--								newItemType = ScriptManager.instance:getItem(newItemType)
								tooltip.description = string.format("%s: <SETX:%d> %d / %d", getText("ContextMenu_WaterName"), tx, 0, 1.0 / newItemType:getCurrentUsesFloat () + 0.0001);
								newItemType = nil
							--else
								tooltip.description = string.format("%s: <SETX:%d> %d / %d", getText("ContextMenu_WaterName"), tx, item:getCurrentUses(), 1.0 / item:getCurrentUsesFloat () + 0.0001);	
							--end
...
local units = math.min(math.ceil(thirst * 10), 10)
units = math.min(units, waterTankContent)

tooltip.description = string.format(
    "%s: <SETX:%d> -%d / %d",
    getText("Tooltip_food_Thirst"),
    tx,
    math.min(units * 10, thirst * 100),
    thirst * 100
)
					tooltip.description = tooltip.description .. "\n" .. string.format("%s: <SETX:%d> %d / %d", getText("ContextMenu_WaterName"), tx, waterTankContent, waterTankCapacity);
					if filter ~= nil then	-- tainted tooltip if Filter installed - Tread
				...
				if #waterStations > 0 then
					if slice then
						...
						for i = 1, #waterStations do
							...
							local tx = getTextManager():MeasureStringX(tooltip.font, getText("ContextMenu_WaterName") .. ":") + 20
							tooltip.description = formatWaterAmount(tx, pump:getFluidAmount(), pump:getFluidCapacity())	
							tooltip.description = tooltip.description .. " <BR>" .. (getText("IGUI_RS_Source") .. getText("IGUI_VehiclePart" .. part:getId()))
							tooltip.description = tooltip.description .. "\n" .. string.format("%s: <SETX:%d> %d / %d", getText("ContextMenu_WaterName"), tx, waterTankContent, waterTankCapacity)
							if filter ~= nil then	-- tainted tooltip if Filter installed - Tread
								if filterCondition >=70 then
				...
				if #waterSources > 0 then
					...
					--	slice:addSlice((getText("IGUI_RS_AddWaterFrom") .. " " .. item:getName()), item:getTex(), ISVehiclePartMenu.addWaterToTank, playerObj, part, item)
						...
						for i = 1, #waterSources do
							...
							tooltip.description = tooltip.description .. "\n" .. string.format("%s: <SETX:%d> %d / %d", getText("ContextMenu_WaterName"), tx, item:getCurrentUses(), 1.0 / item:getCurrentUsesFloat () + 0.0001);
				...
				if #waterStations > 0 then
					if slice then
						...
						for i = 1, #waterStations do
							...
							tooltip.description = tooltip.description .. "\n" .. formatWaterAmount(tx, pump:getFluidAmount(), pump:getFluidCapacity())
