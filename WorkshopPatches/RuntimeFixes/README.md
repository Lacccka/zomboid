...
LCC provides only the missing lookup contract. It first uses `BanditServerZombie.Cache` if a Bandits version populates it, then falls back to an on-demand scan of the server cell zombie list using `BanditUtils.GetZombieID()`. This preserves the intended death-item update when the server has the active zombie instead of always returning nil.
...
- exercise a Bandit state update that reaches `BanditUpdatePart`; dedicated server no longer throws on missing `BanditZombie`, and death-item synchronization remains available when the matching server zombie exists;
- verify the grouped audit rejects any reintroduced `BanditZombie.lua`, `BanditServerWanderers.lua`, `ZAWaterFarm.lua`, or `ZAStompPlant.lua` under this project.
