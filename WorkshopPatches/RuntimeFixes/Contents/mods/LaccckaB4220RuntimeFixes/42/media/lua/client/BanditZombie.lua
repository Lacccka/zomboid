...
-- Upstream source is kept intact except for defensive square checks that prevent
-- transient/despawned IsoZombie instances from entering Bandit client caches.
...
local function onZombieUpdate(zombie)
    ...
    -- B42.20 MP can briefly update an IsoZombie after it has lost its square
    -- during despawn/devirtualization. Never expose that transient object to
    -- BanditUpdate (ManageCombat assumes getSquare() is valid).
...
local function flush()
    ...
    for i = 0, zombieListSize - 1 do
        ...
        -- Same lifecycle guard as onZombieUpdate: the full-cache rebuild must
        -- not reintroduce an object whose square has already disappeared.
        if zombie:getSquare() and not BanditCompatibility.IsReanimatedForGrappleOnly(zombie) and not zombie:isRagdoll() then
