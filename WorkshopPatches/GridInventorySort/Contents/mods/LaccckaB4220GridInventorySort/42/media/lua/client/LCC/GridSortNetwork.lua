...
local function OnServerCommand(module, command, args)
    ...
    if command == GridSortState.COMMANDS.REJECT_LAYOUT then
        applyLayout(args)
        print("[LCC GridSort] server rejected stale/conflicting layout; authoritative snapshot restored")
        return
    end
end
Events.OnServerCommand.Add(OnServerCommand)

-- Preserve upstream networking for the normal first page. Only page-aware
-- operations need the addon protocol; this keeps the patch source-clean and
-- minimizes the surface area that diverges from GridInventory.
...
GridClientNetwork.sendItemMove = function(container, itemId, x, y, rotated, gridContainer, manual)
    ...
    if oldPage > 1 or targetPage > 1 then
        if md then md.gridPage = targetPage > 1 and targetPage or nil end
