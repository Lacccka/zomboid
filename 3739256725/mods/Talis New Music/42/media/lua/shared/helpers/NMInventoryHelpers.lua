...
local function shouldLogBasementWorldLookup()
    return NMCore
        ...
        and NMCore.isDebugKnobOn("runtimeProbe")
end

local function logBasementWorldLookup(tag, detail)
    if shouldLogBasementWorldLookup() then
        NMCore.logChannel("runtimeProbe", tostring(tag or "world_lookup"), tostring(detail or ""))
    end
end
...
function NMInventoryHelpers.rememberVehicleLootStaleReject(traceContext)
...
function NMInventoryHelpers.isVehicleLootStaleReject(descriptor)
...
local function getContainerDebugId(container)
...
local function resolveVehicleContainerIdentity(player, inventory)
    ...
    local vehicle, part = resolveVehiclePartByContainer(player, inventory, {
...
function NMInventoryHelpers.refreshOpenVehicleLootPages(player, vehicleId, traceContext)
    local playerNum = player and player.getPlayerNum and tonumber(player:getPlayerNum()) or nil
    ...
    for i = 1, #backpacks do
        local button = backpacks[i]
        local inventory = button and button.inventory or nil
        local runtimeVehicleId, runtimePartId, containerType = resolveVehicleContainerIdentity(player, inventory)
        local selectedMatch = selectedInventory ~= nil and selectedInventory == inventory
