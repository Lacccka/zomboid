...
function LSPingPong:waitToStart()
	...
	if not self.source and not self.commandSent then
		...
		--print("TA-LSPingPong Sent StopOrStartInteraction command with doAction")
	end
...
function LSPingPong:update()
	...
	if self.character:getModData().LSInteractionState == "none" then
		--print("TA-LSPingPong force stop")
		self:forceStop()
	elseif self.character:getModData().LSInteractionState == "negativeReaction" then
		--print("TA-LSPingPong force stop Negative Reaction")
	...
	elseif self.character:getModData().LSInteractionState == "actionFinished" then
		--print("TA-LSPingPong force complete")
		self:forceComplete()
	...
	if not self.source and self.character:hasTrait(CharacterTrait[self.negativeTrait]) then
		if self.negativeTimer >= self.negativeMaxTime then
			--print("TA-LSPingPong force stop negativeMaxTime")
			self:forceStop()
		end
	...
	if self.countAnimTimer >= self.totalAnimTimer then
		if not getOtherPlayer(self.character, self.otherPlayer) then
			--print("TA-LSPingPong force stop. Reason: false at getOtherPlayer")
			self:forceStop()
		end
...
function LSPingPong:loadParams()
	local t = {
...
function LSPingPong:start()
	...
	if self.character:getModData().LSInteractionState == "sourceStoppedWaiting" then
		--print("TA-LSPingPong LSInteractionState is sourceStoppedWaiting, changing to none")
		self.character:getModData().LSInteractionState = "none"
	else
		--print("TA-LSPingPong LSInteractionState changed to doingAction")
...
function LSPingPong:stop()
	...
	if self.character:getModData().LSInteractionState == "doingAction" then
		--print("TA-LSPingPong action stopped by player, sending StopOrStartInteraction command with none")
...
function LSPingPong:perform()
	...
	self.character:setBlockMovement(false)
	self.character:getModData().LSInteractionState = "none"
	--print("TA-LSPingPong action performed")
