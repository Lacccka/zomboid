...
extends BaseSoundEmitter {
    ...
    private static final ObjectPool<ParameterValue> parameterValuePool = new ObjectPool<ParameterValue>(ParameterValue::new, "FMODSoundEmitter.parameterValuePool");
    ...
    public long playSound(String file) {
        if (GameClient.client) {
            ...
            if (isoObject instanceof IsoMovingObject) {
                ...
                if (!(isoObject2 instanceof IsoPlayer) || !(player = (IsoPlayer)isoObject2).isInvisible()) {
                    INetworkPacket.send(PacketTypes.PacketType.PlaySound, file, (byte)0, movingObject);
                }
            } else {
    ...
    public long playSound(String file, IsoGameCharacter character) {
        if (GameClient.client) {
            if (!character.isInvisible()) {
                INetworkPacket.send(PacketTypes.PacketType.PlaySound, file, (byte)0, character);
            }
            return !character.isInvisible() || DebugOptions.instance.character.debug.playSoundWhenInvisible.getValue() ? this.playSoundImpl(file, (IsoObject)null) : 0L;
        }
        if (GameServer.server) {
            return 0L;
        }
    ...
    public long playSoundLooped(String file) {
        if (GameClient.client) {
            ...
            if (isoObject instanceof IsoMovingObject) {
                ...
                INetworkPacket.send(PacketTypes.PacketType.PlaySound, file, (byte)1, isoMovingObject);
            } else {
                GameClient.instance.PlayWorldSound(file, PZMath.fastfloor(this.x), PZMath.fastfloor(this.y), (byte)PZMath.fastfloor(this.z));
            }
        }
    ...
    public long playSound(String file, IsoObject parent) {
        if (GameClient.client) {
            if (parent instanceof IsoMovingObject) {
                ...
                INetworkPacket.send(PacketTypes.PacketType.PlaySound, file, (byte)0, isoMovingObject);
            } else {
                GameClient.instance.PlayWorldSound(file, PZMath.fastfloor(this.x), PZMath.fastfloor(this.y), (byte)PZMath.fastfloor(this.z));
            }
        }
    ...
    private int findParameterValue(long soundRef, FMOD_STUDIO_PARAMETER_DESCRIPTION parameterDescription) {
        for (int i = 0; i < this.parameterValues.size(); ++i) {
            ...
            if (parameterValue.eventInstance != soundRef || parameterValue.parameterDescription != parameterDescription) continue;
            return i;
        }
        return -1;
    }
    ...
    public void clearParameters() {
        ...
        parameterValuePool.releaseAll((List<ParameterValue>)this.parameterValues);
    ...
    private void startEvent(long eventInstance, GameSoundClip clip, boolean remote) {
        ...
        block0: for (int i = 0; i < eventParameters.size(); ++i) {
            ...
            for (int j = 0; j < myParameters.size(); ++j) {
                ...
                if (fmodParameter.getParameterDescription() != eventParameter) continue;
    ...
    private void stopEvent(long eventInstance, GameSoundClip clip, boolean remote) {
        ...
        block0: for (int i = 0; i < eventParameters.size(); ++i) {
            ...
            int index = this.findParameterValue(eventInstance, eventParameter);
            if (index != -1) {
                ParameterValue parameterValue = this.parameterValues.remove(index);
    ...
    private Sound addSound(GameSoundClip clip, float volume, IsoObject parent) {
        ...
        if (clip.event != null && !clip.event.isEmpty()) {
            ...
            if ((eventInstance = javafmod.FMOD_Studio_System_CreateEventInstance(eventDescription.address)) < 0L) {
                return null;
            }
            if (clip.hasMinDistance()) {
                javafmodJNI.FMOD_Studio_EventInstance_SetProperty(eventInstance, FMOD_STUDIO_EVENT_PROPERTY.FMOD_STUDIO_EVENT_PROPERTY_MINIMUM_DISTANCE.ordinal(), clip.getMinDistance());
            }
            if (clip.hasMaxDistance()) {
                javafmodJNI.FMOD_Studio_EventInstance_SetProperty(eventInstance, FMOD_STUDIO_EVENT_PROPERTY.FMOD_STUDIO_EVENT_PROPERTY_MAXIMUM_DISTANCE.ordinal(), clip.getMaxDistance());
            }
    ...
    private void sendStopSound(String soundName, boolean triggerCue) {
        IsoObject isoObject;
        if (GameClient.client && (isoObject = this.parent) instanceof IsoMovingObject) {
            IsoMovingObject isoMovingObject = (IsoMovingObject)isoObject;
    ...
    private boolean shouldPlayRemoteEvents() {
        IsoPlayer isoPlayer;
        IsoObject isoObject = this.parent;
        if (isoObject instanceof IsoPlayer && !(isoPlayer = (IsoPlayer)isoObject).isLocalPlayer()) {
            return true;
        }
        return this.playRemoteEvents;
    }
    ...
    extends Sound {
        ...
        public boolean tick(boolean isStarting) {
            ...
            if (!isStarting) {
                ...
                if (this.triggeredCue && this.getEventDescription().length > 0L && currentTimeMs - this.checkTimeMs > 1500L) {
            ...
            boolean bl = bPositionChanged = Float.compare(this.emitter.x, this.setX) != 0 || Float.compare(this.emitter.y, this.setY) != 0 || Float.compare(this.emitter.z, this.setZ) != 0;
            if (bPositionChanged) {
    ...
    extends Sound {
        ...
        public boolean tick(boolean isStarting) {
            ...
            float reverb = IsoUtils.DistanceManhatten(x, y, IsoPlayer.getInstance().getX(), IsoPlayer.getInstance().getY(), z, IsoPlayer.getInstance().getZ()) / maxrange;
            ...
            if (current.getRoom() == null) {
                if (!this.ambient) {
                    reverb += IsoPlayer.getInstance().numNearbyBuildingsRooms / 32.0f;
                }
                if (!this.ambient) {
                    reverb += 0.08f;
                }
            ...
            reverb *= 10.0f;
            if (IsoPlayer.getInstance().getCurrentSquare().getRoom() == null && reverb < 0.1f) {
                reverb = 0.1f;
            }
            if (!this.ambient) {
                if (current.getRoom() != null) {
            ...
            if (sq2 != null) {
                if (this.emitter.parent instanceof IsoWindow || this.emitter.parent instanceof IsoDoor) {
                    r = IsoPlayer.getInstance().getCurrentSquare().getRoom();
                    if (r != this.emitter.parent.square.getRoom()) {
                        if (r != null && r.getBuilding() == this.emitter.parent.square.getBuilding()) {
                            ...
                            if (isoObject instanceof IsoDoor) {
                                ...
                                doorother = door.north ? IsoWorld.instance.currentCell.getGridSquare(door.getX(), door.getY() - 1.0f, door.getZ()) : IsoWorld.instance.currentCell.getGridSquare(door.getX() - 1.0f, door.getY(), door.getZ());
                            } else {
                                IsoWindow door = (IsoWindow)this.emitter.parent;
                                doorother = door.isNorth() ? IsoWorld.instance.currentCell.getGridSquare(door.getX(), door.getY() - 1.0f, door.getZ()) : IsoWorld.instance.currentCell.getGridSquare(door.getX() - 1.0f, door.getY(), door.getZ());
                            }
                            if (doorother != null && ((r = IsoPlayer.getInstance().getCurrentSquare().getRoom()) != null || doorother.getRoom() == null)) {
                                if (r != null && doorother.getRoom() != null && r.building == doorother.getBuilding()) {
                    ...
                    if (r != null && sq2.getRoom().getBuilding() != r.getBuilding()) {
                        directOcclude = 1.0f;
                        reverbOcclude = 0.8f;
                    }
                    if (r != null && sq2.getRoom().def.level != IsoPlayer.getInstance().getZi()) {
                        directOcclude = 0.6f;
                        reverbOcclude = 0.6f;
                    }
                } else {
                ...
                if (!sq2.isCouldSee(IsoPlayer.getPlayerIndex()) && sq2 != IsoPlayer.getInstance().getCurrentSquare()) {
                    directOcclude += 0.4f;
                }
            } else {
                ...
                directOcclude = (r = IsoPlayer.getInstance().getCurrentSquare().getRoom()) != null ? (directOcclude += 0.94f) : (directOcclude += 0.6f);
            }
            ...
            if (this.emitter.emitterType == EmitterType.Footstep && z > IsoPlayer.getInstance().getZ() && sq2.getBuilding() == IsoPlayer.getInstance().getBuilding()) {
                directOcclude = 0.0f;
                reverbOcclude = 0.0f;
            }
