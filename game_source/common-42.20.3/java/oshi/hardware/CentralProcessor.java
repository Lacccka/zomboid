...
public interface CentralProcessor {
    ...
    public double getSystemCpuLoadBetweenTicks(long[] var1, long[] var2);

    public long[] getSystemCpuLoadTicks();

    public double[] getSystemLoadAverage(int var1);

    default public double getSystemCpuLoad(long delay) {
    ...
    default public double[] getProcessorCpuLoad(long delay) {
        long start = System.nanoTime();
    ...
    public double[] getProcessorCpuLoadBetweenTicks(long[][] var1, long[][] var2);
    ...
    public static final class ProcessorIdentifier {
        ...
        private String queryMicroarchitecture() {
            ...
            if (ucVendor.contains("AMD")) {
                ...
                if (powerIdx > 0) {
                    arch2 = this.cpuName.substring(powerIdx + 1);
                }
            } else if (ucVendor.contains("APPLE")) {
                sb.append("apple.");
            }
            if (Util.isBlank(arch2) && !sb.toString().equals("arm.")) {
                sb.append(this.cpuFamily);
                arch2 = archProps.getProperty(sb.toString());
            }
            if (Util.isBlank(arch2)) {
            ...
            return Util.isBlank(arch2) ? "unknown" : arch2;
        }
    ...
    public static class ProcessorCache {
        ...
        public String toString() {
            return "ProcessorCache [L" + this.level + " " + String.valueOf((Object)this.type) + ", cacheSize=" + this.cacheSize + ", " + (String)(this.associativity > 0 ? this.associativity + "-way" : "unknown") + " associativity, lineSize=" + this.lineSize + "]";
        }
