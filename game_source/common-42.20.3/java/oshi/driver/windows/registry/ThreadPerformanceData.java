...
public final class ThreadPerformanceData {
    ...
    public static Map<Integer, PerfCounterBlock> buildThreadMapFromRegistry(Collection<Integer> pids) {
        ...
        for (Map<ThreadInformation.ThreadPerformanceProperty, Object> threadInstanceMap : threadInstanceMaps) {
            Integer pid = (Integer)threadInstanceMap.get(ThreadInformation.ThreadPerformanceProperty.IDPROCESS);
    ...
    public static Map<Integer, PerfCounterBlock> buildThreadMapFromPerfCounters(Collection<Integer> pids) {
        return ThreadPerformanceData.buildThreadMapFromPerfCounters(pids, null, -1);
    }

    public static Map<Integer, PerfCounterBlock> buildThreadMapFromPerfCounters(Collection<Integer> pids, String procName, int threadNum) {
        if (PerfmonDisabled.PERF_PROC_DISABLED) {
            return Collections.emptyMap();
        }
        HashMap<Integer, PerfCounterBlock> threadMap = new HashMap<Integer, PerfCounterBlock>();
        Pair<List<String>, Map<ThreadInformation.ThreadPerformanceProperty, List<Long>>> instanceValues = Util.isBlank(procName) ? ThreadInformation.queryThreadCounters() : ThreadInformation.queryThreadCounters(procName, threadNum);
        ...
        for (int inst = 0; inst < instances.size(); ++inst) {
            ...
            if ((startTime = WinBase.FILETIME.filetimeToDate(lowerStartTimeLimit, higherStartTimeLimit).getTime()) > now) {
                startTime = now - 1L;
            }
