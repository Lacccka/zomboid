...
public final class ParseUtil {
    ...
    public static String decodeBinaryToString(byte[] bytes) {
        ...
        return IntStream.range(0, bytes.length).mapToObj(i -> String.format(Locale.ROOT, "%02X", bytes[i])).collect(Collectors.joining(" "));
    }

    public static long parseSpeed(String speed) {
        if (speed.contains("T/s")) {
            return ParseUtil.parseHertz(speed.replace("T/s", HZ));
        }
        return ParseUtil.parseHertz(speed);
    }

    public static long parseHertz(String hertz) {
        double value;
        Matcher matcher = HERTZ_PATTERN.matcher(hertz.trim());
        if (matcher.find() && (value = Double.valueOf(matcher.group(1)) * (double)multipliers.getOrDefault(matcher.group(3), -1L).longValue()) >= 0.0) {
            return (long)value;
        }
        return -1L;
    }
    ...
    public static String parseMmDdYyyyToYyyyMmDD(String dateString) {
        try {
            return String.format(Locale.ROOT, "%s-%s-%s", dateString.substring(6, 10), dateString.substring(0, 2), dateString.substring(3, 5));
        }
    ...
    public static Triplet<String, String, String> parseDeviceIdToVendorProductSerial(String deviceId) {
        ...
        if (m.matches()) {
            ...
            return new Triplet<CallSite, CallSite, String>((CallSite)((Object)vendorId), (CallSite)((Object)productId), !m.group(3).isEmpty() || serial.contains("&") ? "" : serial);
        }
        return null;
    }
    ...
    public static List<Integer> parseHyphenatedIntList(String str) {
        ...
        for (String csvToken : csvTokens = str.split(",")) {
    ...
    public static List<String> parseByteArrayToStrings(byte[] bytes) {
        ...
        int end = 0;
        do {
            if (end != bytes.length && bytes[end] != 0 && bytes[end] != 10) continue;
            if (start == end) break;
    ...
    public static <K extends Enum<K>> Map<K, String> stringToEnumMap(Class<K> clazz, String values2, char delim) {
        EnumMap<Enum, String> map = new EnumMap<Enum, String>(clazz);
        int start = 0;
        int len = values2.length();
        EnumSet<Enum> keys2 = EnumSet.allOf(clazz);
        int keySize = keys2.size();
        for (Enum key : keys2) {
            int idx;
            int n = idx = --keySize == 0 ? len : values2.indexOf(delim, start);
            if (idx >= 0) {
                map.put(key, values2.substring(start, idx));
                start = idx;
                while (++start < len && values2.charAt(start) == delim) {
                }
                continue;
            }
            map.put(key, values2.substring(start));
            break;
        }
        return map;
    }
    ...
    public static String getValueOrUnknown(Map<?, String> map, Object key) {
        return ParseUtil.getStringValueOrUnknown(map.get(key));
    }
    ...
    public static long parseDateToEpoch(String dateString, String datePattern) {
        if (dateString == null || dateString.equals("unknown") || dateString.isEmpty() || datePattern.isEmpty()) {
            return 0L;
        }
        try {
            DateTimeFormatter formatter = new DateTimeFormatterBuilder().appendPattern(datePattern).parseDefaulting(ChronoField.HOUR_OF_DAY, 0L).parseDefaulting(ChronoField.MINUTE_OF_HOUR, 0L).parseDefaulting(ChronoField.SECOND_OF_MINUTE, 0L).parseDefaulting(ChronoField.MILLI_OF_SECOND, 0L).toFormatter(Locale.ROOT);
            LocalDateTime localDateTime = LocalDateTime.parse(dateString, formatter);
            return localDateTime.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
        }
        catch (DateTimeParseException e) {
            LOG.trace("Unable to parse date string: {}", (Object)dateString);
            return 0L;
        }
    }
