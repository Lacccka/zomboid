            ...
            if (0L < pdhQueryHandler.updateQuery()) {
                for (i2 = 1; i2 < props.length; ++i2) {
                    ...
                    for (PerfDataUtil.PerfCounter counter : (List)counterListMap.get(prop)) {
                        values2.add(pdhQueryHandler.queryCounter(counter));
                    }
                    valuesMap.put(prop, values2);
                }
            }
        }
            ...
            for (Enum prop : (Enum[])propertyEnum.getEnumConstants()) {
                ...
                block8: for (int i = 0; i < result.getResultCount(); ++i) {
                    switch (result.getCIMType(prop)) {
                        case 18: {
                            values2.add(Long.valueOf(WmiUtil.getUint16(result, prop, i)));
                            continue block8;
                        }
                        case 19: {
                            values2.add(WmiUtil.getUint32asLong(result, prop, i));
                            continue block8;
                        }
                        case 21: {
                            values2.add(WmiUtil.getUint64(result, prop, i));
                            continue block8;
                        }
                        case 101: {
                            values2.add(WmiUtil.getDateTime(result, prop, i).toInstant().toEpochMilli());
                            continue block8;
                        }
                        default: {
