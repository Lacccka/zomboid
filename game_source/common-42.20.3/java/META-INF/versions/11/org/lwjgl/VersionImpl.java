/*
 * Decompiled with CFR 0.152.
 */
package org.lwjgl;

import java.lang.module.ModuleDescriptor;
import org.lwjgl.Version;

/*
 * Multiple versions of this class in jar - see https://www.benf.org/other/cfr/multi-version-jar.html
 */
final class VersionImpl {
    VersionImpl() {
    }

    static String find() {
        int plusIndex;
        String moduleVersion;
        Package org_lwjgl = Version.class.getPackage();
        String specVersion = org_lwjgl.getSpecificationVersion();
        String implVersion = org_lwjgl.getImplementationVersion();
        if (specVersion != null && implVersion != null) {
            return Version.createImplementation(specVersion, implVersion);
        }
        Module module = Version.class.getModule();
        if ("org.lwjgl".equals(module.getName()) && (moduleVersion = (String)module.getDescriptor().version().map(ModuleDescriptor.Version::toString).orElse(null)) != null && (plusIndex = moduleVersion.indexOf(43)) != -1) {
            return Version.createImplementation(moduleVersion.substring(0, plusIndex), moduleVersion.substring(plusIndex + 1));
        }
        String version = Version.findImplementationFromManifest();
        if (version != null) {
            return version;
        }
        return "-snapshot";
    }
}

