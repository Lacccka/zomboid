...
implements GroupArray<StructLayout, T> {
    ...
    public StructArray<T> slice(long index) {
        return new StructArray<T>(this.binder, this.segment.asSlice(index * this.binder.sizeof()));
    }

    public StructArray<T> slice(long index, long elementCount) {
        long sizeof = this.binder.sizeof();
        return new StructArray<T>(this.binder, this.segment.asSlice(index * sizeof, elementCount * sizeof));
    }
