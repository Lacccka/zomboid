...
Externalizable {
    ...
    public TShortCharHashMap(short[] keys2, char[] values2) {
    ...
    @Override
    public short[] keys() {
        short[] keys2 = new short[this.size()];
        short[] k = this._set;
    ...
    implements TCharCollection {
        ...
        public boolean remove(char entry) {
            char[] values2 = TShortCharHashMap.this._values;
            short[] set = TShortCharHashMap.this._set;
            int i = values2.length;
            while (i-- > 0) {
                if (set[i] == 0 || set[i] == 2 || entry != values2[i]) continue;
