...
Externalizable {
    ...
    public TFloatLinkedList(TFloatList list) {
        ...
        TFloatIterator iterator2 = list.iterator();
    ...
    public void insert(int offset, float[] values2) {
        this.insert(offset, TFloatLinkedList.link(values2, 0, values2.length));
    }

    @Override
    public void insert(int offset, float[] values2, int valOffset, int len) {
        this.insert(offset, TFloatLinkedList.link(values2, valOffset, len));
    }
    ...
    static TFloatLinkedList link(float[] values2, int valOffset, int len) {
    ...
    public void set(int offset, float[] values2, int valOffset, int length) {
        for (int i = 0; i < length; ++i) {
            float value = values2[valOffset + i];
    ...
    public boolean addAll(Collection<? extends Float> collection) {
        ...
        for (Float f : collection) {
            if (!this.add(f.floatValue())) continue;
            ret = true;
        }
        return ret;
    }
    ...
    public void writeExternal(ObjectOutput out) throws IOException {
        ...
        TFloatIterator iterator2 = this.iterator();
        while (iterator2.hasNext()) {
            float next = iterator2.next();
            out.writeFloat(next);
        }
