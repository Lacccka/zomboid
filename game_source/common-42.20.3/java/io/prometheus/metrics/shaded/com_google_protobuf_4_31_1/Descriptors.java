...
import io.prometheus.metrics.shaded.com_google_protobuf_4_31_1.JavaFeaturesProto;
...
public final class Descriptors {
    ...
    private static DescriptorProtos.FeatureSet internFeatures(DescriptorProtos.FeatureSet features) {
        DescriptorProtos.FeatureSet cached = FEATURE_CACHE.putIfAbsent(features.hashCode(), features);
        if (cached == null) {
            return features;
        }
        return cached;
    ...
    FieldSet.FieldDescriptorLite<FieldDescriptor> {
        ...
        public Type getType() {
            if (!(this.type != Type.MESSAGE || this.messageType != null && this.messageType.toProto().getOptions().getMapEntry() || this.containingType != null && this.containingType.toProto().getOptions().getMapEntry() || this.features == null || this.getFeatures().getMessageEncoding() != DescriptorProtos.FeatureSet.MessageEncoding.DELIMITED)) {
                return Type.GROUP;
            }
            return this.type;
        }
        ...
        boolean hasOptionalKeyword() {
            return this.isProto3Optional || this.file.getEdition() == DescriptorProtos.Edition.EDITION_PROTO2 && !this.isRequired() && !this.isRepeated() && this.getContainingOneof() == null;
        }
        ...
        public boolean legacyEnumFieldTreatedAsClosed() {
            ...
            return this.getType() == Type.ENUM && (this.getFeatures().getExtension(JavaFeaturesProto.java_).getLegacyClosedEnum() || this.enumType.isClosed());
        }
                ...
                if (field.isRepeated()) {
                    for (Message m : (List)value) {
                        for (Map.Entry<FieldDescriptor, Object> entry : m.getAllFields().entrySet()) {
                            List<Boolean> result = this.isOptionSensitive(entry.getKey(), entry.getValue());
                            if (!result.get(0).booleanValue()) continue;
                            return result;
                        }
                    }
                } else {
        ...
        private List<Boolean> calculateSensitivityData() {
            if (this.sensitivity == Sensitivity.UNKNOWN) {
                FieldDescriptor fieldDescriptor = this;
                synchronized (fieldDescriptor) {
                    if (this.sensitivity == Sensitivity.UNKNOWN) {
                        boolean isSensitive = this.proto.getOptions().getDebugRedact();
                        for (Map.Entry<FieldDescriptor, Object> entry : this.proto.getOptions().getAllFields().entrySet()) {
                            List<Boolean> result = this.isOptionSensitive(entry.getKey(), entry.getValue());
                            isSensitive = isSensitive || result.get(0) != false;
                            this.isReportable = result.get(1);
                            if (!isSensitive) continue;
                            break;
                        }
                        this.sensitivity = isSensitive ? Sensitivity.SENSITIVE : Sensitivity.NOT_SENSITIVE;
                    }
                }
            }
            return Arrays.asList(this.sensitivity == Sensitivity.SENSITIVE, this.isReportable);
        }

        boolean isSensitive() {
            return this.calculateSensitivityData().get(0);
        }

        boolean isReportable() {
            return this.calculateSensitivityData().get(1);
        }
        ...
        private static enum Sensitivity {
            UNKNOWN,
            SENSITIVE,
            NOT_SENSITIVE;
