...
import io.prometheus.metrics.shaded.com_google_protobuf_4_31_1.PrimitiveNonBoxingCollection;
...
implements MessageLite {
    ...
    protected static <T> void addAll(Iterable<T> values2, List<? super T> list) {
        Builder.addAll(values2, list);
    }
    ...
    implements MessageLite.Builder {
        ...
        private static <T> void addAllCheckingNulls(Iterable<T> values2, List<? super T> list) {
            if (values2 instanceof Collection) {
                int growth = ((Collection)values2).size();
            ...
            int begin = list.size();
            if (values2 instanceof List && values2 instanceof RandomAccess) {
                List valuesList = (List)values2;
        ...
        protected static <T> void addAll(Iterable<T> values2, Collection<? super T> list) {
            Builder.addAll(values2, (List)list);
        }

        protected static <T> void addAll(Iterable<T> values2, List<? super T> list) {
            Internal.checkNotNull(values2);
            if (values2 instanceof LazyStringList) {
                List<?> lazyValues = ((LazyStringList)values2).getUnderlyingElements();
            ...
            } else if (values2 instanceof PrimitiveNonBoxingCollection) {
                list.addAll((Collection)values2);
            } else {
                Builder.addAllCheckingNulls(values2, list);
            }
        }
