...
public class JvmBufferPoolMetrics {
    ...
    private void register(PrometheusRegistry registry) {
        ((GaugeWithCallback.Builder)((GaugeWithCallback.Builder)((GaugeWithCallback.Builder)((GaugeWithCallback.Builder)GaugeWithCallback.builder(this.config).name(JVM_BUFFER_POOL_USED_BYTES)).help("Used bytes of a given JVM buffer pool.")).unit(Unit.BYTES)).labelNames("pool")).callback(callback -> {
            for (BufferPoolMXBean pool : this.bufferPoolBeans) {
                callback.call(pool.getMemoryUsed(), pool.getName());
            }
        }).register(registry);
        ((GaugeWithCallback.Builder)((GaugeWithCallback.Builder)((GaugeWithCallback.Builder)((GaugeWithCallback.Builder)GaugeWithCallback.builder(this.config).name(JVM_BUFFER_POOL_CAPACITY_BYTES)).help("Bytes capacity of a given JVM buffer pool.")).unit(Unit.BYTES)).labelNames("pool")).callback(callback -> {
            for (BufferPoolMXBean pool : this.bufferPoolBeans) {
                callback.call(pool.getTotalCapacity(), pool.getName());
            }
        }).register(registry);
        ((GaugeWithCallback.Builder)((GaugeWithCallback.Builder)((GaugeWithCallback.Builder)GaugeWithCallback.builder(this.config).name(JVM_BUFFER_POOL_USED_BUFFERS)).help("Used buffers of a given JVM buffer pool.")).labelNames("pool")).callback(callback -> {
            for (BufferPoolMXBean pool : this.bufferPoolBeans) {
