...
public final class MemoryUtil {
    ...
    public static long memAddress(Buffer buffer) {
        ...
        return MemoryUtil.address(buffer.position(), elementShift, UNSAFE.getLong(buffer, ADDRESS));
    }
    ...
    static ByteBuffer slice(ByteBuffer source2, long address, int capacity) {
        ByteBuffer target;
        try {
            target = (ByteBuffer)UNSAFE.allocateInstance(BUFFER_BYTE);
        }
        ...
        Object attachment = UNSAFE.getObject(source2, PARENT_BYTE);
        UNSAFE.putObject(target, PARENT_BYTE, attachment == null ? source2 : attachment);
        return target.order(source2.order());
    }

    static <T extends Buffer> T slice(Class<? extends T> clazz, T source2, long address, int capacity, long attachmentOffset) {
        Buffer target;
        try {
        ...
        UNSAFE.putInt(target, LIMIT, capacity);
        UNSAFE.putInt(target, CAPACITY, capacity);
        UNSAFE.putObject(target, attachmentOffset, UNSAFE.getObject(source2, attachmentOffset));
        return (T)target;
    }

    static <T extends Buffer> T duplicate(Class<? extends T> clazz, T source2, long attachmentOffset) {
        Buffer target;
        try {
            target = (Buffer)UNSAFE.allocateInstance(clazz);
        }
        ...
        UNSAFE.putLong(target, ADDRESS, UNSAFE.getLong(source2, ADDRESS));
        UNSAFE.putInt(target, MARK, UNSAFE.getInt(source2, MARK));
        UNSAFE.putInt(target, POSITION, UNSAFE.getInt(source2, POSITION));
        UNSAFE.putInt(target, LIMIT, UNSAFE.getInt(source2, LIMIT));
        UNSAFE.putInt(target, CAPACITY, UNSAFE.getInt(source2, CAPACITY));
        UNSAFE.putObject(target, attachmentOffset, UNSAFE.getObject(source2, attachmentOffset));
        return (T)target;
    }
