...
public final class CacheUtil {
    ...
    public static ByteBuffer createByteBuffer(int size) {
        ...
        if (MemoryUtil.getAddress((ByteBuffer)buffer) % (long)CACHE_LINE_SIZE != 0L) {
            buffer.position(CACHE_LINE_SIZE - (int)(MemoryUtil.getAddress((ByteBuffer)buffer) & (long)(CACHE_LINE_SIZE - 1)));
        }
    ...
    static {
        Integer size = LWJGLUtil.getPrivilegedInteger((String)"org.lwjgl.util.mapped.CacheLineSize");
        if (size != null) {
            if (size < 1) {
            ...
            if (LWJGLUtil.DEBUG) {
                LWJGLUtil.log((CharSequence)"Cannot detect cache line size on single-core CPUs, assuming 64 bytes.");
            }
            CACHE_LINE_SIZE = 64;
        } else {
