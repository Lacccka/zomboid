...
implements Comparable<CLongBuffer> {
    ...
    public static CLongBuffer allocateDirect(int capacity) {
        ByteBuffer source2 = BufferUtils.createByteBuffer(BufferUtils.getAllocationSize(capacity, CLONG_SHIFT));
        return new CLongBuffer(MemoryUtil.memAddress(source2), source2, -1, 0, capacity, capacity);
    }
    ...
    public static CLongBuffer create(ByteBuffer source2) {
        int capacity = source2.remaining() >> CLONG_SHIFT;
        return new CLongBuffer(MemoryUtil.memAddress(source2), source2, -1, 0, capacity, capacity);
    }
    ...
    public static long get(ByteBuffer source2) {
        if (source2.remaining() < CLONG_SIZE) {
            throw new BufferUnderflowException();
        }
        try {
            long l = MemoryUtil.memGetCLong(MemoryUtil.memAddress(source2));
            return l;
        }
        finally {
            source2.position(source2.position() + CLONG_SIZE);
        }
    }
    ...
    public static long get(ByteBuffer source2, int index) {
        CheckIntrinsics.checkFromIndexSize(index, CLONG_SIZE, source2.limit());
        return MemoryUtil.memGetCLong(MemoryUtil.memAddress0(source2) + (long)index);
    }
