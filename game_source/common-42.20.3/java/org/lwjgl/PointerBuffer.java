...
implements Comparable<PointerBuffer> {
    ...
    public static PointerBuffer allocateDirect(int capacity) {
        ByteBuffer source2 = BufferUtils.createByteBuffer(BufferUtils.getAllocationSize(capacity, POINTER_SHIFT));
        return new PointerBuffer(MemoryUtil.memAddress(source2), source2, -1, 0, capacity, capacity);
    }
    ...
    public static PointerBuffer create(ByteBuffer source2) {
        int capacity = source2.remaining() >> POINTER_SHIFT;
        return new PointerBuffer(MemoryUtil.memAddress(source2), source2, -1, 0, capacity, capacity);
    }
    ...
    public static long get(ByteBuffer source2) {
        if (source2.remaining() < POINTER_SIZE) {
            throw new BufferUnderflowException();
        }
        try {
            long l = MemoryUtil.memGetAddress(MemoryUtil.memAddress(source2));
            return l;
        }
        finally {
            source2.position(source2.position() + POINTER_SIZE);
        }
    }
    ...
    public static long get(ByteBuffer source2, int index) {
        CheckIntrinsics.checkFromIndexSize(index, POINTER_SIZE, source2.limit());
        return MemoryUtil.memGetAddress(MemoryUtil.memAddress0(source2) + (long)index);
    }
