...
implements IndexedStringMap {
    ...
    public void putAll(ReadOnlyStringMap source2) {
        if (source2 == this || source2 == null || source2.isEmpty()) {
            return;
        }
        this.assertNotFrozen();
        this.assertNoConcurrentModification();
        if (source2 instanceof SortedArrayStringMap) {
            if (this.size == 0) {
                this.initFrom0((SortedArrayStringMap)source2);
            } else {
                this.merge((SortedArrayStringMap)source2);
            }
        } else if (source2 != null) {
            source2.forEach(PUT_ALL, this);
        }
    }
    ...
    private static int hashCode(Object[] values2, int length) {
        ...
        for (int i = 0; i < length; ++i) {
            result = 31 * result + (values2[i] == null ? 0 : values2[i].hashCode());
        }
        return result;
    }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private static Object unmarshall(byte[] data, ObjectInputStream inputStream2) throws IOException, ClassNotFoundException {
        ObjectInputStream ois;
        ByteArrayInputStream bin = new ByteArrayInputStream(data);
        Collection<String> allowedClasses = null;
        if (inputStream2 instanceof FilteredObjectInputStream) {
            allowedClasses = ((FilteredObjectInputStream)inputStream2).getAllowedClasses();
            ois = new FilteredObjectInputStream(bin, allowedClasses);
        } else {
            try {
                Object obj = getObjectInputFilter.invoke((Object)inputStream2, new Object[0]);
