...
public class TypeUtils {
    private static final AppendableJoiner<Type> AMP_JOINER = AppendableJoiner.builder().setDelimiter(" & ").setElementAppender((a, e) -> a.append(TypeUtils.toString(e))).get();
    private static final AppendableJoiner<TypeVariable<Class<?>>> CTJ_JOINER = AppendableJoiner.builder().setDelimiter(", ").setElementAppender((a, e) -> a.append(TypeUtils.anyToString(e))).get();
    private static final AppendableJoiner<Object> GT_JOINER = AppendableJoiner.builder().setPrefix("<").setSuffix(">").setDelimiter(", ").setElementAppender((a, e) -> a.append(TypeUtils.anyToString(e))).get();
    ...
    private static void appendRecursiveTypes(StringBuilder builder, int[] recursiveTypeIndexes, Type[] argumentTypes) {
        for (int i = 0; i < recursiveTypeIndexes.length; ++i) {
            GT_JOINER.join(builder, (Object[])new Object[]{argumentTypes[i].toString()});
        }
    ...
    private static boolean isCyclical(Class<?> cls) {
        for (TypeVariable<Class<?>> typeParameter : cls.getTypeParameters()) {
            for (AnnotatedType annotatedBound : typeParameter.getAnnotatedBounds()) {
                if (!annotatedBound.getType().getTypeName().contains(cls.getName())) continue;
                return true;
            }
        }
        return false;
    ...
    private static String typeVariableToString(TypeVariable<?> typeVariable) {
        ...
        if (!(bounds.length <= 0 || bounds.length == 1 && Object.class.equals((Object)bounds[0]))) {
            ...
            AMP_JOINER.join(builder, (Type[])typeVariable.getBounds());
        }
        return builder.toString();
    }
        ...
        if (TypeUtils.containsTypeVariables(type)) {
            ...
            if (type instanceof ParameterizedType) {
                ...
                for (int i = 0; i < args2.length; ++i) {
                    Type unrolled = TypeUtils.unrollVariables(parameterizedTypeArguments, args2[i]);
    ...
    private static String wildcardTypeToString(WildcardType wildcardType) {
        ...
        if (lowerBounds.length > 1 || lowerBounds.length == 1 && lowerBounds[0] != null) {
            AMP_JOINER.join(builder.append(" super "), (Type[])lowerBounds);
        } else if (upperBounds.length > 1 || upperBounds.length == 1 && !Object.class.equals((Object)upperBounds[0])) {
            AMP_JOINER.join(builder.append(" extends "), (Type[])upperBounds);
        }
        return builder.toString();
    }
