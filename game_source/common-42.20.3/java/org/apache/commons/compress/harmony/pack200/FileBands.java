...
extends BandSet {
    ...
    public FileBands(CpBands cpBands, SegmentHeader segmentHeader, PackingOptions options, Archive.SegmentUnit segmentUnit, int effort) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            long l = (packingFile.getModtime() + (long)TimeZone.getDefault().getRawOffset()) / 1000L;
            this.file_modtime[i] = (int)(l - (long)archiveModtime);
    ...
    public void finaliseBands() {
        ...
        for (int i = 0; i < this.file_name.length; ++i) {
            ...
            String name;
            if (this.fileName[i].equals(this.cpBands.getCPUtf8("")) && this.options.isPassFile(name = (packingFile = this.fileList.get(i)).getName())) {
    ...
    private int[] flatten(byte[][] bytes) {
        ...
        for (int i = 0; i < n; ++i) {
            byte[] element;
            for (byte element2 : element = byArray[i]) {
