...
public class CodecEncoding {
    ...
    public static int[] getSpecifier(Codec codec, Codec defaultForBand) {
        ...
        if (codec instanceof RunCodec) {
            ...
            int[] aSpecifier = abDef == 1 ? EMPTY_INT_ARRAY : CodecEncoding.getSpecifier(aCodec, defaultForBand);
            int[] bSpecifier = abDef == 2 ? EMPTY_INT_ARRAY : CodecEncoding.getSpecifier(bCodec, defaultForBand);
            ...
            for (n = 0; n < n2; ++n) {
                specifier[index] = element = nArray[n];
                ++index;
            }
            nArray = bSpecifier;
            n2 = nArray.length;
        ...
        if (codec instanceof PopulationCodec) {
            ...
            int[] favouredSpecifier = fDef == 1 ? EMPTY_INT_ARRAY : CodecEncoding.getSpecifier(favouredCodec, defaultForBand);
            int[] tokenSpecifier = tDefL != 0 ? EMPTY_INT_ARRAY : CodecEncoding.getSpecifier(tokenCodec, defaultForBand);
            int[] unfavouredSpecifier = uDef == 1 ? EMPTY_INT_ARRAY : CodecEncoding.getSpecifier(unfavouredCodec, defaultForBand);
