...
implements InputStreamStatistics {
    ...
    public GzipCompressorInputStream(InputStream inputStream2) throws IOException {
        this(inputStream2, false);
    }

    public GzipCompressorInputStream(InputStream inputStream2, boolean decompressConcatenated) throws IOException {
    ...
    private boolean init(boolean isFirstMember) throws IOException {
        ...
        if ((flg & 8) != 0) {
            this.parameters.setFileName(new String(GzipCompressorInputStream.readToNull(inData), GzipUtils.GZIP_ENCODING));
        }
        if ((flg & 0x10) != 0) {
            this.parameters.setComment(new String(GzipCompressorInputStream.readToNull(inData), GzipUtils.GZIP_ENCODING));
        }
        if ((flg & 2) != 0) {
            inData.readShort();
        }
