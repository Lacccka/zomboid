...
public class Archiver {
    ...
    public void create(ArchiveOutputStream<?> target, Path directory, EnumSet<FileVisitOption> fileVisitOptions, LinkOption ... linkOptions) throws IOException {
        Files.walkFileTree(directory, fileVisitOptions, Integer.MAX_VALUE, new ArchiverFileVisitor(target, directory, linkOptions, null));
        target.finish();
    }
    ...
    public void create(final SevenZOutputFile target, final Path directory) throws IOException {
        Files.walkFileTree(directory, (FileVisitor<? super Path>)new ArchiverFileVisitor<ArchiveOutputStream<ArchiveEntry>, ArchiveEntry>(null, directory, new LinkOption[0]){
    ...
    public void create(String format, OutputStream target, File directory, CloseableConsumer closeableConsumer) throws IOException, ArchiveException {
        try (CloseableConsumerAdapter c = new CloseableConsumerAdapter(closeableConsumer);){
            Object archiveOutputStream = ArchiveStreamFactory.DEFAULT.createArchiveOutputStream(format, target);
    ...
    public void create(String format, Path target, Path directory) throws IOException, ArchiveException {
        ...
        try (Object outputStream2 = ArchiveStreamFactory.DEFAULT.createArchiveOutputStream(format, Files.newOutputStream(target, new OpenOption[0]));){
            this.create((ArchiveOutputStream<?>)outputStream2, directory, EMPTY_FileVisitOption, new LinkOption[0]);
        }
    }
    ...
    extends SimpleFileVisitor<Path> {
        ...
        protected FileVisitResult visit(Path path, BasicFileAttributes attrs, boolean isFile) throws IOException {
            ...
            if (!name.isEmpty()) {
                Object archiveEntry = ((ArchiveOutputStream)this.target).createArchiveEntry(path, isFile || name.endsWith("/") ? name : name + "/", this.linkOptions);
