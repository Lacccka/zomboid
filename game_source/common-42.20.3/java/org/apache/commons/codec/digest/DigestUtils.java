...
public class DigestUtils {
    ...
    public static MessageDigest getDigest(String algorithm) {
        try {
            return DigestUtils.getMessageDigest(algorithm);
        }
    ...
    public static MessageDigest updateDigest(MessageDigest digest, File data) throws IOException {
        try (BufferedInputStream inputStream2 = new BufferedInputStream(new FileInputStream(data));){
            MessageDigest messageDigest = DigestUtils.updateDigest(digest, inputStream2);
            return messageDigest;
        }
    }
    ...
    public static MessageDigest updateDigest(MessageDigest digest, InputStream inputStream2) throws IOException {
        byte[] buffer = new byte[1024];
        int read = inputStream2.read(buffer, 0, 1024);
        while (read > -1) {
            digest.update(buffer, 0, read);
            read = inputStream2.read(buffer, 0, 1024);
        }
        return digest;
    }

    public static MessageDigest updateDigest(MessageDigest digest, Path path, OpenOption ... options) throws IOException {
        try (BufferedInputStream inputStream2 = new BufferedInputStream(Files.newInputStream(path, options));){
            MessageDigest messageDigest = DigestUtils.updateDigest(digest, inputStream2);
            return messageDigest;
        }
    }
