...
BinaryDecoder {
    ...
    public static char[] encodeHex(byte[] data, boolean toLowerCase) {
        return Hex.encodeHex(data, Hex.toAlphabet(toLowerCase));
    }

    protected static char[] encodeHex(byte[] data, char[] toDigits) {
        int dataLength = data.length;
        return Hex.encodeHex(data, 0, dataLength, toDigits, new char[dataLength << 1], 0);
    }

    public static char[] encodeHex(byte[] data, int dataOffset, int dataLen, boolean toLowerCase) {
        return Hex.encodeHex(data, dataOffset, dataLen, Hex.toAlphabet(toLowerCase), new char[dataLen << 1], 0);
    }

    public static void encodeHex(byte[] data, int dataOffset, int dataLen, boolean toLowerCase, char[] out, int outOffset) {
        Hex.encodeHex(data, dataOffset, dataLen, Hex.toAlphabet(toLowerCase), out, outOffset);
    }
