...
extends BaseNCodec {
    ...
    private Base32(int lineLength, byte[] lineSeparator, byte[] encodeTable, byte padding, CodecPolicy decodingPolicy) {
        super(5, 8, lineLength, Base32.toLength(lineSeparator), padding, decodingPolicy);
        Objects.requireNonNull(encodeTable, "encodeTable");
        this.encodeTable = encodeTable;
        byte[] byArray = this.decodeTable = encodeTable == HEX_ENCODE_TABLE ? HEX_DECODE_TABLE : DECODE_TABLE;
        if (lineLength > 0) {
    ...
    extends BaseNCodec.AbstractBuilder<Base32, Builder> {
        public Builder() {
            super(ENCODE_TABLE);
        }
