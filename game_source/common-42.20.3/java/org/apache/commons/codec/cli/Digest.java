...
public class Digest {
    ...
    private Digest(String[] args2) {
        ...
        this.inputs = argsLength <= 1 ? null : Arrays.copyOfRange(args2, 1, argsLength);
    }
    ...
    private void run(String prefix, MessageDigest messageDigest) throws IOException {
        ...
        for (String source2 : this.inputs) {
            ...
            if (file.isFile()) {
                this.println(prefix, DigestUtils.digest(messageDigest, file), source2);
                continue;
            }
            if (file.isDirectory()) {
            ...
            byte[] bytes = source2.getBytes(Charset.defaultCharset());
