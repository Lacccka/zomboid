...
public class CertificateStatusRequest {
    ...
    public void encode(OutputStream outputStream2) throws IOException {
        TlsUtils.writeUint8(this.statusType, outputStream2);
        switch (this.statusType) {
            case 1: {
                ((OCSPStatusRequest)this.request).encode(outputStream2);
                break;
            }
            default: {
    ...
    public static CertificateStatusRequest parse(InputStream inputStream2) throws IOException {
        OCSPStatusRequest oCSPStatusRequest;
        short s = TlsUtils.readUint8(inputStream2);
        switch (s) {
            case 1: {
                oCSPStatusRequest = OCSPStatusRequest.parse(inputStream2);
                break;
            }
            default: {
