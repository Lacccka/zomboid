...
extends AbstractTlsKeyExchange {
    ...
    public void processServerKeyExchange(InputStream inputStream2) throws IOException {
        ...
        if (this.tlsSigner != null) {
            ...
            inputStream3 = new TeeInputStream(inputStream2, signerInputBuffer);
        }
        ServerSRPParams serverSRPParams = ServerSRPParams.parse(inputStream3);
        if (signerInputBuffer != null) {
            DigitallySigned digitallySigned = this.parseSignature(inputStream2);
    ...
    public void generateClientKeyExchange(OutputStream outputStream2) throws IOException {
        ...
        TlsSRPUtils.writeSRPParameter(bigInteger, outputStream2);
    ...
    public void processClientKeyExchange(InputStream inputStream2) throws IOException {
        try {
            this.srpPeerCredentials = SRP6Util.validatePublicValue(this.srpGroup.getN(), TlsSRPUtils.readSRPParameter(inputStream2));
        }
        catch (CryptoException cryptoException) {
            throw new TlsFatalAlert(47, (Throwable)cryptoException);
        }
