...
class PEMUtil {
    ...
    private String readLine(InputStream inputStream2) throws IOException {
        ...
        while (true) {
            if ((n = inputStream2.read()) != 13 && n != 10 && n >= 0) {
    ...
    ASN1Sequence readPEMObject(InputStream inputStream2) throws IOException {
        String string;
        StringBuffer stringBuffer = new StringBuffer();
        while (!((string = this.readLine(inputStream2)) == null || string.startsWith(this._header1) || string.startsWith(this._header2) || string.startsWith(this._header3))) {
        }
        while (!((string = this.readLine(inputStream2)) == null || string.startsWith(this._footer1) || string.startsWith(this._footer2) || string.startsWith(this._footer3))) {
