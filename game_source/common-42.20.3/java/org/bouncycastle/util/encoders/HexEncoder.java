...
implements Encoder {
    ...
    public int encode(byte[] byArray, int n, int n2, OutputStream outputStream2) throws IOException {
        for (int i = n; i < n + n2; ++i) {
            ...
            outputStream2.write(this.encodingTable[n3 >>> 4]);
            outputStream2.write(this.encodingTable[n3 & 0xF]);
        }
        return n2 * 2;
    }
    ...
    public int decode(byte[] byArray, int n, int n2, OutputStream outputStream2) throws IOException {
        ...
        while (n5 < n3) {
            byte by;
            while (n5 < n3 && HexEncoder.ignore((char)byArray[n5])) {
                ++n5;
            }
            ...
            if ((by2 | (by = this.decodingTable[byArray[n5++]])) < 0) {
                throw new IOException("invalid characters encountered in Hex data");
            }
            outputStream2.write(by2 << 4 | by);
            ++n4;
        }
        return n4;
    }
