...
public class PEMUtil {
    ...
    ASN1Sequence readPEMObject(InputStream inputStream2) throws IOException {
        String string;
        StringBuffer stringBuffer = new StringBuffer();
        while ((string = this.readLine(inputStream2)) != null && !string.startsWith(this._header1) && !string.startsWith(this._header2)) {
        }
        while ((string = this.readLine(inputStream2)) != null && !string.startsWith(this._footer1) && !string.startsWith(this._footer2)) {
