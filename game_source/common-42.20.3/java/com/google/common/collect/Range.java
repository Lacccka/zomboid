...
Serializable {
    ...
    public boolean containsAll(Iterable<? extends C> values2) {
        if (Iterables.isEmpty(values2)) {
            return true;
        }
        if (values2 instanceof SortedSet) {
            SortedSet<C> set = Range.cast(values2);
