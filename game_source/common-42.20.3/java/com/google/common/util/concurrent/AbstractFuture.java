...
extends FluentFuture<V> {
    ...
    public V get(long timeout2, TimeUnit unit) throws InterruptedException, TimeoutException, ExecutionException {
        ...
        long remainingNanos;
        block11: {
            remainingNanos = unit.toNanos(timeout2);
        ...
        if (this.isDone()) {
            throw new TimeoutException("Waited " + timeout2 + " " + Ascii.toLowerCase(unit.toString()) + " but future completed as timeout expired");
        }
        throw new TimeoutException("Waited " + timeout2 + " " + Ascii.toLowerCase(unit.toString()) + " for " + futureToString);
    }
