...
public final class PairedStatsAccumulator {
    ...
    public void addAll(PairedStats values2) {
        if (values2.count() == 0L) {
            return;
        }
        this.xStats.addAll(values2.xStats());
        this.sumOfProductsOfDeltas = this.yStats.count() == 0L ? values2.sumOfProductsOfDeltas() : (this.sumOfProductsOfDeltas += values2.sumOfProductsOfDeltas() + (values2.xStats().mean() - this.xStats.mean()) * (values2.yStats().mean() - this.yStats.mean()) * (double)values2.count());
        this.yStats.addAll(values2.yStats());
    }
