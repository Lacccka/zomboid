...
final class FieldParser {
    ...
    static String parseFieldsInGeneralPurpose(String rawInformation) throws NotFoundException {
        ...
        for (Object[] dataLength : TWO_DIGIT_DATA_LENGTH) {
            if (!dataLength[0].equals(firstTwoDigits)) continue;
            if (dataLength[1] == VARIABLE_LENGTH) {
                return FieldParser.processVariableAI(2, (Integer)dataLength[2], rawInformation);
            }
            return FieldParser.processFixedAI(2, (Integer)dataLength[1], rawInformation);
        }
        ...
        for (Object[] dataLength : THREE_DIGIT_DATA_LENGTH) {
            if (!dataLength[0].equals(firstThreeDigits)) continue;
            if (dataLength[1] == VARIABLE_LENGTH) {
                return FieldParser.processVariableAI(3, (Integer)dataLength[2], rawInformation);
            }
            return FieldParser.processFixedAI(3, (Integer)dataLength[1], rawInformation);
        }
        for (Object[] dataLength : THREE_DIGIT_PLUS_DIGIT_DATA_LENGTH) {
            if (!dataLength[0].equals(firstThreeDigits)) continue;
            if (dataLength[1] == VARIABLE_LENGTH) {
                return FieldParser.processVariableAI(4, (Integer)dataLength[2], rawInformation);
            }
            return FieldParser.processFixedAI(4, (Integer)dataLength[1], rawInformation);
        }
        ...
        for (Object[] dataLength : FOUR_DIGIT_DATA_LENGTH) {
            if (!dataLength[0].equals(firstFourDigits)) continue;
            if (dataLength[1] == VARIABLE_LENGTH) {
                return FieldParser.processVariableAI(4, (Integer)dataLength[2], rawInformation);
            }
            return FieldParser.processFixedAI(4, (Integer)dataLength[1], rawInformation);
    ...
    private static String processFixedAI(int aiSize, int fieldSize, String rawInformation) throws NotFoundException {
        ...
        String parsedAI = FieldParser.parseFieldsInGeneralPurpose(remaining);
    ...
    private static String processVariableAI(int aiSize, int variableFieldSize, String rawInformation) throws NotFoundException {
        String ai = rawInformation.substring(0, aiSize);
        int maxSize = rawInformation.length() < aiSize + variableFieldSize ? rawInformation.length() : aiSize + variableFieldSize;
