...
extends FinderPatternFinder {
    ...
    private FinderPattern[][] selectMutipleBestPatterns() throws NotFoundException {
        ...
        block0: for (int i1 = 0; i1 < size - 2; ++i1) {
            ...
            block1: for (int i2 = i1 + 1; i2 < size - 1; ++i2) {
                ...
                if (vModSize12A > 0.5f && vModSize12 >= 0.05f) continue block0;
                for (int i3 = i2 + 1; i3 < size; ++i3) {
                    ...
                    if (vModSize23A > 0.5f && vModSize23 >= 0.05f) continue block1;
                    ...
                    if (estimatedModuleCount > 180.0f || estimatedModuleCount < 9.0f || (vABBC = Math.abs((dA - dB) / Math.min(dA, dB))) >= 0.1f || (vPyC = Math.abs((dC - (dCpy = (float)Math.sqrt(dA * dA + dB * dB))) / Math.min(dC, dCpy))) >= 0.1f) continue;
                    results.add(test);
                }
            }
        }
    ...
    public FinderPatternInfo[] findMulti(Map<DecodeHintType, ?> hints) throws NotFoundException {
        ...
        for (ResultPoint[] resultPointArray : patternInfo) {
            ResultPoint.orderBestPatterns(resultPointArray);
            result.add(new FinderPatternInfo((FinderPattern[])resultPointArray));
        }
        if (result.isEmpty()) {
