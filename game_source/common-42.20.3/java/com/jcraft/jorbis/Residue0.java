...
class Residue0
extends FuncResidue {
    private static int[][][] inversePartword01 = new int[2][][];
    static int[][] inversePartword2;
    ...
    static synchronized int _01inverse(Block vb, Object vl, float[][] in, int ch, int decodepart) {
        ...
        if (inversePartword01.length < ch) {
            inversePartword01 = new int[ch][][];
        }
        for (j = 0; j < ch; ++j) {
            if (inversePartword01[j] != null && inversePartword01[j].length >= partwords) continue;
            Residue0.inversePartword01[j] = new int[partwords][];
        }
            ...
            while (i < partvals) {
                if (s == 0) {
                    for (j = 0; j < ch; ++j) {
                        ...
                        Residue0.inversePartword01[j][l] = look.decodemap[temp];
                        if (inversePartword01[j][l] != null) continue;
                        return 0;
                    }
                }
                for (int k = 0; k < partitions_per_word && i < partvals; ++k, ++i) {
                    for (j = 0; j < ch; ++j) {
                        ...
                        int index = inversePartword01[j][l][k];
    ...
    static synchronized int _2inverse(Block vb, Object vl, float[][] in, int ch) {
        ...
        int partvals = n / samples_per_partition;
        int partwords = (partvals + partitions_per_word - 1) / partitions_per_word;
        if (inversePartword2 == null || inversePartword2.length < partwords) {
            inversePartword2 = new int[partwords][];
        }
        for (int s = 0; s < look.stages; ++s) {
            int i = 0;
            int l = 0;
            while (i < partvals) {
                if (s == 0) {
                    ...
                    Residue0.inversePartword2[l] = look.decodemap[temp];
                    if (inversePartword2[l] == null) {
                        return 0;
                    }
                }
                for (int k = 0; k < partitions_per_word && i < partvals; ++k, ++i) {
                    CodeBook stagebook;
                    int offset = info.begin + i * samples_per_partition;
                    int index = inversePartword2[l][k];
    ...
    Object look(DspState vd, InfoMode vm, Object vr) {
        ...
        LookResidue0 look = new LookResidue0(this);
        int acc = 0;
        int maxstage = 0;
        look.info = info;
        look.map = vm.mapping;
    ...
    Object unpack(Info vi, Buffer opb) {
        int j;
        int acc = 0;
        InfoResidue0 info = new InfoResidue0(this);
