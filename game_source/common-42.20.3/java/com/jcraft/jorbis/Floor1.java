...
extends FuncFloor {
    ...
    Object inverse1(Block vb, Object ii, Object memo) {
        ...
        if (vb.opb.read(1) == 1) {
            ...
            if (memo instanceof int[]) {
                int[] ints;
                fit_value = ints = (int[])memo;
            }
            if (fit_value == null || fit_value.length < look.posts) {
                fit_value = new int[look.posts];
            } else {
                for (int i2 = 0; i2 < fit_value.length; ++i2) {
                    fit_value[i2] = 0;
                }
            }
            ...
            for (i = 0; i < info.partitions; ++i) {
                ...
                int csub = 1 << csubbits;
                int cval = 0;
                if (csubbits != 0 && (cval = books[info.classBook[clss]].decode(vb.opb)) == -1) {
                    return null;
                }
    ...
    Object look(DspState vd, InfoMode mi, Object i) {
        ...
        LookFloor1 look = new LookFloor1(this);
        look.vi = info;
        look.n = info.postlist[1];
        for (j2 = 0; j2 < info.partitions; ++j2) {
            _n += info.classDim[info.partitionclass[j2]];
        }
        look.posts = _n += 2;
        for (j2 = 0; j2 < _n; ++j2) {
            sortpointer[j2] = j2;
    ...
    Object unpack(Info vi, Buffer opb) {
        ...
        int count = 0;
        int maxclass = -1;
        InfoFloor1 info = new InfoFloor1(this);
        ...
        for (j = 0; j < maxclass + 1; ++j) {
            ...
            for (k = 0; k < 1 << info.classSubs[j]; ++k) {
                ...
                if (info.classSubbook[j][k] >= -1 && info.classSubbook[j][k] < vi.books) continue;
                info.free();
                return null;
            }
        }
    ...
    class LookFloor1 {
        ...
        LookFloor1(Floor1 this$0) {
            Objects.requireNonNull(this$0);
            this.forwardIndex = new int[65];
            this.hineighbor = new int[63];
            this.loneighbor = new int[63];
            this.reverseIndex = new int[65];
            this.sortedIndex = new int[65];
        }
    ...
    class InfoFloor1 {
        ...
        InfoFloor1(Floor1 this$0) {
            Floor1 floor1 = this$0;
            Objects.requireNonNull(floor1);
            this.this$0 = floor1;
            this.classBook = new int[16];
            this.classDim = new int[16];
            this.classSubbook = new int[16][];
            this.classSubs = new int[16];
            this.partitionclass = new int[31];
            this.postlist = new int[65];
            for (int i = 0; i < this.classSubbook.length; ++i) {
                this.classSubbook[i] = new int[8];
            }
        }
        ...
        Object copy_info() {
            ...
            System.arraycopy(info.classDim, 0, ret.classDim, 0, 16);
            System.arraycopy(info.classSubs, 0, ret.classSubs, 0, 16);
            System.arraycopy(info.classBook, 0, ret.classBook, 0, 16);
            for (int j = 0; j < 16; ++j) {
                System.arraycopy(info.classSubbook[j], 0, ret.classSubbook[j], 0, 8);
            }
            ret.mult = info.mult;
