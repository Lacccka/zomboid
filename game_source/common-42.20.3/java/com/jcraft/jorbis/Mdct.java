...
class Mdct {
    ...
    synchronized void backward(float[] in, float[] out) {
        int i;
        if (this.x.length < this.n / 2) {
            this.x = new float[this.n / 2];
        }
        if (this.w.length < this.n / 2) {
    ...
    void init(int n) {
        ...
        int CO = CE + 1;
        for (i = 0; i < n / 4; ++i) {
            this.trig[0 + i * 2] = (float)Math.cos(Math.PI / (double)n * (double)(4 * i));
            this.trig[1 + i * 2] = (float)(-Math.sin(Math.PI / (double)n * (double)(4 * i)));
