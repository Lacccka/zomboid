...
class StaticCodeBook {
    ...
    int pack(Buffer opb) {
        ...
        switch (this.maptype) {
            ...
            case 2: {
                ...
                opb.write(this.qMin, 32);
                opb.write(this.qDelta, 32);
                opb.write(this.qQuant - 1, 4);
                opb.write(this.qSequencep, 1);
                int quantvals = 0;
                switch (this.maptype) {
                    case 1: {
                ...
                for (i = 0; i < quantvals; ++i) {
                    opb.write(Math.abs(this.quantlist[i]), this.qQuant);
                }
                break;
            }
            default: {
    ...
    int unpack(Buffer opb) {
        ...
        switch (this.maptype) {
            ...
            case 1: 
            case 2: {
                this.qMin = opb.read(32);
                this.qDelta = opb.read(32);
                this.qQuant = opb.read(4) + 1;
                this.qSequencep = opb.read(1);
                int quantvals = 0;
                switch (this.maptype) {
                    case 1: {
                        quantvals = this.maptype1_quantvals();
                ...
                for (i = 0; i < quantvals; ++i) {
                    this.quantlist[i] = opb.read(this.qQuant);
                }
    ...
    float[] unquantize() {
        if (this.maptype == 1 || this.maptype == 2) {
            float mindel = StaticCodeBook.float32_unpack(this.qMin);
            float delta = StaticCodeBook.float32_unpack(this.qDelta);
