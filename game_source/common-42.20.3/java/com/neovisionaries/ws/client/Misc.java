...
class Misc {
    ...
    public static int max(int[] values2) {
        ...
        for (int i = 0; i < values2.length; ++i) {
            if (max >= values2[i]) continue;
            max = values2[i];
        }
        return max;
    }

    public static String join(Collection<?> values2, String delimiter) {
        StringBuilder builder = new StringBuilder();
        Misc.join(builder, values2, delimiter);
        return builder.toString();
    }

    private static void join(StringBuilder builder, Collection<?> values2, String delimiter) {
        boolean first = true;
