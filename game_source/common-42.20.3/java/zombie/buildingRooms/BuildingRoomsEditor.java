...
import zombie.buildingRooms.BuildingRoomsDrawer;
import zombie.buildingRooms.PlayerRoomsFile;
...
public final class BuildingRoomsEditor {
    ...
    final ObjectPool<BuildingRoomsDrawer> drawerPool = new ObjectPool<BuildingRoomsDrawer>(BuildingRoomsDrawer::new, "BuildingRoomsEditor.drawerPool");
    ...
    public boolean canAddRoomRectangle(BRERoom room, int x, int y, int w, int h, int z) {
        ...
        for (BREBuilding building : this.buildings) {
            if (building == this.currentBuilding) continue;
            if (building.intersects(x, y, w, h, z)) {
                return false;
            }
            if (!building.isAdjacent(x, y, w, h, z)) continue;
            return false;
        }
        this.tempBuildings.clear();
        IsoWorld.instance.metaGrid.getBuildingsIntersecting(x - 1, y - 1, w + 2, h + 2, this.tempBuildings);
        for (BuildingDef buildingDef : this.tempBuildings) {
            if (this.preEditBuildings.contains(buildingDef)) continue;
            if (buildingDef.intersects(x, y, w, h, z)) {
                return false;
            }
            if (!buildingDef.isAdjacent(x, y, w, h, z)) continue;
            return false;
        }
        return true;
    }

    public void callLua(String event, Object ... args2) {
        if (this.luaEditor == null) {
            return;
        }
        Object functionObj = UIManager.tableget(this.luaEditor, "OnEvent");
        if (functionObj instanceof LuaClosure) {
            Object[] argsArray = new Object[2 + args2.length];
            argsArray[0] = this.luaEditor;
            argsArray[1] = event;
            System.arraycopy(args2, 0, argsArray, 2, args2.length);
    ...
    public boolean isValid() {
        ...
        for (BREBuilding building : this.buildings) {
            if (building.getRoomCount() == 0) {
                this.invalidString = Translator.getText("IGUI_BuildingRoomsEditor_Invalid_BuildingHasNoRooms", new Object[0]);
                return false;
            }
            for (BRERoom room : building.rooms) {
                if (room.getRectangleCount() != 0) continue;
                this.invalidString = Translator.getText("IGUI_BuildingRoomsEditor_Invalid_RoomHasNoRectangles", new Object[0]);
                return false;
            }
        }
        return true;
    ...
    public void applyChanges(boolean bLoading) {
        ...
        if (!bLoading) {
            ...
            this.callLua("BeforeClear", new Object[0]);
            this.buildings.clear();
            this.callLua("AfterClear", new Object[0]);
        }
    }
    ...
    public void checkBuildingAndRoomIDs(IsoMetaCell metaCell) {
        ...
        for (i = 0; i < metaCell.buildings.size(); ++i) {
            ...
            expected = BuildingID.makeID(cellX, cellY, i);
            ...
            for (RoomDef roomDef : buildingDef.getRooms()) {
                index = metaCell.roomList.indexOf(roomDef);
                if (index == -1) {
                    DebugType.General.error("roomDef missing from IsoMetaCell.RoomList");
                }
                if ((expected = RoomID.makeID(cellX, cellY, index)) == roomDef.id) continue;
        ...
        for (i = 0; i < metaCell.roomList.size(); ++i) {
            ...
            expected = RoomID.makeID(cellX, cellY, i);
            if (expected == roomDef.id) continue;
            DebugType.General.error("roomDef.ID=%d expected=%d", roomDef.id, expected);
        }
    ...
    public void renderMain() {
        ...
        Object object = this.luaEditor.rawget("javaObject");
        if (!(object instanceof UIElement) || !(ui = (UIElement)object).isVisible().booleanValue()) {
            return;
        }
    ...
    public static void Reset() {
        ...
        BuildingRoomsEditor.instance.buildings.clear();
        BuildingRoomsEditor.instance.preEditBuildings.clear();
        BuildingRoomsEditor.instance.currentBuilding = null;
        BuildingRoomsEditor.instance.currentRoom = null;
        BuildingRoomsEditor.instance.currentLevel = 0;
        BuildingRoomsEditor.instance.luaEditor = null;
    }
