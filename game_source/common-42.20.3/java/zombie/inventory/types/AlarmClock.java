...
implements IAlarmClock {
    ...
    public void syncStopRinging() {
        IsoPlayer player = this.getOwnerPlayer(this.container);
        if (player != null) {
            INetworkPacket.sendToRelative(PacketTypes.PacketType.SyncPlayerAlarmClock, player.getX(), player.getY(), player, this.id, true, 0, 0, false, this.forceDontRing);
    ...
    public void stopRinging() {
        ...
        if (this.ringSound != -1L) {
            ...
            IsoGridSquare sq = this.getAlarmSquare();
            if (sq != null) {
                BaseSoundEmitter emitter = IsoWorld.instance.getFreeEmitter((float)sq.x + 0.5f, (float)sq.y + 0.5f, sq.z);
