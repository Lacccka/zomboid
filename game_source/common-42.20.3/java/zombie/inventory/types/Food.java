...
extends InventoryItem {
        ...
        if (outermostContainer != null) {
            ...
            if (this.isCookable && !this.isFrozen()) {
                if (this.heat > 1.6f) {
                    ...
                    int currentCookMinute2 = GameTime.getInstance().getMinutes();
                    if (currentCookMinute2 != this.lastCookMinute) {
                        if (GameServer.server) {
                            GameServer.sendItemStats(this);
                        }
                        this.lastCookMinute = currentCookMinute2;
            ...
            } else if (this.isTainted && this.heat > 1.6f && !this.isFrozen() && (currentCookMinute = GameTime.getInstance().getMinutes()) != this.lastCookMinute) {
    ...
    private void updateTemperature() {
        ...
        float temp;
        this.temperatureTimeAccum += this.timeMultiplier / 1.6f;
        ...
        float f = temp = (outermostContainer = this.getOutermostContainer()) == null ? 1.0f : outermostContainer.getTemprature();
        if (this.heat > temp) {
            this.heat -= 0.001f * this.temperatureTimeAccum;
            if (this.heat < Math.max(0.2f, temp)) {
                this.heat = Math.max(0.2f, temp);
            }
        }
        if (this.heat < temp) {
            this.heat += temp / 1000.0f * this.temperatureTimeAccum;
    ...
    public void setAutoAge() {
        ...
        if ((this.isInFridge(outermostContainer) || this.isInFreezer(outermostContainer)) && (electShutModifier = SandboxOptions.instance.elecShutModifier.getValue()) > -1) {
    ...
    public void DoTooltip(ObjectTooltip tooltipUI, ObjectTooltip.Layout layout) {
        ...
        if (this.isbDangerousUncooked() && !this.isCooked() && !this.isBurnt()) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_food_Dangerous_uncooked", new Object[0]), Core.getInstance().getBadHighlitedColor().getR(), Core.getInstance().getBadHighlitedColor().getG(), Core.getInstance().getBadHighlitedColor().getB(), 1.0f);
        ...
        if (this.getScriptItem().removeUnhappinessWhenCooked && !this.isCooked() && !this.isBurnt()) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_food_CookToRemoveUnhappiness", new Object[0]), Core.getInstance().getBadHighlitedColor().getR(), Core.getInstance().getBadHighlitedColor().getG(), Core.getInstance().getBadHighlitedColor().getB(), 1.0f);
        }
    ...
    public boolean isFreezing() {
        ItemContainer outermostContainer = this.getOutermostContainer();
        if (!this.canBeFrozen() || this.getFreezingTime() >= 100.0f || !this.isInFreezer(outermostContainer)) {
            return false;
        }
        return outermostContainer.isPowered();
    }
    ...
    public boolean isThawing() {
        ...
        if (!this.isInFreezer(outermostContainer)) {
            return true;
        }
        return !outermostContainer.isPowered();
    }
    ...
    public void syncItemFields() {
        ...
        if (outer != null) {
            IsoObject isoObject = outer.getParent();
            if (isoObject instanceof IsoPlayer) {
                ...
                INetworkPacket.sendToRelative(PacketTypes.PacketType.ItemStats, sq.getX(), (float)sq.getY(), this.getContainer(), this);
            }
            return;
        }
        if (GameClient.client) {
    ...
    private void initAnimalIcons() {
        ...
        if (corpse.isSkeleton()) {
            icon = isBaby ? breed.invIconBabySkel : (corpse.isFemale() ? breed.invIconFemaleSkel : breed.invIconMaleSkel);
        } else if (isBaby) {
            icon = breed.invIconBabyDead;
        } else {
            String string = icon = corpse.isFemale() ? breed.invIconFemaleDead : breed.invIconMaleDead;
        }
        if (icon == null) {
            return;
        }
        ...
        if ((textureRotten = Texture.getSharedTexture(icon + "Rotten")) != null) {
