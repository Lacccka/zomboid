...
IUpdater {
    ...
    private boolean canBecomeWetInRain() {
        ...
        return this.is(ItemKey.Drainable.BATH_TOWEL, ItemKey.Drainable.DISH_CLOTH);
    }

    private void replaceWetTowel() {
        Object item = InventoryItemFactory.CreateItem(this.getReplaceOnDeplete());
        this.getWorldItem().swapItem((InventoryItem)item);
    }

    private void updateTowelWetness() {
        if (!RainManager.isRaining().booleanValue() || !this.canBecomeWetInRain()) {
        ...
        this.towelWetnessAccumulator += 0.5f * RainManager.getRainIntensity() / 1000.0f * this.timeMultiplier;
        ...
        this.syncItemFields();
        this.towelWetnessAccumulator -= (float)use * this.getUseDelta();
        if (uses <= 0) {
            this.replaceWetTowel();
        }
    }
    ...
    public void update() {
        ...
        if (outermostContainer != null) {
            ...
            if (this.heat > temp) {
                this.heat -= 0.001f * this.timeMultiplier;
                if (this.heat < Math.max(0.2f, temp)) {
                    this.heat = Math.max(0.2f, temp);
                }
            }
            if (this.heat < temp) {
                this.heat += temp / 1000.0f * this.timeMultiplier;
        ...
        if (this.useWhileUnequiped && this.uses > 0 && (this.canBeActivated() && this.isActivated() || !this.canBeActivated())) {
            this.ticks += this.timeMultiplier;
