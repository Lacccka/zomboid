...
public final class ItemContainer {
    ...
    public void setCapacity(int capacity) {
        if (this.getVehiclePart() != null && this.capacity > 1000) {
            DebugType.General.warn("Attempting to set capacity of " + String.valueOf(this) + "over maximum capacity of 1000");
        } else if (this.containingItem != null && this.containingItem instanceof InventoryItem && this.capacity > 50) {
            DebugType.General.warn("Attempting to set capacity of " + String.valueOf(this.containingItem) + "over maximum capacity of 50");
        } else if (capacity > 100) {
            DebugType.General.warn("Attempting to set capacity of " + String.valueOf(this) + "over maximum capacity of 100");
        }
    ...
    public boolean canHumanCorpseFit(IsoGameCharacter chr) {
        ...
        if (this.isVehiclePart()) {
            ...
            float availableSpace = hasChr ? (float)this.getEffectiveCapacity(chr) - this.getContentsWeight() : this.getAvailableWeightCapacity();
        ...
        String[] allowedContainers = new String[]{"bin", "cardboardbox", "crate", "militarycrate", "clothingdryer", "clothingdryerbasic", "clothingwasher", "coffin", "doghouse", "dumpster", "fireplace", TYPE_FRIDGE, TYPE_FREEZER, "locker", "militarylocker", "postbox", "shelter", "tent", "wardrobe"};
    ...
    public static boolean isObjectPowered(IsoObject parent, boolean includeGenerators) {
        ...
        if (tempObjects.isEmpty()) {
            ...
            if (parent.getPipedFuelAmount() > 0) {
                return includeGenerators && sq.haveElectricity() || sq.hasGridPower();
            }
            return ItemContainer.isSquarePowered(sq, includeGenerators);
        }
        ...
        for (i = 0; i < tempObjects.size(); ++i) {
            ...
            if (!ItemContainer.isSquarePowered(testSq, includeGenerators)) continue;
            return true;
        }
        return false;
    }
    ...
    public float getTemprature() {
        ...
        if (this.isPowered() && (this.isStove() || "microwave".equals(this.type)) && (isoObject = this.parent) instanceof IsoStove) {
        ...
        if ((this.isFridge() || this.isFreezer()) && (float)(GameTime.getInstance().getWorldAgeHours() / 24.0 + (double)((SandboxOptions.instance.timeSinceApo.getValue() - 1) * 30)) == (float)SandboxOptions.instance.getElecShutModifier() && GameTime.instance.getTimeOfDay() < 13.0f) {
    ...
    public boolean isFreezer() {
        ...
        return this.type.equals(ContainerType.FREEZER.toString()) || this.type.equals(TYPE_FREEZER);
    }
    ...
    public boolean isFridge() {
        ...
        if (this.type.equals(ContainerType.FRIDGE.toString()) || this.type.equals(TYPE_FRIDGE)) {
            return true;
        }
        if (this.parent != null) {
            return this.parent.getProperties() != null && this.parent.getProperties().has(IsoPropertyType.IS_FRIDGE);
        }
        return false;
    }
    ...
    extends ObjectPool<InventoryItemList> {
        public InventoryItemListPool() {
            super(InventoryItemList::new, InventoryItemList.class.getSimpleName());
        }
    ...
    private static final class Predicates {
        private final ObjectPool<CategoryPredicate> category = new ObjectPool<CategoryPredicate>(CategoryPredicate::new, "Predicates.category");
        private final ObjectPool<EvalPredicate> eval = new ObjectPool<EvalPredicate>(EvalPredicate::new, "Predicates.eval");
        private final ObjectPool<EvalArgPredicate> evalArg = new ObjectPool<EvalArgPredicate>(EvalArgPredicate::new, "Predicates.evalArg");
        private final ObjectPool<TagPredicate> tag = new ObjectPool<TagPredicate>(TagPredicate::new, "Predicates.tag");
        private final ObjectPool<TagEvalPredicate> tagEval = new ObjectPool<TagEvalPredicate>(TagEvalPredicate::new, "Predicates.tagEval");
        private final ObjectPool<TagEvalArgPredicate> tagEvalArg = new ObjectPool<TagEvalArgPredicate>(TagEvalArgPredicate::new, "Predicates.tagEvalArg");
        private final ObjectPool<TypePredicate> type = new ObjectPool<TypePredicate>(TypePredicate::new, "Predicates.type");
        private final ObjectPool<TypeEvalPredicate> typeEval = new ObjectPool<TypeEvalPredicate>(TypeEvalPredicate::new, "Predicates.typeEval");
        private final ObjectPool<TypeEvalArgPredicate> typeEvalArg = new ObjectPool<TypeEvalArgPredicate>(TypeEvalArgPredicate::new, "Predicates.typeEvalArg");
    ...
    private static final class Comparators {
        ObjectPool<ConditionComparator> condition = new ObjectPool<ConditionComparator>(ConditionComparator::new, "Comparators.condition");
        ObjectPool<EvalComparator> eval = new ObjectPool<EvalComparator>(EvalComparator::new, "Comparators.eval");
        ObjectPool<EvalArgComparator> evalArg = new ObjectPool<EvalArgComparator>(EvalArgComparator::new, "Comparators.evalArg");
