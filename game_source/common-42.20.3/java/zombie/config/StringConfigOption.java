...
extends ConfigOption {
    ...
    private final Set<String> valueCacheSet = new HashSet<String>();
    ...
    public StringConfigOption(String name, String defaultValue, String[] values2) {
        super(name);
        for (int i = 0; i < values2.length; ++i) {
            if (!values2[i].equals(defaultValue)) continue;
            this.setValueInternal(defaultValue);
            break;
        }
        if (this.value == null) {
            defaultValue = values2[0];
            this.setValueInternal(defaultValue);
        }
        this.defaultValue = defaultValue;
        this.values = values2;
