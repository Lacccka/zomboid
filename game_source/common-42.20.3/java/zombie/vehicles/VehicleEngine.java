...
public final class VehicleEngine {
    ...
    private final Set<IVehicleEngineListener> listeners = new HashSet<IVehicleEngineListener>();
    ...
    public void setFeatures(int quality, int loudness, int power) {
        ...
        this.loudness = (int)((float)loudness * 0.37037036f);
        this.power = power;
    }
    ...
    public void addToWorld() {
        if (this.getState() != BaseVehicle.engineStateTypes.Idle) {
            this.speed = this.getOwner().getScript() == null ? 1000.0 : (double)this.getOwner().getScript().getEngineIdleSpeed();
        }
    }
    ...
    public void scriptReloaded() {
        if (this.isRunning()) {
            ...
            this.notifyListeners(this.state, this.state, VehicleEngineStateChangeReason.ScriptReloaded);
        }
    }
    ...
    private void updateWorldSounds() {
        int newEngineLoudness = (int)((double)this.loudness * this.speed / 2500.0);
        ...
        if (Rand.Next((int)(baseRand * (double)GameTime.instance.getInvMultiplier())) == 0) {
            WorldSoundManager.instance.addSoundRepeating(this.getOwner(), this.getXi(), this.getYi(), this.getZi(), Math.max(8, newEngineLoudness), Math.max(6, newEngineLoudness / 3), false, true);
        }
        if (Rand.Next((int)((baseRand - 85.0) * (double)GameTime.instance.getInvMultiplier())) == 0) {
            WorldSoundManager.instance.addSoundRepeating(this.getOwner(), this.getXi(), this.getYi(), this.getZi(), Math.max(8, newEngineLoudness / 2), Math.max(6, newEngineLoudness / 3), false, true);
        }
        if (Rand.Next((int)((baseRand - 110.0) * (double)GameTime.instance.getInvMultiplier())) == 0) {
            WorldSoundManager.instance.addSoundRepeating(this.getOwner(), this.getXi(), this.getYi(), this.getZi(), Math.max(8, newEngineLoudness / 4), Math.max(6, newEngineLoudness / 3), false, true);
        }
        WorldSoundManager.instance.addSoundRepeating(this.getOwner(), this.getXi(), this.getYi(), this.getZi(), Math.max(8, newEngineLoudness / 6), Math.max(6, newEngineLoudness / 3), false, true);
        this.emitAnimalFleeingWorldSound(newEngineLoudness);
    }

    private void emitAnimalFleeingWorldSound(int loudness) {
        boolean flags = true;
        WorldSoundManager.instance.addSoundRepeating((Object)this.getOwner(), this.getXi(), this.getYi(), this.getZi(), Math.max(MIN_ANIMAL_FLEE_SOUND_RADIUS, loudness / 6), Math.max(6, loudness / 3), (short)1);
    }
    ...
    private void updateStarting() {
        ...
        if (this.shouldFailToStartDueToCold()) {
    ...
    public boolean isStarting() {
        return this.state == BaseVehicle.engineStateTypes.Starting || this.state == BaseVehicle.engineStateTypes.StartingFailed || this.state == BaseVehicle.engineStateTypes.StartingSuccess;
    }

    public boolean isStarted() {
        return this.state == BaseVehicle.engineStateTypes.Starting || this.state == BaseVehicle.engineStateTypes.StartingFailed || this.state == BaseVehicle.engineStateTypes.StartingSuccess || this.state == BaseVehicle.engineStateTypes.RetryingStarting;
    }
