...
public final class VehicleManager {
    ...
    public void removeVehicles(IsoPlayer player) {
        ...
        ArrayList<BaseVehicle> vehiclesToRemove = new ArrayList<BaseVehicle>();
        if (player != null) {
            for (BaseVehicle baseVehicle : this.getVehicles()) {
                if (baseVehicle.hasPassenger() || !baseVehicle.isInRange(player, radius)) continue;
        ...
        if (this.idToVehicle.containsKey(vehicle.vehicleId)) {
            ...
            if (GameServer.server) {
                for (int i = 0; i < GameServer.udpEngine.connections.size(); ++i) {
                    GameServer.udpEngine.connections.get((int)i).vehicleStates.remove(vehicle.vehicleId);
                }
            }
        }
    }
        ...
        if (vehicle.vehicleId != -1) {
            ...
            if (GameServer.server) {
                for (int j = 0; j < GameServer.udpEngine.connections.size(); ++j) {
                    ...
                    if (!connection.vehicleStates.containsKey(vehicle.vehicleId)) continue;
    ...
    public void serverUpdate() {
        ...
        if (this.updatePassengers.Check()) {
            for (BaseVehicle vehicle : vehicles) {
                vehicle.updateFlags = (short)(vehicle.updateFlags | 0x4000);
            }
        }
        for (int j = 0; j < GameServer.udpEngine.connections.size(); ++j) {
            UdpConnection connection = GameServer.udpEngine.connections.get(j);
            if (GameServer.isDelayedDisconnect(connection)) continue;
            this.sendVehicles(connection);
        }
        for (BaseVehicle vehicle : vehicles) {
            if ((vehicle.updateFlags & 0xBF0) != 0) {
    ...
    private void sendVehicles(UdpConnection connection) {
        ...
        for (BaseVehicle vehicle : IsoWorld.instance.currentCell.getVehicles()) {
            BaseVehicle.ServerVehicleState state;
            if (vehicle.vehicleId == -1) {
                vehicle.vehicleId = this.idToVehicle.allocateID();
                this.registerVehicle(vehicle);
            }
            if (!connection.isRelevantTo(vehicle.getX(), vehicle.getY()) || !(state = this.getVehicleState(connection, vehicle)).shouldSend(vehicle)) continue;
            if ((state.flags & 1) != 0) {
                INetworkPacket.send(connection, PacketTypes.PacketType.VehicleFullUpdate, vehicle);
                state.flags = (short)(state.flags | 0x6002);
            } else {
    ...
    private void vehiclePosUpdate(BaseVehicle vehicle, float[] ff) {
        ...
        if ((double)tempVector3f.x < 0.99 || (double)tempVector3f.y < 0.99 || (double)tempVector3f.z < 0.99) {
        ...
        vehicle.setSpeedKmHour(vehicle.jniLinearVelocity.length() * 3.6f * (float)PZMath.sign(forward.dot(vehicle.jniLinearVelocity)));
        ...
        if (vehicle.interpolation.interpolationDataGet(dd, this.engineSound, towedByInterpolation)) {
            ...
            if (vehicle.isNetPlayerAuthorization(BaseVehicle.Authorization.Remote) || vehicle.isNetPlayerAuthorization(BaseVehicle.Authorization.RemoteCollide)) {
                for (int i = 0; i < this.vehicles.size(); ++i) {
                    ...
                    if (vehicle2 == vehicle || vehicle2 == vehicle.getVehicleTowedBy() || vehicle2 == vehicle.getVehicleTowing() || !vehicle2.isNetPlayerAuthorization(BaseVehicle.Authorization.Remote) && !vehicle2.isNetPlayerAuthorization(BaseVehicle.Authorization.RemoteCollide) || !((distance = IsoUtils.DistanceTo(x, y, z, vehicle2.getX(), vehicle2.getY(), vehicle2.getZ())) < 10.0f)) continue;
                    collide = true;
                    break;
                }
            }
    ...
    public void clientUpdate() {
        ...
        for (int i = 0; i < vehicles.size(); ++i) {
            ...
            if (vehicle.getVehicleTowing() == null) continue;
    ...
    public short getTowedVehicleID(short towingID) {
        if (this.towedVehicleMap.containsKey(towingID)) {
            return this.towedVehicleMap.get(towingID);
        }
        return -1;
    }
