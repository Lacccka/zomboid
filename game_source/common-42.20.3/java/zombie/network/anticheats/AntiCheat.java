...
public enum AntiCheat {
    ...
    public boolean isValid(UdpConnection connection, INetworkPacket packet) {
        String result;
        if (GameServer.server && this.isEnabled() && (result = this.anticheat.validate(connection, packet)) != null) {
            this.anticheat.react(connection, packet);
            if ("".equals(result)) {
                AntiCheat.log(connection, this, connection.getValidator().getCounters().getOrDefault((Object)this, 0), packet.getClass().getSimpleName() + ": skip");
            } else {
    ...
    public static void update(UdpConnection connection) {
        for (AntiCheat antiCheat : AntiCheat.values()) {
            ...
            antiCheat.act(connection, REASON_UPDATE_FAILED);
        }
    }

    public static void log(UdpConnection connection, String message) {
        message = (String)message + " ping=" + connection.getLastPing();
        DebugType.Multiplayer.warn(message);
        LoggerManager.getLogger("user").write((String)message);
        ChatServer.getInstance().sendMessageToAdminChat((String)message);
        EventManager.instance().report((String)message);
    }

    public static void log(UdpConnection connection, AntiCheat antiCheat, int counter, String text) {
        if (antiCheat.isEnabled()) {
            try {
                String message = String.format("Anti-cheat=\"%s\" is triggered for connection=\"%s\" server-option=\"%s\" reason=\"%s\" counter=%d/%d action=\"%s\"", antiCheat.name(), connection.getUserName(), antiCheat.option.getName(), text, counter, antiCheat.maxSuspiciousCounter, Policy.name(antiCheat.option.getValue()));
                AntiCheat.log(connection, message);
            }
            catch (Exception e) {
