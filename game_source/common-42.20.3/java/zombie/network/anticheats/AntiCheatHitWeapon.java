...
public class AntiCheatHitWeapon
extends AbstractAntiCheat {
    ...
    public String validate(UdpConnection connection, INetworkPacket packet) {
        ...
        if (this.isDistanceExceeded(weapon, distance)) {
            return String.format("distance=%f is greater than weapon range", Float.valueOf(distance));
        }
        if (weapon.isAimedFirearm() && !field.isIgnoreDamage() && !player.isUnlimitedAmmo() && this.isAmmoExceeded(player, weapon, field.getShotID())) {
            return "not enough ammo";
        }
        if (this.isRateExceeded(player, weapon)) {
            return "attack rate is too high";
        }
        return result;
    }
    ...
    private boolean isRateExceeded(IsoPlayer player, HandWeapon weapon) {
        ...
        int n = hitCount = SandboxOptions.instance.multiHitZombies.getValue() ? PZMath.max(3, weaponHitCount) : weaponHitCount;
        if (!weapon.isAimedFirearm()) {
            return attackRateChecker.check(hitCount, 400L);
        }
        if ("Auto".equals(weapon.getFireMode())) {
            return attackRateChecker.check(hitCount, 15L);
        }
        return attackRateChecker.check(hitCount, 150L);
    }
