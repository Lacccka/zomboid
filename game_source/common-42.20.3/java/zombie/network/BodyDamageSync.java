...
public class BodyDamageSync {
    ...
    public void startSendingUpdates(short localId, short remoteID) {
        ...
        BodyDamageSync.noise("start sending updates to " + remoteID);
        for (Updater updater2 : this.updaters) {
            if (updater2.localId != localId || updater2.remoteId != remoteID) continue;
            updater2.bdSent.RestoreToFullHealth();
            updater2.bdSent.setBodyPartsLastState();
            return;
        }
        IsoPlayer player = GameServer.IDToPlayerMap.get(localId);
        updater2 = new Updater();
        updater2.localId = localId;
        updater2.remoteId = remoteID;
        updater2.bdLocal = player.getBodyDamage();
        updater2.bdSent = new BodyDamage(player);
    ...
    public void stopSendingUpdates(short localIndex, short remoteID) {
        ...
        BodyDamageSync.noise("stop sending updates to " + remoteID);
        for (int i = 0; i < this.updaters.size(); ++i) {
            Updater updater = this.updaters.get(i);
            if (updater.localId != localIndex || updater.remoteId != remoteID) continue;
    ...
    public static final class Updater {
        ...
        private void update() {
            ...
            if (this.compareFloats(this.bdLocal.getOverallBodyHealth(), (int)this.bdSent.getOverallBodyHealth()) || painLevelLocal != this.bdSent.getRemotePainLevel() || this.bdLocal.isFakeInfected != this.bdSent.isFakeInfected || this.compareFloats(localZombieInfection, remoteZombieInfection)) {
                bb.put((byte)50);
        ...
        private boolean compareFloats(float value1, float value2) {
            if (Float.compare(value1, 0.0f) != Float.compare(value2, 0.0f)) {
                return true;
            }
