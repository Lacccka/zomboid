...
public class PVPLogTool {
    ...
    public static void logSafety(IsoPlayer player, String event) {
        ...
        String text = String.format("Safety: \"%s\" %s %s %s", player.getUsername(), LoggerManager.getPlayerCoords(player), event, player.getSafety().isEnabled());
        PVPLogTool.log(message, text, "LOG");
    }
    ...
    public static void logKill(IsoPlayer wielder, IsoPlayer target) {
        ...
        String text = String.format("Kill: \"%s\" %s killed \"%s\" %s", wielder.getUsername(), LoggerManager.getPlayerCoords(wielder), targetName, LoggerManager.getPlayerCoords(target));
        boolean announceAnimalDeath = ServerOptions.instance.announceAnimalDeath.getValue();
        if (!target.isAnimal() || announceAnimalDeath) {
            PVPLogTool.log(message, text, "IMPORTANT");
        }
        boolean bl = shouldAnnounce = target.isAnimal() ? announceAnimalDeath : ServerOptions.instance.announceDeath.getValue();
        if (shouldAnnounce) {
            ChatServer.getInstance().sendMessageToServerChat(message);
        }
    }

    public static void logCombat(String wielder, String wielderPosition, String target, String targetPosition, float x, float y, float z, String weapon, float damage) {
        String message = String.format("Combat: \"%s\" hit \"%s\" \"%s\" %f", wielder, target, weapon, Float.valueOf(damage));
        String text = String.format("Combat: \"%s\" %s hit \"%s\" %s weapon=\"%s\" damage=%f", wielder, wielderPosition, target, targetPosition, weapon, Float.valueOf(damage));
        PVPLogTool.log(message, text, "INFO");
        PVPEvent event = events.remove(9);
        event.reset(wielder, target, x, y, z);
        events.add(0, event);
        for (UdpConnection connection : GameServer.udpEngine.connections) {
            if (!connection.isFullyConnected() || !connection.getRole().hasCapability(Capability.PVPLogTool)) continue;
    ...
    static {
        for (int i = 0; i < 10; ++i) {
            events.add(new PVPEvent(null, null, 0.0f, 0.0f, 0.0f));
        }
    }
