...
IPositional {
    ...
    public void set(IsoPlayer player) {
        ...
        this.playerIndex = (byte)(player.isLocal() ? (int)player.getIndex() : -1);
        this.player = player;
    }
    ...
    private void parsePlayer(IConnection connection) {
        this.player = GameServer.server ? (connection != null && this.playerIndex != -1 ? GameServer.getPlayerFromConnection(connection, this.playerIndex) : GameServer.IDToPlayerMap.get(this.getID())) : (GameClient.client ? GameClient.IDToPlayerMap.get(this.getID()) : IsoPlayer.getPlayer(this.playerIndex));
    }
