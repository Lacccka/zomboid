        ...
        try {
            ...
            for (int i = 0; i < numParts; ++i) {
                ...
                if (part == null) {
                    ...
                    DebugType.Multiplayer.warn("%s: could not resolve VehiclePart index: %d. Skipping forward %d bytes.", this.getClass().getSimpleName(), partIndex, lengthOfDoorData);
                    continue;
                }
                part.getDoor().load(bb.bb, 249);
                if (bb.position() != positionOfPartDataEnd) {
                    DebugType.Multiplayer.warn("%s: unexpected buffer position for part %d. The DoorData.load hasn't loaded the same number of bytes (%d) that was written (%d).", this.getClass().getSimpleName(), partIndex, bb.position() - positionPartDataStart, positionOfPartDataEnd - positionPartDataStart);
                }
                bb.position(positionOfPartDataEnd);
            }
