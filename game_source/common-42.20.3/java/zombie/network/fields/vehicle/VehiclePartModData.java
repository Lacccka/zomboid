        ...
        try {
            ...
            for (int i = 0; i < numParts; ++i) {
                ...
                if (part == null) {
                    ...
                    DebugType.Multiplayer.warn("%s: could not resolve VehiclePart index: %d. Skipping forward %d bytes.", this.getClass().getSimpleName(), partIndex, lengthOfModData);
                    continue;
                }
                ...
                if (bb.bb.position() != positionModDataEnd) {
                    DebugType.Multiplayer.warn("%s: unexpected buffer position for part %d. The ModData.load hasn't loaded the same number of bytes (%d) that was written (%d).", this.getClass().getSimpleName(), partIndex, bb.bb.position() - positionModDataStart, positionModDataEnd - positionModDataStart);
                }
                bb.bb.position(positionModDataEnd);
            }
