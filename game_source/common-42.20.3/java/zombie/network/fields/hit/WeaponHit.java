...
implements INetworkPacketField {
    ...
    public void set(float damage, float range, float hitForce, float hitDirectionX, float hitDirectionY, boolean hitHead, boolean hitLegs, boolean removeKnife) {
        ...
        this.range = range;
        this.hitPart = hitHead ? (byte)1 : (hitLegs ? (byte)2 : (byte)4);
    ...
    public void process(IsoGameCharacter wielder, IsoGameCharacter target, HandWeapon weapon, boolean ignore) {
        ...
        if (this.isHitHead()) {
            ...
            if (GameServer.server && target instanceof IsoPlayer) {
                IsoPlayer player = (IsoPlayer)target;
                player.syncVisuals();
            }
        }
        if ((!((IsoLivingCharacter)wielder).isDoShove() || wielder.isAimAtFloor()) && wielder.DistToSquared(target) < 2.0f && Math.abs(wielder.getZ() - target.getZ()) < 0.5f && !(wielder instanceof IsoPlayer)) {
