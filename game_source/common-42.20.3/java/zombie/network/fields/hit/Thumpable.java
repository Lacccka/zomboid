...
INetworkPacketField {
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        ...
        this.index = b.getInt();
        this.isoObject = this.square != null && this.index > -1 && this.index < this.square.getObjects().size() ? this.square.getObjects().get(this.index) : null;
    }
        ...
        if (GameServer.server) {
            if (thumpable != null) {
                if (animal != null) {
                    ...
                    if ((float)thumpable.health <= 0.0f) {
                        thumpable.destroy();
