                ...
                if (this.prometheusEnabled) {
                    ...
                    for (int n = 0; n < GameServer.udpEngine.connections.size(); ++n) {
                        ...
                        for (int j = 0; j < c.players.length; ++j) {
                            ...
                            ((GaugeDataPoint)this.playerLon.labelValues(new String[]{onlineId, username})).set(5.3648E-6 * (double)(player.getX() - player.getY()) + -179.913143);
                            ((GaugeDataPoint)this.playerLat.labelValues(new String[]{onlineId, username})).set(-2.3152E-7 * (double)(player.getX() + player.getY()) + 85.052166);
                        }
                    }
                }
                for (UdpConnection connection : GameServer.udpEngine.connections) {
                    if (connection.getRole() == null || !connection.getRole().hasCapability(Capability.GetStatistic) || !connection.statisticTransmissionEnabled) continue;
                    INetworkPacket.send(connection, PacketTypes.PacketType.Statistics, new Object[0]);
                }
            }
        }
    }
