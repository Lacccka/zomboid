...
public class ServerMap {
    ...
    public void SaveAll() {
        ...
        if (GameServer.softReset || this.loadedCells.size() < 10) {
            for (ServerCell cell : this.loadedCells) {
                cell.Save(false);
                cell.UpdateVehicle();
            ...
            int i;
            for (i = 0; i < 4; ++i) {
                ServerMap.workerThreads[i] = new WorkerThread(this);
                workerThreads[i].setDaemon(true);
                workerThreads[i].start();
            }
            for (int n = 0; n < this.loadedCells.size(); ++n) {
                ServerCell cell = this.loadedCells.get(n);
                workerThreads[n % 4].putCommand(EThreadCommand.SaveCell, cell);
                cell.UpdateVehicle();
            }
            for (i = 0; i < 4; ++i) {
                workerThreads[i].putCommand(EThreadCommand.Quit, null);
            }
            while (true) {
                boolean running = false;
                for (int i2 = 0; i2 < 4; ++i2) {
                    if (ServerMap.workerThreads[i2].quit) continue;
                    running = true;
                    break;
                }
                if (!running) break;
    ...
    public void loadOrKeepRelevent(int x, int y) {
        ...
        if (cell == null) {
            ...
            if (mapLoading) {
                DebugType.MapLoading.debugln("Loading cell: " + cell.wx + ", " + cell.wy + " (" + this.toWorldCellX(cell.wx) + ", " + this.toWorldCellY(cell.wy) + ")");
            }
    ...
    public void QueuedSaveAll(boolean quit) {
        ...
        DebugLog.log("Saving took " + (double)(System.nanoTime() - this.saveStartTime) / 1000000.0 + " ms");
    }
    ...
    static {
        workerThreads = new WorkerThread[4];
    ...
    public static final class ServerCell {
        ...
        public boolean Load2() {
            ...
            for (int i = 0; i < loaded2.size(); ++i) {
                ...
                loaded2.remove(i);
                if (mapLoading) {
                    DebugType.MapLoading.debugln("loaded2=" + String.valueOf(loaded2));
                }
        ...
        public void Unload() {
            ...
            if (mapLoading) {
                DebugType.MapLoading.debugln("Unloading cell: " + this.wx + ", " + this.wy + " (" + instance.toWorldCellX(this.wx) + ", " + instance.toWorldCellY(this.wy) + ")");
            }
            for (int x = 0; x < 8; ++x) {
                for (int y = 0; y < 8; ++y) {
    ...
    extends Thread {
        ...
        public WorkerThread(ServerMap this$0) {
        ...
        void putCommand(EThreadCommand e, ServerCell cell) {
