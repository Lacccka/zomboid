        ...
        try {
            if (this.ticketId == -1) {
                for (UdpConnection c : GameServer.udpEngine.connections) {
                    if (!c.isFullyConnected() || c.getRole() == null || !c.getRole().hasCapability(Capability.CanSeeMessageForAdmin) && !c.getRole().hasCapability(Capability.AnswerTickets)) continue;
                    INetworkPacket.send(c, PacketTypes.PacketType.MessageForAdmin, "user " + this.author + " added a ticket <LINE> <LINE> " + this.message, -1, -1, -1);
                }
            }
