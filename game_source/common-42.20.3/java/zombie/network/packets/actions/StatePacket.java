...
implements INetworkPacket {
    ...
    @JSONField
    protected PZNetKahluaTableImpl params = new PZNetKahluaTableImpl(new LinkedHashMap<Object, Object>());
    @JSONField
    ...
    public void setData(Object ... values2) {
        IsoGameCharacter character = (IsoGameCharacter)values2[0];
        this.set(character.getX(), character.getY(), character.getZ());
        this.characterId.set(character);
        this.state.set((State)values2[1]);
        this.stage = (State.Stage)((Object)values2[2]);
        this.params.wipe();
        this.state.getState().setParams(character, this.stage);
        for (Map.Entry<State.Param<?>, Object> entry : this.state.getState().getParams(character).entrySet()) {
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        if (State.Stage.Exit == this.stage) {
            ...
            if (FitnessState.instance() == this.getState() && (isoGameCharacter = this.characterId.getCharacter()) instanceof IsoPlayer && (player = (IsoPlayer)isoGameCharacter).isCurrentState(FitnessState.instance())) {
    ...
    public boolean isAway() {
        ...
        boolean isClimbingRope = this.getCharacter().isCurrentState(ClimbSheetRopeState.instance()) || this.getCharacter().isCurrentState(ClimbDownSheetRopeState.instance());
        boolean isGoingToClimbWindow = this.getState().equals(ClimbThroughWindowState.instance());
        boolean wrongFloor = PZMath.fastfloor(this.getZ()) != PZMath.fastfloor(this.characterId.getCharacter().getZ()) && (!isClimbingRope || isGoingToClimbWindow);
        return distance > this.getState().awayCheckDistance() || wrongFloor;
    }

    public boolean isReady() {
        return this.isSubState || this.getCharacter().getCurrentState() == null || this.getCharacter().isCurrentState(this.getState()) || this.getCharacter().isCurrentState(ClimbSheetRopeState.instance()) && (this.getState() == ClimbDownSheetRopeState.instance() || this.getState() == PlayerFallingState.instance()) || this.getCharacter().isCurrentState(ClimbDownSheetRopeState.instance()) && (this.getState() == ClimbSheetRopeState.instance() || this.getState() == PlayerFallingState.instance()) || this.getCharacter().isCurrentState(IdleState.instance()) || this.getCharacter().isCurrentState(PlayerMovementState.instance()) && this.getState() == PlayerSitOnFurnitureState.instance() && (this.getCharacter().getStateMachine().getSubStateCount() == 0 || this.getCharacter().getStateMachine().getSubStateAt(0) == null);
    }
    ...
    public void apply() {
        ...
        if (instance.isSyncOnSquare()) {
            for (Map.Entry<String, String> event : this.events.get().entrySet()) {
                boolean isClimbRopeState;
                if (!StringUtils.isNullOrEmpty(event.getValue())) continue;
                ...
                boolean bl = isClimbRopeState = ClimbSheetRopeState.instance().equals(this.getState()) || ClimbDownSheetRopeState.instance().equals(this.getState());
                if (isActionsState && isClimbEvent || isClimbRopeState && isClimbWindowEvent) continue;
                character.reportEvent(event.getKey());
            }
        } else if (IdleState.instance().equals(this.getState()) && ClimbSheetRopeState.instance().equals(character.getCurrentState()) && !FallingConstants.isFallingHeight(this.getZ())) {
