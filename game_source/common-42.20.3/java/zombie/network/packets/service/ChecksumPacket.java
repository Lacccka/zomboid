    ...
    public void parseServer(ByteBufferReader b, UdpConnection connection) {
        ...
        switch (this.pkt) {
            case 1: {
                ...
                connection.checksumState = this.okLua && this.okScript && this.okAnim ? UdpConnection.ChecksumState.Done : UdpConnection.ChecksumState.Different;
                connection.checksumTime = System.currentTimeMillis();
                if (!(this.okLua && this.okScript && this.okAnim)) {
                    DebugType.Multiplayer.warn("user " + connection.getUserName() + " will be kicked in " + AntiCheatChecksumUpdate.DIFFERENT_CHECKSUM_STATE_TIMEOUT + "ms because Lua/script checksums do not match");
                    Object text = "";
                    if (!this.okLua) {
                    ...
                    if (!this.okAnim) {
                        text = (String)text + "Anim";
                    }
                    AntiCheat.log(connection, AntiCheat.ChecksumUpdate, 1, (String)text + " incorrect checksum");
