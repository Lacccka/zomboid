    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        ...
        block17: {
            block16: {
                ...
                if (this.spawnRegionName == null || this.spawnRegionName.length() <= "WithPlayer".length() || !this.spawnRegionName.startsWith("WithPlayer")) break block16;
                int playerId = this.spawnRegionName.charAt("WithPlayer".length()) - 49;
                if (playerId < 0 || playerId >= 4) break block17;
                boolean indexIsValid = ServerOptions.getInstance().playerRespawnWithSelf.getValue() && this.playerIndex == playerId || ServerOptions.getInstance().playerRespawnWithOther.getValue() && this.playerIndex != playerId;
