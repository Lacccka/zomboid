...
implements INetworkPacket {
    ...
    public void postpone() {
        this.characterId.getCharacter().getNetworkCharacterAI().setCorpse(this, IsoWorld.instance.currentCell.getGridSquare(this.x, this.y, this.z), this.index);
    }
    ...
    public void processClient(UdpConnection connection) {
        ...
        if (character.getCurrentSquare() != square) {
            DebugType.Multiplayer.debugln(String.format("Corpse %s(%d) teleport: position (%f ; %f) => (%f ; %f)", character.getClass().getSimpleName(), this.characterId.getID(), Float.valueOf(character.getX()), Float.valueOf(character.getY()), Float.valueOf(this.x), Float.valueOf(this.y)));
        ...
        if (character.getAnimAngleRadians() - this.angle > 1.0E-4f) {
            DebugType.Multiplayer.debugln(String.format("Corpse %s(%d) teleport: direction (%f) => (%f)", character.getClass().getSimpleName(), this.characterId.getID(), Float.valueOf(character.getAnimAngleRadians()), Float.valueOf(this.angle)));
            character.setDirectionAngle(this.angle * 57.295776f);
        ...
        if (this.isFallOnFront != character.isFallOnFront()) {
            DebugType.Multiplayer.debugln(String.format("Corpse %s(%d) teleport: pose (%s) => (%s)", character.getClass().getSimpleName(), this.characterId.getID(), character.isFallOnFront() ? "front" : "back", this.isFallOnFront ? "front" : "back"));
            character.setFallOnFront(this.isFallOnFront);
        }
        character.setCurrent(square);
        if (character instanceof IsoPlayer && !(player = (IsoPlayer)character).isAnimal() && player.getVehicle() == null && !player.getEmitter().isPlaying(SoundKey.BODY_HIT_GROUND.getSoundName())) {
