            ...
            if (GameClient.client) {
                try {
                    ...
                    if (animal == null) {
                        switch (packet.location) {
                            ...
                            case 4: {
                                ...
                                animal.setDirectionAngle(packet.prediction.direction * 57.295776f);
                                break;
                            }
                            default: {
                                DebugType.Animal.debugln("BAD animal response creation id=%d", onlineID2);
                            }
                        }
                    ...
                    switch (packet.location) {
                        ...
                        default: {
                            DebugType.Animal.debugln("BAD animal response id=%d", onlineID2);
                        }
                    }
                    animal.getNetworkCharacterAI().parse(packet);
                    DebugType.Animal.noise("Receive animal response id=%d created in %s", onlineID2, animal.hutch == null ? "word" : (animal.nestBox == -1 ? "hutch" : "nest box"));
                }
                catch (IOException e) {
                    DebugType.Animal.printException(e, String.format("Can't load animal id=%d", onlineID2), LogSeverity.Error);
                }
                continue;
            }
