...
AntiCheatPlayer.IAntiCheat {
    ...
    public void processClient(UdpConnection connection) {
        try {
            ...
            if (player == null) {
                ...
                if (player.getVehicle() == null && 2 != this.prediction.type && (networkAi.distance.getLength() > 7.0f || IsoUtils.DistanceTo(this.prediction.x, this.prediction.y, this.prediction.z, player.getX(), player.getY(), player.getZ()) > 1.0f && !PolygonalMap2.instance.lineClearCollide(player.getX(), player.getY(), this.prediction.x, this.prediction.y, this.prediction.z) && this.prediction.z > player.getZi() && (float)this.prediction.z < (float)player.getZi() - player.getHeightAboveFloor()) && (player.getCurrentState() == null || !player.getCurrentState().isSyncOnSquare())) {
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        ...
        try {
            if (player == null) {
                DebugType.General.error("receivePlayerUpdate: Server received position for unknown player (id:" + this.id.getDescription() + "). Server will ignore this data.");
            } else {
                NetworkPlayerAI networkAi = player.getNetworkCharacterAI();
                if (!networkAi.isHitByVehicle()) {
                    networkAi.parse(this);
                }
                if (networkAi.distance.getLength() > (float)IsoChunkMap.chunkWidthInTiles) {
        ...
        if (player != null) {
            for (int n = 0; n < GameServer.udpEngine.connections.size(); ++n) {
                ...
                if (connection.getConnectedGUID() == c.getConnectedGUID() || !c.isFullyConnected() || (!player.checkCanSeeClient(c) || !c.isRelevantTo(this.prediction.x, this.prediction.y)) && (packetType != PacketTypes.PacketType.PlayerUpdateReliable || c.getRole().getPosition() <= connection.getRole().getPosition() && c.getRole() != Roles.getDefaultForAdmin())) continue;
    ...
    public boolean validateHitVehicleDistance() {
        return this.hitVehicleId.getVehicle() != null && this.hitVehicleId.getVehicle().validateHitVehicleDistance(this.prediction.position.x, this.prediction.position.y);
    }

    @Override
    public void resetMovable() {
        if (!this.getPlayer().isDead()) {
            this.getPlayer().getNetworkCharacterAI().speedChecker.set(this.prediction.position.x, this.prediction.position.y, this.getPlayer().isSeatedInVehicle(), this.hitVehicleId.getVehicle());
        }
    }
}
