...
public class RemoveItemFromSquarePacket
implements INetworkPacket {
    ...
    public void setData(Object ... values2) {
        IsoObject obj = (IsoObject)values2[0];
    ...
    public void processClient(UdpConnection connection) {
        ...
        if (this.index >= 0 && this.index < sq.getObjects().size()) {
            ...
            RemoveItemFromSquarePacket.handleRemoveRadio(o, sq);
            ...
            if (o instanceof IsoWorldInventoryObject && (isoWorldInventoryObject = (IsoWorldInventoryObject)o).getItem() != null) {
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        RemoveItemFromSquarePacket.removeItemFromMap(connection, this.x, this.y, this.z, this.index);
    ...
    public static void removeItemFromMap(UdpConnection connection, int x, int y, int z, int index) {
        ...
        if (sq != null && index >= 0 && index < sq.getObjects().size()) {
            ...
            if (o instanceof IsoWorldInventoryObject) {
                ...
                RemoveItemFromSquarePacket.handleRemoveRadio(o, sq);
                if (connection != null) {
                    LoggerManager.getLogger("item").write(connection.getIDStr() + " \"" + connection.getUserName() + "\" floor -1 " + x + "," + y + "," + z + " [" + isoWorldInventoryObject.getItem().getFullType() + "]");
                }
            } else {
                Object name;
                Object object = name = o.getName() != null ? o.getName() : o.getObjectName();
                if (o.getSprite() != null && o.getSprite().getName() != null) {
                    name = (String)name + " (" + o.getSprite().getName() + ")";
                }
                if (connection != null) {
                    LoggerManager.getLogger("map").write(connection.getIDStr() + " \"" + connection.getUserName() + "\" removed " + (String)name + " at " + x + "," + y + "," + z);
                }
            }
            if (o.isTableSurface()) {
    ...
    private static void handleRemoveRadio(IsoObject o, IsoGridSquare sq) {
        ...
        for (int i = sq.getObjects().size() - 1; i >= 0; --i) {
            ...
            if (!(objI instanceof IsoRadio) || objI.getModData().isEmpty() || (idRadio = objI.getModData().rawget("RadioItemID")) == null || !(idRadio instanceof Double) || (d = (Double)idRadio).intValue() != invItem.getID()) continue;
