...
implements INetworkPacket {
    ...
    public boolean isRelevant(UdpConnection connection) {
        ...
        if (!connection.isRelevantTo(this.playerId.getPlayer().square.x, this.playerId.getPlayer().square.y)) {
            DebugType.Multiplayer.noise("[SyncItemActivated] not relevant client isoPlayer[x,y]=" + this.playerId.getPlayer().getX() + "," + this.playerId.getPlayer().getY());
            return false;
        }
        return true;
    }
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        ...
        if (inventoryItem == null || !inventoryItem.canBeActivated()) {
            DebugType.Multiplayer.noise("[SyncItemActivated] unable to find item for getPlayerNum()=" + player.getIndex() + " getOnlineID()=" + player.getOnlineID() + " itemID:" + this.itemId);
            return;
        }
