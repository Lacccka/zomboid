...
implements INetworkPacket {
    ...
    private void process() {
        ...
        for (ItemDescription itemDescription : this.items) {
            Clothing wornItem = Type.tryCastTo(this.playerId.getPlayer().getWornItems().getItemById(itemDescription.itemId), Clothing.class);
            if (wornItem == null || !itemDescription.location.equals(wornItem.getBodyLocation())) {
                InventoryItem itemForAdd = this.playerId.getPlayer().getInventory().getItemWithID(itemDescription.itemId);
                if (itemForAdd == null && (itemForAdd = InventoryItemFactory.CreateItem(itemDescription.itemType)) != null) {
                    itemForAdd.setID(itemDescription.itemId);
                }
