...
@PacketSetting(ordering=0, priority=0, reliability=3, requiredCapability=Capability.LoginOnServer, handlingType=3, anticheats={AntiCheat.HitDamage, AntiCheat.HitShortDistance, AntiCheat.Safety, AntiCheat.Speed, AntiCheat.HitVehicle})
...
AntiCheatHitVehicle.IAntiCheat {
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        ...
        if (GameServer.server) {
            this.vehicleHit.vehicleSpeed = this.vehicleId.getVehicle().getDriver() != null && this.vehicleId.getVehicle().getDriver() == this.wielder.getPlayer() ? this.vehicleId.getVehicle().getDriver().getNetworkCharacterAI().speedChecker.getSpeed() : 0.0f;
        }
    }
