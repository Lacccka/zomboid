...
@PacketSetting(ordering=0, priority=1, reliability=2, requiredCapability=Capability.LoginOnServer, handlingType=1, anticheats={AntiCheat.SafeHouseOwner})
...
AntiCheatSafeHouseNotMember.IAntiCheat {
    ...
    public boolean isConsistent(IConnection connection) {
        ...
        if (SafeHouse.hasNotSurvivedEnoughToClaim(isoPlayer)) {
            DebugType.Multiplayer.error("player is not survived enough");
            return false;
        }
        if (this.getSafehouse().isOwner(isoPlayer)) {
