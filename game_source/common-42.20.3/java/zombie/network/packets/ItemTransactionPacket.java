...
implements INetworkPacket {
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        ...
        if (GameServer.server) {
            for (Transaction.TransactionEntry entry : this.entries) {
                this.consistent = TransactionManager.isConsistent(entry.itemId, null, entry.sourceId.getContainer(), entry.destinationId.getContainer(), this.extra, this, this.playerId.getPlayer());
                if (this.consistent == 0) continue;
                break;
            }
        }
    }
    ...
    public void processClient(UdpConnection connection) {
        ...
        if (Transaction.TransactionState.Done == this.state) {
            Transaction.TransactionEntry firstEntry = (Transaction.TransactionEntry)this.entries.getFirst();
