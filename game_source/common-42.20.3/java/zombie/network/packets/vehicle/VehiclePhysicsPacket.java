...
public class VehiclePhysicsPacket
...
AntiCheatSpeed.IAntiCheat {
    ...
    public void setData(Object ... values2) {
        this.set((BaseVehicle)values2[0]);
    }
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        if (this.vehicleId.getVehicle().hasAuthorization(connection)) {
            ...
            this.vehicleId.getVehicle().jniTransform.origin.set(this.vehicleId.getVehicle().getX() - WorldSimulation.instance.offsetX, this.vehicleId.getVehicle().getZ(), this.vehicleId.getVehicle().getY() - WorldSimulation.instance.offsetY);
    ...
    public IMovable getMovable(int index) {
        ...
        if (passenger != null && (isoGameCharacter = passenger.character) instanceof IsoPlayer) {
            ...
            return player.getNetworkCharacterAI().speedChecker;
        }
        return null;
    }
    ...
    public void resetMovable() {
        for (int i = 0; i < this.getVehicle().getMaxPassengers(); ++i) {
            ...
            if (passenger == null || !((isoGameCharacter = passenger.character) instanceof IsoPlayer) || (player = (IsoPlayer)isoGameCharacter).isDead()) continue;
