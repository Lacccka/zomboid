...
implements INetworkPacket {
    ...
    private void findAlarmClockPlayer(IsoPlayer player) {
        ...
        if (inventoryItem != null && inventoryItem instanceof IAlarmClock) {
            ...
            DebugType.Multiplayer.noise("[SyncAlarmClock] unable to find alarmClock for getPlayerNum()=" + player.getIndex() + " getOnlineID()=" + player.getOnlineID() + " itemID:" + this.itemId);
        }
    }
    ...
    public boolean isRelevant(UdpConnection connection) {
        ...
        if (!connection.isRelevantTo(this.playerId.getPlayer().square.x, this.playerId.getPlayer().square.y)) {
            DebugType.Multiplayer.noise("[SyncAlarmClock] not relevant client isoPlayer[x,y]=" + this.playerId.getPlayer().getX() + "," + this.playerId.getPlayer().getY());
            return false;
        }
        return true;
    }
