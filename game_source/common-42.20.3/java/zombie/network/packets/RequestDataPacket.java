...
implements INetworkPacket {
    ...
    public void setData(Object ... values2) {
        this.setRequest((RequestID)((Object)values2[0]));
    }
    ...
    private void doProcessRequest(UdpConnection connection) {
        ...
        if (this.id == RequestID.PlayerVisited) {
            this.allocateLargeFileBB();
            largeFileBb.clear();
            WorldMapVisitedServer.getInstance().sendRequestData(connection, largeFileBbWriter);
            this.doSendRequest(connection);
        }
    ...
    private void doProcessData(ByteBufferReader bb) {
        ...
        if (this.id == RequestID.PlayerVisited) {
            try {
                WorldMapVisited.getInstance(false).receiveRequestData(bb);
            }
            catch (Exception ex) {
                ExceptionLogger.logException(ex, "Visited map data loading error", DebugType.Multiplayer, LogSeverity.Error);
            }
        }
        this.sendNextRequest(this.id);
    }

    public static void requestFromServer(RequestID id) {
        INetworkPacket.send(PacketTypes.PacketType.RequestData, new Object[]{id});
    }
    ...
    public static enum RequestID {
        ...
        public String getDescriptor() {
            return Translator.getText("IGUI_RequestID_" + this.name(), new Object[0]);
        }
    }
}
