...
public class FactionChangeTagPacket
extends FactionId
implements INetworkPacket {
    ...
    public boolean isConsistent(IConnection connection) {
        ...
        if (!this.factionTag.isConsistent(connection)) {
            DebugType.Multiplayer.error("tag is null");
            return false;
        }
        if (Faction.tagExist(this.factionTag.getTag()) && !connection.hasPlayer(this.getFaction().getOwner())) {
        ...
        if (!this.getFaction().canCreateTag()) {
