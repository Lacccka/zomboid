...
public class BanSystem {
    ...
    public static String BanUserBySteamID(String steamID, UdpConnection adminConnection, String argument, boolean ban) throws SQLException {
        ...
        if (SteamUtils.isSteamModeEnabled()) {
            ...
            DebugType.Multiplayer.println((String)message);
        }
        return message;
    }
    ...
    public static String BanIP(String ip, UdpConnection adminConnection, String argument, boolean ban) throws SQLException {
        ...
        String usernameAdmin = adminConnection != null ? adminConnection.getUserName() : "System";
        if (ban && BanSystem.isIpUsedBySteamRelayConnection(ip)) {
            return "Cannot ban IP " + ip + " (Steam Relay shared address). Use bansteamid or banuser instead.";
        }
        String connectionTypeNote = BanSystem.formatConnectionTypeNoteForIp(ip);
    ...
    private static String formatConnectionTypeNoteForIp(String ip) {
        ...
        return " (connection-type=" + c.getConnectionType().name() + ")";
    }
        ...
        if (ban) {
            ...
            if (pl != null) {
                ...
                if (c != null) {
                    ...
                    LoggerManager.getLogger("admin").write(usernameAdmin + " banned IP " + c.getIP() + " (" + c.getUserName() + ", connection-type=" + c.getConnectionType().name() + ")" + (argument != null ? argument : ""), "IMPORTANT");
