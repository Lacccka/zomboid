...
public abstract class PacketsCache {
    ...
    public boolean isLimitExceeded(PacketTypes.PacketType packetType) {
        ...
        if (!this.limits.containsKey((Object)packetType)) {
            this.limits.put(packetType, new LinkedList());
        }
        List<Long> limit = this.limits.get((Object)packetType);
        ...
        if (limit.size() > ServerOptions.getInstance().maxPacketsPerSecond.getValue()) {
            ...
            if (Core.debug) {
                StringBuilder stringBuilder = new StringBuilder();
                for (Map.Entry<PacketTypes.PacketType, List<Long>> temp : this.limits.entrySet()) {
                    if (temp.getValue().isEmpty()) continue;
                    stringBuilder.append((Object)temp.getKey()).append("=").append(temp.getValue().size()).append(", ");
                }
