...
import zombie.network.packets.ChunkNotReadyPacket;
...
public class PacketTypes {
    ...
    public static final Map<Short, PacketType> packetTypes = new TreeMap<Short, PacketType>();
    ...
    static {
        for (PacketType packetType : PacketType.values()) {
            PacketType previous = packetTypes.put(packetType.getId(), packetType);
    ...
    public static enum PacketType {
        ...
        ChunkNotReady(ChunkNotReadyPacket.class),
        ...
        private PacketType(int priority, int reliability, Capability requiredCapability, CallbackServerProcess server, CallbackClientProcess client, CallbackClientProcess clientLoading) {
            this(priority, reliability, 0, requiredCapability, PacketSetting.HandlingType.getType(server != null, client != null, clientLoading != null), server, client, clientLoading, null);
        }

        private PacketType(int priority, int reliability, int ordering, Capability requiredCapability, CallbackServerProcess server, CallbackClientProcess client, CallbackClientProcess clientLoading) {
            this(priority, reliability, (byte)ordering, requiredCapability, PacketSetting.HandlingType.getType(server != null, client != null, clientLoading != null), server, client, clientLoading, null);
        }

        private PacketType(Class<? extends INetworkPacket> handler) {
            this(2, 0, 0, Capability.LoginOnServer, 7, null, null, null, handler);
        }

        private PacketType(int priority, int reliability, byte ordering, Capability requiredCapability, int handlingType, CallbackServerProcess server, CallbackClientProcess client, CallbackClientProcess clientLoading, Class<? extends INetworkPacket> handler) {
            ...
            if (handler != null) {
                PacketSetting setting = PacketType.getPacketSetting(handler);
                if (setting == null) {
        ...
        private static PacketSetting getPacketSetting(Class<? extends INetworkPacket> cls) {
            PacketSetting[] settings = (PacketSetting[])cls.getAnnotationsByType(PacketSetting.class);
        ...
        public void onServerPacket(ByteBufferReader bb, UdpConnection connection) throws Exception {
            ...
            if (PacketAuthorization.isAuthorized(connection, this)) {
                ...
                if ((this.handlingType & 1) != 0) {
                    if (this.handler != null) {
                        ...
                        if (packet.isPostponed() || packet.shouldInstantiate()) {
                            packet = this.handler.getDeclaredConstructor(new Class[0]).newInstance(new Object[0]);
                        }
                        ...
                        if (this.anticheats != null) {
                            for (AntiCheat anticheat : this.anticheats) {
                                if (AntiCheat.None == anticheat || !anticheat.isEnabled() || anticheat.isValid(connection, packet)) continue;
        ...
        public String getFlags() {
            String separator = "-";
            Object result = String.valueOf(this.orderingChannel);
            ...
            switch (this.packetReliability) {
                ...
                case 3: {
                    result = (String)result + "RO";
                    break;
                }
                case 4: {
                    result = (String)result + "RS";
                }
            }
            return result;
        }
    ...
    public static interface CallbackClientProcess {
        public void call(ByteBufferReader var1, short var2) throws IOException;
    }

    public static interface CallbackServerProcess {
        public void call(ByteBufferReader var1, UdpConnection var2, short var3) throws Exception;
    }

    public static class PacketAuthorization {
        private static boolean isAuthorized(UdpConnection connection, PacketType type) {
            boolean isAuthorized;
            boolean bl = isAuthorized = type.requiredCapability == Capability.None || connection.getRole() != null && connection.getRole().hasCapability(type.requiredCapability);
            if (!isAuthorized || type.serverHandler == null && type.handler == null) {
                PacketAuthorization.onUnauthorized(connection, type);
            }
            return isAuthorized;
        }

        public static void onUnauthorized(UdpConnection connection, PacketType type) {
