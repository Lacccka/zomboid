...
extends UIElement {
    ...
    private boolean shouldShowRemotePlayer(WorldMapRemotePlayer remotePlayer) {
        ...
        for (int pn = 0; pn < IsoPlayer.numPlayers; ++pn) {
            ...
            if (Faction.isInSameFaction(player, remotePlayer.getUsername()) || SafeHouse.isInSameSafehouse(player.getUsername(), remotePlayer.getUsername())) {
                sameFaction = true;
            }
            ...
            for (IsoPlayer isoPlayer : GameClient.instance.getPlayers()) {
                if (!isoPlayer.getUsername().equals(remotePlayer.getUsername())) continue;
                remoteIsoPlayer = isoPlayer;
                break;
            }
            if (remoteIsoPlayer == null || !player.checkCanSeeClient(remoteIsoPlayer)) continue;
            visible = true;
        }
        GameServer.MapRemotePlayerVisibility mapRemotePlayerVisibility = GameServer.MapRemotePlayerVisibility.values()[ServerOptions.getInstance().mapRemotePlayerVisibility.getValue() - 1];
        return switch (mapRemotePlayerVisibility) {
            case GameServer.MapRemotePlayerVisibility.FACTION_ONLY -> sameFaction;
            case GameServer.MapRemotePlayerVisibility.FACTION_AND_VISIBLE_ONLY -> {
                if (sameFaction || visible) {
                    yield true;
                }
                yield false;
            }
            case GameServer.MapRemotePlayerVisibility.ALL -> cansee;
            default -> false;
        };
    }
