...
extends WorldMapBaseSymbol {
    static final ObjectPool<WorldMapTextureSymbol> s_pool = new ObjectPool<WorldMapTextureSymbol>(WorldMapTextureSymbol::new, "WorldMapTextureSymbol.s_pool");
    ...
    public void render(WorldMapRenderer.Drawer drawer) {
        ...
        if (layout.collided) {
            ...
            double x0 = 0.0 - dx;
            double y0 = 0.0 - dy;
            double x1 = x0 + (double)this.width;
            double y1 = y0 + (double)this.height;
            WorldMapTextureSymbol.getAbsolutePosition(x0, y0, pointOfRotationX, pointOfRotationY, this.getCosA(), this.getSinA(), scale, scale, leftTop);
            WorldMapTextureSymbol.getAbsolutePosition(x1, y0, pointOfRotationX, pointOfRotationY, this.getCosA(), this.getSinA(), scale, scale, rightTop);
            WorldMapTextureSymbol.getAbsolutePosition(x1, y1, pointOfRotationX, pointOfRotationY, this.getCosA(), this.getSinA(), scale, scale, rightBottom);
            WorldMapTextureSymbol.getAbsolutePosition(x0, y1, pointOfRotationX, pointOfRotationY, this.getCosA(), this.getSinA(), scale, scale, leftBottom);
            if (this.isMatchPerspective()) {
            ...
            vbor.addQuad((float)leftTop.x, (float)leftTop.y, this.texture.xStart, this.texture.yStart, (float)rightTop.x, (float)rightTop.y, this.texture.xEnd, this.texture.yStart, (float)rightBottom.x, (float)rightBottom.y, this.texture.xEnd, this.texture.yEnd, (float)leftBottom.x, (float)leftBottom.y, this.texture.xStart, this.texture.yEnd, 0.0f, color.r, color.g, color.b, color.a);
            vbor.endRun();
