...
extends AssetManager {
    ...
    protected void startLoading(Asset asset) {
        ...
        FileTask fileTask = Files.exists(Paths.get(filePath + ".bin", new String[0]), new LinkOption[0]) ? new FileTask_LoadWorldMapBinary(worldMapData, filePath + ".bin", fileSystem, result -> this.loadCallback(worldMapData, result)) : new FileTask_LoadWorldMapXML(worldMapData, filePath, fileSystem, result -> this.loadCallback(worldMapData, result));
    ...
    private void loadCallback(WorldMapData worldMapData, Object result) {
        if (result == Boolean.TRUE) {
            ...
            DebugType.General.warn("Failed to load asset: " + String.valueOf(worldMapData.getPath()));
