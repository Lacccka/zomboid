...
extends ComponentScript {
    ...
    public Texture getIconTexture() {
        ...
        if (uiConfig != null) {
            ...
            DebugType.General.error("CraftRecipeComponentScript: Recipe " + this.craftRecipe.getName() + " missing UiConfigScript - falling back to SpriteConfigScript first world object face for recipe icon.");
    ...
    private String getTranslationName() {
        String displayName = Translator.getText("EC_Entity_DisplayName_Default", new Object[0]);
        GameEntityScript gameEntityScript = (GameEntityScript)this.getParent();
        UiConfigScript uiConfig = (UiConfigScript)gameEntityScript.getComponentScriptFor(ComponentType.UiConfig);
        if (uiConfig != null) {
            XuiSkin.EntityUiStyle style;
