...
extends CraftRecipe.IOScript {
    ...
    private List<Item> itemScriptCache = new ArrayList<Item>();
    ...
    protected boolean isValid() {
        ...
        if (this.type == ResourceType.Energy) {
            return this.acceptsAnyEnergy || !this.energies.isEmpty();
        }
        return false;
    }
    ...
    public boolean hasPossibleFrozenFoodInputItems() {
        for (int i = 0; i < this.itemScriptCache.size(); ++i) {
            ...
            if (!item.isItemType(ItemType.FOOD) || item.isCantBeFrozen()) continue;
            return true;
        }
        return false;
    }
    ...
    public ArrayList<Fluid> getPossibleInputFluids() {
        if (this.acceptsAnyFluid) {
            return Fluid.getAllFluids();
        }
        return this.filteredFluidCache;
    }

    public String getInputFluidFilterDisplayName() {
        if (this.acceptsAnyFluid) {
            return Translator.getText("Fluid_Name_Any", new Object[0]);
        }
        return this.fluidFilter.getFilterDisplayName();
    }
    ...
    public String getInputFluidFilterTooltip() {
        ...
        return this.fluidFilter.getFilterTooltipText();
    }

    public ArrayList<Energy> getPossibleInputEnergies() {
        if (this.acceptsAnyEnergy) {
            return Energy.getAllEnergies();
        }
    ...
    public boolean isUsesPartialItem(Item item) {
        ...
        if (item != null && item.isItemType(ItemType.DRAINABLE)) {
            return item.getUseDelta() < 1.0f;
        }
        if (item != null && item.isItemType(ItemType.FOOD)) {
            return Math.abs(item.getHungerChange()) > 1.0f;
        }
        return false;
    }
    ...
    public boolean dontAllowFrozenItem() {
        if (this.getResourceType() != ResourceType.Item || !this.hasPossibleFrozenFoodInputItems()) {
            return false;
        }
        return !this.hasFlag(InputFlag.AllowFrozenItem);
    }
    ...
    public float getAmount(int idx) {
        if (idx < this.amounts.size()) {
            return this.amounts.get(idx).floatValue();
        }
        return this.amount;
    }
    ...
    public float getMaxAmount(int idx) {
        if (idx < this.maxamounts.size()) {
            return this.maxamounts.get(idx).floatValue();
        }
        return this.maxamount;
    }
    ...
    public float getAmount(String item) {
        ...
        for (int i = 0; i < this.items.size(); ++i) {
            Object it = this.items.get(i);
            if (!((String)it).contains(".")) {
                it = "Base." + (String)it;
            }
            if (!((String)item).equalsIgnoreCase((String)it)) continue;
    ...
    public boolean isProp1() {
        return this.flags.contains((Object)InputFlag.Prop1);
    }

    public boolean isProp2() {
        return this.flags.contains((Object)InputFlag.Prop2);
    }
    ...
    public boolean isHandcraftOnly() {
        return this.flags.contains((Object)InputFlag.HandcraftOnly);
    }

    public boolean isAutomationOnly() {
        return this.flags.contains((Object)InputFlag.AutomationOnly);
    }
    ...
    public boolean isFluidMatch(FluidContainer container) {
        ...
        if (this.isFluidExact()) {
            ...
            for (int i = 0; i < sample.size(); ++i) {
                if (this.containsFluid(sample.getFluid(i))) continue;
                fluidMatch = false;
                break;
            }
            sample.release();
        } else {
            fluidMatch = this.isFluidPrimary() ? this.containsFluid(container.getPrimaryFluid()) : true;
        }
        return fluidMatch;
    }
    ...
    protected static InputScript LoadBlock(CraftRecipe parentRecipe, ScriptParser.Block block) throws Exception {
        ...
        for (ScriptParser.Value value : block.values) {
            ...
            input = InputScript.Load(parentRecipe, s);
        }
        if (input == null) {
    ...
    protected static InputScript Load(CraftRecipe parentRecipe, String line) throws Exception {
        return InputScript.Load(parentRecipe, line, false);
    }
    ...
    protected static InputScript Load(CraftRecipe parentRecipe, String line, boolean isInternal) throws Exception {
        ...
        if (inputAmount.contains("variable")) {
            ...
            upperamount = baseamount = PZMath.max(0.0f, Float.parseFloat(inputAmount));
        }
        ...
        block5: for (int i = 2; i < elems.length; ++i) {
            String entry;
            int z;
            String[] split;
            String s = elems[i];
                ...
                for (z = 0; z < split.length; ++z) {
                    ...
                    if (type == ResourceType.Item) {
                        ...
                        if (entry.contains(":")) {
                            ...
                            input.amounts.add(Float.valueOf(itembaseamount));
                            input.maxamounts.add(Float.valueOf(itembaseamount * (upperamount / baseamount)));
                            continue;
                        }
                        input.loadedItems.add(entry);
                        input.amounts.add(Float.valueOf(baseamount));
                        input.maxamounts.add(Float.valueOf(upperamount));
                        continue;
                    }
                    if (type == ResourceType.Fluid) {
                        if ("*".equals(entry)) {
            ...
            if (s.startsWith("mode:")) {
                ...
                switch (type) {
                    case Item: {
                        if (s.equalsIgnoreCase("use")) continue block5;
                        ...
                        if (s.equalsIgnoreCase("useprop1")) {
                            DebugType.General.error("Deprecated parameter in inputscript! for recipe " + recipeName);
                            continue block5;
                        }
                        if (s.equalsIgnoreCase("useprop2")) {
                            DebugType.General.error("Deprecated parameter in inputscript! for recipe " + recipeName);
                            continue block5;
                        }
                        if (s.equalsIgnoreCase("keepprop1")) {
                            DebugType.General.error("Deprecated parameter in inputscript! for recipe " + recipeName);
                            continue block5;
                        }
                        if (s.equalsIgnoreCase("keepprop2")) {
                            DebugType.General.error("Deprecated parameter in inputscript! for recipe " + recipeName);
                            continue block5;
                        }
                        if (s.equalsIgnoreCase("prop1")) {
                            DebugType.General.error("Deprecated parameter in inputscript! for recipe " + recipeName);
                            continue block5;
                        }
                    ...
                    default: {
                        DebugType.General.warn("Cannot set mode for type = " + String.valueOf((Object)type) + " for recipe " + recipeName);
                        if (!Core.debug) continue block5;
    ...
    protected void OnPostWorldDictionaryInit() throws Exception {
        ...
        if (this.acceptsAnyItem) {
            ...
            if (this.flags.contains((Object)InputFlag.ItemIsFluid) || this.consumeFromItemScript != null && this.consumeFromItemScript.getResourceType() == ResourceType.Fluid || this.createToItemScript != null && this.createToItemScript.getResourceType() == ResourceType.Fluid) {
                allItems = ScriptManager.instance.getAllItems();
                for (int i = 0; i < allItems.size(); ++i) {
                    item = allItems.get(i);
                    if (!item.containsComponent(ComponentType.FluidContainer)) continue;
                    this.itemScriptCache.add(item);
                }
            } else if (this.flags.contains((Object)InputFlag.ItemIsEnergy) || this.consumeFromItemScript != null && this.consumeFromItemScript.getResourceType() == ResourceType.Energy || this.createToItemScript != null && this.createToItemScript.getResourceType() == ResourceType.Energy) {
                allItems = ScriptManager.instance.getAllItems();
                for (int i = 0; i < allItems.size(); ++i) {
                    item = allItems.get(i);
                    if (!item.isItemType(ItemType.DRAINABLE)) continue;
                    this.itemScriptCache.add(item);
                }
            } else {
            ...
            for (String s : this.loadedItems) {
                ...
                if (item != null) {
                    ...
                    if (!this.getParentRecipe().isBuildableRecipe()) continue;
    ...
    public boolean canUseItem(String item) {
        ...
        for (int i = 0; i < inputItems.size(); ++i) {
            ...
            if (!Objects.equals(inputItem.getName(), item) && !Objects.equals(inputItem.getFullName(), item)) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean passesSealedTest(InventoryItem item) {
        ...
        if (this.hasFlag(InputFlag.IsNotSealed)) {
            return item.getName() != null && !item.getName().contains(Translator.getFluidText("Fluid_Sealed"));
        }
        return true;
    }
    ...
    public boolean doesItemPassRoutineStatusTests(InventoryItem item, IsoGameCharacter character) {
        IsoPlayer isoPlayer;
        if (character != null && character instanceof IsoPlayer && item.isNoRecipes(isoPlayer = (IsoPlayer)character)) {
            return false;
        }
        return this.passesFavoriteTest(item) && this.passesRottenTest(item) && this.passesFrozenTest(item) && this.passesBrokenTest(item) && this.passesSealedTest(item);
    }
    ...
    public boolean doesItemPassClothingTypeStatusTests(InventoryItem item) {
        ...
        if (this.hasFlag(InputFlag.IsWorn) && !item.isWorn()) {
            return false;
        }
        return !this.hasFlag(InputFlag.IsNotWorn) || !item.isWorn() && !item.isEquipped();
    }
    ...
    public boolean doesItemPassSharpnessStatusTests(InventoryItem item) {
        ...
        if (this.hasFlag(InputFlag.IsSharpenable) && item.hasSharpness() && !(item.getSharpness() < item.getMaxSharpness())) {
            return false;
        }
        if (this.hasFlag(InputFlag.IsSharpenable) && !item.hasSharpness()) {
            if (item.hasHeadCondition()) {
                return item.getHeadCondition() > 0;
            }
    ...
    public boolean doesItemPassDamageStatusTests(InventoryItem item) {
        if (this.hasFlag(InputFlag.IsDamaged) && !item.isDamaged()) {
            return false;
        }
        return !this.hasFlag(InputFlag.IsUndamaged) || !item.isDamaged();
    }
    ...
    public boolean doesItemPassIsOrNotEmptyAndFullTests(InventoryItem item) {
        ...
        if (item instanceof InventoryContainer) {
            InventoryContainer containerItem = (InventoryContainer)item;
            if (this.hasFlag(InputFlag.IsEmpty) && !containerItem.isEmpty()) {
                return false;
            }
        }
        if (item instanceof DrainableComboItem) {
            comboItem = (DrainableComboItem)item;
        ...
        if (item instanceof Radio && (radio = (Radio)item).getDeviceData().getIsBatteryPowered()) {
        ...
        if (item instanceof WeaponPart) {
            WeaponPart weaponPart = (WeaponPart)item;
    ...
    public boolean doesItemPassFoodAndCookingTests(InventoryItem item) {
        ...
        if (this.hasFlag(InputFlag.IsWholeFoodItem) && item instanceof Food && !(food = (Food)item).isWholeFoodItem()) {
            return false;
        }
        if (this.hasFlag(InputFlag.IsUncookedFoodItem) && item instanceof Food) {
            food = (Food)item;
            if (item.isCookable() && !food.isUncooked()) {
                return false;
            }
        }
        if (this.hasFlag(InputFlag.IsCookedFoodItem) && item instanceof Food) {
            food = (Food)item;
            if (item.isCookable() && food.isUncooked()) {
                return false;
            }
    ...
    public boolean isRecordInput() {
        ...
        if (this.hasFlag(InputFlag.DontRecordInput)) {
            return false;
        }
        return this.getParentRecipe().hasTag(CraftRecipeTag.ENTITY_RECIPE);
    }
