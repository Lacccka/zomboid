    ...
    public void LoadScripts(ScriptLoadMode loadMode) {
        for (LoadData<E> data : this.loadData.values()) {
            try {
                ...
                if (this.isVerbose()) {
                    DebugType.General.debugln("[" + this.scriptType.getScriptTag() + "] load script = " + ((BaseScriptObject)script).getScriptObjectName());
                }
                ...
                if (script != null) {
                    if (((BaseScriptObject)script).getObsolete()) {
                        DebugType.Script.debugln("[" + this.scriptType.getScriptTag() + "] ignoring script, obsolete = " + ((BaseScriptObject)script).getScriptObjectName());
                        continue;
                    }
                    if (!((BaseScriptObject)script).isEnabled()) {
                        DebugType.Script.debugln("[" + this.scriptType.getScriptTag() + "] ignoring script, disabled = " + ((BaseScriptObject)script).getScriptObjectName());
                        continue;
                    }
                    if (((BaseScriptObject)script).isDebugOnly() && !Core.debug) {
                        DebugType.Script.debugln("[" + this.scriptType.getScriptTag() + "] ignoring script, is debug only = " + ((BaseScriptObject)script).getScriptObjectName());
                        continue;
                    }
