...
public abstract class BaseScriptObject {
    ...
    public final String debugString() {
        return "[type=" + String.valueOf((Object)this.scriptObjectType) + ", module=" + (this.module != null ? this.module.getName() : "null") + ", name=" + this.scriptObjectName + ", fulltype=" + this.getScriptObjectFullType() + "]";
    }
    ...
    public void calculateScriptVersion() {
        ...
        for (int i = 0; i < this.loadedScriptBodies.size(); ++i) {
            body = this.loadedScriptBodies.get(i);
            len += body.length();
        }
    ...
    public final String getScriptObjectFullType() {
        ...
        String string = scriptName = this.scriptObjectName != null ? this.scriptObjectName : null;
        if (moduleName == null || scriptName == null) {
            throw new RuntimeException("[" + String.valueOf((Object)this.scriptObjectType) + "] Module or name missing, module: " + moduleName + ", script: " + scriptName);
        }
    ...
    public final void LoadCommonBlock(ScriptParser.Block block) throws Exception {
        for (ScriptParser.Value value : block.values) {
            ...
            if (!key.equalsIgnoreCase("debugOnly")) continue;
    ...
    public final ArrayList<String> getAllScriptLines(ArrayList<String> list) {
        ...
        for (int i = this.loadedScriptBodies.size() - 2; i >= 0; i -= 2) {
            ...
            int emptyLines = 0;
            for (String s : rLines) {
                emptyLines = StringUtils.isNullOrWhitespace(s) ? ++emptyLines : 0;
    ...
    public final ArrayList<String> getBodyScriptLines(int bodyIndex, ArrayList<String> list) {
        if ((bodyIndex = bodyIndex * 2 + 1) >= 0 && bodyIndex < this.loadedScriptBodies.size()) {
