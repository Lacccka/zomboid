        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter((String)filename));){
            ...
            for (PhysicsHitReaction physicsHitReaction : physicsHitReactionList) {
                ...
                for (i = 0; i < physicsHitReaction.impulse.length; ++i) {
                    writer.write("        impulse_" + Translator.getText(RagdollBodyPart.values()[i].name(), new Object[0]).replace(" ", "_").toLowerCase() + " = " + physicsHitReaction.impulse[i] + ",\n");
                }
                for (i = 0; i < physicsHitReaction.upwardImpulse.length; ++i) {
                    writer.write("        upwardImpulse_" + Translator.getText(RagdollBodyPart.values()[i].name(), new Object[0]).replace(" ", "_").toLowerCase() + " = " + physicsHitReaction.upwardImpulse[i] + ",\n");
                }
                writer.write("    }\n\n");
            }
            writer.write("}\n");
