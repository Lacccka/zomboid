...
extends RecipeCodeHelper {
    ...
    public static void makeCoffee(CraftRecipeData data, IsoGameCharacter character) {
        InventoryItem inventoryItem = data.getAllCreatedItems().getFirst();
        if (inventoryItem instanceof Food) {
            Food result = (Food)inventoryItem;
            result.setName(Translator.getText("ContextMenu_FoodType_Coffee", new Object[0]) + " " + Translator.getText("ContextMenu_EvolvedRecipe_HotDrink", new Object[0]));
    ...
    public static void refillBlowTorch(CraftRecipeData data, IsoGameCharacter character) {
        ...
        if (inventoryItem instanceof DrainableComboItem) {
            DrainableComboItem newTorch = (DrainableComboItem)inventoryItem;
            inventoryItem = data.getAllConsumedItems().getFirst();
            if (inventoryItem instanceof DrainableComboItem) {
                DrainableComboItem oldTorch = (DrainableComboItem)inventoryItem;
                inventoryItem = data.getAllKeepInputItems().getFirst();
                if (inventoryItem instanceof DrainableComboItem) {
                    DrainableComboItem propaneTank = (DrainableComboItem)inventoryItem;
    ...
    public static void refillLighter(CraftRecipeData data, IsoGameCharacter character) {
        InventoryItem inventoryItem = data.getAllKeepInputItems().getFirst();
        if (inventoryItem instanceof DrainableComboItem) {
            DrainableComboItem lighter = (DrainableComboItem)inventoryItem;
            inventoryItem = data.getAllConsumedItems().getFirst();
            if (inventoryItem instanceof DrainableComboItem) {
                DrainableComboItem lighterFluid = (DrainableComboItem)inventoryItem;
    ...
    public static void torchBatteryInsert(CraftRecipeData data, IsoGameCharacter character) {
        ...
        InventoryItem item = data.getAllKeepInputItems().getFirst();
    ...
    public static void cutFish(CraftRecipeData data, IsoGameCharacter character) {
        Food fish = RecipeCodeOnCreate.getConsumedItems(data, Food.class).getFirst();
        if (fish != null) {
            ...
            if (Rand.NextBool(roeChance)) {
                roeWeightMult = fishName.contains(Translator.getText("IGUI_Fish_Legendary", new Object[0])) ? 0.15f : (fishName.contains(Translator.getText("IGUI_Fish_Big", new Object[0])) ? 0.1f : 0.05f);
            }
            if (!fish.isCooked()) {
    ...
    public static void createLogStack(CraftRecipeData data, IsoGameCharacter character) {
        ...
        data.getAllCreatedItems().getFirst().getModData().rawset("ropeItems", (Object)table);
    }
    ...
    public static void ripClothing(CraftRecipeData data, IsoGameCharacter character) {
        Clothing clothing = RecipeCodeOnCreate.getConsumedItems(data, Clothing.class).getFirst();
        ...
        for (int i = 0; i < nbr; ++i) {
            ...
            Object scrap = InventoryItemFactory.CreateItem(!dirty ? material : materialDirty);
            if (!dirty && ((InventoryItem)scrap).is(ItemKey.Normal.RIPPED_SHEETS)) {
                ((InventoryItem)scrap).getModData().rawset("filterLife", (Object)Float.valueOf(calculatedFilterLife));
                ((InventoryItem)scrap).syncItemFields();
            }
            RecipeCodeOnCreate.addItemToCharacterInventory(character, scrap);
        }
    ...
    public static void addToPack(CraftRecipeData data, IsoGameCharacter character) {
        DrainableComboItem item = RecipeCodeOnCreate.getKeepItems(data, DrainableComboItem.class).getFirst();
    ...
    public static void drawRandomCard(CraftRecipeData data, IsoGameCharacter character) {
        String card = Translator.getText(ServerOptions.getRandomCard(), new Object[0]);
        ...
        if (GameServer.server) {
            KahluaTableImpl table = (KahluaTableImpl)LuaManager.platform.newTable();
            table.rawset("onlineID", (Object)player.getOnlineID());
    ...
    public static void refillHurricaneLantern(CraftRecipeData data, IsoGameCharacter character) {
        ...
        DrainableComboItem result = RecipeCodeOnCreate.getCreatedItems(data, DrainableComboItem.class).getFirst();
    ...
    public static void scratchTicket(CraftRecipeData data, IsoGameCharacter character) {
        Literature result = RecipeCodeOnCreate.getCreatedItems(data, Literature.class).getFirst();
    ...
    public static void removeGasFilter(CraftRecipeData data, IsoGameCharacter character) {
        RecipeCodeOnCreate.removeTankOrFilter(character, RecipeCodeOnCreate.getKeepItems(data, Clothing.class).getFirst(), (DrainableComboItem)InventoryItemFactory.CreateItem(RecipeCodeOnCreate.getKeepItems(data, Clothing.class).getFirst().getFilterType()), true);
    }

    public static void removeOxygenTank(CraftRecipeData data, IsoGameCharacter character) {
        RecipeCodeOnCreate.removeTankOrFilter(character, RecipeCodeOnCreate.getKeepItems(data, Clothing.class).getFirst(), (DrainableComboItem)InventoryItemFactory.CreateItem(RecipeCodeOnCreate.getKeepItems(data, Clothing.class).getFirst().getTankType()), false);
    }
    ...
    private static void removeTankOrFilter(IsoGameCharacter character, Clothing maskOrSuit, DrainableComboItem tankOrFilter, boolean isFilter) {
        ...
        if (tankOrFilter.is(ItemKey.Drainable.RAG_FILTER)) {
            if (maskOrSuit.getUsedDelta() >= 0.3f) {
                Object rag = InventoryItemFactory.CreateItem(ItemKey.Normal.RIPPED_SHEETS);
                ((InventoryItem)rag).getModData().rawset("filterLife", (Object)Float.valueOf(maskOrSuit.getUsedDelta()));
                RecipeCodeOnCreate.addItemToCharacterInventory(character, rag);
            } else {
                RecipeCodeOnCreate.addItemToCharacterInventory(character, ItemKey.Normal.RIPPED_SHEETS_DIRTY);
            }
        } else {
        ...
        if ((isoPlayer = maskOrSuit.getPlayer()) instanceof IsoPlayer && (player = isoPlayer).isEquipped(maskOrSuit)) {
    ...
    public static void assignRagFilterLife(CraftRecipeData data, IsoGameCharacter character) {
        ...
        Object object = rag.getModData().rawget("filterLife");
        ...
        float filterLife = f;
        mask.getModData().rawset("filterLife", (Object)filterLife);
    ...
    public static void addGasFilter(CraftRecipeData data, IsoGameCharacter character) {
        RecipeCodeOnCreate.attachTankOrFilter((Clothing)data.getAllKeepInputItems().getFirst(), data.getAllDestroyInputItems().getFirst(), true);
    }

    public static void addOxygenTank(CraftRecipeData data, IsoGameCharacter character) {
        RecipeCodeOnCreate.attachTankOrFilter((Clothing)data.getAllKeepInputItems().getFirst(), data.getAllDestroyInputItems().getFirst(), false);
    }
    ...
    private static void attachTankOrFilter(Clothing maskOrSuit, InventoryItem tankOrFilter, boolean isFilter) {
        ...
        if (tankOrFilter.is(ItemKey.Normal.RIPPED_SHEETS)) {
            float f;
            object = tankOrFilter.getModData().rawget("filterLife");
            if (object instanceof Number) {
                Number number = (Number)object;
                f = number.floatValue();
            } else {
        ...
        object = maskOrSuit.getPlayer();
        if (object instanceof IsoPlayer && ((IsoGameCharacter)(player = object)).isEquipped(maskOrSuit)) {
            ((IsoGameCharacter)player).resetModelNextFrame();
        }
    }
    ...
    public static void scrapJewellery(CraftRecipeData data, IsoGameCharacter character) {
        ...
        for (InventoryItem item : data.getAllConsumedItems()) {
            ...
            if (item.hasTag(ItemTag.DIAMOND_JEWELLERY)) {
                addedItem = inv.addItem(ItemKey.Normal.DIAMOND);
                if (GameServer.server) {
                    GameServer.sendAddItemToContainer(inv, addedItem);
                }
            }
            if (item.hasTag(ItemTag.TWO_DIAMOND_JEWELLERY)) {
                addedItems = inv.addItems(ItemKey.Normal.DIAMOND, 2);
                if (GameServer.server) {
                    GameServer.sendAddItemsToContainer(inv, (ArrayList)addedItems);
                }
            }
            if (item.hasTag(ItemTag.EMERALD_JEWELLERY)) {
                addedItem = inv.addItem(ItemKey.Normal.EMERALD);
                if (GameServer.server) {
                    GameServer.sendAddItemToContainer(inv, addedItem);
                }
            }
            if (item.hasTag(ItemTag.TWO_EMERALD_JEWELLERY)) {
                addedItems = inv.addItems(ItemKey.Normal.EMERALD, 2);
                if (GameServer.server) {
                    GameServer.sendAddItemsToContainer(inv, (ArrayList)addedItems);
                }
            }
            if (item.hasTag(ItemTag.RUBY_JEWELLERY)) {
                addedItem = inv.addItem(ItemKey.Normal.RUBY);
                if (GameServer.server) {
                    GameServer.sendAddItemToContainer(inv, addedItem);
                }
            }
            if (item.hasTag(ItemTag.TWO_RUBY_JEWELLERY)) {
                addedItems = inv.addItems(ItemKey.Normal.RUBY, 2);
                if (GameServer.server) {
                    GameServer.sendAddItemsToContainer(inv, (ArrayList)addedItems);
                }
            }
            if (item.hasTag(ItemTag.SAPPHIRE_JEWELLERY)) {
                addedItem = inv.addItem(ItemKey.Normal.SAPPHIRE);
                if (GameServer.server) {
                    GameServer.sendAddItemToContainer(inv, addedItem);
                }
            }
            if (item.hasTag(ItemTag.TWO_SAPPHIRE_JEWELLERY)) {
                addedItems = inv.addItems(ItemKey.Normal.SAPPHIRE, 2);
                if (GameServer.server) {
                    GameServer.sendAddItemsToContainer(inv, (ArrayList)addedItems);
                }
            }
            if (!item.hasTag(ItemTag.AMETHYST_JEWELLERY)) continue;
            addedItem = inv.addItem(ItemKey.Normal.AMETHYST);
    ...
    public static void cutChicken(CraftRecipeData data, IsoGameCharacter character) {
        Food chicken = RecipeCodeOnCreate.getConsumedItems(data, Food.class).getFirst();
    ...
    public static void placeItemTypeInBox(CraftRecipeData data, IsoGameCharacter character) {
        for (int i = 0; i < RecipeCodeOnCreate.getConsumedItems(data, InventoryItem.class).size(); ++i) {
            ...
            data.getAllCreatedItems().getFirst().getModData().rawset("packedItem" + i, (Object)packedItem.getFullType());
            data.getAllCreatedItems().getFirst().getModData().rawset("numberOfPackedItems", (Object)RecipeCodeOnCreate.getConsumedItems(data, InventoryItem.class).size());
        }
    }
    ...
    public static void untieHeadband(CraftRecipeData data, IsoGameCharacter character) {
        Clothing clothing;
        InventoryItem inventoryItem = data.getAllConsumedItems().getFirst();
        if (inventoryItem instanceof Clothing && ((clothing = (Clothing)inventoryItem).isDirty() || clothing.isBloody())) {
            RecipeCodeOnCreate.removeItemFromCharacterInventory(character, data.getAllCreatedItems().getFirst());
    ...
    public static void openAndEat(CraftRecipeData data, IsoGameCharacter character) {
        ...
        if (GameServer.server) {
            ...
            table.rawset("itemId", (Object)data.getAllCreatedItems().getFirst().getID());
