...
public class GameEntityManager {
    ...
    static void RegisterEntity(GameEntity gameEntity) {
        ...
        DebugType.Entity.noise("Registering entity id = " + gameEntity.getEntityNetID() + " [type:" + String.valueOf(gameEntity.getGameEntityType()) + ", name:" + gameEntity.getEntityFullTypeDebug() + ", comps: " + gameEntity.componentSize() + "]");
    ...
    static void UnregisterEntity(GameEntity gameEntity, boolean offloadToMeta) {
        ...
        DebugType.Entity.noise("Unregistering entity id = " + gameEntity.getEntityNetID() + " [type:" + String.valueOf(gameEntity.getGameEntityType()) + ", name:" + gameEntity.getEntityFullTypeDebug() + ", tryOffloadToMeta:" + offloadToMeta + "]");
        long entityNetID = gameEntity.getEntityNetID();
        LongMap<GameEntity> longMap = idToEntityMap;
        synchronized (longMap) {
        ...
        if (offloadToMeta && gameEntity instanceof IsoObject && ComponentType.bitsRunInMeta.intersects(gameEntity.getComponentBits())) {
            ...
            if (!shouldStoreMeta) {
                DebugType.Entity.println("IsoObject Entity despawn - ignoring meta storage for entity: " + gameEntity.getEntityNetID() + ", no components actually require meta updating right now...");
            } else {
