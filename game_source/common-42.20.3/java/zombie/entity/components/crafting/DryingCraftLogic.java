...
extends CraftLogic {
    ...
    public void doProgressTooltip(ObjectTooltip.Layout layout, Resource resource, CraftRecipeData craftRecipeData) {
        ...
        String temperatureText = Translator.getText("EC_CraftLogicTooltip_TemperatureNormal", new Object[0]);
        if (dryingFactor <= 0.0f) {
            temperatureText = Translator.getText("EC_CraftLogicTooltip_TemperatureFrozen", new Object[0]);
        }
        if (dryingFactor > 0.0f && (double)dryingFactor < 0.9) {
            temperatureText = Translator.getText("EC_CraftLogicTooltip_TemperatureCold", new Object[0]);
        }
        if ((double)dryingFactor > 1.1 && dryingFactor < 2.0f) {
            temperatureText = Translator.getText("EC_CraftLogicTooltip_TemperatureWarm", new Object[0]);
        }
        if (dryingFactor >= 2.0f) {
            temperatureText = Translator.getText("EC_CraftLogicTooltip_TemperatureHot", new Object[0]);
        }
        ObjectTooltip.LayoutItem item = layout.addItem();
        item.setLabel(Translator.getText("EC_CraftLogicTooltip_Temperature", new Object[0]) + ":", 1.0f, 1.0f, 1.0f, 1.0f);
        item.setValue(temperatureText, 1.0f, 1.0f, 0.8f, 1.0f);
        if (wetness > 0.0) {
            String wetnessText = String.format(Locale.ENGLISH, "%.0f%%", wetness * 100.0);
            item = layout.addItem();
            item.setLabel(Translator.getText("EC_CraftLogicTooltip_Wetness", new Object[0]) + ":", 1.0f, 1.0f, 1.0f, 1.0f);
        ...
        if (paused) {
            dryingRateText = Translator.getText("EC_CraftLogicTooltip_Paused", new Object[0]);
            ArrayList<String> reasons = new ArrayList<String>();
            if (dryingFactor <= 0.0f) {
                reasons.add(Translator.getText("EC_CraftLogicTooltip_TemperatureCold", new Object[0]));
            }
            if (wetness > 0.0) {
                reasons.add(Translator.getText("EC_CraftLogicTooltip_Wet", new Object[0]));
            }
            dryingRateText = (String)dryingRateText + " (" + String.join((CharSequence)", ", reasons) + ")";
        }
        item = layout.addItem();
        item.setLabel(Translator.getText("EC_CraftLogicTooltip_DryingRate", new Object[0]) + ":", 1.0f, 1.0f, 1.0f, 1.0f);
