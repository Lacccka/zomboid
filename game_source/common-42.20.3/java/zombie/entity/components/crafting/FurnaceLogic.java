...
extends Component {
    ...
    private final FurnaceSlot[] furnaceSlots = new FurnaceSlot[16];
    private int furnaceSlotSize;

    private static FurnaceSlot allocFurnaceSlot(int index) {
        FurnaceSlot o = SLOT_POOL.poll();
        if (o == null) {
            o = new FurnaceSlot();
        }
        o.index = index;
    ...
    private static void releaseFurnaceSlot(FurnaceSlot o) {
        try {
            if (o == null) {
    ...
    protected void clearSlots() {
        ...
        for (int i = 0; i < this.furnaceSlotSize; ++i) {
            FurnaceSlot slot = this.furnaceSlots[i];
            if (slot == null) continue;
            FurnaceLogic.releaseFurnaceSlot(slot);
    ...
    public FurnaceSlot getSlot(int index) {
        if (index >= 0 && index < this.furnaceSlotSize) {
            return this.furnaceSlots[index];
        }
        return null;
    }

    protected FurnaceSlot createSlot(int index) {
        if (index >= 0 && index < this.furnaceSlots.length) {
            FurnaceSlot slot = FurnaceLogic.allocFurnaceSlot(index);
            if (this.furnaceSlots[index] != null) {
                FurnaceLogic.releaseFurnaceSlot(this.furnaceSlots[index]);
            }
    ...
    public ResourceItem getInputSlotResource(int index) {
        ...
        FurnaceSlot slot = this.getSlot(index);
    ...
    public List<Resource> getFurnaceInputResources() {
        ResourceGroup group;
        Resources resources = (Resources)this.getComponent(ComponentType.Resources);
        if (resources != null && (group = resources.getResourceGroup(this.furnaceInputsGroupName)) != null) {
    ...
    public List<Resource> getFurnaceOutputResources() {
        ResourceGroup group;
        Resources resources = (Resources)this.getComponent(ComponentType.Resources);
        if (resources != null && (group = resources.getResourceGroup(this.furnaceOutputsGroupName)) != null) {
            return group.getResources();
        }
        return _emptyResourceList;
    }
    ...
    protected void load(ByteBuffer input, int worldVersion) throws IOException {
        ...
        if (header.hasFlags(128)) {
            ...
            if (recipe == null || scriptVersion != recipe.getScriptVersion()) {
                ...
                DebugType.General.warn("CraftRecipe '" + recipeName + "' is null (" + (recipe == null) + ", or has script version mismatch. Cancelling current craft.");
            }
        ...
        private void load(ByteBuffer input, int worldVersion) throws IOException {
            if (input.get() != 0) {
                ...
                if (this.currentRecipe == null || scriptVersion != this.currentRecipe.getScriptVersion()) {
                    this.currentRecipe = null;
                    this.elapsedTime = 0;
                    DebugType.General.warn("FurnaceSlot[" + this.index + "] CraftRecipe '" + recipeName + "' is null (" + (this.currentRecipe == null) + ", or has script version mismatch. Cancelling current craft.");
                }
            }
