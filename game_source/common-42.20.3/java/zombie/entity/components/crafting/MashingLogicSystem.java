...
public class MashingLogicSystem
extends EngineSystem {
    ...
    public void update() {
        ...
        for (int i = 0; i < entities.size(); ++i) {
            ResourceFluid fluidBarrel;
            ResourceGroup inputResources;
            GameEntity entity = entities.get(i);
            if (!this.isValidEntity(entity)) continue;
            MashingLogic mashingLogic = (MashingLogic)entity.getComponent(ComponentType.MashingLogic);
            Resources resources = (Resources)entity.getComponent(ComponentType.Resources);
            if (!mashingLogic.isValid() || !resources.isValid() || (inputResources = resources.getResourceGroup(mashingLogic.getInputsGroupName())) == null || inputResources.getResources().isEmpty() || (fluidBarrel = (ResourceFluid)inputResources.get(mashingLogic.getResourceFluidID())) == null) continue;
    ...
    private void updateMashingLogic(MashingLogic logic, ResourceGroup inputResources, ResourceFluid fluidBarrel) {
        if (logic.isRunning()) {
            ...
            if (temperature < 12.0f) {
                ...
                if (logic.getElapsedTime() > (double)logic.getCurrentRecipe().getTime()) {
    ...
    private void createResultFluid(MashingLogic logic, boolean isCancelled, ResourceFluid fluidBarrel) {
        ...
        if (isCancelled) {
            ...
            ArrayList<OutputScript> outputs = recipe.getOutputs();
            for (int i = 0; i < outputs.size(); ++i) {
                OutputScript output = (OutputScript)outputs.get(i);
                if (output.getResourceType() != ResourceType.Fluid) continue;
