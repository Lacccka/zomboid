...
extends BaseCraftingLogic {
    ...
    public void startCraftAction(KahluaTableImpl actionTable) {
        ...
        if (this.getRecipe() == actionTable.rawget("craftRecipe") && (items = (List)actionTable.rawget("items")) != null) {
    ...
    public float getResidualFluidFromInput(InputScript inputScript) {
        ...
        for (int i = 0; i < inputScripts.size(); ++i) {
            CraftRecipeData.InputScriptData inputScriptData;
            if (inputScripts.get(i).getConsumeFromItemScript() != inputScript || (inputScriptData = this.recipeData.getDataForInputScript(inputScripts.get(i))) == null) continue;
    ...
    public void getCreatedOutputItems(ArrayList<InventoryItem> list) {
        ...
        if (this.recipeData != null && this.recipeData.isAllowOutputItems() && (dataList = this.recipeData.getToOutputItems()).hasUnprocessed()) {
    ...
    protected void rebuildCachedRecipeInfo() {
        ...
        for (i = 0; i < this.completeRecipeList.size(); ++i) {
            CraftRecipe recipe = (CraftRecipe)this.completeRecipeList.get(i);
    ...
    public Texture getResultTexture() {
        ...
        if (this.recipeData != null && this.recipeData.getRecipe() != null && this.recipeData.getRecipe().getOutputs() != null) {
            for (int i = 0; i < this.recipeData.getRecipe().getOutputs().size(); ++i) {
                ...
                if (output.getOutputMapper() == null || output.getOutputMapper().getEntrees() == null) continue;
                for (int j = 0; j < output.getOutputMapper().getEntrees().size(); ++j) {
                    ...
                    for (int k = 0; k < entree.pattern.size(); ++k) {
                        ...
                        for (int l = 0; l < this.recipeData.inputs.size(); ++l) {
                            ...
                            if (!consumedItem.getFullType().equals(testItem.moduleDotType)) continue;
        ...
        if (this.recipeData != null && this.recipeData.getRecipe() != null && this.recipeData.getRecipe().getIconTexture() != null) {
            return this.recipeData.getRecipe().getIconTexture();
        }
        return null;
    }
    ...
    public boolean isUsingRecipeAtHandBenefit() {
        ...
        if (!this.getRecipe().canBenefitFromRecipeAtHand(this.getPlayer())) {
            return false;
        }
        return this.isRecipeAtHand();
    }
    ...
    public InventoryItem getUsingRecipeAtHandItem() {
        ...
        for (int i = 0; i < this.containers.size(); ++i) {
            ItemContainer cont = (ItemContainer)this.containers.get(i);
            if (!cont.hasRecipe(this.getRecipe().getName(), this.getPlayer(), false)) continue;
    ...
    public IsoObject findCraftSurface(IsoGameCharacter player, int radius) {
        ...
        for (int x = chrX - radius; x < chrX + radius; ++x) {
            for (int y = chrY - radius; y < chrY + radius; ++y) {
                ...
                if (square == null || !player.getSquare().canReachTo(square)) continue;
                for (int i = 0; i < square.getObjects().size(); ++i) {
                    IsoObject object = square.getObjects().get(i);
                    if (!player.canUseAsGenericCraftingSurface(object)) continue;
                    return object;
                }
            }
        }
