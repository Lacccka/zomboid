...
public class CraftRecipeManager {
    ...
    private static boolean isCurrentCraftActionPending(IsoGameCharacter player, KahluaTable action) {
        ...
        return currentAction != null && currentAction.getTable() == action && (craftStarted == null || craftStarted == false);
    }

    private static boolean consumesEntireItem(InputScript inputScript, InventoryItem item) {
        return inputScript.isDestroy() || inputScript.hasFlag(InputFlag.DontReplace) && (item.getReplaceOnUse() != null || item.getScriptItem().getReplaceOnDeplete() != null) || inputScript.isItemCount();
    }
    ...
    public static float itemGetQueuedAmount(IsoGameCharacter player, InventoryItem item, CraftRecipeData.CacheData cacheData) {
        ...
        while (actionsIt.advance()) {
            KahluaTable manualInputs;
            KahluaTable action = (KahluaTable)actionsIt.getValue();
            if (!"ISHandcraftAction".equals(action.getMetatable().getString("Type")) || CraftRecipeManager.isCurrentCraftActionPending(player, action) || (manualInputs = (KahluaTable)action.rawget("manualInputs")) == null) continue;
            ...
            while (inputsIt.advance()) {
                ...
                if (manualInputItems == null || manualInputItems.size() != 1 || !manualInputItems.contains(item) || (index = (Double)inputsIt.getKey()) == null || (inputScript = (InputScript)recipe.getIOForIndex((int)index.doubleValue())) == null) continue;
                if (inputScript.hasConsumeFromItem()) {
                    switch (inputScript.getConsumeFromItemScript().getResourceType()) {
                        case Item: 
                        case Energy: {
                            amount += (float)((int)inputScript.getAmount());
                            break;
                        }
                        case Fluid: {
                ...
                if (CraftRecipeManager.consumesEntireItem(inputScript, item)) {
                    return Float.MAX_VALUE;
                }
                if (inputScript.isKeep()) continue;
                float variableInputRatio = cacheData != null && cacheData.getRecipeData() != null ? cacheData.getRecipeData().getVariableInputRatio() : 1.0f;
                float variableAmount = inputScript.isVariableAmount() ? (float)inputScript.getIntAmount() * variableInputRatio : (float)inputScript.getIntAmount();
                ...
                amount += (float)((int)(amountTruncated * itemScale));
            }
        }
        return amount;
    }
    ...
    protected static boolean consumeInputFromResources(InputScript input, List<Resource> resources, boolean testOnly, CraftRecipeData.CacheData cacheData, IsoGameCharacter character, Set<Resource> usedResources) {
        ...
        switch (input.getResourceType()) {
            ...
            case Fluid: {
                return CraftRecipeManager.consumeInputFluid(input, resources, testOnly, cacheData, 0.0f);
            }
            case Energy: {
    ...
    protected static boolean consumeInputItem(InputScript input, InventoryItem inventoryItem, boolean testOnly, CraftRecipeData.CacheData cacheData, IsoGameCharacter character) {
        ...
        float f = useQueued = success && testOnly ? CraftRecipeManager.itemGetQueuedAmount(character, inventoryItem, cacheData) : 0.0f;
        ...
        if (success && input.hasConsumeFromItem()) {
            ...
            switch (((InputScript)script).getResourceType()) {
                case Item: {
                    success = CraftRecipeManager.consumeInputItemUsesInternal((InputScript)script, inventoryItem, testOnly, cacheData, (int)useQueued);
                    if (!success || input.isDestroy()) break;
                    moveToOutput = (float)inventoryItem.getCurrentUses() < input.getAmount() + useQueued;
                    break;
                }
                case Fluid: {
                    success = CraftRecipeManager.consumeInputFluidInternal((InputScript)script, inventoryItem.getFluidContainer(), testOnly, cacheData, useQueued);
                    if (!success) break;
                    moveToOutput = inventoryItem.getFluidContainer().getAmount() < input.getAmount() + useQueued;
                    break;
                }
                case Energy: {
                    success = CraftRecipeManager.consumeInputEnergyFromItemInternal((InputScript)script, inventoryItem, testOnly, cacheData, (int)useQueued);
                    if (!success) break;
                    boolean bl = moveToOutput = (float)inventoryItem.getCurrentUses() < input.getAmount() + useQueued;
                }
            }
        }
        if (success && input.hasCreateToItem()) {
            ...
            switch (((OutputScript)script).getResourceType()) {
                case Item: {
                    success = CraftRecipeManager.createOutputItemUsesInternal((OutputScript)script, inventoryItem, testOnly, cacheData, (int)useQueued);
                    break;
                }
                case Fluid: {
                    success = CraftRecipeManager.createOutputFluidInternal((OutputScript)script, inventoryItem.getFluidContainer(), testOnly, cacheData, useQueued);
                    break;
                }
                case Energy: {
                    success = CraftRecipeManager.createOutputEnergyToItemInternal((OutputScript)script, inventoryItem, testOnly, cacheData, (int)useQueued);
                }
            }
        }
        if (success) {
    ...
    private static boolean consumeInputItemInternal(InputScript input, InventoryItem inventoryItem, boolean testOnly, CraftRecipeData.CacheData cacheData, IsoGameCharacter character) {
        ...
        if (input.containsItem(item)) {
            ...
            if (!input.doesItemPassRoutineStatusTests(inventoryItem, character)) {
                DebugType.CraftLogic.debugln(inputItemType + " fails routine status test for " + input.getParentRecipe().getName() + " - " + input.getOriginalLine());
                return false;
            }
            if (!input.doesItemPassClothingTypeStatusTests(inventoryItem)) {
                DebugType.CraftLogic.debugln(inputItemType + " fails clothing status test for " + input.getParentRecipe().getName() + " - " + input.getOriginalLine());
                return false;
            }
            if (!input.doesItemPassSharpnessStatusTests(inventoryItem)) {
                DebugType.CraftLogic.debugln(inputItemType + " fails sharpness status test for " + input.getParentRecipe().getName() + " - " + input.getOriginalLine());
                return false;
            }
            if (!input.doesItemPassDamageStatusTests(inventoryItem)) {
                DebugType.CraftLogic.debugln(inputItemType + " fails damage status test for " + input.getParentRecipe().getName() + " - " + input.getOriginalLine());
                return false;
            }
            if (!input.doesItemPassIsOrNotEmptyAndFullTests(inventoryItem)) {
                DebugType.CraftLogic.debugln(inputItemType + " fails empty/full status test for " + input.getParentRecipe().getName() + " - " + input.getOriginalLine());
                return false;
            }
            if (!input.doesItemPassFoodAndCookingTests(inventoryItem)) {
                DebugType.CraftLogic.debugln(inputItemType + " fails food/cooking status test for " + input.getParentRecipe().getName() + " - " + input.getOriginalLine());
                return false;
            }
            if (input.hasFlag(InputFlag.IsEmptyContainer) && inventoryItem instanceof InventoryContainer && !(bag = (InventoryContainer)inventoryItem).isEmpty()) {
                DebugType.CraftLogic.debugln(inputItemType + " fails empty bag status test for " + input.getParentRecipe().getName() + " - " + input.getOriginalLine());
            }
            if (cacheData != null) {
    ...
    private static boolean consumeInputItemUsesInternal(InputScript input, InventoryItem inventoryItem, boolean testOnly, CraftRecipeData.CacheData cacheData, int useQueued) {
    ...
    private static boolean consumeInputFluid(InputScript input, List<Resource> resources, boolean testOnly, CraftRecipeData.CacheData cacheData, float useQueued) {
        if (!CraftRecipeManager.validateInputScript(input, ResourceType.Fluid, testOnly)) {
            return false;
        }
        for (Resource resource : resources) {
            if (resource == null || resource.getType() != ResourceType.Fluid || resource.isEmpty() || !CraftRecipeManager.consumeInputFluidInternal(input, ((ResourceFluid)resource).getFluidContainer(), testOnly, cacheData, useQueued)) continue;
            return true;
        }
        return false;
    }

    private static boolean consumeInputFluidInternal(InputScript input, FluidContainer fc, boolean testOnly, CraftRecipeData.CacheData cacheData, float useQueued) {
    ...
    private static boolean consumeInputEnergyFromItemInternal(InputScript input, InventoryItem inventoryItem, boolean testOnly, CraftRecipeData.CacheData cacheData, int useQueued) {
        if (inventoryItem instanceof DrainableComboItem) {
            int useAmount;
            DrainableComboItem comboItem = (DrainableComboItem)inventoryItem;
            if (input.hasFlag(InputFlag.IsFull) && !comboItem.isFullUses()) {
    ...
    protected static boolean createOutputItem(OutputScript output, Item item, boolean testOnly, CraftRecipeData.CacheData cacheData, IsoGameCharacter character) {
        ...
        if (success && !testOnly && output.hasCreateToItem() && cacheData.getMostRecentItem() != null) {
            ...
            switch (script.getResourceType()) {
                case Item: {
                    b = CraftRecipeManager.createOutputItemUsesInternal(script, inventoryItem, testOnly, cacheData, 0);
                    break;
                }
                case Fluid: {
                    b = CraftRecipeManager.createOutputFluidInternal(script, inventoryItem.getFluidContainer(), testOnly, cacheData, 0.0f);
                    break;
                }
                case Energy: {
                    b = CraftRecipeManager.createOutputEnergyToItemInternal(script, inventoryItem, testOnly, cacheData, 0);
                }
            }
            if (!b) {
    ...
    private static boolean createOutputItemUsesInternal(OutputScript output, InventoryItem targetItem, boolean testOnly, CraftRecipeData.CacheData cacheData, int useQueued) {
    ...
    private static boolean createOutputFluid(OutputScript output, Resource resource, boolean testOnly, CraftRecipeData.CacheData cacheData) {
        ...
        return CraftRecipeManager.createOutputFluidInternal(output, ((ResourceFluid)resource).getFluidContainer(), testOnly, cacheData, 0.0f);
    }

    private static boolean createOutputFluidInternal(OutputScript output, FluidContainer fc, boolean testOnly, CraftRecipeData.CacheData cacheData, float useQueued) {
    ...
    private static boolean createOutputEnergyToItemInternal(OutputScript output, InventoryItem targetItem, boolean testOnly, CraftRecipeData.CacheData cacheData, int useQueued) {
        if (targetItem instanceof DrainableComboItem) {
            DrainableComboItem comboItem = (DrainableComboItem)targetItem;
