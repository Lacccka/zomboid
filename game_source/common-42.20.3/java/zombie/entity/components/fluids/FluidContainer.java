...
extends Component {
    ...
    public void DoTooltip(ObjectTooltip tooltipUI, ObjectTooltip.Layout layout) {
        ...
        if (this.fluids.size() == 1) {
            ...
            for (int i = 0; i < tempFluidUI.size(); ++i) {
                ...
                while (i + 1 < tempFluidUI.size() && (SandboxOptions.instance.enableTaintedWaterText.getValue() ? tempFluidUI.get(i + 1).getFluid() == fluidInstance.getFluid() : tempFluidUI.get(i + 1).getFluid().getTranslatedName().equals(fluidInstance.getFluid().getTranslatedName()))) {
        ...
        for (FluidInstance fluidInstance : this.fluids) {
            ...
            if (fluid.getFluidType() == FluidType.TaintedWater || !fluid.isPoisonous()) continue;
            hasOtherPoison = true;
            break;
        }
        if (this.isPoisonous() && this.getPoisonEffect() != PoisonEffect.None && (hasOtherPoison || !this.isTainted() || SandboxOptions.instance.enableTaintedWaterText.getValue())) {
            ColorInfo badColor = Core.getInstance().getBadHighlitedColor();
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_tainted", new Object[0]), badColor.getR(), badColor.getG(), badColor.getB(), 1.0f);
        }
        if (Core.debug) {
            ...
            item = layout.addItem();
            item.setLabel(Translator.getText("IGUI_Fluids_Categories", new Object[0]) + ":", 1.0f, 1.0f, 0.8f, 1.0f);
    ...
    public String getUiName() {
        if (this.cacheInvalidated || this.nameCache == null) {
            ...
            ArrayList<FluidInstance> visibleFluids = new ArrayList<FluidInstance>(this.fluids);
            ...
            if (!SandboxOptions.instance.enableTaintedWaterText.getValue()) {
                ...
                for (FluidInstance fluidinstance : visibleFluids) {
                    ...
                    if (!fluid.isCategory(FluidCategory.Poisons) && !fluid.isPoisonous()) continue;
                    hasRealPoison = true;
                }
                if (tainted != null) {
                    for (FluidInstance fluidInstance : visibleFluids) {
                        if (fluidInstance == tainted || !fluidInstance.getTranslatedName().equals(tainted.getTranslatedName())) continue;
            ...
            if (visibleFluids.isEmpty()) {
                ...
                this.nameCache = Translator.getText("Fluid_HoldingOneType", containerName, ((FluidInstance)visibleFluids.getFirst()).getFluid().getTranslatedName());
                ...
                fluidInstanceA = (FluidInstance)visibleFluids.get(0);
                FluidInstance fluidInstanceB = (FluidInstance)visibleFluids.get(1);
                this.nameCache = !fluidInstanceA.getFluid().isCategory(FluidCategory.Poisons) ? Translator.getText("Fluid_HoldingOneType", containerName, fluidInstanceA.getFluid().getTranslatedName()) : Translator.getText("Fluid_HoldingOneType", containerName, fluidInstanceB.getFluid().getTranslatedName());
                if (this.contains(Fluid.TaintedWater) && this.getPoisonEffect() != PoisonEffect.None && SandboxOptions.instance.enableTaintedWaterText.getValue()) {
                    this.nameCache = this.nameCache + " " + Translator.getFluidText("Fluid_Tainted");
                }
                ...
                for (FluidInstance fluidInstance : visibleFluids) {
                    ...
                    if (!fluidInstance.getFluid().isCategory(FluidCategory.Beverage) || !fluidInstance.getFluid().isCategory(FluidCategory.Alcoholic)) continue;
                    ++alcoholics;
                }
    ...
    private void recalculateCaches(boolean force, boolean removeInvalid) {
        if (this.cacheInvalidated || force) {
            ...
            if (this.amountCache > 0.0f) {
                Iterator<FluidInstance> i = this.fluids.iterator();
                while (i.hasNext()) {
                    FluidInstance fluidInstance;
                    fluid = fluidInstance = i.next();
            ...
            this.nameCache = null;
            this.cacheInvalidated = false;
            if (this.owner != null && (gameEntity = this.owner) instanceof InventoryItem && (item = (InventoryItem)gameEntity).isInPlayerInventory() && (itemContainer = item.getContainer()) != null) {
    ...
    public boolean isTainted() {
        for (FluidInstance fluid : this.fluids) {
            if (fluid.getFluid().getFluidType() != FluidType.TaintedWater) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean isWaterOnlySource() {
        ...
        for (FluidInstance instance : this.fluids) {
            ...
            if (fluid != null && fluid.isCategory(FluidCategory.Water)) continue;
            return false;
        }
        return true;
    }
    ...
    public void save(ByteBuffer output) throws IOException {
        ...
        if (!this.fluids.isEmpty()) {
            ...
            if (this.fluids.size() == 1) {
                ...
                Iterator<FluidInstance> iterator2 = this.fluids.iterator();
                while (iterator2.hasNext()) {
                    FluidInstance fluid;
                    FluidInstance fi = fluid = iterator2.next();
    ...
    public float getTransferRate() {
        InventoryItem item;
        Item itemScript;
        FluidContainerScript componentScript;
        GameEntity gameEntity = this.getOwner();
        if (gameEntity instanceof InventoryItem && (componentScript = (FluidContainerScript)(itemScript = (item = (InventoryItem)gameEntity).getScriptItem()).getComponentScriptFor(ComponentType.FluidContainer)) != null) {
