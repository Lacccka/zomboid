...
public class FluidUtil {
    ...
    public static String getFractionFormatted(float numerator, float denominator) {
        ...
        if (amount >= 1000.0f) {
            return String.format("%s / %s", df_liter1000.format(numerator), df_liter1000.format(denominator) + Translator.getText("IGUI_Liter", new Object[0]));
        }
        if (amount >= 10.0f) {
            return String.format("%s / %s", df_liter10.format(numerator), df_liter10.format(denominator) + Translator.getText("IGUI_Liter", new Object[0]));
        }
        if (amount >= 1.0f) {
            return String.format("%s / %s", df_liter.format(numerator), df_liter.format(denominator) + Translator.getText("IGUI_Liter", new Object[0]));
        }
        return String.format("%s / %s", Math.round(numerator * 1000.0f), Math.round(denominator * 1000.0f) + Translator.getText("IGUI_Milliliter", new Object[0]));
    }

    public static String getAmountLiter1000(float amount) {
        return df_liter1000.format(amount) + Translator.getText("IGUI_Liter", new Object[0]);
    }

    public static String getAmountLiter10(float amount) {
        return df_liter10.format(amount) + Translator.getText("IGUI_Liter", new Object[0]);
    }

    public static String getAmountLiter(float amount) {
        return df_liter.format(amount) + Translator.getText("IGUI_Liter", new Object[0]);
    }

    public static String getAmountMilli(float amount) {
        int ml = Math.round(amount * 1000.0f);
        return ml + Translator.getText("IGUI_Milliliter", new Object[0]);
    }
