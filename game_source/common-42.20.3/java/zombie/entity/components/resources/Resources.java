...
extends Component {
    ...
    private final Map<String, Resource> idToResourceMap = new HashMap<String, Resource>();
    private final ArrayList<ResourceGroup> namedGroups = new ArrayList();
    private final Map<String, ResourceGroup> namedGroupMap = new HashMap<String, ResourceGroup>();
    ...
    private final Map<ResourceChannel, ResourceGroup> inputChannelMap = new HashMap<ResourceChannel, ResourceGroup>();
    private final Map<ResourceChannel, ResourceGroup> outputChannelMap = new HashMap<ResourceChannel, ResourceGroup>();
    ...
    void resetDirty() {
        if (this.dirty) {
            int i;
            for (i = 0; i < this.namedGroups.size(); ++i) {
    ...
    protected void readFromScript(ComponentScript componentScript) {
        ...
        ArrayList<String> groups2 = script.getGroupNames();
        for (int i = 0; i < groups2.size(); ++i) {
            String groupName = groups2.get(i);
    ...
    private void resetResources() {
        ...
        for (i = 0; i < this.channelGroups.size(); ++i) {
    ...
    public void createResourceFromSerial(String blueprintSerial) {
        this.createResourceFromSerial(defaultGroup, blueprintSerial);
    }
    ...
    public void createResourceFromSerial(String groupName, String blueprintSerial) {
        try {
            ...
            DebugType.General.printException(e, LogSeverity.Error, "ResourceBlueprint serial: " + blueprintSerial, new Object[0]);
        }
    }

    public void createResource(ResourceBlueprint blueprint) {
        this.createResource(defaultGroup, blueprint);
    }
    ...
    private void addResourceInternal(String groupName, Resource resource) {
        ...
        if (this.idToResourceMap.containsKey(resource.getId())) {
    ...
    public Resource getResource(String nameId) {
        if (nameId != null) {
            for (int i = 0; i < this.resources.size(); ++i) {
                ...
                if (resource == null || resource.getId() == null || !resource.getId().equalsIgnoreCase(nameId)) continue;
                return resource;
            }
        }
        return null;
    ...
    private List<Resource> getResources(List<Resource> sources, List<Resource> list, ResourceIO io, ResourceType type, ResourceChannel channel, boolean clear) {
        ...
        for (int i = 0; i < sources.size(); ++i) {
            ...
            if (resource == null || io != ResourceIO.Any && resource.getIO() != io || type != ResourceType.Any && resource.getType() != type || channel != null && resource.getChannel() != channel) continue;
            list.add(resource);
        }
        return list;
    }
