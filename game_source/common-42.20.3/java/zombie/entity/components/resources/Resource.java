...
public abstract class Resource {
    ...
    public void setProgress(double progress) {
        ...
        if (this.progress != (progress = PZMath.clampDouble_01(progress))) {
    ...
    public void saveSync(ByteBuffer output) throws IOException {
        output.put(this.isLocked ? (byte)1 : 0);
    ...
    public void sync() {
        if (GameServer.server) {
            INetworkPacket.sendToRelative(PacketTypes.PacketType.SyncEntityResource, this.getResourcesComponent().getOwner().getX(), this.getResourcesComponent().getOwner().getY(), this);
        }
        if (GameClient.client) {
