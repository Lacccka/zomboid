        ...
        try {
            if (packetType.getGroup() != PacketGroup.GameEntity) {
                ...
                if (component != null && !(success = component.onReceivePacket(b, packetType, senderConnection)) && Core.debug) {
                    DebugType.Entity.error("ReadPacketContents returned failure. Component = " + String.valueOf((Object)component.getComponentType()) + ", packetType = " + String.valueOf((Object)packetType));
                }
            } else {
                boolean success = gameEntity.onReceiveEntityPacket(b, packetType, senderConnection);
                if (!success && Core.debug) {
                    DebugType.Entity.error("ReadPacketContents returned failure. PacketType = " + String.valueOf((Object)packetType));
                }
            }
        }
        catch (Exception e) {
