...
public class EntityDebugger {
    ...
    ArrayList<GameEntity> getIsoEntitiesDebug() {
        if (Core.debug) {
            ...
            for (int i = 0; i < entities.size(); ++i) {
                ...
                if (gameEntity.getGameEntityType() != GameEntityType.IsoObject || !(gameEntity instanceof IsoObject) || gameEntity.hasComponent(ComponentType.SpriteConfig) && !((SpriteConfig)gameEntity.getComponent(ComponentType.SpriteConfig)).isMultiSquareMaster()) continue;
                list.add(gameEntity);
            }
            return list;
        }
    ...
    void reloadDebug() throws Exception {
        if (Core.debug && !GameClient.client && !GameServer.server) {
            ...
            this.bytesCache = bytes = this.bytesCache != null ? this.bytesCache : new byte[0x200000];
    ...
    void reloadEntityFromScriptDebug(GameEntity gameEntity) throws Exception {
        if (Core.debug && !GameClient.client && !GameServer.server) {
            ...
            EntityScriptInfo entityScript = (EntityScriptInfo)gameEntity.getComponent(ComponentType.Script);
