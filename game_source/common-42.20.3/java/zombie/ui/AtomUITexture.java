...
extends AtomUI {
    ...
    public void render() {
        ...
        if (!this.textureIsReady) {
            boolean bl = this.textureIsReady = this.tex == null || this.tex.isReady();
            if (this.textureIsReady) {
                this.updateSlices();
            }
            ...
            if (this.isAnimPlay) {
                ...
                this.frameTimer += time - this.beforeTime;
    ...
    void drawTextureAnim() {
        ...
        Slice slice = this.slices.get(this.frameIndex);
        SpriteRenderer.instance.render(this.tex, leftTop[0], leftTop[1], rightTop[0], rightTop[1], rightDown[0], rightDown[1], leftDown[0], leftDown[1], slice.uvLeft, slice.uvTop, slice.uvRight, slice.uvTop, slice.uvRight, slice.uvDown, slice.uvLeft, slice.uvDown, this.colorR, this.colorG, this.colorB, this.colorA);
    }
    ...
    void drawTexture() {
        ...
        double[] rightDown = this.getAbsolutePosition(this.rightSide, this.downSide);
        double[] leftDown = this.getAbsolutePosition(this.leftSide, this.downSide);
        SpriteRenderer.instance.render(this.tex, leftTop[0], leftTop[1], rightTop[0], rightTop[1], rightDown[0], rightDown[1], leftDown[0], leftDown[1], this.colorR, this.colorG, this.colorB, this.colorA, null);
    }
    ...
    private void updateSlicesAnim() {
        ...
        double texUVW = (double)this.tex.getXEnd() - texUVX;
        double texUVH = (double)this.tex.getYEnd() - texUVY;
        double frameUVWidth = texUVW / (double)this.animFrameColumns;
        double frameUVHeight = texUVH / (double)this.animFrameRows;
        for (int y = 0; y < this.animFrameRows; ++y) {
            for (int x = 0; x < this.animFrameColumns; ++x) {
                ...
                this.slices.add(new Slice(0.0, 0.0, 0.0, 0.0, texUVX + frameUVWidth * (double)x, texUVX + frameUVWidth * (double)(x + 1), texUVY + frameUVHeight * (double)y, texUVY + frameUVHeight * (double)(y + 1)));
            }
        }
    }
    ...
    private void updateSlices9() {
        ...
        if (this.sliceLeft != 0.0 && this.sliceTop != 0.0) {
            this.slices.add(new Slice(x0, x1, y0, y1, u0, u1, v0, v1));
        }
        if (this.sliceTop != 0.0) {
            this.slices.add(new Slice(x1, x2, y0, y1, u1, u2, v0, v1));
        }
        if (this.sliceRight != 0.0 && this.sliceTop != 0.0) {
            this.slices.add(new Slice(x2, x3, y0, y1, u2, u3, v0, v1));
        }
        if (this.sliceLeft != 0.0) {
            this.slices.add(new Slice(x0, x1, y1, y2, u0, u1, v1, v2));
        }
        this.slices.add(new Slice(x1, x2, y1, y2, u1, u2, v1, v2));
        if (this.sliceRight != 0.0) {
            this.slices.add(new Slice(x2, x3, y1, y2, u2, u3, v1, v2));
        }
        if (this.sliceLeft != 0.0 && this.sliceDown != 0.0) {
            this.slices.add(new Slice(x0, x1, y2, y3, u0, u1, v2, v3));
        }
        if (this.sliceDown != 0.0) {
            this.slices.add(new Slice(x1, x2, y2, y3, u1, u2, v2, v3));
        }
        if (this.sliceRight != 0.0 && this.sliceDown != 0.0) {
            this.slices.add(new Slice(x2, x3, y2, y3, u2, u3, v2, v3));
        }
    }
    ...
    Texture tryGetTexture(String key) {
        Object value;
        Object object = value = UIManager.tableget(this.table, key);
        int n = 0;
        switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{Texture.class, KahluaTable.class}, (Object)object, n)) {
            case 0: {
                Texture texture = (Texture)object;
                return texture;
            }
            case 1: {
                KahluaTable valueTable = (KahluaTable)object;
                KahluaThread thread2 = UIManager.getDefaultThread();
                Object getTextureFunc = thread2.tableget(valueTable, "getTexture");
                if (KahluaUtil.isFunction(getTextureFunc)) {
                    return Type.tryCastTo(thread2.call(getTextureFunc, valueTable, null, null), Texture.class);
                }
                return null;
            }
        }
