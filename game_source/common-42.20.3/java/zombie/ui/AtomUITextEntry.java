...
implements UITextEntryInterface {
    ...
    public void init() {
        ...
        id = 42;
        this.maskDef = this.fontToUse.chars[id];
    }
        ...
        if (this.toSelectionIndex != this.textEntryCursorPos) {
            ...
            for (int i = startOffset; i <= endOffset; ++i) {
                ...
                if (i < this.textData.size()) {
                    ch = this.textData.get(i);
                    if (x0 == 9999999.0 || i > 0 && this.textData.get((int)(i - 1)).id == 10) {
                        x0 = ch.x - dx;
                        y0 = ch.y - dy;
                    }
                }
                if (i < this.textData.size() - 2 && this.textData.get((int)(i + 1)).id == 10) {
                    ch = this.textData.get(i);
                    advance = ch.def == null ? 0.0 : (double)ch.def.xadvance;
                    x1 = ch.x - dx + advance;
                    y1 = y0 + (double)this.fontToUse.getLineHeight();
                    leftTop = this.getAbsolutePosition(x0, y0);
                    rightTop = this.getAbsolutePosition(x1, y0);
                    rightDown = this.getAbsolutePosition(x1, y1);
                    leftDown = this.getAbsolutePosition(x0, y1);
                    SpriteRenderer.instance.render(null, leftTop[0], leftTop[1], rightTop[0], rightTop[1], rightDown[0], rightDown[1], leftDown[0], leftDown[1], 0.39215687f, 0.39215687f, 0.8627451f, 0.627451f, null);
                    continue;
                }
                if (i != endOffset) continue;
                ch = this.textData.get(i - 1);
                advance = ch.def == null ? 0.0 : (double)ch.def.xadvance;
                x1 = ch.x - dx + advance;
                y1 = y0 + (double)this.fontToUse.getLineHeight();
                leftTop = this.getAbsolutePosition(x0, y0);
    ...
    void drawCursor() {
        ...
        if (this.textData.isEmpty()) {
            double x0 = (double)(-this.cursorDef.width) / 2.0 - dx;
            double y0 = (double)this.cursorDef.yoffset - dy;
            double x1 = x0 + (double)this.cursorDef.width;
            double y1 = y0 + (double)this.cursorDef.height;
            ...
            SpriteRenderer.instance.render(this.cursorDef.image, leftTop[0], leftTop[1], rightTop[0], rightTop[1], rightDown[0], rightDown[1], leftDown[0], leftDown[1], this.colorR, this.colorG, this.colorB, this.colorA, null);
        } else {
            double y0;
            double x0;
            if (this.textEntryCursorPos == 0) {
                CharData ch = this.textData.get(0);
                x0 = ch.x - (double)this.cursorDef.xoffset - (double)this.cursorDef.width / 4.0 - dx;
                y0 = ch.y + (double)this.cursorDef.yoffset - dy;
            } else if (this.textEntryCursorPos >= this.textData.size()) {
                CharData ch = this.textData.get(this.textData.size() - 1);
                double width = ch.def == null ? 0.0 : (double)ch.def.xadvance;
                x0 = ch.x - (double)this.cursorDef.xoffset + width - (double)this.cursorDef.width / 4.0 - dx;
                y0 = ch.y + (double)this.cursorDef.yoffset - dy;
            } else {
                CharData ch = this.textData.get(this.textEntryCursorPos);
                x0 = ch.x - (double)this.cursorDef.xoffset - (double)this.cursorDef.width / 4.0 - dx;
                y0 = ch.y + (double)this.cursorDef.yoffset - dy;
            }
            if (this.textEntryCursorPos >= this.textData.size() && this.text.substring(this.textEntryCursorPos - 1).equals("\n")) {
                x0 = (double)this.cursorDef.xoffset - dx;
                y0 += (double)this.fontToUse.getLineHeight();
            }
            double x1 = x0 + (double)this.cursorDef.width;
            double y1 = y0 + (double)this.cursorDef.height;
            double[] leftTop = this.getAbsolutePosition(x0, y0);
    ...
    void updateInternalValues() {
        ...
        for (int i = 0; i < numChars; ++i) {
            ...
            int id = data[i];
            if (id == 10) {
                charDef = this.fontToUse.chars[id];
    ...
    public void putCharacter(char eventChar) {
        if (this.textEntryCursorPos == this.toSelectionIndex) {
            ...
            this.text = textOffset < this.text.length() ? this.text.substring(0, textOffset) + eventChar + this.text.substring(textOffset) : this.text + eventChar;
            ...
            this.text = !this.text.isEmpty() ? this.text.substring(0, l) + eventChar + this.text.substring(h) : "" + eventChar;
    ...
    int getCursorPos(double x, double y) {
        ...
        for (int i = 0; i < this.textData.size(); ++i) {
            CharData ch = this.textData.get(i);
            if (ch.id == 10) continue;
            double x0 = ch.x + (double)ch.def.xoffset - dx;
            double y0 = ch.y + (double)this.fontToUse.getLineHeight() / 2.0 - dy;
            ...
            if (i != this.textData.size() - 1 && this.textData.get((int)(i + 1)).id != 10 || !((dist = Math.hypot(clickPos[0] - (chPos = this.getAbsolutePosition(x0 += (double)ch.def.width, y0))[0], clickPos[1] - chPos[1])) < minDist)) continue;
            pos = i + 1;
            if (i + 1 < this.textData.size() && this.textData.get((int)(i + 1)).id == 10) {
                pos = i + 1;
            }
            minDist = dist;
        }
    ...
    public boolean onConsumeMouseButtonDown(int btn, double x, double y) {
        ...
        if (this.isEditable || this.isSelectable) {
            ...
            this.toSelectionIndex = this.textEntryCursorPos = this.getCursorPos(local[0], local[1]);
            return true;
        }
        return consumed;
    }
    ...
    public void onKeyEnter() {
        ...
        if (this.textEntryCursorPos != this.toSelectionIndex) {
            int l = Math.min(this.textEntryCursorPos, this.toSelectionIndex);
            int h = Math.max(this.textEntryCursorPos, this.toSelectionIndex);
            this.text = !this.text.isEmpty() ? this.text.substring(0, l) + "\n" + this.text.substring(h) : "\n";
            this.textEntryCursorPos = l + 1;
        } else {
            int textOffset = this.textEntryCursorPos;
    ...
    public void onKeyUp() {
        ...
        for (int i = this.textEntryCursorPos; i > 0; --i) {
            CharData ch = this.textData.get(i - 1);
            if (ch.id == 10 || i == 1) {
                if (!checkFirstEnter) {
                    this.textEntryCursorPos = i - 1;
                    for (int j = 1; j <= shift && i + j - 1 != this.textData.size() && (i + j - 1 >= this.textData.size() || this.textData.get((int)(i + j - 1)).id != 10); ++j) {
                        ++this.textEntryCursorPos;
                    }
                    break;
                }
    ...
    public void onKeyDown() {
        ...
        boolean isShiftKeyDown = GameKeyboard.isKeyDownRaw(42) || GameKeyboard.isKeyDownRaw(54);
        int shift = 0;
        for (i = this.textEntryCursorPos; i > 0; --i) {
            ch = this.textData.get(i - 1);
            if (ch.id == 10 || i == 1) break;
            ++shift;
        }
        DebugType.General.warn(shift);
        for (i = this.textEntryCursorPos; i < this.textData.size(); ++i) {
            ch = this.textData.get(i);
            if (ch.id != 10) continue;
            this.textEntryCursorPos = i - 1;
            for (int j = 1; j <= shift && i + j - 1 != this.textData.size() && (i + j + 1 >= this.textData.size() || this.textData.get((int)(i + j + 1)).id != 10); ++j) {
    ...
    public void onKeyDelete() {
        ...
        this.text = this.textEntryCursorPos > 0 ? this.text.substring(0, this.textEntryCursorPos) + this.text.substring(this.textEntryCursorPos + 1) : this.text.substring(1);
        if (this.luaOnTextChange != null) {
            LuaManager.caller.pcallvoid(UIManager.getDefaultThread(), this.luaOnTextChange, this.table);
        }
        this.updateInternalValues();
    ...
    public void pasteFromClipboard() {
        ...
        if (this.textEntryCursorPos != this.toSelectionIndex) {
            ...
            this.text = this.textEntryCursorPos < this.text.length() ? this.text.substring(0, this.textEntryCursorPos) + clip + this.text.substring(this.textEntryCursorPos) : this.text + clip;
    ...
    public void copyToClipboard() {
        ...
        int l = Math.min(this.textEntryCursorPos, this.toSelectionIndex);
        String newClip = this.text.substring(l, h = Math.max(this.textEntryCursorPos, this.toSelectionIndex));
        if (!newClip.isEmpty()) {
