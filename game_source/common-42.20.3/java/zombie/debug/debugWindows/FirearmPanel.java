...
extends BaseDebugWindow {
    ...
    protected void doWindowContents() {
        if (this.attachments.isEmpty()) {
            this.attachments = ScriptManager.instance.getAllItems().stream().filter(i -> i.isItemType(ItemType.WEAPON_PART)).map(i -> (WeaponPart)InventoryItemFactory.CreateItem(i.getFullName())).collect(Collectors.toCollection(ArrayList::new));
        }
    ...
    private void doSettingTweaks() {
        if (this.doTreeNode("gunplaySettings", "Global Settings", false, false)) {
            ImGui.text("General");
            Wrappers.sliderDouble("Noise", 0.3f, 2.0, SandboxOptions.instance.firearmNoiseMultiplier::getValue, SandboxOptions.instance.firearmNoiseMultiplier::setValue);
            this.combatConfig.set(CombatConfigKey.FIREARM_RECOIL_MUSCLE_STRAIN_MODIFIER, PZMath.roundFloat(Wrappers.sliderFloat("Firearm Muscle Strain", this.combatConfig.get(CombatConfigKey.FIREARM_RECOIL_MUSCLE_STRAIN_MODIFIER), 0.0f, 0.1f), 4));
            ImGui.treePop();
        }
    }
    ...
    private void doPlayerTweaks(IsoPlayer player) {
        if (this.doTreeNode("gunplayPlayer", "Player", false, false)) {
            ...
            player.getStats().set(CharacterStat.PANIC, Wrappers.sliderFloat("Panic", player.getStats().get(CharacterStat.PANIC), CharacterStat.PANIC.getMinimumValue(), CharacterStat.PANIC.getMaximumValue()));
            ImGui.sameLine();
            ...
            player.getCharacterTraits().set(CharacterTrait.MARKSMAN, Wrappers.checkbox(CharacterTrait.MARKSMAN.toString(), player.hasTrait(CharacterTrait.MARKSMAN)));
            player.getCharacterTraits().set(CharacterTrait.DEXTROUS, Wrappers.checkbox(CharacterTrait.DEXTROUS.toString(), player.hasTrait(CharacterTrait.DEXTROUS)));
            player.getCharacterTraits().set(CharacterTrait.NIGHT_VISION, Wrappers.checkbox(CharacterTrait.NIGHT_VISION.toString(), player.hasTrait(CharacterTrait.NIGHT_VISION)));
            player.getCharacterTraits().set(CharacterTrait.EAGLE_EYED, Wrappers.checkbox(CharacterTrait.EAGLE_EYED.toString(), player.hasTrait(CharacterTrait.EAGLE_EYED)));
            player.getCharacterTraits().set(CharacterTrait.SHORT_SIGHTED, Wrappers.checkbox(CharacterTrait.SHORT_SIGHTED.toString(), player.hasTrait(CharacterTrait.SHORT_SIGHTED)));
            ImGui.treePop();
        }
    }
            ...
            if (this.doTreeNode("gunplayAttach", "Attachments", false, false)) {
                ...
                if (ImGui.beginCombo("Add attachment", this.selectedAttachment)) {
                    for (WeaponPart i : this.attachments) {
                        ...
                        if (!Wrappers.valueBoolean.get()) continue;
                ...
                if (ImGui.button("Add") && !StringUtils.isNullOrEmpty(this.selectedAttachment)) {
                    weapon.attachWeaponPart((WeaponPart)InventoryItemFactory.CreateItem(this.selectedAttachment));
                }
                ImGui.treePop();
            }
            ImGui.treePop();
        ...
        if (this.doTreeNode("gunplayTargets", "Targets", false, false)) {
            if (ImGui.beginTable("gunplayTargetTable", 10, 1984)) {
                ...
                for (int i = 0; i < hitInfoList.size(); ++i) {
                    ...
                    IsoGameCharacter obj = (IsoGameCharacter)isoMovingObject;
                    ...
                    ImGui.tableSetColumnIndex(columnIndex++);
                    ImGui.text(Float.toString(PZMath.max(0.0f, this.combatConfig.get(CombatConfigKey.LOW_LIGHT_TO_HIT_MAXIMUM_PENALTY) * (1.0f - obj.getCurrentSquare().getLightLevel(player.getIndex()) / this.combatConfig.get(CombatConfigKey.LOW_LIGHT_THRESHOLD)))));
                    ImGui.tableSetColumnIndex(columnIndex++);
