...
extends PZDebugWindow {
    ...
    private void physicsHitReactionTab(PhysicsHitReaction physicsHitReaction) {
        ...
        for (int i = 0; i < physicsHitReaction.impulse.length; ++i) {
            if (!physicsHitReaction.useImpulseOverride) {
                physicsHitReaction.impulse[i] = PZImGui.sliderFloat(Translator.getText(RagdollBodyPart.values()[i].name(), new Object[0]) + " Forward Impulse", physicsHitReaction.impulse[i], 0.0f, this.maxForwardImpulse.floatValue());
                physicsHitReaction.upwardImpulse[i] = PZImGui.sliderFloat(Translator.getText(RagdollBodyPart.values()[i].name(), new Object[0]) + " Upward Impulse", physicsHitReaction.upwardImpulse[i], 0.0f, this.maxUpwardImpulse.floatValue());
                continue;
            }
