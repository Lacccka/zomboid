...
public class RagdollDebugWindow
extends PZDebugWindow {
    ...
    private void jointConstraintsTab() {
        ...
        for (RagdollConstraint ragdollConstraint : RagdollScript.getRagdollConstraintList()) {
            int constraintPartB;
            ImInt constraintPartBImInt;
            int constraintPartA;
            ImInt constraintPartAImInt;
            if (!PZImGui.collapsingHeader(RagdollJoint.values()[ragdollConstraint.joint].toString())) continue;
            ...
            if (PZImGui.combo("Constraint Type", constraintTypeImInt, this.constraintTypeArray)) {
                int n = ragdollConstraint.constraintType = constraintTypeImInt.get() == 0 ? 4 : 5;
            }
            if (PZImGui.combo("Constraint Part A", constraintPartAImInt = new ImInt(constraintPartA = ragdollConstraint.constraintPartA), this.bodyPartArray)) {
                ragdollConstraint.constraintPartA = constraintPartAImInt.get();
            }
            if (PZImGui.combo("Constraint Part B", constraintPartBImInt = new ImInt(constraintPartB = ragdollConstraint.constraintPartB), this.bodyPartArray)) {
                ragdollConstraint.constraintPartB = constraintPartAImInt.get();
            }
            ragdollConstraint.constraintAxisA.x = PZImGui.sliderFloat("constraintAxisA.x", ragdollConstraint.constraintAxisA.x, (float)(-Math.PI), (float)Math.PI);
            ragdollConstraint.constraintAxisA.y = PZImGui.sliderFloat("constraintAxisA.y", ragdollConstraint.constraintAxisA.y, (float)(-Math.PI), (float)Math.PI);
            ragdollConstraint.constraintAxisA.z = PZImGui.sliderFloat("constraintAxisA.z", ragdollConstraint.constraintAxisA.z, (float)(-Math.PI), (float)Math.PI);
            ragdollConstraint.constraintAxisB.x = PZImGui.sliderFloat("constraintAxisB.x", ragdollConstraint.constraintAxisB.x, (float)(-Math.PI), (float)Math.PI);
            ragdollConstraint.constraintAxisB.y = PZImGui.sliderFloat("constraintAxisB.y", ragdollConstraint.constraintAxisB.y, (float)(-Math.PI), (float)Math.PI);
            ragdollConstraint.constraintAxisB.z = PZImGui.sliderFloat("constraintAxisB.z", ragdollConstraint.constraintAxisB.z, (float)(-Math.PI), (float)Math.PI);
            ragdollConstraint.constraintPositionOffsetA.x = PZImGui.sliderFloat("constraintPositionOffsetA.x", ragdollConstraint.constraintPositionOffsetA.x, -0.25f, 0.25f);
            ragdollConstraint.constraintPositionOffsetA.y = PZImGui.sliderFloat("constraintPositionOffsetA.y", ragdollConstraint.constraintPositionOffsetA.y, -0.25f, 0.25f);
            ragdollConstraint.constraintPositionOffsetA.z = PZImGui.sliderFloat("constraintPositionOffsetA.z", ragdollConstraint.constraintPositionOffsetA.z, -0.25f, 0.25f);
            ragdollConstraint.constraintPositionOffsetB.x = PZImGui.sliderFloat("constraintPositionOffsetB.x", ragdollConstraint.constraintPositionOffsetB.x, -0.25f, 0.25f);
            ragdollConstraint.constraintPositionOffsetB.y = PZImGui.sliderFloat("constraintPositionOffsetB.y", ragdollConstraint.constraintPositionOffsetB.y, -0.25f, 0.25f);
            ragdollConstraint.constraintPositionOffsetB.z = PZImGui.sliderFloat("constraintPositionOffsetB.z", ragdollConstraint.constraintPositionOffsetB.z, -0.25f, 0.25f);
            ragdollConstraint.constraintLimit.x = PZImGui.sliderFloat("constraintLimit.x", ragdollConstraint.constraintLimit.x, (float)(-Math.PI), (float)Math.PI);
            ragdollConstraint.constraintLimit.y = PZImGui.sliderFloat("constraintLimit.y", ragdollConstraint.constraintLimit.y, (float)(-Math.PI), (float)Math.PI);
            ragdollConstraint.constraintLimit.z = PZImGui.sliderFloat("constraintLimit.z", ragdollConstraint.constraintLimit.z, (float)(-Math.PI), (float)Math.PI);
            ImGui.endChild();
        }
        if (RagdollBuilder.instance.isInitialized()) {
            if (PZImGui.button(Translator.getText("IGUI_PlayerStats_ResetToDefault", new Object[0]))) {
    ...
    private void skeletonTab() {
        if (ImGui.treeNode("Skeleton")) {
            ...
            for (SkeletonBone skeletonBone : skeletonBones) {
                ...
                if (!ImGui.treeNode(parentBone.name())) continue;
    ...
    private void boneToBodyPartAnchorsTab() {
        ...
        for (RagdollAnchor ragdollAnchor : RagdollScript.getRagdollAnchorList()) {
            if (!PZImGui.collapsingHeader(SkeletonBone.values()[ragdollAnchor.bone].toString())) continue;
            ...
            ragdollAnchor.enabled = PZImGui.checkbox(Translator.getText("IGUI_DebugMenu_Enabled", new Object[0]), enabledTypeImInt::get, enabledTypeImInt::set);
            if (enabled) {
    ...
    private void physicsPropertiesTab() {
        RagdollSettingsManager.RagdollSetting ragdollSetting;
        int i;
        int count = RagdollSettingsManager.getInstance().getSettingsCount();
        ImGui.beginChild("Physics Properties");
        for (i = 0; i < count; ++i) {
            ragdollSetting = RagdollSettingsManager.getInstance().getSetting(i);
            float ragdollSettingValue = PZImGui.sliderFloat(ragdollSetting.getName(), ragdollSetting.getAdminValue(), ragdollSetting.getMin(), ragdollSetting.getMax());
            if (!RagdollBuilder.instance.isInitialized()) continue;
            ragdollSetting.setAdminValue(ragdollSettingValue);
        }
        if (RagdollBuilder.instance.isInitialized() && PZImGui.button(Translator.getText("IGUI_PlayerStats_ResetToDefault", new Object[0]))) {
            for (i = 0; i < count; ++i) {
                ragdollSetting = RagdollSettingsManager.getInstance().getSetting(i);
    ...
    private void hitReactionsTab() {
        RagdollSettingsManager.HitReactionSetting hitReactionSetting;
        int i;
        ...
        for (i = 0; i < hitReactionSettingCount; ++i) {
            hitReactionSetting = RagdollSettingsManager.getInstance().getHitReactionSetting(i);
            if (!PZImGui.collapsingHeader(hitReactionSetting.getName())) continue;
            ImGui.beginChild(hitReactionSetting.getName());
            if (i != 0) {
                RagdollSettingsManager.ForceHitReactionLocation forceHitReactionLocation = RagdollSettingsManager.getInstance().getForceHitReactionLocation(i - 1);
                boolean forceHitReactionLocationValue = PZImGui.checkbox("Force " + forceHitReactionLocation.getName() + " Hit Reaction", forceHitReactionLocation.getAdminValue());
                if (RagdollBuilder.instance.isInitialized()) {
            ...
            for (int j = 0; j < 3; ++j) {
                ...
                float ragdollSettingValue = PZImGui.sliderFloat(hitReactionSetting.getName(), hitReactionSetting.getAdminValue(), hitReactionSetting.getMin(), hitReactionSetting.getMax());
                if (!RagdollBuilder.instance.isInitialized()) continue;
    ...
    private void bodyPartInfoTab() {
        ...
        for (RagdollBodyPartInfo ragdollBodyPartInfo : RagdollScript.getRagdollBodyPartInfoList()) {
            if (!PZImGui.collapsingHeader(RagdollBodyPart.values()[ragdollBodyPartInfo.part].toString())) continue;
            ...
            ragdollBodyPartInfo.calculateLength = PZImGui.checkbox(Translator.getText("Calculate Length", new Object[0]), calculateLengthTypeImInt::get, calculateLengthTypeImInt::set);
    ...
    private void bodyDynamicsTab() {
        ...
        for (RagdollBodyDynamics ragdollBodyDynamics : RagdollScript.getRagdollBodyDynamicsList()) {
            if (!PZImGui.collapsingHeader(RagdollBodyPart.values()[ragdollBodyDynamics.part].toString())) continue;
            ...
            ragdollBodyDynamics.linearSleepingThreshold = PZImGui.sliderFloat("Linear Sleep Threshold", ragdollBodyDynamics.linearSleepingThreshold, 0.0f, 2.0f);
            ragdollBodyDynamics.angularSleepingThreshold = PZImGui.sliderFloat("Angular Sleep Threshold", ragdollBodyDynamics.angularSleepingThreshold, 0.0f, 3.0f);
