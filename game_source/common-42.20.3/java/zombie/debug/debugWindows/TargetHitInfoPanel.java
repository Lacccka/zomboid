...
public class TargetHitInfoPanel
extends PZDebugWindow {
    ...
    public PZArrayList<HitInfo> hitInfoList = new PZArrayList<HitInfo>(HitInfo.class, 8);
    ...
    private void displayBaseVehicleRow(BaseVehicle baseVehicle, HandWeapon weapon, HitInfo hitInfo) {
        ...
        ImGui.text("null");
        ImGui.tableSetColumnIndex(columnIndex++);
        ImGui.text(Float.toString(PZMath.max(0.0f, this.combatConfig.get(CombatConfigKey.LOW_LIGHT_TO_HIT_MAXIMUM_PENALTY) * (1.0f - 0.0f / this.combatConfig.get(CombatConfigKey.LOW_LIGHT_THRESHOLD)))));
        ImGui.tableSetColumnIndex(columnIndex++);
        ImGui.text("NA");
        ImGui.tableSetColumnIndex(columnIndex++);
    ...
    private void displayRow(IsoMovingObject obj, HandWeapon weapon, HitInfo hitInfo) {
        ...
        ImGui.tableSetColumnIndex(columnIndex++);
        ImGui.text(Float.toString(PZMath.max(0.0f, this.combatConfig.get(CombatConfigKey.LOW_LIGHT_TO_HIT_MAXIMUM_PENALTY) * (1.0f - lightLevel / this.combatConfig.get(CombatConfigKey.LOW_LIGHT_THRESHOLD)))));
        ImGui.tableSetColumnIndex(columnIndex++);
        if (gameCharacter != null) {
    ...
    protected void doWindowContents() {
        ...
        if (ballisticsController != null) {
            ...
            for (int i = 0; i < numberOfTargets; ++i) {
                ...
                z = ballisticsTargets[arrayIndex++];
                ImGui.tableNextRow();
                ImGui.tableSetColumnIndex(columnIndex++);
                ImGui.text(Integer.toString(id));
                ImGui.tableSetColumnIndex(columnIndex++);
        ...
        if ((weapon = this.isoGameCharacter.getUseHandWeapon()) == null) {
            return;
        }
        ...
        if (ImGui.beginTable("Spread Data", 4, 1984)) {
            ...
            for (int i = 0; i < numberOfSpreadTargets; ++i) {
                ...
                z = spreadTargets[arrayIndex++] / 2.44949f;
