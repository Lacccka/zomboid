    ...
    public static void doCSV() throws IOException {
        try (PrintWriter writer = new PrintWriter(new FileWriter("FluidContainersCSV.txt"));){
            ...
            for (Item item : ScriptManager.instance.getAllItems()) {
                ...
                writer.println("%s.%s,%s,%s".formatted(item.getModuleName(), item.getName(), Float.valueOf(((FluidContainerScript)item.getComponentScriptFor(ComponentType.FluidContainer)).getCapacity()), Float.valueOf(((FluidContainerScript)item.getComponentScriptFor(ComponentType.FluidContainer)).getTransferRate())));
            }
        }
    }
}
