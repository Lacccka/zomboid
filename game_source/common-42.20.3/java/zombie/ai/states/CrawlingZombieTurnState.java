...
extends State {
    ...
    public void animEvent(IsoGameCharacter owner, AnimLayer layer, AnimationTrack track, AnimEvent event) {
        if (event.eventName.equalsIgnoreCase("TurnSome")) {
            ...
            Vector2 endDir = "left".equalsIgnoreCase(event.parameterValue) ? owner.getForwardIsoDirection().RotLeft().ToVector() : owner.getForwardIsoDirection().RotRight().ToVector();
    ...
    public static boolean calculateDir(IsoGameCharacter owner, IsoDirections targetDir) {
        if (targetDir.ordinal() > owner.getForwardIsoDirection().ordinal()) {
            return targetDir.ordinal() - owner.getForwardIsoDirection().ordinal() <= 4;
        }
        return targetDir.ordinal() - owner.getForwardIsoDirection().ordinal() < -4;
    }
}
