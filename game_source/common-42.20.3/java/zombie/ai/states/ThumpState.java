...
extends State {
    ...
    public void execute(IsoGameCharacter owner) {
        ...
        int thumpCount = 0;
        if (owner.isAnimationUpdatingThisFrame() && (track = ThumpAnimations.getAnimationTrack(owner)) != null) {
        ...
        for (int i = 0; i < thumpCount; ++i) {
            ...
            boolean bl = listener = GameServer.server || SoundManager.instance.isListenerInRange(owner.getX(), owner.getY(), 70.0f);
            if (!listener || IsoPlayer.allPlayersAsleep()) continue;
            ...
            if (barricade != null && (barricade.isMetal() || barricade.isMetalBar())) {
                ...
                IsoDoor door3 = (IsoDoor)thumpableFor;
                thumpSound = door3.getThumpSound();
                if ("WoodDoor".equalsIgnoreCase(door3.getSoundPrefix()) && owner.isVariable("ThumpType", "DoorClaw")) {
                    thumpSound = SoundKey.ZOMBIE_THUMP_WOOD.getSoundName();
                }
        ...
        if (thump instanceof IsoDoor && ((door2 = (IsoDoor)thump).isOpen() || thump.isDestroyed()) && this.lungeThroughDoor(zombie, sq3 = door2.getSquare(), sq22 = door2.getOppositeSquare())) {
            return;
        }
