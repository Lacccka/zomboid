...
public final class GrappledThrownOverFenceState
extends State {
    ...
    public void execute(IsoGameCharacter owner) {
        ...
        if (owner.getVariableBoolean("ClimbFenceStarted")) {
            ...
            if (owner.getXi() != PZMath.fastfloor(x) && (dir == IsoDirections.W || dir == IsoDirections.E)) {
                this.slideX(owner, x);
            } else if (owner.getXi() == owner.get(END_X).intValue()) {
                owner.setNextX(PZMath.clamp(owner.getNextX(), (float)owner.get(END_X).intValue() + 0.01f, (float)owner.get(END_X).intValue() + 0.99f));
            }
            if (owner.getYi() != PZMath.fastfloor(y) && (dir == IsoDirections.N || dir == IsoDirections.S)) {
                this.slideY(owner, y);
            } else if (owner.getYi() == owner.get(END_Y).intValue()) {
                owner.setNextY(PZMath.clamp(owner.getNextY(), (float)owner.get(END_Y).intValue() + 0.01f, (float)owner.get(END_Y).intValue() + 0.99f));
            }
        }
    }
    ...
    public void exit(IsoGameCharacter owner) {
        ...
        if (owner.hasAnimationPlayer() && owner.getAnimationPlayer().isReady()) {
            if (owner.get(DIR).dy() != 0) {
                owner.setX((float)owner.get(END_X).intValue() + 0.5f);
                owner.setY(PZMath.clamp(owner.getY(), (float)owner.get(END_Y).intValue() + 0.01f, (float)owner.get(END_Y).intValue() + 0.99f));
            } else {
                owner.setX(PZMath.clamp(owner.getX(), (float)owner.get(END_X).intValue() + 0.01f, (float)owner.get(END_X).intValue() + 0.99f));
                owner.setY((float)owner.get(END_Y).intValue() + 0.5f);
            }
