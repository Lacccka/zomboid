...
extends State {
    ...
    public void execute(IsoGameCharacter owner) {
        ...
        if (zombie.getZ() == (float)networkAi.targetZ && (networkAi.predictionType == 3 || networkAi.predictionType == 4)) {
        ...
        } else if (zombie.getZ() == (float)networkAi.targetZ && !PolygonalMap2.instance.lineClearCollide(zombie.getX(), zombie.getY(), networkAi.targetX, networkAi.targetY, networkAi.targetZ, null)) {
            if (networkAi.usePathFind) {
                pfb2.reset();
                zombie.setPath2(null);
                networkAi.usePathFind = false;
            ...
            pfb2.moveToPoint(networkAi.targetX, networkAi.targetY, 1.0f);
            zombie.setVariable("bMoving", IsoUtils.DistanceManhatten(networkAi.targetX, networkAi.targetY, zombie.getNextX(), zombie.getNextY()) > 0.5f);
        } else if (zombie.getZ() == (float)networkAi.targetZ && !PolygonalMap2.instance.lineClearCollide(zombie.getX(), zombie.getY(), zombie.realx, zombie.realy, zombie.realz, null)) {
            if (networkAi.usePathFind) {
                pfb2.reset();
                zombie.setPath2(null);
                networkAi.usePathFind = false;
