...
extends State {
    ...
    private void doAttack(IsoPlayer ownerPlayer, float chargeDelta, String clickSound, AttackVars vars) {
        ...
        if (attackItem instanceof HandWeapon) {
            ...
            if (ownerPlayer.playerIndex == 0 && ownerPlayer.getJoypadBind() == -1 && UIManager.getPicked() != null && (!GameClient.client || ownerPlayer.isLocalPlayer())) {
    ...
    public void enter(IsoGameCharacter owner) {
        ...
        if (!player.isDoShove() && !bDoShove && !weapon.isRanged() && player.isLocalPlayer()) {
            ...
            if (isStompAnim && SwipeStatePlayer.isStompingDisabled(owner, true)) {
                isStompAnim = false;
            }
            if (isStompAnim) {
    ...
    public void setParams(IsoGameCharacter owner, State.Stage stage) {
        if (owner.isLocal()) {
            ...
            if (owner.getUseHandWeapon() != null && owner instanceof IsoPlayer && (isoPlayer = (IsoPlayer)owner).isInitiateAttack() && isoPlayer.isAttackStarted()) {
