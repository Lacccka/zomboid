...
public final class GameWindow {
    ...
    private static void initShared() throws Exception {
        ...
        GameWindow.LoadTexturePack("Mechanics", 2);
        ...
        ModRegistries.init();
        GameWindow.DoLoadingText(Translator.getText("UI_Loading_Scripts", new Object[0]));
        ScriptManager.instance.Load();
        CustomizationManager.getInstance().load();
        SpriteModelManager.getInstance().init();
        GameWindow.DoLoadingText(Translator.getText("UI_Loading_Clothing", new Object[0]));
        ClothingDecals.init();
        BeardStyles.init();
        HairStyles.init();
        OutfitManager.init();
        ...
        for (int m = 0; m < counta; ++m) {
            ...
            if (count > 1) {
                ...
                DebugType.Input.println("----------------------------------------------");
                for (n = 0; n < count; ++n) {
                    name = controller.getAxisName(n);
                    DebugType.Input.println("Axis: " + name);
                }
            }
            if ((count = controller.getButtonCount()) <= 1) continue;
            DebugType.Input.println("----------------------------------------------");
            DebugType.Input.println("--    Button definitions for controller " + m);
            DebugType.Input.println("----------------------------------------------");
            for (n = 0; n < count; ++n) {
                name = controller.getButtonName(n);
    ...
    private static void logic() {
        ...
        try (GameProfiler.ProfileArea profileArea = profiler.profile("Steam Loop");){
            SteamUtils.runLoop();
        }
        profileArea = profiler.profile("Mouse");
        try {
            Mouse.update();
        }
        finally {
        ...
        profileArea = profiler.profile("Keyboard");
        try {
            GameKeyboard.update();
        }
        finally {
        ...
        profileArea = profiler.profile("UI");
        try {
            UIManager.update();
        }
        finally {
        ...
        if (JoypadManager.instance.isAPressed(-1)) {
            for (int n = 0; n < JoypadManager.instance.joypadList.size(); ++n) {
                joypad = JoypadManager.instance.joypadList.get(n);
                if (!((JoypadManager.Joypad)joypad).isAPressed()) continue;
                if (activatedJoyPad == null) {
                    activatedJoyPad = joypad;
                }
                if (IsoPlayer.getInstance() != null) {
                    LuaEventManager.triggerEvent("OnJoypadActivate", ((JoypadManager.Joypad)joypad).getID());
                    break;
                }
                LuaEventManager.triggerEvent("OnJoypadActivateUI", ((JoypadManager.Joypad)joypad).getID());
                break;
            }
        }
        boolean doUpdate = !GameTime.isGamePaused();
        joypad = profiler.profile("Collision Data");
        try {
            MapCollisionData.instance.updateGameState();
        }
        finally {
            if (joypad != null) {
                ((GameProfiler.ProfileArea)joypad).close();
            }
        }
        ...
        joypad = profiler.profile("UI Resize");
        try {
            UIManager.resize();
        }
        finally {
    ...
    public static boolean isIngameState() {
        return IngameState.instance != null && (GameWindow.states.current == IngameState.instance || GameWindow.states.states.contains(IngameState.instance));
    }
    ...
    protected static void renderInternal() {
        ...
        try (AbstractPerformanceProfileProbe abstractPerformanceProfileProbe = s_performance.statesRender.profile();){
            states.render();
        }
    }
    ...
    private static void uncaughtExceptionMainThread(Thread thread2, Throwable e) {
        try {
    ...
    private static void uncaughtGlobalException(Thread thread2, Throwable e) {
        GameWindow.uncaughtException(thread2, e);
    }

    public static void uncaughtException(Thread thread2, Throwable e) {
        String exceptionMessage = String.format("Unhandled %s thrown by thread %s.", e.getClass().getName(), thread2.getName());
    ...
    private static void mainThreadStep() {
        ...
        if (PerformanceSettings.instance.isFramerateUncapped()) {
            ...
            if ((accumulator += timeDiffNS) >= desiredDt) {
                GameWindow.frameStep();
    ...
    private static void mainThreadInit() {
        ...
        TextureID.useCompression = TextureID.useCompressionOption = Core.safeModeForced || Core.getInstance().getOptionTextureCompression();
    ...
    private static void enter() {
        ...
        flags |= 0x40;
        if (Core.tileScale == 1) {
            GameWindow.LoadTexturePack("Tiles1x", flags);
            GameWindow.LoadTexturePack("Overlays1x", flags);
            GameWindow.LoadTexturePack("JumboTrees1x", flags);
            GameWindow.LoadTexturePack("Tiles1x.floor", flags & 0xFFFFFFFB);
        }
        if (Core.tileScale == 2) {
            ...
            GameWindow.LoadTexturePack("JumboTreesBigs2x", flags);
            GameWindow.LoadTexturePack("Tiles2x.floor", flags & 0xFFFFFFFB);
            GameWindow.LoadTexturePack("B42ChunkCaching2x", flags);
            GameWindow.LoadTexturePack("B42ChunkCaching2x.floor", flags & 0xFFFFFFFB);
            GameWindow.LoadTexturePack("Clock2x", flags);
        }
        GameWindow.setTexturePackLookup();
        ...
        s_fpsTracking.init();
        GameWindow.DoLoadingText(Translator.getText("UI_Loading_ModelsAnimations", new Object[0]));
        ...
        GameWindow.DoLoadingText(Translator.getText("UI_Loading_OnGameBoot", new Object[0]));
    ...
    private static void frameStep() {
        ...
        ++IsoCamera.frameState.frameCount;
        IsoCamera.frameState.updateUnPausedAccumulator();
        try (AbstractPerformanceProfileProbe abstractPerformanceProfileProbe = s_performance.frameStep.profile();){
            GameProfiler.ProfileArea profileArea;
            s_fpsTracking.frameStep();
            try (AbstractPerformanceProfileProbe abstractPerformanceProfileProbe2 = s_performance.logic.profile();){
                GameWindow.logic();
            }
            ...
            if (!GameServer.server) {
                profileArea = profiler.profile("IsoObjectAnimations.update");
                try {
            ...
            GameWindow.renderInternal();
            if (doRenderEvent) {
                profileArea = profiler.profile("On Render");
                try {
                    GameWindow.onRender();
                }
                finally {
            ...
            profileArea = profiler.profile("Lighting");
            try {
            ...
            if (Core.debug) {
                ...
                if (GameKeyboard.isKeyPressed("ToggleLuaConsole") && (console = UIManager.getDebugConsole()) != null) {
    ...
    public static void setTexturePackLookup() {
        ...
        for (int i = texturePacks.size() - 1; i >= 0; --i) {
            TexturePack texturePack = texturePacks.get(i);
            if (texturePack.modId == null || !modIDs.contains(texturePack.modId)) continue;
            texturePackTextures.putAll(texturePack.textures);
        }
    ...
    private static void checkRequiredLibraries() {
        if (System.getProperty("os.name").startsWith("Win")) {
            ...
            try {
                ...
                GameWindow.installRequiredLibrary("_CommonRedist\\vcredist\\2010\\vcredist_x64.exe", "the Microsoft Visual C++ 2010 Redistributable.");
                GameWindow.installRequiredLibrary("_CommonRedist\\vcredist\\2012\\vcredist_x64.exe", "the Microsoft Visual C++ 2012 Redistributable.");
                GameWindow.installRequiredLibrary("_CommonRedist\\vcredist\\2013\\vcredist_x64.exe", "the Microsoft Visual C++ 2013 Redistributable.");
            }
        }
    }
    ...
    private static void init() throws Exception {
        ...
        GameWindow.doEpilepsyWarningText();
        LuaManager.init();
        GameWindow.initShared();
        GameWindow.DoLoadingText(Translator.getText("UI_Loading_Lua", new Object[0]));
    ...
    public static void save(boolean bDoChars) throws IOException {
        ...
        try (FileOutputStream fos = new FileOutputStream(outFile);){
            ...
            try {
                ((DataOutputStream)output).writeInt(249);
                GameWindow.WriteString((DataOutputStream)output, Core.gameMap);
            }
            finally {
                output.close();
            }
        ...
        try {
            ...
            try {
                ...
                ((BufferedOutputStream)output).write(SliceY.SliceBuffer.array(), 0, SliceY.SliceBuffer.position());
            }
            finally {
                output.close();
            }
        ...
        try {
            try {
                ...
                try (FileOutputStream outStream = new FileOutputStream(outFile);){
                    ...
                    IsoWorld.instance.currentCell.save((DataOutputStream)output, bDoChars);
                }
                catch (Exception ex) {
                    ExceptionLogger.logException(ex);
                }
    ...
    public static void doEpilepsyWarningText() {
        ...
        int y = (int)((float)Core.getInstance().getScreenHeight() / 2.0f - 3.5f * (float)lineHeight);
        ...
        for (int i = 1; i <= 8; ++i) {
            TextManager.instance.DrawStringCentre(font, (double)((float)Core.getInstance().getScreenWidth() / 2.0f), (double)y, Translator.getText("UI_EpilepsyWarning_" + i, new Object[0]), 1.0, 1.0, 1.0, 1.0);
            y += lineHeight;
        }
