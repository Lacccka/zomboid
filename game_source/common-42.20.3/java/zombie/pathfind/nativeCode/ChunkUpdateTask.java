...
implements IPathfindTask {
    ...
    static final ObjectPool<ChunkUpdateTask> pool;
    ...
    private static ByteBuffer ensureCapacity(ByteBuffer bb, int capacity) {
        if (bb == null || bb.capacity() < capacity) {
            bb = ByteBuffer.allocateDirect(ChunkUpdateTask.bufferSize(capacity));
        }
        return bb;
    }
    ...
    private static ByteBuffer ensureCapacity(ByteBuffer bb) {
        ...
        if (bb.capacity() - bb.position() < 256) {
            ByteBuffer newBB = ChunkUpdateTask.ensureCapacity(null, bb.position() + 256);
    ...
    ChunkUpdateTask init(IsoChunk chunk) {
        ...
        this.bb = ChunkUpdateTask.ensureCapacity(this.bb);
        this.bb.clear();
        this.bb.putInt(chunk.minLevel + 32);
        this.bb.putInt(chunk.maxLevel + 32);
        bbTemp = ChunkUpdateTask.ensureCapacity(bbTemp);
        bbTemp.clear();
        ...
        if (numSlopedSurfaces > 0) {
            ...
            if (numBytes > this.bb.capacity()) {
                ByteBuffer newBB = ByteBuffer.allocateDirect(ChunkUpdateTask.bufferSize(numBytes));
    ...
    static {
        pool = new ObjectPool<ChunkUpdateTask>(ChunkUpdateTask::new, "ChunkUpdateTask.pool", 2048);
    }
}
