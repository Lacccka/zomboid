...
import zombie.pathfind.nativeCode.ChunkRemoveTask;
...
import zombie.pathfind.nativeCode.VehicleAddTask;
import zombie.pathfind.nativeCode.VehicleRemoveTask;
import zombie.pathfind.nativeCode.VehicleState;
import zombie.pathfind.nativeCode.VehicleUpdateTask;
...
public class PathfindNative {
    ...
    public void init(IsoMetaGrid metaGrid) {
        PathfindNative.initWorld(metaGrid.getMinX(), metaGrid.getMinY(), metaGrid.getWidth(), metaGrid.getHeight(), GameServer.server);
    ...
    public void updateMain() {
        ...
        PathFindRequest request1 = requestToMain.poll();
        while (request1 != null) {
    ...
    public int findPath(PathFindRequest request, ByteBuffer pathBB, boolean bRender) {
        ...
        if (mover instanceof IsoPlayer) {
            IsoPlayer isoPlayer = (IsoPlayer)mover;
            if (!(request.mover instanceof IsoAnimal)) {
                moverType = 1;
        ...
        pathBB.clear();
        return PathfindNative.findPath(this.requestBb, pathBB);
    }
        ...
        if (this.testRequestAdded) {
            if (this.finder.done) {
                ...
                if (GameWindow.states.current == IngameState.instance && !GameTime.isGamePaused() && Mouse.isButtonDown(0) && GameKeyboard.isKeyDown(42)) {
                    ...
                    if (obj != null) {
                        LuaManager.caller.pcall(LuaManager.thread, obj, Float.valueOf(this.request.targetX), Float.valueOf(this.request.targetY), Float.valueOf(this.request.targetZ));
                    }
                }
            }
        } else {
