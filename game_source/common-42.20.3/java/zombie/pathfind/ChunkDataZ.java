...
final class ChunkDataZ {
    ...
    public void init(Chunk chunk, int z) {
        ...
        for (int y = cy - 2; y < cy + 8 + 2; ++y) {
            for (int x = cx - 2; x < cx + 8 + 2; ++x) {
                ...
                if (square == null || !square.has(512)) {
                    clipper.addAABB(x, y, (float)x + 1.0f, (float)y + 1.0f);
                    continue;
                }
                ...
                if (square.has(256) && (square2 = PolygonalMap2.instance.getSquare(x + 1, y, z)) != null) {
                    clipper.addAABBBevel((float)(x + 1) - 0.3f, (float)y - 0.3f, (float)(x + 1) + 0.3f, (float)y + 1.0f + 0.3f, 0.19800001f);
                }
                if (!square.has(32) || (square2 = PolygonalMap2.instance.getSquare(x, y + 1, z)) == null) continue;
                clipper.addAABBBevel((float)x - 0.3f, (float)(y + 1) - 0.3f, (float)x + 1.0f + 0.3f, (float)(y + 1) + 0.3f, 0.19800001f);
            }
        }
        ...
        for (int i = 0; i < polyCount; ++i) {
            ...
            int holeCount = xyBuffer.getShort();
        ...
        chunkBounds.init(x1 -= 2, y1 -= 2, (x2 += 2) - x1, (y2 += 2) - y1);
        ...
        for (int i = 0; i < PolygonalMap2.instance.vehicles.size(); ++i) {
            ...
            if (!chunkBounds.intersects(vehicleBounds)) continue;
    ...
    static {
        pool = new ObjectPool<ChunkDataZ>(ChunkDataZ::new, "ChunkLevel.pool");
    }
}
