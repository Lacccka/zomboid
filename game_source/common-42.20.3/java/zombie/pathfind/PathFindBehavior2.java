...
implements IPathfinder {
    ...
    private static final ObjectPool<DebugPt> actualPool = new ObjectPool<DebugPt>(DebugPt::new, "PathFindBehavior2.actualPool");
    ...
    public void pathToCharacter(IsoGameCharacter target) {
        ...
        if (target.isSittingOnFurniture() && (target.getSquare().isSolid() || target.getSquare().isSolidTrans())) {
            ...
            switch (dir) {
                case N: {
                    IsoGridSquare sqN = IsoWorld.instance.getCell().getGridSquare((float)x + 0.5f, (float)y - 0.3f, (double)z);
                    if (!this.isGoodChairAdjacentSquare(target.getSquare(), sqN)) break;
                    this.setData((float)x + 0.5f, (float)y - 0.3f, z);
                    return;
                }
                case S: {
                    IsoGridSquare sqS = IsoWorld.instance.getCell().getGridSquare((float)x + 0.5f, (float)(y + 1) + 0.3f, (double)z);
                    if (!this.isGoodChairAdjacentSquare(target.getSquare(), sqS)) break;
                    this.setData((float)x + 0.5f, (float)(y + 1) + 0.3f, z);
                    return;
                }
                case W: {
                    IsoGridSquare sqW = IsoWorld.instance.getCell().getGridSquare((float)x - 0.3f, (float)y + 0.5f, (double)z);
                    if (!this.isGoodChairAdjacentSquare(target.getSquare(), sqW)) break;
                    this.setData((float)x - 0.3f, (float)y + 0.5f, z);
                    return;
                }
                case E: {
                    IsoGridSquare sqE = IsoWorld.instance.getCell().getGridSquare((float)(x + 1) + 0.3f, (float)y + 0.5f, (double)z);
            ...
            for (int i = 0; i < 8; ++i) {
                if (IsoDirections.fromIndex(i) == dir) continue;
    ...
    public BehaviorResult update() {
        ...
        if (this.chr.isPerformingNoAimShortStrafe()) {
            ...
            this.chr.set(PlayerStrafeState.STRAFE_SPEED, Float.valueOf(1.0f));
