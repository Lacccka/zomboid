...
extends RenderTarget {
    ...
    public RenderTexture(Descriptor desc) {
        super(desc.name);
        ...
        this.descriptor = new Descriptor(desc);
    }
    ...
    private void Copy(Descriptor desc) {
        this.width = desc.width;
        this.height = desc.height;
        this.length = desc.length;
        this.colourFormat = desc.colourFormat;
    ...
    public void CopyTexture(RenderTarget dest, Rectangle srcRect, Rectangle dstRect) {
        ...
        GL44.glBlitFramebuffer(srcRect.getX(), srcRect.getY(), srcRect.getX() + srcRect.getWidth(), srcRect.getY() + srcRect.getHeight(), dstRect.getX(), dstRect.getY(), dstRect.getX() + dstRect.getWidth(), dstRect.getY() + dstRect.getHeight(), 16384, 9728);
    }
    ...
    public RenderTarget Recreate() {
        ...
        if (this.colour >= 0) {
            if (this.colourFormat == 0) {
                ...
                int type = RenderTexture.GetFormatType(this.colourFormat);
    ...
    public static RenderTexture GetTarget(String name, boolean createIfNull) {
        RenderTarget rt = RenderTexture.GetTarget(name);
    ...
    public static RenderTexture GetTexture(Descriptor desc) {
        RenderTarget rt = RenderTexture.GetTarget(desc.name);
        assert (rt == null || rt instanceof RenderTexture);
        if (rt == null) {
