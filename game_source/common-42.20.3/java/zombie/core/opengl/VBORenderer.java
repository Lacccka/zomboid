...
import zombie.core.opengl.VBORendererCommands;
...
public final class VBORenderer {
    ...
    private final ObjectPool<Run> runPool = new ObjectPool<Run>(Run::new, "VBORenderer.runPool");
    private final ArrayList<Run> runs = new ArrayList();
    private Run runInProgress;
    private static final Vector3f tempVector3f_1;
    private static final Vector3f tempVector3f_2;
    ...
    private void setFormat(VertexBufferObject.VertexFormat format) {
        ...
        this.uv2Offset = -1;
        this.depthOffset = -1;
        block7: for (int i = 0; i < this.format.getNumElements(); ++i) {
    ...
    public void setFloats2(int byteOffset, float f1, float f2) {
        ...
        int index = this.elements.position() - this.elementSize + byteOffset;
        this.elements.putFloat(index, f1);
        this.elements.putFloat(index += 4, f2);
    }
    ...
    public void setFloats3(int byteOffset, float f1, float f2, float f3) {
        ...
        this.elements.putFloat(index, f1);
        this.elements.putFloat(index += 4, f2);
        this.elements.putFloat(index += 4, f3);
    }
    ...
    public void setFloats4(int byteOffset, float f1, float f2, float f3, float f4) {
        ...
        this.elements.putFloat(index += 4, f2);
        this.elements.putFloat(index += 4, f3);
        this.elements.putFloat(index += 4, f4);
    }
    ...
    boolean isFull() {
        ...
        if (run.mode == 4 && run.vertexCount % 3 == 0 && this.elements.position() + 3 * this.elementSize > this.elementSize * this.numElements) {
            return true;
        }
        if (run.mode == 7 && run.vertexCount % 4 == 0 && this.elements.position() + 4 * this.elementSize > this.elementSize * this.numElements) {
            return true;
        }
        return this.elements.position() >= this.elementSize * this.numElements;
    }
    ...
    private VBOLinesShader initShader(String name, VBOLinesShader shader) {
        if (shader == null) {
            return new VBOLinesShader(name);
        }
        return shader;
    }
    ...
    private VBOLinesShader getShaderForFormat() {
        if (this.format == this.formatPositionColor) {
            this.shaderPositionColor = this.initShader("vboRenderer_PositionColor", this.shaderPositionColor);
            return this.shaderPositionColor;
        }
        if (this.format == this.formatPositionColorUv) {
            this.shaderPositionColorUv = this.initShader("vboRenderer_PositionColorUV", this.shaderPositionColorUv);
            return this.shaderPositionColorUv;
        }
        if (this.format == this.formatPositionColorUvDepth) {
            this.shaderPositionColorUvDepth = this.initShader("vboRenderer_PositionColorUVDepth", this.shaderPositionColorUvDepth);
            return this.shaderPositionColorUvDepth;
        }
        if (this.format == this.formatPositionNormalColor) {
            this.shaderPositionNormalColor = this.initShader("vboRenderer_PositionNormalColor", this.shaderPositionNormalColor);
            return this.shaderPositionNormalColor;
        }
        if (this.format == this.formatPositionNormalColorUv) {
            this.shaderPositionNormalColorUv = this.initShader("vboRenderer_PositionNormalColorUV", this.shaderPositionNormalColorUv);
            return this.shaderPositionNormalColorUv;
        }
        return null;
    }
    ...
    public void flush() {
        ...
        this.commandsReady.clear();
        this.runPool.releaseAll((List<Run>)this.runs);
        this.runs.clear();
    ...
    private void setVertexAttribArrays(VertexBufferObject.VertexFormat format) {
        ...
        for (i = 0; i < format.getNumElements(); ++i) {
        ...
        for (i = format.getNumElements(); i < 5; ++i) {
    ...
    private void startRun(Run run) {
        ...
        if (run.shaderProgram != null) {
            run.shaderProgram.setValue("userDepth", this.forceUserDepth == null ? run.userDepth : this.forceUserDepth.floatValue());
        }
    ...
    private void renderRun(Run run) {
        ...
        GL12.glDrawRangeElements(run.mode, run.startVertex, run.startVertex + run.vertexCount, run.vertexCount, 5123, (long)run.startIndex * 2L);
    }

    void startNextRun() {
        Run run;
        Run run2 = run = this.runs.isEmpty() ? this.runInProgress : this.runs.get(0);
        if (run == null) {
            return;
        }
        if (run.vertexCount == 0) {
    ...
    public void startRun(VertexBufferObject.VertexFormat format) {
        ...
        Run run = this.runPool.alloc().init();
        run.format = format;
        if (this.elements != null) {
        ...
        run.shaderProgram = (shader = this.getShaderForFormat()) == null ? null : shader.getProgram();
        this.commands.cmdStartRun();
    ...
    public void addCylinder(int drawStyle, int orientation, float baseRadius, float topRadius, float height, int slices, int stacks, float r1, float g1, float b1, float a1, ShaderProgram shaderProgram) {
        float nsign = orientation == 100021 ? -1.0f : 1.0f;
    ...
    public void addTorus(double r, double c, int rSeg, int cSeg, float r1, float g1, float b1, UI3DScene.Ray cameraRay) {
        ...
        for (int i = 0; i < rSeg; ++i) {
            ...
            for (int j = 0; j <= cSeg; ++j) {
                this.reserve(2);
                for (int k = 0; k <= 1; ++k) {
                    double s = (double)((i + k) % rSeg) + 0.5;
                    double t = j % (cSeg + 1);
                    double x = (c + r * Math.cos(s * (Math.PI * 2) / (double)rSeg)) * Math.cos(t * (Math.PI * 2) / (double)cSeg);
                    double y = (c + r * Math.cos(s * (Math.PI * 2) / (double)rSeg)) * Math.sin(t * (Math.PI * 2) / (double)cSeg);
                    double z = r * Math.sin(s * (Math.PI * 2) / (double)rSeg);
