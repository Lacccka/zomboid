...
public final class ShaderProgram {
    ...
    public final org.joml.Matrix4f modelView = new org.joml.Matrix4f();
    ...
    private void invokeProgramCompiledEvent() {
        ...
        ArrayList<IShaderProgramListener> listenersInvoking = new ArrayList<IShaderProgramListener>(this.onCompiledListeners);
        for (IShaderProgramListener listener : listenersInvoking) {
    ...
    public void compile() {
        ...
        if (GL20.glGetProgrami(this.shaderId, 35714) == 0) {
            ...
            DebugType.Shader.error(ShaderProgram.getLogInfo(this.shaderId));
            this.destroy();
            return;
        }
    ...
    void addCombinedVertexSource(String code) {
        this.vertexUnits.get((int)0).processedCode = this.vertexUnits.get((int)0).processedCode + System.lineSeparator() + code;
    }

    void addCombinedFragmentSource(String code) {
        this.fragmentUnits.get((int)0).processedCode = this.fragmentUnits.get((int)0).processedCode + System.lineSeparator() + code;
    }
    ...
    private boolean compileAllShaderUnits() {
        ...
        for (ShaderUnit unit : this.getShaderUnits()) {
            if (unit.isCompiled() || unit.compile()) continue;
    ...
    private ArrayList<ShaderUnit> getShaderUnits() {
        ArrayList<ShaderUnit> shaderUnits = new ArrayList<ShaderUnit>();
    ...
    private ShaderUnit findShader(String fileName, ShaderUnit.Type unitType) {
        ...
        for (ShaderUnit unit : shaderList) {
            if (!unit.getFileName().equals(fileName)) continue;
            foundUnit = unit;
            break;
        }
        return foundUnit;
    ...
    public void setSamplerUnit(String loc, int textureUnit) {
        Uniform u = this.getUniform(loc, 35678);
        if (u != null) {
    ...
    public void setValueColor(String loc, int rgba) {
        ...
        this.setVector4(loc, 0.003921569f * (float)(rgba >> 24 & 0xFF), 0.003921569f * (float)(rgba >> 16 & 0xFF), 0.003921569f * (float)(rgba >> 8 & 0xFF), 0.003921569f * (float)(rgba & 0xFF));
    }
    ...
    public void setValue(String loc, float val) {
        Uniform u = this.getUniform(loc, 5126);
        if (u != null) {
    ...
    public void setValue(String loc, int val) {
        Uniform u = this.getUniform(loc, 5124);
        if (u != null) {
    ...
    public void setVector2(String loc, float valX, float valY) {
        Uniform u = this.getUniform(loc, 35664);
        if (u != null) {
    ...
    public void setVector3(String loc, float valX, float valY, float valZ) {
        Uniform u = this.getUniform(loc, 35665);
        if (u != null) {
    ...
    public void setVector4(String loc, float valX, float valY, float valZ, float valW) {
        Uniform u = this.getUniform(loc, 35666);
        if (u != null) {
            this.setVector4(u.loc, valX, valY, valZ, valW);
        }
    }

    public final Uniform getUniform(String loc, int type) {
        return this.getUniform(loc, type, false);
    }

    public Uniform getUniform(String loc, int type, boolean bWarn) {
        Uniform u = this.uniformsByName.get(loc);
        if (u == null) {
            if (bWarn) {
    ...
    public void setValue(String loc, Matrix4f matrix4f) {
        Uniform u = this.getUniform(loc, 35676);
        if (u != null) {
            this.setTransformMatrix(u.loc, matrix4f);
        }
    }

    public void setValue(String loc, org.joml.Matrix4f matrix4f) {
        Uniform u = this.getUniform(loc, 35676);
        if (u != null) {
            this.setTransformMatrix(u.loc, matrix4f);
        }
    ...
    public void setValue(String loc, Texture tex, int samplerUnit) {
        ...
        Vector2 uvScale = tex.getUVScale(L_setValue.vector2);
    ...
    private void setUVScale(int samplerUnit, float x, float y) {
        ...
        if (samplerUnit >= this.uvScaleUniforms.length) {
            Object unitName = "UVScale";
            if (samplerUnit > 0) {
                unitName = "UVScale" + samplerUnit;
            }
            this.setVector2((String)unitName, x, y);
            return;
        }
    ...
    void setTransformMatrix(int loc, org.joml.Matrix4f matrix4f) {
        floatBuffer.clear();
