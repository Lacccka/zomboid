...
public class PZGLUtil {
    ...
    public static void checkGLErrorThrow(String format, Object ... args2) throws OpenGLException {
        PZGLUtil.checkGLErrorThrowInternal(false, format, args2);
    }

    public static void checkGLErrorThrowTexture(String format, Object ... args2) throws OpenGLException {
        PZGLUtil.checkGLErrorThrowInternal(false, format, args2);
    }

    private static void checkGLErrorThrowInternal(boolean glErrorCheckEnabled, String format, Object ... args2) throws OpenGLException {
        if (!glErrorCheckEnabled) {
            return;
        }
        int glErrorCode = GL11.glGetError();
        if (glErrorCode != 0) {
            throw new OpenGLException(PZGLUtil.createErrorMessage(glErrorCode, format, args2));
        }
    }

    private static String createErrorMessage(int glErrorCode, String format, Object ... args2) {
        String nl = System.lineSeparator();
        return "  GL Error code (" + glErrorCode + ") encountered." + nl + "  Error translation: " + PZGLUtil.createErrorMessage(glErrorCode) + nl + "  While performing: " + String.format(format, args2);
    }
    ...
    private static void glDebugOutput(int source2, int type, int id, int severity, int length, long message, long userParam) {
        if (severity > 37191 || severity == 33387 && 37191 != severity) {
            return;
        }
        String sourceStr = switch (source2) {
            case 33350 -> "API";
    ...
    public static void printGLState(DebugType out) {
        PZGLUtil.printGLState(out.getLogStream());
    }
    ...
    public static void pushAndMultMatrix(int mode, Matrix4f matrix) {
        ...
        MatrixStack matrixStack2 = matrixStack = mode == 5888 ? Core.getInstance().modelViewMatrixStack : Core.getInstance().projectionMatrixStack;
