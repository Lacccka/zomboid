...
public class Bullet {
    ...
    public static void init() {
        ...
        if (System.getProperty("os.name").contains("OS X")) {
            Bullet.loadLibrary("PZBullet");
        } else {
            Bullet.loadLibrary("PZBullet" + noOpenGl + "64" + libSuffix);
        }
    ...
    public static void initWorld(int minCellX, int minCellY, int maxCellX, int maxCellY, int offsetX, int offsetY, int wx, int wy, int chunkGridWidth) {
        Bullet.initWorld(minCellX, minCellY, maxCellX, maxCellY, offsetX, offsetY, GameServer.server);
        Bullet.activateChunkMap(0, wx, wy, chunkGridWidth);
    }
    ...
    public static void startLoadingPhysicsMeshes() {
        ...
        for (int i = 0; i < physicsShapeScripts.size(); ++i) {
            ...
            PhysicsShape physicsShape = (PhysicsShape)PhysicsShapeAssetManager.instance.load(new AssetPath(physicsShapeScript.meshName), assetParams);
        }
    }
    ...
    public static void initPhysicsMeshes() {
        ...
        Bullet.clearPhysicsMeshes();
        ArrayList<PhysicsShapeScript> physicsShapeScripts = ScriptManager.instance.getAllPhysicsShapes();
        int shapeCount = 0;
        for (int i = 0; i < physicsShapeScripts.size(); ++i) {
            PhysicsShapeScript physicsShapeScript = physicsShapeScripts.get(i);
            PhysicsShape asset = (PhysicsShape)PhysicsShapeAssetManager.instance.getAssetTable().get(physicsShapeScript.meshName);
            if (asset == null || !asset.isReady()) continue;
            for (j = 0; j < asset.meshes.size(); ++j) {
                PhysicsShape.OneMesh oneMesh = asset.meshes.get(j);
                boolean bCompoundShape = asset.meshes.size() > 1;
                Bullet.definePhysicsMesh(shapeCount, bCompoundShape, Bullet.transformPhysicsMeshPoints(physicsShapeScript.translate, physicsShapeScript.rotate, physicsShapeScript.scale, oneMesh.transform, oneMesh.points, false));
            }
        ...
        for (int i = 0; i < vehicleScripts.size(); ++i) {
            ...
            for (j = 0; j < vehicleScript.getPhysicsShapeCount(); ++j) {
                ...
                if (vehicleScriptPhysicsShape.type != 3 || (physicsShapeScript = ScriptManager.instance.getPhysicsShape(vehicleScriptPhysicsShape.physicsShapeScript)) == null || (physicsShape = (PhysicsShape)PhysicsShapeAssetManager.instance.getAssetTable().get(physicsShapeScript.meshName)) == null || !physicsShape.isReady()) continue;
                for (int k = 0; k < physicsShape.meshes.size(); ++k) {
                    PhysicsShape.OneMesh oneMesh = physicsShape.meshes.get(k);
                    if (oneMesh.points.length == 0) continue;
                    Matrix4f xfrm = BaseVehicle.allocMatrix4f().scaling(vehicleScript.getModelScale());
                    Bullet.postMultiplyTranslateRotateScale(physicsShapeScript.translate, physicsShapeScript.rotate, physicsShapeScript.scale, xfrm);
                    oneMesh.transform.transpose();
                    xfrm.mul(oneMesh.transform);
                    oneMesh.transform.transpose();
                    float[] points = Bullet.transformPhysicsMeshPoints(xfrm, oneMesh.points, false);
                    boolean firstUserShape = true;
                    Bullet.defineVehiclePhysicsMesh(vehicleScript.getFullName(), 1 + j, points);
    ...
    public static float[] transformPhysicsMeshPoints(org.joml.Vector3f translate, org.joml.Vector3f rotate, float scale, Matrix4f meshXfrm, float[] points, boolean bSwapYZ) {
        Matrix4f m = Bullet.translationRotateScale(translate, rotate, scale, BaseVehicle.allocMatrix4f());
        if (meshXfrm != null) {
            meshXfrm.transpose();
            m.mul(meshXfrm);
            meshXfrm.transpose();
    ...
    public static Matrix4f translationRotateScale(org.joml.Vector3f translate, org.joml.Vector3f rotate, float scale, Matrix4f result) {
        Quaternionf q = BaseVehicle.allocQuaternionf();
        q.rotationXYZ(rotate.x * ((float)Math.PI / 180), rotate.y * ((float)Math.PI / 180), rotate.z * ((float)Math.PI / 180));
        result.translationRotateScale((Vector3fc)translate, (Quaternionfc)q, scale);
        BaseVehicle.releaseQuaternionf(q);
        return result;
    }

    public static Matrix4f postMultiplyTranslateRotateScale(org.joml.Vector3f translate, org.joml.Vector3f rotate, float scale, Matrix4f result) {
        Matrix4f m2 = Bullet.translationRotateScale(translate, rotate, scale, BaseVehicle.allocMatrix4f());
        result.mul(m2);
    ...
    public static void onVehicleConstraintImpulse(int constraintID, int vidA, int vidB, float impulse) {
        ...
        float f = breakImpulse = vehicleA.getUpVectorDot() > 0.8f && vehicleB.getUpVectorDot() > 0.8f ? 1500.0f : 500.0f;
        if (impulse < breakImpulse) {
            return;
        }
        boolean bl = recentlyConnected = vehicleA.constraintChangedTime + 2000L > System.currentTimeMillis();
    ...
    public static native int add6DofConstraint(int var0, int var1, float var2, float var3, float var4, float var5, float var6, float var7, float var8, float var9, float var10, float var11, float var12, float var13, float var14, float var15, float var16, float var17, float var18, float var19);
    ...
    public static void addRagdoll(int id, Vector3f worldPosition, Quaternion worldRotation) {
        Bullet.addRagdoll(id, worldPosition.x, worldPosition.y, worldPosition.z, worldRotation.x, worldRotation.y, worldRotation.z, worldRotation.w);
    }
    ...
    public static void updateRagdoll(int id, Vector3f worldPosition, Quaternion worldRotation) {
        Bullet.updateRagdoll(id, worldPosition.x, worldPosition.y, worldPosition.z, worldRotation.x, worldRotation.y, worldRotation.z, worldRotation.w);
    }
    ...
    static {
        physicsShapeNameToIndex = new HashMap();
    }
}
