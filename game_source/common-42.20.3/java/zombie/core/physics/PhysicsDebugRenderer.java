...
extends TextureDraw.GenericDrawer {
    private static final ObjectPool<PhysicsDebugRenderer> POOL = new ObjectPool<PhysicsDebugRenderer>(PhysicsDebugRenderer::new, "PhysicsDebugRenderer.POOL");
    ...
    public void init(IsoPlayer player) {
        ...
        this.camOffX = IsoCamera.getRightClickOffX() + (float)IsoCamera.playerOffsetX;
        this.camOffY = IsoCamera.getRightClickOffY() + (float)IsoCamera.playerOffsetY;
        this.camOffX += this.XToScreenExact(player.getX() - (float)PZMath.fastfloor(player.getX()), player.getY() - (float)PZMath.fastfloor(player.getY()), 0.0f, 0);
        this.camOffY += this.YToScreenExact(player.getX() - (float)PZMath.fastfloor(player.getX()), player.getY() - (float)PZMath.fastfloor(player.getY()), 0.0f, 0);
        ...
        if (DebugOptions.instance.physicsRenderBallisticsTargets.getValue()) {
            ...
            for (BallisticsTarget ballisticsTarget : renderedBallisticsTargets) {
                if (!ballisticsTarget.isValidIsoGameCharacter()) continue;
    ...
    public void render() {
        ...
        modelView.translate(-(dx += this.camOffX), -(dy += this.camOffY), 0.0f);
        int side = 32 * Core.tileScale;
        float f = (float)Math.sqrt(side * side + side * side);
        modelView.scale(f, f, f);
        modelView.rotate(3.6651914f, 1.0f, 0.0f, 0.0f);
    ...
    public native void renderVehicle(int var1, int var2, int var3);
}
