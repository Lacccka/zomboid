...
public final class CarController {
    ...
    private void steerInDirectionOfTowing() {
        ...
        Vector2 attachPointSelfToTowedByDirection = BaseVehicle.allocVector2().set(vehicleTowedBy.getX() - attachPointSelfPos.x, vehicleTowedBy.getY() - attachPointSelfPos.y);
        ...
        float angle = attachPointSelfToTowedByDirection.angleBetween(forwardAdjusted) * 57.295776f;
        angle = PZMath.clamp(angle / 2.0f, 0.0f, 45.0f) / 45.0f;
        float p0x = this.vehicleObject.getX();
        if (PZMath.isLeft(p0x, p0y = this.vehicleObject.getY(), p1x = p0x + forwardSelf.x, p1y = p0y + forwardSelf.z, vehicleTowedBy.getX(), vehicleTowedBy.getY()) > 0.0f) {
            angle *= -1.0f;
        }
    ...
    public void control_NoControl() {
        ...
        if (!this.vehicleObject.isEngineRunning()) {
            if (this.vehicleObject.getEngineSpeed() > 0.0) {
                this.vehicleObject.setEngineSpeed(Math.max(this.vehicleObject.getEngineSpeed() - (double)(50.0f * fpsMult), 0.0));
            }
        } else if (this.vehicleObject.getEngineSpeed() > (double)this.vehicleObject.getScript().getEngineIdleSpeed()) {
        ...
        this.engineForce = 0.0f;
        this.brakingForce = this.vehicleObject.getEngineSpeed() > 1000.0 ? 15.0f : 10.0f;
    }

    private void control_Braking() {
        float fpsMult = GameTime.getInstance().getMultiplier() / 0.8f;
        if (this.vehicleObject.getEngineSpeed() > (double)this.vehicleObject.getScript().getEngineIdleSpeed()) {
            this.vehicleObject.addEngineSpeed((float)(-Rand.Next(10, 30)) * fpsMult);
        } else {
            this.vehicleObject.addEngineSpeed((float)Rand.Next(20) * fpsMult);
        }
        this.vehicleObject.transmissionNumber = TransmissionNumber.N;
        this.engineForce = 0.0f;
    ...
    private void control_Forward(float speed) {
        ...
        this.vehicleObject.addEngineSpeed(-Math.min(0.5 * (this.vehicleObject.getEngineSpeed() - (double)engineSpeedCalc), 100.0) * (double)fpsMult);
        if (trainFastDriver) {
            if (speed < 50.0f) {
                this.vehicleObject.addEngineSpeed(-Math.min(0.06 * (this.vehicleObject.getEngineSpeed() - 7000.0), (double)(30.0f - speed)) * (double)fpsMult);
            }
        } else if (speed < 30.0f) {
            this.vehicleObject.addEngineSpeed(-Math.min(0.02 * (this.vehicleObject.getEngineSpeed() - 7000.0), (double)(30.0f - speed)) * (double)fpsMult);
        }
        this.engineForce = (float)((double)this.vehicleObject.getEnginePower() * (0.5 + this.vehicleObject.getEngineSpeed() / 24000.0));
        ...
        if (this.vehicleObject.getEngineSpeed() > 6000.0) {
            this.engineForce = (float)((double)this.engineForce * ((7000.0 - this.vehicleObject.getEngineSpeed()) / 1000.0));
        }
        if (trainSlowDriver) {
    ...
    private void control_ForwardNew(float speed) {
        ...
        if (this.vehicleObject.transmissionNumber.getIndex() < gearRatioCount && this.vehicleObject.transmissionNumber.getIndex() - 1 >= 0) {
            this.vehicleObject.setEngineSpeed(Math.min(this.vehicleObject.getEngineSpeed(), (double)(carRpmData[this.vehicleObject.transmissionNumber.getIndex() - 1].gearChange + 100.0f)));
        }
        if (this.vehicleObject.getEngineSpeed() > (double)engineSpeedCalc) {
            this.vehicleObject.addEngineSpeed(-Math.min(0.5 * (this.vehicleObject.getEngineSpeed() - (double)engineSpeedCalc), 10.0) * (double)fpsMult);
        } else {
        ...
        this.engineForce = (float)((double)(enginePower *= (switch (this.vehicleObject.transmissionNumber) {
            ...
            default -> 1.0f;
        })) * ((double)0.3f + this.vehicleObject.getEngineSpeed() / 30000.0));
        this.engineForce -= this.engineForce * (speed / 200.0f);
        if (speed < 15.0f && this.vehicleObject.getVehicleTowing() != null && this.vehicleObject.getVehicleTowing().isBurnt()) {
            this.engineForce *= Math.min(1.2f, this.vehicleObject.getVehicleTowing().getMass() / 500.0f);
            this.engineForce *= 2.0f;
        }
        if (this.vehicleObject.getEngineSpeed() > 6000.0) {
            this.engineForce *= (7000.0f - (float)this.vehicleObject.getEngineSpeed()) / 1000.0f;
        }
        if (trainSlowDriver) {
            this.engineForce *= 0.75f;
            if (speed > this.vehicleObject.getMaxSpeed() * 0.6f) {
    ...
    private void control_Reverse(float speed) {
        ...
        this.engineForce = (float)((double)(-1.0f * (float)this.vehicleObject.getEnginePower()) * (0.75 + this.vehicleObject.getEngineSpeed() / 24000.0));
        if (this.vehicleObject.getEngineSpeed() > 6000.0) {
            this.engineForce = (float)((double)this.engineForce * ((7000.0 - this.vehicleObject.getEngineSpeed()) / 1000.0));
        }
        if (trainSlowDriver) {
    ...
    protected boolean shouldBeActive() {
        ...
        if (!this.vehicleObject.isAtRest()) {
            return true;
        }
