...
public class ActionManager {
    ...
    public static void stopPlayerActions(PlayerID playerId) {
        for (Action action : actions) {
            if (action.playerId.getID() != playerId.getID()) continue;
    ...
    public void setStateFromPacket(Action packet) {
        for (Action action : actions) {
            if (packet.id != action.id || packet.playerId.getID() != action.playerId.getID()) continue;
    ...
    public static NetTimedAction getAction(byte id, PlayerID playerId) {
        for (Action action : actions) {
            if (!(action instanceof NetTimedAction)) continue;
            NetTimedAction netAction = (NetTimedAction)action;
            if (netAction.id != id || netAction.playerId.getID() != playerId.getID()) continue;
            return netAction;
        }
        return null;
    }
