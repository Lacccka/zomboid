...
implements ISteamWorkshopCallback {
    ...
    public ArrayList<String> getStageFolders() {
        ...
        Path dir = FileSystems.getDefault().getPath(this.getWorkshopFolder(), new String[0]);
        try {
    ...
    public boolean SubmitWorkshopItem(SteamWorkshopItem item) {
        ...
        if (!this.n_SetItemPreview(item.getPreviewImage())) {
            return false;
        }
        return this.n_SubmitItemUpdate(item.getChangeNote());
    }
