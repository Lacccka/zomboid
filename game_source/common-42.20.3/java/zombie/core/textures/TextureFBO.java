...
public final class TextureFBO {
    ...
    private static final TIntArrayList stack;
    private int id;
    ...
    public void swapTextureAndDepth(ITexture newTex, ITexture newDepth) {
        ...
        if (this.depthTexture != null && newDepth != null && newDepth != this.depthTexture && newDepth.getWidth() == this.width && newDepth.getHeight() == this.height) {
            if (newDepth.getID() == -1) {
                newDepth.bind();
            }
    ...
    private void initInternal(ITexture destination, ITexture depthTex, boolean bStencilBuffer) {
        ...
        try {
            ...
            ITexture iTexture = mainTexture = destination != null ? destination : depthTex;
            if (mainTexture == null) {
    ...
    public void endDrawing() {
        lastID = stack.isEmpty() ? 0 : stack.removeAt(stack.size() - 1);
        IGLFramebufferObject funcs = TextureFBO.getFuncs();
        funcs.glBindFramebuffer(funcs.GL_FRAMEBUFFER(), lastID);
    }
    ...
    public void startDrawing(boolean clear, boolean clearToAlphaZero) {
        ...
        IGLFramebufferObject funcs = TextureFBO.getFuncs();
        funcs.glBindFramebuffer(funcs.GL_FRAMEBUFFER(), this.id);
        ITexture iTexture = mainTexture = this.texture != null ? this.texture : this.depthTexture;
        if (mainTexture == null) {
            return;
        }
        if (clear) {
