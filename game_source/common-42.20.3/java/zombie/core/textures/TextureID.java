...
Serializable {
    ...
    public TextureID(AssetPath path, AssetManager manager, TextureIDAssetParams params) {
        super(path, manager);
    ...
    public TextureID(String path, int red, int green, int blue) throws Exception {
        ...
        this.data = new ImageData((String)path);
        this.data.makeTransp((byte)red, (byte)green, (byte)blue);
    ...
    public static TextureID createSteamAvatar(long steamID) {
        ...
        return new TextureID(imageData);
    }
    ...
    private void debugBoundTexture() {
        int current;
        int unit;
        if (DebugOptions.instance.checks.boundTextures.getValue() && Texture.lastTextureID != -1 && (unit = GL11.glGetInteger(34016)) == 33984 && (current = GL11.glGetInteger(32873)) != Texture.lastTextureID) {
            ...
            for (Asset asset : TextureIDAssetManager.instance.getAssetTable().values()) {
                ...
                if (textureID.id != current) continue;
                currentName = textureID.getPath().getPath();
            }
            DebugType.General.error("Texture.lastTextureID %d name=%s != GL_TEXTURE_BINDING_2D %d name=%s", Texture.lastTextureID, lastName, current, currentName);
        }
    }
    ...
    public void setImageData(ImageData data) {
        this.data = data = this.limitMaxSize(data);
    ...
    private void generateHwId(boolean setPixels) {
        ...
        int flags = this.assetParams == null ? (useCompressionOption ? 4 : 0) : this.assetParams.flags;
        boolean bFilterMinNearest = (flags & 1) != 0;
        boolean bFilterMagNearest = (flags & 2) != 0;
        boolean filterFbo = (flags & 0x10) != 0;
        boolean bFilterMipMaps = (flags & 0x40) != 0 && !filterFbo && setPixels;
        ...
        if ((flags & 0x200) != 0) {
            ...
            int n = this.glMagFilter = bFilterMagNearest ? 9728 : 9729;
        ...
        if (setPixels) {
            if (bFilterMipMaps) {
                ...
                for (int mipLevel = mipLevelStart; mipLevel < mipLevelEnd; ++mipLevel) {
                    ...
                    totalMemUsed += (float)mipMap.getDataSize();
    ...
    private void assignFilteringFlags() {
        ...
        if (PerformanceSettings.fboRenderChunk && Core.getInstance().getOffscreenBuffer() != null && this == ((Texture)Core.getInstance().getOffscreenBuffer().getTexture()).dataid) {
            this.glMagFilter = this.glMinFilter = Core.getInstance().getScreenFilter();
        }
        GL11.glTexParameteri(3553, 10241, this.glMinFilter);
        GL11.glTexParameteri(3553, 10240, this.glMagFilter);
        if ((this.flags & 0x40) != 0 && DebugOptions.instance.isoSprite.nearestMagFilterAtMinZoom.getValue() && this.isMinZoomLevel() && this.glMagFilter != 9728) {
            GL11.glTexParameteri(3553, 10240, 9728);
        }
        if (PerformanceSettings.fboRenderChunk && (!DebugOptions.instance.fboRenderChunk.mipMaps.getValue() || FBORenderChunkManager.instance.renderThreadCurrent != null) && (this.flags & 0x40) != 0) {
            GL11.glTexParameteri(3553, 10241, 9728);
            GL11.glTexParameteri(3553, 10240, 9728);
        }
    ...
    public void setAssetParams(AssetManager.AssetParams params) {
        this.assetParams = (TextureIDAssetParams)params;
