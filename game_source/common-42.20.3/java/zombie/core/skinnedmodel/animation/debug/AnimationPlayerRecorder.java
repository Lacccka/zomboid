...
public final class AnimationPlayerRecorder {
    ...
    private static final Set<Class<? extends IsoMovingObject>> animationRecorderTypeMask;
    ...
    public static PrintStream openFileStream(String key, boolean append, Consumer<String> fileNameConsumer) {
        ...
        try {
            ...
            DebugType.General.printException(e, LogSeverity.Error, "Exception thrown trying to create animation player recording file.", new Object[0]);
            return null;
        }
    }
    ...
    public void logAnimNode(LiveAnimNode liveNode) {
        if (liveNode.isTransitioningIn()) {
            this.animationNodeFrame.logWeight("transition(" + liveNode.getTransitionFrom() + "->" + liveNode.getName() + ")", liveNode.getTransitionTrackId(), liveNode.getTransitionLayerIdx(), liveNode.getTransitionInWeight());
        }
        if (liveNode.runningRagdollTrack != null) {
            this.animationNodeFrame.logWeight(liveNode.getName() + "." + liveNode.runningRagdollTrack.getName(), liveNode.runningRagdollTrack.getId(), liveNode.getLayerIdx(), liveNode.runningRagdollTrack.getBlendWeight());
        }
    ...
    public static void initMessaging() {
        if (setAnimationRecordingTriggerWatcher == null) {
            setAnimationRecordingTriggerWatcher = new PredicatedFileWatcher(ZomboidFileSystem.instance.getMessagingDirSub("Trigger_AnimationRecorder.xml"), TriggerSetAnimationRecorderFile.class, AnimationPlayerRecorder::onTrigger_setAnimationRecorderTriggerFile);
    ...
    public static boolean isAnimationRecorderActiveForType(Class<? extends IsoMovingObject> objectType) {
        Set<Class<? extends IsoMovingObject>> set = animationRecorderTypeMask;
        synchronized (set) {
            return AnimationPlayerRecorder.isAnimationRecorderActiveAll() || animationRecorderTypeMask.contains(objectType);
        }
    }
    ...
    public static boolean isAnimationRecorderActive(IsoMovingObject object) {
        return object != null && object.getSquare() != null && (AnimationPlayerRecorder.isAnimationRecorderActiveForType(object.getClass()) || IsoPlayer.anyPlayer(object, AnimationPlayerRecorder::shouldPlayerActivateRecordingOnObject));
    }
    ...
    static {
        animationRecorderTypeMask = new HashSet<Class<? extends IsoMovingObject>>();
    }
}
