...
public final class ModelManager {
    ...
    private final ArrayList<ModelSlot> modelSlots = new ArrayList();
    private final ReferencedObjectPool<ModelInstance> modelInstancePool = new ReferencedObjectPool<ModelInstance>(ModelInstance::new, ModelInstance.class.getSimpleName());
    private ModelMesh animModel;
    private final HashMap<String, AnimationAsset> animationAssets = new HashMap();
    private final ModAnimations gameAnimations = new ModAnimations("game");
    private final HashMap<String, ModAnimations> modAnimations = new HashMap();
    ...
    private final ArrayList<ModelSlot> resetAfterRender = new ArrayList();
    ...
    private static final HashMap<String, String> toLower;
    private static final HashMap<String, String> toLowerTex;
    private static final HashMap<String, String> toLowerKeyRoot;
    ...
    private void loadAnimsFromDir(String dir, ModelMesh animationsModel, ArrayList<String> prefixes) {
        ...
        this.loadAnimsFromDir(ZomboidFileSystem.instance.base.lowercaseUri, ZomboidFileSystem.instance.getMediaLowercaseURI(), file, animationsModel, this.gameAnimations, prefixes);
    }

    private void loadAnimsFromDir(URI baseURI, URI mediaURI, File dir, ModelMesh animationsModel, ModAnimations modAnimations, ArrayList<String> prefixes) {
        if (!dir.exists()) {
        ...
        for (File child : children) {
            ...
            if (!this.checkAnimationMeshPrefixes(child.getName(), prefixes)) continue;
        ...
        if (bHasAnims) {
            DebugFileWatcher.instance.add(new AnimDirReloader(this, baseURI, mediaURI, dir.getPath(), animationsModel, modAnimations).GetFileWatcher());
        }
    }
    ...
    private boolean checkAnimationMeshPrefixes(String fileName, ArrayList<String> prefixes) {
        ...
        for (String prefix : prefixes) {
            ...
            if (!StringUtils.startsWithIgnoreCase(fileName, prefix)) continue;
            prefixOK = true;
        }
        return prefixOK;
    }
    ...
    public void RenderWater(TextureDraw texd, int shaderID, int playerIndex, int firstSquare, int numSquares, boolean bShore) {
        ...
        if (shader instanceof WaterShader) {
            WaterShader waterShader = (WaterShader)shader;
    ...
    public void RenderParticles(TextureDraw texd, int userId, int va11) {
        ...
        GL11.glPushMatrix();
        GL11.glLoadIdentity();
        GL11.glViewport(0, 0, SpriteRenderer.instance.getRenderingPlayerCamera((int)userId).offscreenWidth, SpriteRenderer.instance.getRenderingPlayerCamera((int)userId).offscreenHeight);
        ...
        if (shader instanceof FireShader) {
            FireShader fireShader = (FireShader)shader;
        ...
        if (shader instanceof SmokeShader) {
            SmokeShader smokeShader = (SmokeShader)shader;
    ...
    public void Reset(IsoGameCharacter chr) {
        ...
        ModelSlot modelSlot = chr.legsSprite.modelSlot;
        ...
        for (int i = 0; i < modelSlot.sub.size(); ++i) {
            ...
            if (inst == chr.primaryHandModel || inst == chr.secondaryHandModel || modelSlot.attachedModels.contains(inst)) continue;
    ...
    public void Add(IsoGameCharacter chr) {
        ...
        ModelSlot slot = this.getSlot(chr);
    ...
    public Model getBodyModel(IsoGameCharacter chr) {
        ...
        if (chr.isZombie() && ((IsoZombie)chr).isSkeleton()) {
            if (chr.isFemale()) {
                return this.skeletonFemaleModel;
            }
            return this.skeletonMaleModel;
        }
        if (chr.isFemale()) {
    ...
    public boolean shouldHideModel(ItemVisuals itemVisuals, ItemBodyLocation itemBodyLocation) {
        return BodyLocations.getGroup("Human").getLocation(itemBodyLocation) != null && PopTemplateManager.instance.isItemModelHidden(itemVisuals, itemBodyLocation);
    }

    private void DoCharacterModelEquipped(IsoGameCharacter chr, ModelSlot slot) {
        ...
        for (int i = 0; i < chr.getAttachedItems().size(); ++i) {
            ...
            if (StringUtils.isNullOrWhitespace(modelName) || this.shouldHideModel(itemVisuals, ItemBodyLocation.get(ResourceLocation.of(attachmentName = chr.getAttachedItems().getGroup().getLocation(attachedItem.getLocation()).getAttachmentName()))) || (modelInstance = this.addStatic(slot.model, modelName, attachmentName, attachmentName)) == null) continue;
            ...
            if (inventoryItem instanceof HandWeapon) {
                ...
                if (!Core.getInstance().getOptionSimpleWeaponTextures()) {
                    textureInitializer = ModelInstanceTextureInitializer.alloc();
        ...
        BaseAction baseAction = action = chr.getCharacterActions().isEmpty() ? null : (BaseAction)chr.getCharacterActions().get(0);
        ...
        if (primaryItem != null) {
            replacement = primaryItem.getItemReplacementPrimaryHand();
        ...
        if (secondaryItem != null && primaryItem != secondaryItem) {
            replacement = secondaryItem.getItemReplacementSecondHand();
            chr.secondaryHandModel = this.addEquippedModelInstance(chr, slot, secondaryItem, "Bip01_Prop2", replacement, overrideHandModels);
        }
    }

    private ModelInstance addEquippedModelInstance(IsoGameCharacter chr, ModelSlot slot, InventoryItem item, String boneName, ItemReplacement replacement, boolean overrideHandModels) {
        ...
        if (item != null) {
            if (!(replacement == null || StringUtils.isNullOrEmpty(replacement.maskVariableValue) || replacement.clothingItem == null && StringUtils.isNullOrWhitespace(item.getStaticModel()))) {
            ...
            if (overrideHandModels && !StringUtils.isNullOrWhitespace(item.getStaticModel())) {
                ...
                Object staticModel = item.getStaticModel();
                if (item.hasComponent(ComponentType.FluidContainer) && item.getFluidContainer().getFilledRatio() > 0.5f && (modelScriptFluid = ScriptManager.instance.getModelScript(item.getWorldStaticItem() + "_Fluid")) != null) {
                    staticModel = item.getWorldStaticItem() + "_Fluid";
                }
                ModelInstance modelInstance = this.addStatic(slot, (String)staticModel, boneName);
                if (this.requiresSmartTexture(item)) {
                    ModelInstanceTextureInitializer textureInitializer = ModelInstanceTextureInitializer.alloc();
                    textureInitializer.init(modelInstance, item);
                    modelInstance.setTextureInitializer(textureInitializer);
    ...
    private ModelInstance addMaskingModel(ModelSlot slot, IsoGameCharacter chr, InventoryItem item, ItemReplacement replacement, String maskVariableValue, String animMaskAttachment, String bone) {
        ...
        if (replacement.clothingItem != null && itemVisual != null) {
            ...
            result = !StringUtils.isNullOrEmpty(animMaskAttachment) ? this.addStaticForcedTex(slot.model, item.getStaticModel(), animMaskAttachment, animMaskAttachment, forcedTex) : this.addStaticForcedTex(slot, item.getStaticModel(), bone, forcedTex);
    ...
    private void addWeaponPartModels(ModelSlot slot, HandWeapon weapon, ModelInstance parent) {
    ...
    public void update() {
        IsoGameCharacter toReset;
        int i;
        for (i = 0; i < this.toResetNextFrame.size(); ++i) {
            toReset = this.toResetNextFrame.get(i);
            this.Reset(toReset);
        }
        this.toResetNextFrame.clear();
        for (i = 0; i < this.toResetEquippedNextFrame.size(); ++i) {
            toReset = this.toResetEquippedNextFrame.get(i);
            this.ResetCharacterEquippedHands(toReset);
        }
        this.toResetEquippedNextFrame.clear();
        for (i = 0; i < this.toRemove.size(); ++i) {
            IsoGameCharacter toRemove = this.toRemove.get(i);
            this.DoRemove(toRemove);
        }
        this.toRemove.clear();
        for (i = 0; i < this.resetAfterRender.size(); ++i) {
            ModelSlot slot = this.resetAfterRender.get(i);
            if (slot.isRendering()) continue;
            slot.reset();
        ...
        if (IsoWorld.instance != null && IsoWorld.instance.currentCell != null) {
            for (BaseVehicle vehicle : IsoWorld.instance.currentCell.getVehicles()) {
                ...
                if (square == null || vehicle.sprite == null || !vehicle.sprite.hasActiveModel()) continue;
                ...
                for (int j = 0; j < IsoPlayer.numPlayers; ++j) {
                    ...
                    boolean bl = bForceSeen = player.getVehicle() == vehicle;
                    if (!bForceSeen && !square.lighting[j].bSeen() || !bForceSeen && !square.lighting[j].bCouldSee()) continue;
                    playerIndex = j;
                    break;
                }
                if (playerIndex == -1) {
    ...
    private ModelSlot addNewSlot(IsoGameCharacter character) {
        ModelSlot newSlot = new ModelSlot(this.modelSlots.size(), null, character);
        this.modelSlots.add(newSlot);
        return newSlot;
    }

    public ModelSlot getSlot(IsoGameCharacter chr) {
        for (int i = 0; i < this.modelSlots.size(); ++i) {
            ModelSlot slot = this.modelSlots.get(i);
            if (slot.remove || slot.isRendering() || slot.active) continue;
            return slot;
        }
    ...
    private boolean DoRemove(IsoGameCharacter chr) {
        ...
        for (int i = 0; i < this.modelSlots.size(); ++i) {
            ModelSlot slot = this.modelSlots.get(i);
            if (slot.character != chr) continue;
    ...
    public void Remove(BaseVehicle vehicle) {
        if (vehicle.sprite != null && vehicle.sprite.modelSlot != null) {
            ModelSlot slot = vehicle.sprite.modelSlot;
            if (!this.resetAfterRender.contains(slot)) {
                this.resetAfterRender.add(slot);
            }
    ...
    public void ResetNextFrame(IsoGameCharacter isoGameCharacter) {
        if (ModelManager.isUpdateDisabled()) {
            return;
        }
        ...
        RenderThread.invokeOnRenderContext(() -> {
            ...
            try {
                if (!this.contains.isEmpty()) {
                    IsoGameCharacter[] remove;
                    for (IsoGameCharacter chr : remove = this.contains.toArray(new IsoGameCharacter[0])) {
                        this.DoRemove(chr);
                    }
                }
    ...
    public void addVehicle(BaseVehicle vehicle) {
        ...
        ModelSlot slot = this.getSlot(null);
        slot.model = inst;
        inst.setOwner(slot);
        inst.object = vehicle;
        slot.sub.clear();
    ...
    public ModelInstance addStatic(ModelSlot slot, String meshName, String texName, String boneName, String shaderName) {
    ...
    public ModelInstance newStaticInstance(ModelSlot slot, String meshName, String texName, String boneName, String shaderName) {
    ...
    private ModelInstance addStatic(ModelSlot slot, String staticModel, String boneName) {
        return this.addStaticForcedTex(slot, staticModel, boneName, null);
    }

    private ModelInstance addStaticForcedTex(ModelSlot slot, String staticModel, String boneName, String forcedTex) {
        ModelScript modelScript;
        String meshName = ScriptManager.getItemName(staticModel);
        String texName = ScriptManager.getItemName(staticModel);
        String shaderName = null;
        ModelMetaData metaData = modelMetaData.get(staticModel);
        if (metaData != null) {
        ...
        if ((modelScript = ScriptManager.instance.getModelScript(staticModel)) != null) {
            meshName = modelScript.getMeshName();
            ModelInstance inst = this.addStatic(slot, meshName, texName = modelScript.getTextureName(), boneName, shaderName = modelScript.getShaderName());
            if (inst != null) {
    ...
    public ModelInstance addStaticForcedTex(ModelInstance parentInst, String modelName, String attachNameSelf, String attachNameParent, String forcedTex) {
        ...
        if ((model = this.tryGetLoadedModel(meshName, texName, true, shaderName, false)) == null && meshName != null) {
            this.loadStaticModel(meshName, texName, shaderName);
            model = this.getLoadedModel(meshName, texName, true, shaderName);
            if (model == null) {
                DebugType.ModelManager.error("Model not found. model:%s tex:%s", meshName, texName);
    ...
    private String modifyShaderName(String shaderName) {
        if ((StringUtils.equals((String)shaderName, "vehicle") || StringUtils.equals((String)shaderName, "vehicle_multiuv") || StringUtils.equals((String)shaderName, "vehicle_norandom_multiuv")) && (!Core.getInstance().getPerfReflectionsOnLoad() || Core.getInstance().getUseOpenGL21())) {
            shaderName = (String)shaderName + "_noreflect";
        }
        return shaderName;
    }
    ...
    private Model loadModelInternal(String meshName, String texName, String shaderName, ModelMesh animationsModel, boolean isStatic, String postProcess) {
        ...
        if (shaderName != null && StringUtils.startsWithIgnoreCase(shaderName, "vehicle")) {
            ...
            params.textureFlags |= 0x100;
        } else {
            params.textureFlags = this.getTextureFlags();
        }
        if (!this.shouldLimitTextureSize(texName)) {
            params.textureFlags &= 0xFFFFFE7F;
        }
        if ((model = (Model)ModelAssetManager.instance.load(new AssetPath(modelId = this.createModelKey(meshName, texName, isStatic, shaderName)), params)) != null) {
    ...
    public int getTextureFlags() {
        int flags;
        int n = flags = TextureID.useCompression ? 4 : 0;
    ...
    public boolean shouldLimitTextureSize(String textureName) {
        ...
        return !textureName.equals("IsoObject/MODELS_fixtures_doors_fences_01");
    }
    ...
    public Model getLoadedModel(String meshName) {
        ...
        if (modelScript != null) {
            ...
            Model model = this.tryGetLoadedModel(modelScript.getMeshName(), modelScript.getTextureName(), modelScript.isStatic, modelScript.getShaderName(), false);
            if (model != null) {
            ...
            modelScript.loadedModel = model = modelScript.isStatic ? this.loadModelInternal(modelScript.getMeshName(), modelScript.getTextureName(), modelScript.getShaderName(), null, true, modelScript.postProcess) : this.loadModelInternal(modelScript.getMeshName(), modelScript.getTextureName(), modelScript.getShaderName(), animsModel, false, modelScript.postProcess);
            return model;
        }
        ModelMetaData metaData = modelMetaData.get(meshName);
        if (metaData != null) {
            ...
            return metaData.isStatic ? this.loadStaticModel(metaData.meshName, metaData.textureName, metaData.shaderName) : this.loadModel(metaData.meshName, metaData.textureName, this.animModel);
        }
        ...
        for (Map.Entry<String, Model> entry : this.modelMap.entrySet()) {
            ...
            if (!key.startsWith(nameKey) || (value = entry.getValue()) == null || key.length() != nameKey.length() && key.charAt(nameKey.length()) != '&') continue;
            model = value;
            break;
        }
        if (model == null) {
    ...
    public Model tryGetLoadedModel(ModelScript modelScript, boolean logError) {
        if (modelScript.getModelManagerKey() == null) {
            modelScript.setModelManagerKey(this.createModelKey(modelScript.getMeshName(), modelScript.getTextureName(), modelScript.isStatic, modelScript.getShaderName()));
        }
    ...
    private String createModelKey2(String name, String tex, boolean isStatic, String shaderName) {
        ...
        String keyParams = shaderEquals + shaderName.toLowerCase().trim();
        if (!StringUtils.isNullOrWhitespace(tex)) {
            keyParams = keyParams + texA + tex.toLowerCase().trim();
        }
        if (isStatic) {
            keyParams = keyParams + isStaticTrue;
        }
        String keyRoot = name.toLowerCase(Locale.ENGLISH).trim();
        return keyRoot + amp + keyParams;
    }

    private AnimationAsset loadAnim(String name, ModelMesh animationsModel, ModAnimations modAnimations) {
    ...
    private void putAnimationAsset(String name, AnimationAsset anim, ModAnimations modAnimations) {
    ...
    public void reloadModelsMatching(String meshName) {
        ...
        for (String key : keySet) {
            Model model;
            if (!key.contains(meshName) || (model = this.modelMap.get(key)).isEmpty()) continue;
            ...
            assetParams.isStatic = model.mesh.vb == null ? key.contains(isStaticTrue) : model.mesh.vb.isStatic;
            MeshAssetManager.instance.reload(model.mesh, assetParams);
        }
    }

    public void loadModAnimations() {
        for (ModAnimations modAnimations : this.modAnimations.values()) {
        ...
        for (int i = 0; i < modIDs.size(); ++i) {
            ...
            if (mod == null || !mod.animsXFile.common.absoluteFile.isDirectory() && !mod.animsXFile.version.absoluteFile.isDirectory()) continue;
            ModAnimations modAnimations = this.modAnimations.get(modID);
            if (modAnimations != null) {
                modAnimations.setPriority(i + 1);
                continue;
            }
            modAnimations = new ModAnimations(modID);
            modAnimations.setPriority(i + 1);
            this.modAnimations.put(modID, modAnimations);
            for (AnimationsMesh am2 : animationsMeshes) {
                for (String dir : am2.animationDirectories) {
                    if (!am2.modelMesh.isReady()) continue;
                    File subDir = new File(mod.animsXFile.common.canonicalFile, dir);
                    if (subDir.exists()) {
                        this.loadAnimsFromDir(mod.baseFile.common.lowercaseUri, mod.mediaFile.common.lowercaseUri, subDir, am2.modelMesh, modAnimations, am2.animationPrefixes);
                    }
                    if (!(subDir = new File(mod.animsXFile.version.canonicalFile, dir)).exists()) continue;
                    this.loadAnimsFromDir(mod.baseFile.version.lowercaseUri, mod.mediaFile.version.lowercaseUri, subDir, am2.modelMesh, modAnimations, am2.animationPrefixes);
                }
            }
    ...
    void setActiveAnimations() {
        ...
        for (AnimationsMesh am : animationsMeshes) {
            if (!am.modelMesh.isReady()) continue;
            am.modelMesh.skinningData.animationClips.clear();
            if (!am.keepMeshAnimations) continue;
        ...
        for (ModAnimations modAnimations : this.modAnimations.values()) {
            ...
            for (AnimationAsset animationAsset : modAnimations.animationAssetList) {
                ...
                if (existing != null && existing != animationAsset && existing.modAnimations.priority > modAnimations.priority) continue;
    ...
    public void initAnimationMeshes(boolean bReloading) {
        ...
        for (AnimationsMesh am : animationsMeshes) {
            for (String dir : am.animationDirectories) {
                File file;
                if (!am.modelMesh.isReady() || !(file = new File(ZomboidFileSystem.instance.getAnimsXFile(), dir)).exists()) continue;
    ...
    private boolean isLoadingAnimationMeshes() {
        ArrayList<AnimationsMesh> animationsMeshes = ScriptManager.instance.getAllAnimationsMeshes();
        for (AnimationsMesh am : animationsMeshes) {
            if (am.modelMesh.isFailure() || am.modelMesh.isReady()) continue;
            return true;
        }
        return false;
    }
    ...
    private void loadHumanAnimations(ChooseGameInfo.Mod mod, ModAnimations modAnimations) {
        ...
        for (File file : files) {
            ...
            animName = ZomboidFileSystem.instance.getAnimName(dirURI, file);
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                if (this.isAnimationsMeshDirectory(file.getName())) continue;
                this.loadAnimsFromDir(mod.baseFile.version.lowercaseUri, mod.mediaFile.version.lowercaseUri, file, humanAM.modelMesh, modAnimations, null);
                continue;
            }
            animName = ZomboidFileSystem.instance.getAnimName(dirURI, file);
            this.loadAnim(animName, humanAM.modelMesh, modAnimations);
    ...
    private boolean isAnimationsMeshDirectory(String dir) {
        ArrayList<AnimationsMesh> animationsMeshes = ScriptManager.instance.getAllAnimationsMeshes();
        for (AnimationsMesh am : animationsMeshes) {
            if (!am.animationDirectories.contains(dir)) continue;
            return true;
        }
        return false;
    }
    ...
    private boolean requiresSmartTexture(InventoryItem item) {
        ...
        if (item.getColorGreen() != 1.0f) {
            return true;
        }
        if (item.getColorBlue() != 1.0f) {
            return true;
        }
        return item.hasComponent(ComponentType.FluidContainer) && !item.getFluidContainer().isEmpty();
    }
    ...
    implements PredicatedFileWatcher.IPredicatedFileWatcherCallback {
        ...
        public AnimDirReloader(ModelManager this$0, URI baseURI, URI mediaURI, String dir, ModelMesh animationsModel, ModAnimations modAnimations) {
            ModelManager modelManager = this$0;
        ...
        private boolean IsInDir(String filename) {
            ...
            try {
                if (this.dirSecondary != null) {
                    return filename.startsWith(this.dirAbsolute) || filename.startsWith(this.dirSecondaryAbsolute);
                }
                return filename.startsWith(this.dirAbsolute);
            }
            catch (Exception e) {
        ...
        public void call(String entryKey) {
            ...
            AnimationAsset anim = this.this$0.getAnimationAsset(animName);
            if (anim != null) {
                if (!anim.isEmpty()) {
                    DebugType.General.debugln("Reloading animation: %s", this.this$0.animAssetToString(anim));
            ...
            this.this$0.loadAnim(animName, this.animationsModel, this.modAnimations);
        }
        ...
        public void Update(float deltaT) {
            ...
            synchronized (object) {
                ...
                for (int n = 0; n < this.sub.size(); ++n) {
                    this.sub.get((int)n).animPlayer = this.model.animPlayer;
                }
            }
        }
