...
public class OutfitManager {
    ...
    @XmlTransient
    private final Hashtable<String, ClothingItemEntry> cachedClothingItems = new Hashtable();
    @XmlTransient
    ...
    private void loaded() {
        for (String modID : ZomboidFileSystem.instance.getModIDs()) {
            ...
            OutfitManager outfitManager = OutfitManager.tryParse(modID, mod.getVersionDir(), "media/clothing/clothing.xml");
            if (outfitManager != null) {
                for (Outfit outfitMod : outfitManager.maleOutfits) {
                    outfit = this.FindMaleOutfit(outfitMod.name);
                    if (outfit == null) {
                ...
                for (Outfit outfitMod : outfitManager.femaleOutfits) {
                    outfit = this.FindFemaleOutfit(outfitMod.name);
                    if (outfit == null) {
            ...
            outfitManager = OutfitManager.tryParse(modID, mod.getCommonDir(), "media/clothing/clothing.xml");
            if (outfitManager == null) continue;
            for (Outfit outfitMod : outfitManager.maleOutfits) {
                outfit = this.FindMaleOutfit(outfitMod.name);
                if (outfit == null) {
                    this.maleOutfits.add(outfitMod);
        ...
        DebugFileWatcher.instance.add(new PredicatedFileWatcher(ZomboidFileSystem.instance.getString("media/clothing/clothing.xml"), entryKey -> OutfitManager.onClothingXmlFileChanged()));
    ...
    private void unload() {
        for (ClothingItemEntry entry : this.cachedClothingItems.values()) {
    ...
    public Outfit GetRandomOutfit(boolean female) {
        Outfit randomOutfit = female ? PZArrayUtil.pickRandom(this.femaleOutfits) : PZArrayUtil.pickRandom(this.maleOutfits);
        return randomOutfit;
    }
    ...
    public Outfit GetRandomNonSillyOutfit(boolean female) {
        Outfit randomOutfit;
        do {
            randomOutfit = female ? PZArrayUtil.pickRandom(this.femaleOutfits) : PZArrayUtil.pickRandom(this.maleOutfits);
    ...
    public Outfit GetRandomNonProfessionalOutfit(boolean female) {
        Object outfit = "Generic0" + (Rand.Next(5) + 1);
        if (Rand.NextBool(4)) {
            if (female) {
        ...
        return this.GetSpecificOutfit(female, (String)outfit);
    }

    public Outfit GetSpecificOutfit(boolean female, String outfitName) {
        Outfit specificOutfit = female ? this.FindFemaleOutfit(outfitName) : this.FindMaleOutfit(outfitName);
        return specificOutfit;
    }

    private static OutfitManager tryParse(String modID, String modDir, String filename) {
        try {
            return OutfitManager.parse(modID, modDir, filename);
        }
    ...
    private static OutfitManager parse(String modID, String modDir, String filename) throws PZXmlParserException {
        filename = "game".equals(modID) ? ZomboidFileSystem.instance.base.canonicalFile.getAbsolutePath() + File.separator + ZomboidFileSystem.processFilePath((String)filename, File.separatorChar) : modDir + File.separator + ZomboidFileSystem.processFilePath((String)filename, File.separatorChar);
        if (!new File((String)filename).exists()) {
            return null;
        }
        OutfitManager rootElement = PZXmlUtil.parse(OutfitManager.class, (String)filename);
        if (rootElement != null) {
            ...
            PZArrayUtil.forEach(rootElement.maleOutfits, outfit -> rootElement.maleOutfitMap.put(outfit.name, (Outfit)outfit));
            PZArrayUtil.forEach(rootElement.femaleOutfits, outfit -> rootElement.femaleOutfitMap.put(outfit.name, (Outfit)outfit));
        }
        return rootElement;
    }
    ...
    private static void tryWrite(OutfitManager out, String filename) {
        try {
            OutfitManager.write(out, filename);
        }
    ...
    private Outfit FindOutfit(ArrayList<Outfit> outfitsList, String outfitName) {
        ...
        for (int i = 0; i < outfitsList.size(); ++i) {
            ...
            if (!o.name.equalsIgnoreCase(outfitName)) continue;
            foundOutfit = o;
            break;
        }
        return foundOutfit;
    ...
    public ClothingItem getClothingItem(String itemGUID) {
        ...
        ClothingItemEntry cachedElement = this.cachedClothingItems.get(itemGUID);
        if (cachedElement == null) {
            cachedElement = new ClothingItemEntry();
        ...
        if (cachedElement.fileWatcher == null) {
            ClothingItemEntry cachedElement2 = cachedElement;
    ...
    private String extractClothingItemName(String filePath) {
        ...
        itemName = StringUtils.trimSuffix(itemName, ".xml");
        return itemName;
    }

    private void onClothingItemFileChanged(ClothingItemEntry cachedElement) {
    ...
    public void loadAllClothingItems() {
        ...
        for (int i = 0; i < allItems.size(); ++i) {
            ...
            if (scriptItem.replacePrimaryHand != null && (guid = ZomboidFileSystem.instance.getGuidFromFilePath("media/clothing/clothingItems/" + scriptItem.replacePrimaryHand.clothingItemName + ".xml")) != null) {
                scriptItem.replacePrimaryHand.clothingItem = this.getClothingItem(guid);
            }
            if (scriptItem.replaceSecondHand != null && (guid = ZomboidFileSystem.instance.getGuidFromFilePath("media/clothing/clothingItems/" + scriptItem.replaceSecondHand.clothingItemName + ".xml")) != null) {
                scriptItem.replaceSecondHand.clothingItem = this.getClothingItem(guid);
            }
            if (StringUtils.isNullOrWhitespace(scriptItem.getClothingItem()) || (guid = ZomboidFileSystem.instance.getGuidFromFilePath("media/clothing/clothingItems/" + scriptItem.getClothingItem() + ".xml")) == null) continue;
    ...
    private void debugOutfit(Outfit outfit) {
        ...
        for (ClothingItemReference itemRef : outfit.items) {
            ...
            if (clothingItem == null || clothingItem.isEmpty() || (itemType = ScriptManager.instance.getItemTypeForClothingItem(clothingItem.mame)) == null || (scriptItem = ScriptManager.instance.getItem(itemType)) == null || !scriptItem.isItemType(ItemType.CONTAINER)) continue;
            ItemBodyLocation itemBodyLocation = location = scriptItem.getBodyLocation() != null ? scriptItem.canBeEquipped : scriptItem.getBodyLocation();
