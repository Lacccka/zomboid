...
implements IRuntimeAnimationCommand {
    ...
    public void parse(ScriptParser.Block block) {
        for (ScriptParser.Value value : block.values) {
            ...
            if ("frame".equalsIgnoreCase(k)) {
                this.frame = PZMath.tryParseInt(v, 1);
                continue;
            }
            if (!"sourceFrame".equalsIgnoreCase(k)) continue;
    ...
    public void exec(List<Keyframe> keyframes) {
        ...
        for (int i = 0; i < 60; ++i) {
            ...
            if (keyframes1.length == 0) continue;
            Keyframe keyframe1 = keyframes1[0];
            Keyframe newKeyframe = new Keyframe();
            newKeyframe.bone = keyframe1.bone;
            newKeyframe.boneName = keyframe1.boneName;
            newKeyframe.time = (float)(this.frame - 1) / (float)this.fps;
            newKeyframe.position = KeyframeUtil.GetKeyFramePosition(keyframes1, (float)(this.sourceFrame - 1) / (float)this.sourceFps, clip.getDuration());
            newKeyframe.rotation = KeyframeUtil.GetKeyFrameRotation(keyframes1, (float)(this.sourceFrame - 1) / (float)this.sourceFps, clip.getDuration());
