...
implements IAnimEventCallback {
    ...
    private final Set<IAnimEventCallback> animCallbackHandlers = new HashSet<IAnimEventCallback>();
    ...
    public void init(IAnimatable character, String animationSetName) {
        ...
        if (character instanceof IAnimEventCallback) {
            IAnimEventCallback animEventListener = (IAnimEventCallback)((Object)character);
    ...
    public void FindTransitionsFromProxy(TransitionNodeProxy proxy) {
        for (int i = 0; i < proxy.allNewNodes.size(); ++i) {
            ...
            for (int j = 0; j < proxy.allOutgoingNodes.size(); ++j) {
                ...
                if (toNode == fromNodePair.liveAnimNode.getSourceNode() || (animTransition = fromNodePair.liveAnimNode.findTransitionTo(this.character, toNodePair.liveAnimNode.getSourceNode())) == null) continue;
    ...
    private static List<String> collectBaseGameFiles() {
        ...
        try {
            ...
            files.addAll(AdvancedAnimator.searchFolders(ZomboidFileSystem.instance.base.lowercaseUri, animSetDir.toPath()));
            files.addAll(AdvancedAnimator.searchFolders(ZomboidFileSystem.instance.base.lowercaseUri, actionGroupsDir.toPath()));
        }
        catch (IOException e) {
    ...
    private static List<String> loadModMedia(String modDir) {
        ...
        try {
            ...
            files.addAll(AdvancedAnimator.searchFolders(lowercaseURI, canonicalAnimSets.toPath()));
            files.addAll(AdvancedAnimator.searchFolders(lowercaseURI, canonicalActionGroups.toPath()));
        }
        catch (IOException e) {
            ExceptionLogger.logException(e);
        }
    ...
    private static List<String> collectModFiles() throws Exception {
        ...
        for (String s : root) {
            ...
            modFiles.addAll(AdvancedAnimator.loadModMedia(mod.getCommonDir()));
            modFiles.addAll(AdvancedAnimator.loadModMedia(mod.getVersionDir()));
    ...
    public static void load() throws Exception {
        ...
        List<String> gameFiles = AdvancedAnimator.collectBaseGameFiles();
        gameFiles.addAll(AdvancedAnimator.collectModFiles());
        AdvancedAnimator.buildChecksum(gameFiles);
        AdvancedAnimator.finalizeChecksum();
    }
