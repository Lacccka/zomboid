...
implements IAnimListener {
    ...
    private static final Pool<AnimLayer> s_pool = new Pool<AnimLayer>(AnimLayer::new);
    ...
    public boolean transitionTo(AnimState newState, boolean force) {
        DebugType.AnimationDetailed.debugln("TransitionTo: from Anim <%s> to State <%s>", this.getLiveAnimNodes().isEmpty() ? "NoAnim" : this.getLiveAnimNodes().getFirst().getName(), newState != null ? newState.name : "NoState");
    ...
    private void updateInternalWhileGrappling(IGrappleable thisGrappleable, LiveAnimNode liveAnimNode) {
        ...
        for (int trackIdx = 0; trackIdx < liveNodeTracksCount; ++trackIdx) {
            ...
            if (++numGrapplingNodes > 1) {
                DebugType.Grapple.warn("More than one AnimNode is grappling. The node '%s' is being overwritten by node '%s'.", thisGrappleable.getSharedGrappleAnimNode(), liveAnimNode.getName());
            }
