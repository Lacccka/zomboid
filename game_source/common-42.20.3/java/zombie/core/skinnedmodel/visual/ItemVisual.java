...
public final class ItemVisual {
    ...
    public Item getScriptItem() {
        if (StringUtils.isNullOrWhitespace(this.fullType)) {
            return null;
        }
        return ScriptManager.instance.getItem(this.fullType);
    }
    ...
    public ClothingItem getClothingItem() {
        ...
        if (!StringUtils.isNullOrWhitespace(this.alternateModelName)) {
            ...
            if ("WithoutDrainable".equalsIgnoreCase(this.alternateModelName) && (withoutDrainableItem = ScriptManager.instance.getItem(scriptItem.getWithoutDrainable())) != null && withoutDrainableItem.getClothingItemAsset() != null) {
    ...
    public void setHue(float hue) {
        ...
        this.hue = hue = Math.min(hue, 1.0f);
    }
    ...
    public float getHue(ClothingItem clothingItem) {
        if (clothingItem.allowRandomHue) {
            if (this.hue == Float.POSITIVE_INFINITY) {
                this.hue = (float)OutfitRNG.Next(200) / 100.0f - 1.0f;
            }
            return this.hue;
        }
        this.hue = 0.0f;
    ...
    public void save(ByteBuffer output) throws IOException {
        ...
        if (this.basicPatches != null) {
            ...
            for (i = 0; i < this.basicPatches.length; ++i) {
        ...
        if (this.denimPatches != null) {
            ...
            for (i = 0; i < this.denimPatches.length; ++i) {
        ...
        if (this.leatherPatches != null) {
            ...
            for (i = 0; i < this.leatherPatches.length; ++i) {
    ...
    public void load(ByteBuffer input, int worldVersion) throws IOException {
        ...
        if ((flags1 & 1) != 0) {
            int r = input.get() & 0xFF;
            int g = input.get() & 0xFF;
        ...
        if ((count = input.get()) > 0 && this.blood == null) {
        ...
        count = input.get();
        if (count > 0 && this.basicPatches == null) {
            this.basicPatches = new byte[BloodBodyPartType.MAX.index()];
        }
        for (i = 0; i < count; ++i) {
            amount = input.get();
            if (i >= this.basicPatches.length) continue;
            this.basicPatches[i] = amount;
        }
        count = input.get();
        if (count > 0 && this.denimPatches == null) {
            this.denimPatches = new byte[BloodBodyPartType.MAX.index()];
        }
        for (i = 0; i < count; ++i) {
            amount = input.get();
            if (i >= this.denimPatches.length) continue;
            this.denimPatches[i] = amount;
        }
        count = input.get();
        if (count > 0 && this.leatherPatches == null) {
            this.leatherPatches = new byte[BloodBodyPartType.MAX.index()];
        }
        for (i = 0; i < count; ++i) {
            amount = input.get();
            if (i >= this.leatherPatches.length) continue;
    ...
    public float getDenimPatch(BloodBodyPartType bodyPartType) {
        ...
        return (float)(this.denimPatches[bodyPartType.index()] & 0xFF) / 255.0f;
    }
    ...
    public float getLeatherPatch(BloodBodyPartType bodyPartType) {
        ...
        return (float)(this.leatherPatches[bodyPartType.index()] & 0xFF) / 255.0f;
    }
    ...
    public float getBasicPatch(BloodBodyPartType bodyPartType) {
        ...
        return (float)(this.basicPatches[bodyPartType.index()] & 0xFF) / 255.0f;
    }
    ...
    public String getLastStandString() {
        ...
        if ((textureChoice = this.getTextureChoice()) != -1) {
            sb.append("textureChoice=");
            sb.append(textureChoice);
            sb.append(";");
        }
        if ((hue = this.getHue(clothingItem)) != 0.0f) {
            sb.append("hue=");
            sb.append(hue);
            sb.append(";");
        }
        if (!StringUtils.isNullOrWhitespace(decal = this.getDecal(clothingItem))) {
            sb.append("decal=");
            sb.append(decal);
            sb.append(";");
        }
    ...
    public static InventoryItem createLastStandItem(String saveStr) {
        ...
        block24: for (int i = 0; i < ss.length; ++i) {
            ...
            switch (key) {
                case "version": {
                    ...
                    if (version >= 1 && version <= 1) continue block24;
                    return null;
                }
                case "baseTexture": {
                    try {
                ...
                case "decal": {
                    if (StringUtils.isNullOrWhitespace(value)) continue block24;
                ...
                case "tint": {
                    ImmutableColor tint = ItemVisual.colorFromString(value);
                    if (tint == null) continue block24;
                    itemVisual.setTint(tint);
                    continue block24;
                }
                case "type": {
                    item = (InventoryItem)InventoryItemFactory.CreateItem(value);
                    if (item == null) {
                        return null;
                    }
