...
extends Asset {
    ...
    public Model(AssetPath path, AssetManager manager, ModelAssetParams params) {
        super(path, manager);
        ...
        if (!StringUtils.isNullOrWhitespace(params.textureName)) {
            this.tex = params.textureName.contains("media/") ? Texture.getSharedTexture(params.textureName, params.textureFlags) : Texture.getSharedTexture("media/textures/" + params.textureName + ".png", params.textureFlags);
        }
    ...
    public static void boneToWorldCoords(AnimationPlayer animPlayer, float x, float y, float z, float animalSize, int boneIndex, Vector3 vec) {
        vec.x = animPlayer.getModelTransformAt((int)boneIndex).m03;
        vec.y = animPlayer.getModelTransformAt((int)boneIndex).m13;
        vec.z = animPlayer.getModelTransformAt((int)boneIndex).m23;
        vec.x *= animalSize;
        vec.y *= animalSize;
        vec.z *= animalSize;
        Model.vectorToWorldCoords(x, y, z, animPlayer.getRenderedAngle(), vec);
    }
    ...
    public static void vectorToWorldCoords(IsoGameCharacter character, Vector3 vec) {
        ...
        Model.vectorToWorldCoords(character.getX(), character.getY(), character.getZ(), angle, vec);
    }
    ...
    public static void boneToWorldCoords(IsoGameCharacter character, int boneIndex, Vector3 vec) {
        ...
        if (character instanceof IsoAnimal) {
            ...
            vec.x *= isoAnimal.getAnimalSize();
            vec.y *= isoAnimal.getAnimalSize();
            vec.z *= isoAnimal.getAnimalSize();
        }
        Model.vectorToWorldCoords(character, vec);
    }

    public static void boneToWorldCoords(IsoGameCharacter character, SkinningBone bone, Vector3 vec) {
        Model.boneToWorldCoords(character, bone.index, vec);
    }

    public static void boneZDirectionToWorldCoords(IsoGameCharacter character, int boneIndex, Vector3 vec, float length) {
        AnimationPlayer animPlayer = character.getAnimationPlayer();
        vec.x = animPlayer.getModelTransformAt((int)boneIndex).m02 * length;
        vec.y = animPlayer.getModelTransformAt((int)boneIndex).m12 * length;
        vec.z = animPlayer.getModelTransformAt((int)boneIndex).m22 * length;
        vec.x += animPlayer.getModelTransformAt((int)boneIndex).m03;
        vec.y += animPlayer.getModelTransformAt((int)boneIndex).m13;
        vec.z += animPlayer.getModelTransformAt((int)boneIndex).m23;
        Model.vectorToWorldCoords(character, vec);
    ...
    public static void boneZDirectionToWorldCoords(IsoGameCharacter character, SkinningBone bone, Vector3 vec, float length) {
        Model.boneZDirectionToWorldCoords(character, bone.index, vec, length);
    }

    public static void boneYDirectionToWorldCoords(IsoGameCharacter character, int boneIndex, Vector3 vec, float length) {
        AnimationPlayer animPlayer = character.getAnimationPlayer();
        vec.x = animPlayer.getModelTransformAt((int)boneIndex).m01 * length;
        vec.y = animPlayer.getModelTransformAt((int)boneIndex).m11 * length;
        vec.z = animPlayer.getModelTransformAt((int)boneIndex).m21 * length;
        vec.x += animPlayer.getModelTransformAt((int)boneIndex).m03;
        vec.y += animPlayer.getModelTransformAt((int)boneIndex).m13;
        vec.z += animPlayer.getModelTransformAt((int)boneIndex).m23;
        Model.vectorToWorldCoords(character, vec);
    ...
    public static void vectorToWorldCoords(ModelSlotRenderData slotData, Vector3 vec) {
        ...
        Model.vectorToWorldCoords(slotData.x, slotData.y, slotData.z, angle, vec);
    }
    ...
    public static void worldToModel(IsoGameCharacter isoGameCharacter, Vector3 vector3) {
        ...
        float angle = animPlayer.getRenderedAngle();
        vector3.x -= isoGameCharacter.getX();
        vector3.y -= isoGameCharacter.getY();
        vector3.z -= isoGameCharacter.getZ();
        vector3.x /= 1.5f;
        vector3.y /= 1.5f;
        float t = vector3.z;
        vector3.z = vector3.y;
    ...
    public static void CharacterModelCameraBegin(ModelSlotRenderData slotData) {
        ...
        if (slotData.inVehicle) {
            org.joml.Matrix4f mv = Core.getInstance().modelViewMatrixStack.peek();
            mv.translate(0.0f, slotData.centerOfMassY, 0.0f);
            mv.rotate(slotData.vehicleAngleZ * ((float)Math.PI / 180), 0.0f, 0.0f, 1.0f);
            mv.rotate(slotData.vehicleAngleY * ((float)Math.PI / 180), 0.0f, 1.0f, 0.0f);
            mv.rotate(slotData.vehicleAngleX * ((float)Math.PI / 180), 1.0f, 0.0f, 0.0f);
    ...
    private void DrawWireframe(ModelSlotRenderData slotData, ModelInstanceRenderData instData) {
        ...
        if (effect2 != null) {
            ...
            float targetDepth = PerformanceSettings.fboRenderChunk ? (instData.modelInstance.parent == null ? instData.modelInstance.targetDepth : instData.modelInstance.parent.targetDepth) : 0.5f;
    ...
    private void DrawSolid(ModelSlotRenderData slotData, ModelInstanceRenderData instData) {
        ...
        if (this.effect.isInstanced()) {
            ...
            org.joml.Matrix4f mv = Core.getInstance().modelViewMatrixStack.peek();
            org.joml.Matrix4f p = Core.getInstance().projectionMatrixStack.peek();
            p.mul(mv, mvp);
        ...
        if (!DebugOptions.instance.debugDrawSkipDrawNonSkinnedModel.getValue()) {
            try (GameProfiler.ProfileArea profileArea = GameProfiler.getInstance().profile("Mesh.Draw.Call");){
    ...
    public void DrawChar(ModelSlotRenderData slotData, ModelInstanceRenderData instData) {
        ...
        if ((modelScript = instData.modelInstance.modelScript) != null && modelScript.cullFace != -1) {
    ...
    private void drawVehicleLights(ModelSlotRenderData slotData) {
        ...
        for (int i = 0; i < slotData.effectLights.length; ++i) {
            ...
            if ((float)el.radius <= 0.0f) continue;
            float x = el.x;
            float y = el.y;
            float swap = z = el.z;
            z = y;
            y = swap;
            vbor.addLine(0.0f, 0.0f, 0.0f, x *= -54.0f, y *= 54.0f, z *= 54.0f, 1.0f, 1.0f, 0.0f, 1.0f);
        }
        vbor.endRun();
        vbor.flush();
        GL11.glEnable(2929);
    ...
    public static void drawBoneMtx(Matrix4f boneMtx) {
        Model.drawBoneMtxInternal(boneMtx);
        GL11.glColor3f(1.0f, 1.0f, 1.0f);
        GL11.glEnable(2929);
    }

    private static void drawBoneMtxInternal(Matrix4f boneMtx) {
        float scale = 0.5f;
        ...
        Model.drawArrowInternal(posx, posy, posz, xAxisx, xAxisy, xAxisz, zAxisx, zAxisy, zAxisz, 0.5f, 0.15f, 0.1f, 1.0f, 0.0f, 0.0f);
        Model.drawArrowInternal(posx, posy, posz, yAxisx, yAxisy, yAxisz, zAxisx, zAxisy, zAxisz, 0.5f, 0.15f, 0.1f, 0.0f, 1.0f, 0.0f);
        Model.drawArrowInternal(posx, posy, posz, zAxisx, zAxisy, zAxisz, xAxisx, xAxisy, xAxisz, 0.5f, 0.15f, 0.1f, 0.0f, 0.0f, 1.0f);
    }

    private static void drawArrowInternal(float posx, float posy, float posz, float fwdAxisx, float fwdAxisy, float fwdAxisz, float leftAxisx, float leftAxisy, float leftAxisz, float scale, float arrowHeadLength, float arrowHeadRadius, float red, float green, float blue) {
        ...
        vbor.addLine(posx + fwdAxisx * scale, posy + fwdAxisy * scale, posz + fwdAxisz * scale, posx + (fwdAxisx * arrowHeadInvLength + leftAxisx * arrowHeadRadius) * scale, posy + (fwdAxisy * arrowHeadInvLength + leftAxisy * arrowHeadRadius) * scale, posz + (fwdAxisz * arrowHeadInvLength + leftAxisz * arrowHeadRadius) * scale, red, green, blue, 1.0f);
        vbor.addLine(posx + fwdAxisx * scale, posy + fwdAxisy * scale, posz + fwdAxisz * scale, posx + (fwdAxisx * arrowHeadInvLength - leftAxisx * arrowHeadRadius) * scale, posy + (fwdAxisy * arrowHeadInvLength - leftAxisy * arrowHeadRadius) * scale, posz + (fwdAxisz * arrowHeadInvLength - leftAxisz * arrowHeadRadius) * scale, red, green, blue, 1.0f);
    }

    public void debugDrawLightSource(IsoLightSource ls, float cx, float cy, float cz, float radians) {
        Model.debugDrawLightSource(ls.x, ls.y, ls.z, cx, cy, cz, radians);
    }
    ...
    public static void debugDrawLightSource(float lx, float ly, float lz, float cx, float cy, float cz, float radians) {
        ...
        float vx = x *= 0.67f;
        float vy = y *= 0.67f;
        x = (float)((double)vx * Math.cos(radians) - (double)vy * Math.sin(radians));
        y = (float)((double)vx * Math.sin(radians) + (double)vy * Math.cos(radians));
        x *= -1.0f;
        float swap = z;
        z = y;
        y = swap;
    ...
    public void DrawVehicle(ModelSlotRenderData slotData, ModelInstanceRenderData instData) {
        ...
        GL11.glDepthFunc(513);
        ModelCamera.instance.Begin();
        org.joml.Matrix4f modelView = Core.getInstance().modelViewMatrixStack.peek();
    ...
    public static void debugDrawAxis(float x, float y, float z, boolean flipX, boolean flipY, boolean flipZ, float length, float thickness) {
        ...
        vbor.addLine(x, y, z, x + length * (float)(flipX ? -1 : 1), y, z, 1.0f, 0.0f, 0.0f, 1.0f);
        vbor.addLine(x, y, z, x, y + length * (float)(flipY ? -1 : 1), z, 0.0f, 1.0f, 0.0f, 1.0f);
        vbor.addLine(x, y, z, x, y, z + length * (float)(flipZ ? -1 : 1), 0.0f, 0.0f, 1.0f, 1.0f);
        vbor.endRun();
        vbor.flush();
        GL11.glColor3f(1.0f, 1.0f, 1.0f);
        GL11.glEnable(2929);
    ...
    public static void debugDrawAxis(float x, float y, float z, float length, float thickness) {
        Model.debugDrawAxis(x, y, z, false, false, false, length, thickness);
    }
    ...
    public void setLightsInst(ModelSlotRenderData slotData, int lightCount) {
        ...
        for (i = 0; i < lightCount; ++i) {
            ModelInstance.EffectLight el = slotData.effectLights[i];
            this.effect.setLightInst(i, el.x, el.y, el.z, el.r, el.g, el.b, el.radius, slotData.animPlayerAngle, slotData.x, slotData.y, slotData.z, slotData.properties);
        }
        for (i = lightCount; i < 5; ++i) {
    ...
    public void CreateShader(String name) {
        ...
        Lambda.invoke(RenderThread::invokeOnRenderContext, this, name, (lThis, lName) -> {
            lThis.effect = ShaderManager.instance.getOrCreateShader((String)lName, lThis.isStatic, false);
        });
    }
