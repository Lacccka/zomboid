...
public final class ModelOutlines {
    ...
    private final ObjectPool<Drawer> drawerPool = new ObjectPool<Drawer>(Drawer::new, "ModelOutlines.drawerPool");
    ...
    private void renderTexture(ITexture texture, int x0, int y0, int w, int h, ShaderProgram shaderProgram) {
        ...
        float u0 = (float)x0 / (float)texture.getWidthHW();
        float v0 = (float)(y0 + h) / (float)texture.getHeightHW();
        float u1 = (float)(x0 + w) / (float)texture.getWidthHW();
        float v1 = (float)y0 / (float)texture.getHeightHW();
        float z = 0.0f;
        y0 = 0;
        x0 = 0;
        vbor.addQuad(x0, y0, u0, v0, x0 + w, y0 + h, u1, v1, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f);
        vbor.endRun();
        vbor.flush();
    }
    ...
    extends TextureDraw.GenericDrawer {
        ...
        public void render() {
            if (this.startFrame) {
                instance.startFrame(this.playerIndex);
            } else {
                instance.endFrame(this.playerIndex);
            }
        }
