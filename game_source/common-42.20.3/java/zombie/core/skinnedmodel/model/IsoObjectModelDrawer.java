...
extends TextureDraw.GenericDrawer {
    private static final ObjectPool<IsoObjectModelDrawer> s_modelDrawerPool = new ObjectPool<IsoObjectModelDrawer>(IsoObjectModelDrawer::new, "IsoObjectModelDrawer.s_modelDrawerPool");
    private static final TIntObjectHashMap<FloatBufferPool> s_matrixPalettePool = new TIntObjectHashMap();
    ...
    private void initMatrixPalette() {
        ...
        if (skinningData == null) {
            this.matrixPalette = IsoObjectModelDrawer.releaseMatrixPalette(this.matrixPalette);
            return;
        }
        if (this.animationPlayer == null) {
            if (this.model.isStatic || this.spriteModel.getAnimationName() == null) {
                ...
                if (this.matrixPalette == null || this.matrixPalette.capacity() < matrixPalette.capacity()) {
                    this.matrixPalette = IsoObjectModelDrawer.releaseMatrixPalette(this.matrixPalette);
                    this.matrixPalette = IsoObjectModelDrawer.allocMatrixPalette(matrixPalette.capacity());
                }
        ...
        if (this.matrixPalette == null || this.matrixPalette.capacity() < skinTransforms.length * 16) {
            this.matrixPalette = IsoObjectModelDrawer.releaseMatrixPalette(this.matrixPalette);
            this.matrixPalette = IsoObjectModelDrawer.allocMatrixPalette(skinTransforms.length * 16);
        }
        this.matrixPalette.clear();
    ...
    private static FloatBuffer allocMatrixPalette(int capacity) {
        FloatBufferPool buffers = s_matrixPalettePool.get(capacity);
        if (buffers == null) {
            buffers = new FloatBufferPool(capacity, "IsoObjectModelDrawer.s_matrixPalettePool." + capacity);
            s_matrixPalettePool.put(capacity, buffers);
        }
        return (FloatBuffer)buffers.alloc();
    }
    ...
    private static FloatBuffer releaseMatrixPalette(FloatBuffer matrixPalette) {
        ...
        FloatBufferPool buffers = s_matrixPalettePool.get(matrixPalette.capacity());
    ...
    private static final class FloatBufferPool
    extends ObjectPool<FloatBuffer> {
        final int capacity;
