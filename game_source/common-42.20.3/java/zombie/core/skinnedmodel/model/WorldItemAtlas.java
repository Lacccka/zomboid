...
public final class WorldItemAtlas {
    ...
    private final ArrayList<Atlas> atlasListMain = new ArrayList();
    private final ConcurrentLinkedQueue<Atlas> newAtlasQ = new ConcurrentLinkedQueue();
    ...
    private final ObjectPool<ItemTextureDrawer> itemTextureDrawerPool = new ObjectPool<ItemTextureDrawer>(ItemTextureDrawer::new, "WorldItemAtlas.itemTextureDrawerPool");
    private final ObjectPool<ItemTextureDepthDrawer> itemTextureDepthDrawerPool = new ObjectPool<ItemTextureDepthDrawer>(ItemTextureDepthDrawer::new, "WorldItemAtlas.itemTextureDepthDrawerPool");
    private final ObjectPool<WeaponPartParams> weaponPartParamPool = new ObjectPool<WeaponPartParams>(WeaponPartParams::new, "WorldItemAtlas.weaponPartParamPool");
    ...
    private void updateAtlasList() {
        Atlas atlas = this.newAtlasQ.poll();
        while (atlas != null) {
            this.atlasListMain.add(atlas);
            atlas = this.newAtlasQ.poll();
        }
    }
    ...
    public void render() {
        ...
        for (i = 0; i < this.atlasListMain.size(); ++i) {
            Atlas atlas = this.atlasListMain.get(i);
        ...
        boolean init(InventoryItem item, boolean bHighRes) {
            ...
            if (worldStaticItem != null) {
                ...
                if ((clothing = Type.tryCastTo(item, Clothing.class)) != null || item.getClothingItem() != null) {
                    ...
                    ImmutableColor immutableColor = clothingTint = item.isCustomColor() ? new ImmutableColor(item.getColorRed(), item.getColorGreen(), item.getColorBlue()) : itemVisual.getTint(clothingItem);
            ...
            if (item instanceof Clothing) {
                ...
                if (clothingItem != null && itemVisual != null && !StringUtils.isNullOrWhitespace(meshName) && "Bip01_Head".equalsIgnoreCase(clothingItem.attachBone) && (!clothing.isCosmetic() || item.isBodyLocation(ItemBodyLocation.EYES))) {
                    ...
                    ImmutableColor tint = item.isCustomColor() ? new ImmutableColor(item.getColorRed(), item.getColorGreen(), item.getColorBlue()) : itemVisual.getTint(clothingItem);
