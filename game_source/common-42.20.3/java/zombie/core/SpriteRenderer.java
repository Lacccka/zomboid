...
public final class SpriteRenderer {
    ...
    public void render(Texture tex, double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, float r1, float g1, float b1, float a1, float r2, float g2, float b2, float a2, float r3, float g3, float b3, float a3, float r4, float g4, float b4, float a4, Consumer<TextureDraw> texdModifier) {
        this.states.getPopulatingActiveState().render(tex, x1, y1, x2, y2, x3, y3, x4, y4, r1, g1, b1, a1, r2, g2, b2, a2, r3, g3, b3, a3, r4, g4, b4, a4, texdModifier);
    }
    ...
    public void renderdebug(Texture tex, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float r1, float g1, float b1, float a1, float r2, float g2, float b2, float a2, float r3, float g3, float b3, float a3, float r4, float g4, float b4, float a4, Consumer<TextureDraw> texdModifier) {
        this.states.getPopulatingActiveState().renderdebug(tex, x1, y1, x2, y2, x3, y3, x4, y4, r1, g1, b1, a1, r2, g2, b2, a2, r3, g3, b3, a3, r4, g4, b4, a4, texdModifier);
    }
    ...
    public void renderClamped(Texture tex, int x, int y, int width, int height, int clampMinX, int clampMinY, int clampWidth, int clampHeight, float r, float g, float b, float a, Consumer<TextureDraw> texdModifier) {
        ...
        float v1 = 1.0f;
        if (tex != null) {
            u0 = (float)dx1 / (float)width;
            v0 = (float)dy1 / (float)height;
            u1 = (float)(width - dx2) / (float)width;
            v1 = (float)(height - dy2) / (float)height;
    ...
    public void postRender() {
        ...
        try (AbstractPerformanceProfileProbe abstractPerformanceProfileProbe = this.buildStateUiDrawBuffer.profile();){
            this.buildStateUIDrawBuffer(renderState);
        }
        if (!UIManager.useUiFbo) {
            WorldMapImages.startFrame();
        }
        abstractPerformanceProfileProbe = this.buildStateDrawBuffer.profile();
        try {
    ...
    public void notifyRenderStateQueue() {
        SpriteRendererStates spriteRendererStates = this.states;
    ...
    public void setExtraWallShaderParams(WallShaderTexRender wallTexRender) {
    ...
    public void pushFrameDown() {
        ...
        synchronized (spriteRendererStates) {
            ...
            try (AbstractPerformanceProfileProbe abstractPerformanceProfileProbe = this.waitForReadySlotToOpen.profile();){
    ...
    public SpriteRenderState acquireStateForRendering(BooleanSupplier waitCallback) {
        SpriteRendererStates spriteRendererStates = this.states;
        synchronized (spriteRendererStates) {
            try (AbstractPerformanceProfileProbe abstractPerformanceProfileProbe = this.waitForReadyState.profile();){
                if (!this.waitForReadyState(waitCallback)) {
                    SpriteRenderState spriteRenderState = null;
    ...
    public static final class RingBuffer {
        ...
        void create() {
            ...
            for (int n = 0; n < 5000; ++n) {
                this.stateRun[n] = new StateRun(this);
            }
            this.vbo = new GLVertexBufferObject[this.numBuffers];
            this.ibo = new GLVertexBufferObject[this.numBuffers];
            for (int i = 0; i < this.numBuffers; ++i) {
                this.vbo[i] = new GLVertexBufferObject(this.bufferSize, GLVertexBufferObject.funcs.GL_ARRAY_BUFFER(), GLVertexBufferObject.funcs.GL_STREAM_DRAW());
                this.vbo[i].create();
                this.ibo[i] = new GLVertexBufferObject(this.indexBufferSize, GLVertexBufferObject.funcs.GL_ELEMENT_ARRAY_BUFFER(), GLVertexBufferObject.funcs.GL_STREAM_DRAW());
                this.ibo[i].create();
            }
        }
        ...
        private boolean isStateChanged(TextureDraw draw, TextureDraw prevDraw, Style newStyle, Texture newTexture0, Texture newTexture1, Texture newTexture2, byte newUseAttribArray) {
        ...
        void growStateRuns() {
            ...
            for (int i = this.numRuns; i < newStateRun.length; ++i) {
                newStateRun[i] = new StateRun(this);
            }
        ...
        private void drawElements(int start, int length, int startIndex, int endIndex) {
            ...
            GL12.glDrawRangeElements(4, start, start + length, endIndex - startIndex, 5123, (long)startIndex * 2L);
        }
            ...
            if (GL11.glGetInteger(34016) == unit) {
                ...
                if (texture0 == null && current != 0) {
                    for (Asset asset : TextureAssetManager.instance.getAssetTable().values()) {
                        ...
                        if (texture.getID() != current) continue;
                    ...
                    DebugType.General.error("SpriteRenderer.lastBoundTexture0 id=" + texture0.getID() + " doesn't match OpenGL texture id=" + current + " " + currentName);
                }
            }
        }
        ...
        private class StateRun {
            ...
            public String toString() {
                ...
                return this.getClass().getSimpleName() + "{ " + nl + "  ops:" + PZArrayUtil.arrayToString(this.ops, "{", "}", ", ") + nl + "  texture0:" + String.valueOf(this.texture0) + nl + "  texture1:" + String.valueOf(this.texture1) + nl + "  useAttribArray:" + this.useAttribArray + nl + "  style:" + String.valueOf(this.style) + nl + "  start:" + this.start + nl + "  length:" + this.length + nl + "  indices:" + String.valueOf(this.indices) + nl + "  startIndex:" + this.startIndex + nl + "  endIndex:" + this.endIndex + nl + "}";
            }
            ...
            void render() {
                ...
                if (this.style != this.this$0.lastRenderedStyle) {
                    if (this.this$0.lastRenderedStyle != null && (!ignoreStyles || this.this$0.lastRenderedStyle != AdditiveStyle.instance && this.this$0.lastRenderedStyle != TransparentStyle.instance && this.this$0.lastRenderedStyle != LightingStyle.instance)) {
                        this.this$0.lastRenderedStyle.resetState();
                    }
                    if (this.style != null && (!ignoreStyles || this.style != AdditiveStyle.instance && this.style != TransparentStyle.instance && this.style != LightingStyle.instance)) {
                ...
                if (this.this$0.restoreBoundTextures) {
                    ...
                    this.this$0.lastRenderedTexture2 = null;
                ...
                if (this.texture2 != this.this$0.lastRenderedTexture2) {
                    ...
                    GL13.glActiveTexture(33984);
                    this.this$0.lastRenderedTexture2 = this.texture2;
                }
                if (this.length == 0) {
                    return;
                }
                ...
                assert (GL11.glGetInteger(34964) == this.this$0.vbo[this.this$0.sequence].getID());
                if (this.style.getRenderSprite()) {
                    try (AbstractPerformanceProfileProbe abstractPerformanceProfileProbe = this.this$0.drawRangleElements.profile();){
                        this.this$0.drawElements(this.start, this.length, this.startIndex, this.endIndex);
                    }
                } else {
