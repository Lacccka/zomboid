...
public abstract class GenericSpriteRenderState {
    private static final int INITIAL_SLOTS = 2048;
    private static final int RELEASE_PER_FRAME = 8;
    public final int index;
    public TextureDraw[] sprite = new TextureDraw[2048];
    public Style[] style = new Style[2048];
    public int numSprites;
    private int liveSprites;
    ...
    public void CheckSpriteSlots() {
        if (this.numSprites == this.sprite.length) {
            int newLength = this.numSprites * 3 / 2 + 1;
            this.sprite = Arrays.copyOf(this.sprite, newLength);
            this.style = Arrays.copyOf(this.style, newLength);
        }
        if (this.sprite[this.numSprites] == null) {
            this.sprite[this.numSprites] = new TextureDraw();
            this.liveSprites = this.numSprites + 1;
        }
    }
    ...
    public void clear() {
        ...
        this.releaseUnusedSprites();
        this.numSprites = 0;
        this.cachedRenderChunkIndexMap.clear();
    }

    private void releaseUnusedSprites() {
        for (int n = 0; n < 8 && this.liveSprites > this.numSprites; ++n) {
            this.sprite[--this.liveSprites] = null;
        }
    }
