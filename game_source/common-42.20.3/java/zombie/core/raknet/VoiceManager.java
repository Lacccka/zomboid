...
public class VoiceManager {
    ...
    void VoiceOpenChannelReply(long uuid, ByteBufferReader buf) {
        if (this.isEnable) {
            ...
            DebugType.Voice.debugln("enabled=%b, sample-rate=%d, period=%d, complexity=%d, buffering=%d, is3D=%b", serverVOIPEnable, sampleRate, period, 1, buffering, is3D);
            try {
    ...
    private static void logFrame(IsoPlayer me, IsoPlayer player, float distance) {
        ...
        if (currentTime > timestamp) {
            ...
            DebugType.Multiplayer.warn(String.format("\"%s\" (%b) received VOIP frame from \"%s\" (%b) at distance=%f", me.getUsername(), me.canHearAll(), player.getUsername(), player.canHearAll(), Float.valueOf(distance)));
        }
    }
