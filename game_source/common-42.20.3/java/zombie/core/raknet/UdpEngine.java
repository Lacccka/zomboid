...
public class UdpEngine {
    ...
    public void connected() {
        ...
        if (GameClient.client) {
            if (!(GameClient.askPing || GameClient.sendQR || GameClient.askCustomizationData)) {
                ...
                INetworkPacket.send(PacketTypes.PacketType.Login, new Object[0]);
    ...
    private void decode(ByteBufferReader buf) {
        int packetIdentifier = buf.getByte() & 0xFF;
        switch (packetIdentifier) {
            ...
            case 134: {
                ...
                if (GameServer.server) {
                    long guid = this.peer.getGuidOfPacket();
                    UdpConnection con = this.connectionMap.get(guid);
                    if (con == null) {
                        DebugType.Network.warn("GOT PACKET FROM UNKNOWN CONNECTION guid=%d packetId=%d", guid, userPacketId);
                        return;
                    }
            ...
            default: {
                ...
                long guid = this.peer.getGuidOfPacket();
                UdpConnection connection = this.connectionMap.get(guid);
                AntiCheat.PacketRakNet.act(connection, packetIdentifier + ": unknown RakNet packet");
            }
        }
    }
        ...
        if (con != null) {
            ...
            if (GameClient.client) {
                ...
                if (cooldown > 0.0f) {
                    ...
                    LoggerManager.getLogger("user").write(String.format("Connection delayed disconnect index=%d guid=%d id=%s", con.getIndex(), con.getConnectedGUID(), con.getIDStr()));
                    ...
                    LoggerManager.getLogger("user").write(String.format("Connection disconnect index=%d guid=%d id=%s", con.getIndex(), con.getConnectedGUID(), con.getIDStr()));
                }
            }
        }
    }
    ...
    public void Connect(String hostname, int port, String serverPassword, boolean useSteamRelay) {
        if (port == 0 && SteamUtils.isSteamModeEnabled()) {
            ...
            try {
                ...
                DebugType.General.printException(e, LogSeverity.Error, "Exception thrown trying to convert String to SteamID..", new Object[0]);
                LuaEventManager.triggerEvent("OnConnectFailed", Translator.getText("UI_OnConnectFailed_UnknownHost", new Object[0]));
                return;
            }
            ...
            try {
                ...
                DebugType.Network.error(Translator.getText("UI_OnConnectFailed_UnknownHost", new Object[0]));
                LuaEventManager.triggerEvent("OnConnectFailed", Translator.getText("UI_OnConnectFailed_UnknownHost", new Object[0]));
                return;
            }
