...
implements IConnection {
    ...
    public String getDescription() {
        if (GameServer.server) {
            return String.format("guid=\"%s\" ip=\"%s\" steam-id=\"%s\" role=\"%s\" username=\"%s\" connection-type=\"%s\"", new Object[]{this.getConnectedGUID(), this.getIP(), this.getSteamId(), this.getRoleName(), this.getUserName(), this.getConnectionType()});
        }
        return String.format("guid=%s", this.getConnectedGUID());
    }
    ...
    public boolean hasPlayer(short playerId) {
        for (int i = 0; i < this.players.length; ++i) {
            if (this.players[i] == null || this.players[i].getOnlineID() != playerId) continue;
            return true;
        }
        return false;
    }
