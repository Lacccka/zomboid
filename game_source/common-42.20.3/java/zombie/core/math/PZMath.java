...
public final class PZMath {
    ...
    public static float getLength(float x, float y) {
        return PZMath.sqrt(PZMath.getLengthSq(x, y));
    }
    ...
    public static boolean isNullOrZero(Vector2 vec) {
        return vec == null || PZMath.isZero(vec);
    }
    ...
    public static double closestPointOnLineSegment(float x1, float y1, float x2, float y2, float px, float py, double endpointSnapEpsilon, Vector2f out) {
        ...
        double u = ((double)(px - x1) * dx + (double)(py - y1) * dy) / segmentLengthSquared;
        ...
        double xu = (double)x1 + u * dx;
        double yu = (double)y1 + u * dy;
        if (out != null) {
            out.set((float)xu, (float)yu);
        }
        double ddx = (double)px - xu;
        double ddy = (double)py - yu;
        return ddx * ddx + ddy * ddy;
    }
