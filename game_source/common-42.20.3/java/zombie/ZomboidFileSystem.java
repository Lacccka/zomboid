...
public final class ZomboidFileSystem {
    ...
    private final ResettableLazyValue<List<Path>> allowedPrefixes = new ResettableLazyValue<List>(() -> {
        ArrayList<String> knownBases = new ArrayList<String>();
        instance.getAllModFolders(knownBases);
        knownBases.add(instance.getCacheDir());
        knownBases.add(ZomboidFileSystem.instance.base.absoluteFile.getAbsolutePath());
        ...
        try (DirectoryStream<Path> dstrm = Files.newDirectoryStream(dir, x$0 -> Files.isDirectory(x$0, new LinkOption[0]));){
            for (Path path : dstrm) {
                if (path.getFileName().toString().equalsIgnoreCase("examplemod")) {
                    DebugType.Mod.println("refusing to list " + String.valueOf(path.getFileName()));
                    continue;
                }
                String commonDirName = "common";
                String versionDirName = this.getModVersionDirName(path);
                if (!Files.exists(path.resolve("common", "mod.info"), new LinkOption[0]) && !Files.exists(path.resolve(versionDirName, "mod.info"), new LinkOption[0])) continue;
    ...
    private static Path normalizeToPath(String path) {
        return Path.of(Path.of(path, new String[0]).normalize().toAbsolutePath().toString().toLowerCase(), new String[0]);
    }

    public void validatePrefix(String input) {
        Path inputPath = ZomboidFileSystem.normalizeToPath(input);
        List allowedPrefixes = (List)this.allowedPrefixes.get();
        int size = allowedPrefixes.size();
        for (int i = 0; i < size; ++i) {
            Path allowedPrefix = (Path)allowedPrefixes.get(i);
            if (!inputPath.startsWith(allowedPrefix)) continue;
            return;
        }
    ...
    public static String getFileName(String filePath) {
        ...
        return filePath.substring(lastSlashIndex + 1).trim();
    }

    public static String getFileNameNoExtension(String filePath) {
        String fileName = ZomboidFileSystem.getFileName(filePath);
        ...
        return fileName.substring(0, lastDotIdx).trim();
    }
