...
import zombie.characters.NetworkState;
...
public abstract class NetworkCharacterAI {
    ...
    private final XpChecker xpChecker = new XpChecker();
    private final PostponedPacket deadCharacterPacket = new PostponedPacket();
    private final PostponedPacket removeCorpsePacket = new PostponedPacket();
    private final PostponedPacket vehicleHitPacket = new PostponedPacket();
    ...
    public void onDied() {
        ...
        if (isoGameCharacter instanceof IsoPlayer && (player = (IsoPlayer)isoGameCharacter).isLocalPlayer() || this.deadCharacterPacket.isNextServerSquareIndex()) {
            this.deadCharacterPacket.process();
            this.removeCorpsePacket.process();
        } else {
            DebugType.Multiplayer.noise("Character [%d][%s] died, but staticMovingObjects order does not match with the server - [%d - %d]", this.getOnlineID(), this.getCharacter().getName(), this.deadCharacterPacket.serverIndex, this.deadCharacterPacket.square != null ? this.deadCharacterPacket.square.getStaticMovingObjects().size() - 1 : -1);
        }
    }
    ...
    public boolean isNoCollisionTimeout() {
        boolean isTimeout;
        boolean bl = isTimeout = GameTime.getServerTimeMills() > this.noCollisionTime;
        if (isTimeout) {
    ...
    public BaseVehicle getVehile() {
        if (this.character instanceof IsoAnimal) {
            return this.character.getVehicle();
        }
        return null;
    }
    ...
    public float setXp(PerkFactory.Perk perk, float value) {
        Float previous = this.xpChecker.xpMap.put(perk, Float.valueOf(value));
        return previous != null ? previous.floatValue() : 0.0f;
    }
    ...
    public float setXpMultiplier(PerkFactory.Perk perk, float value) {
        Float previous = this.xpChecker.xpMapMultiplier.put(perk, Float.valueOf(value));
        return previous != null ? previous.floatValue() : 1.0f;
    }
    ...
    public void updateXpChecker() {
        ...
        this.xpChecker.xpMapMultiplier.putAll(this.character.getXp().getMultiplierMap().entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, e -> Float.valueOf(((IsoGameCharacter.XPMultiplier)e.getValue()).multiplier))));
    }
    ...
    public static class XpChecker {
        ...
        private final Map<PerkFactory.Perk, Float> xpMap = new HashMap<PerkFactory.Perk, Float>();
        private final Map<PerkFactory.Perk, Integer> xpBoostMap = new HashMap<PerkFactory.Perk, Integer>();
        private final Map<PerkFactory.Perk, Float> xpMapMultiplier = new HashMap<PerkFactory.Perk, Float>();
    }
    ...
    public static class PostponedPacket {
        ...
        public void set(INetworkPacket packet, long timeout2, IsoGridSquare square, int serverIndex) {
            this.packet = packet;
            this.timestamp = timeout2;
