...
public class Role {
    ...
    public boolean hasCapability(Capability capability) {
        return Role.isUsingDebugMode() || this.capabilities.contains((Object)capability);
    }
    ...
    public boolean hasAdminTool() {
        return this.hasAdminPower() || this.hasCapability(Capability.CanSeePlayersStats) || this.hasCapability(Capability.AddItem) || this.hasCapability(Capability.SeePublicServerOptions) || this.hasCapability(Capability.CanSetupNonPVPZone) || this.hasCapability(Capability.FactionCheat) || this.hasCapability(Capability.RolesRead) || this.hasCapability(Capability.SeeNetworkUsers) || this.hasCapability(Capability.CanSetupSafehouses) || this.hasCapability(Capability.AnswerTickets) || this.hasCapability(Capability.SeePlayersConnected) || this.hasCapability(Capability.GetStatistic) || this.hasCapability(Capability.SandboxOptions) || this.hasCapability(Capability.GetStatistic) || this.hasCapability(Capability.PVPLogTool);
    }

    public boolean hasAdminPower() {
        return this.hasCapability(Capability.ToggleInvisibleHimself) || this.hasCapability(Capability.ToggleGodModHimself) || this.hasCapability(Capability.ToggleNoclipHimself) || this.hasCapability(Capability.UseFastMoveCheat) || this.hasCapability(Capability.UseTimedActionInstantCheat) || this.hasCapability(Capability.ToggleUnlimitedCarry) || this.hasCapability(Capability.ToggleUnlimitedEndurance) || this.hasCapability(Capability.ToggleUnlimitedAmmo) || this.hasCapability(Capability.ToggleKnowAllRecipes) || this.hasCapability(Capability.UseBuildCheat) || this.hasCapability(Capability.UseFarmingCheat) || this.hasCapability(Capability.UseFishingCheat) || this.hasCapability(Capability.UseHealthCheat) || this.hasCapability(Capability.UseMechanicsCheat) || this.hasCapability(Capability.UseMovablesCheat) || this.hasCapability(Capability.CanSeeAll) || this.hasCapability(Capability.CanHearAll) || this.hasCapability(Capability.ManipulateZombie) || this.hasCapability(Capability.UseBrushToolManager) || this.hasCapability(Capability.UseLootZed) || this.hasCapability(Capability.UseLootLog) || this.hasCapability(Capability.AnimalCheats) || this.hasCapability(Capability.ClimateManager);
    }
}
