...
implements IHumanVisual {
    ...
    private void registerVariableCallbacks() {
        ...
        this.setVariable("bMovingNetwork", () -> {
            ...
            return !(!this.isLocal() && this.isBumped() || !(IsoUtils.DistanceManhatten(networkAi.targetX, networkAi.targetY, this.getX(), this.getY()) > 0.5f) && this.getZ() == (float)networkAi.targetZ);
        }, (IAnimationVariableSource owner) -> "Is this network Zed moving. <br />returns FALSE if bumped and non-local..");
        ...
        this.setVariable("bPathFindPrediction", () -> 2 == this.getNetworkCharacterAI().predictionType, (IAnimationVariableSource owner) -> "Network. The type of pathfinding prediction. One of: <br />  Moving, <br />  Static, <br />  Thump, <br />  Climb, <br />  Lunge, <br />  LungeHalf, <br />  Walk, <br />  WalkHalf, <br />  PathFind.");
    ...
    public void spottedNew(IsoMovingObject other, boolean bForced) {
        ...
        thisToOther.x -= this.getX();
        thisToOther.y -= this.getY();
        float carObstacleMod = intersectVision && otherCharacter.getVehicle() == null ? (thisToOther.getLength() < 1.5f ? 0.5f : 0.0f) : 1.0f;
    ...
    public void moveUnmodded(float dirX, float dirY) {
        ...
        if (GameClient.client && this.isRemoteZombie()) {
            ...
            float movingScale = Math.abs(dist) < 0.01f ? 1.0f : 0.5f + IsoUtils.smoothstep(0.5f, 1.5f, IsoUtils.DistanceTo(this.getX(), this.getY(), networkAi.targetX, networkAi.targetY) / dist);
            dirX *= movingScale;
            dirY *= movingScale;
    ...
    public boolean allowsInvisibleAnimationSkips() {
        ...
        if (GameTime.getInstance().perObjectMultiplier > 1.0f) {
            return false;
        }
        if (!DebugOptions.instance.zombieAnimationDelay.getValue()) {
            return false;
        }
        return super.allowsInvisibleAnimationSkips();
    }
    ...
    private void updateInternal() {
        ...
        if (GameClient.client && this.isRemoteZombie() && this.lastRemoteUpdate < 2000 && this.lastRemoteUpdate + 1000 / PerformanceSettings.getLockFPS() > 2000) {
    ...
    public boolean tryThump(IsoGridSquare square) {
        ...
        if (door != null && door.isHoppable() && !door.isTallHoppable()) {
    ...
    public static int getSpeedTypeFromWalkType(String walkType) {
        ...
        if (walkType.startsWith("sprint")) {
            return 1;
        }
        if (walkType.startsWith("slow")) {
            return 3;
        }
        return 2;
    }
    ...
    public void setSpeedTypeFromWalkType() {
        this.speedType = IsoZombie.getSpeedTypeFromWalkType(this.getWalkType());
    }
    ...
    public void setFemaleEtc(boolean female) {
        ...
        if (this.getDescriptor() != null) {
            ...
            this.getDescriptor().setVoicePrefix(String.valueOf((Object)this.getCharacterGender()) + "Zombie");
        }
    ...
    public boolean shouldGetUpFromCrawl() {
        ...
        if (this.isCurrentState(PathFindState.instance())) {
            if (this.getStateMachine().getPrevious() == ZombieGetDownState.instance() && ZombieGetDownState.instance().isNearStartXY(this)) {
                return false;
            }
    ...
    private void updateVisionRadius() {
        ...
        float darknessPenalty = character != null && character.getCurrentSquare() != null ? (1.0f - this.target.getCurrentSquare().getLightLevel(character.getIndex())) * 5.0f : (1.0f - Math.min(cm.getDayLightStrength(), cm.getAmbient())) * 5.0f;
    ...
    public boolean helmetFallFromVisuals(boolean hitHead) {
        ...
        for (int i = 0; i < tempItemVisuals.size(); ++i) {
            ...
            this.onWornItemsChanged();
            removed = true;
            this.createFallingItem((InventoryItem)item);
