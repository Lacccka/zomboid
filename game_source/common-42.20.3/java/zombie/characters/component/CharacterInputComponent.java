...
IAnimationVariableLogger {
    ...
    private final TimedInputHandler toggleAim = new TimedInputHandler(this::isToggleAimKeyDown, this::isToggleAimKeyMouse, () -> !Options.isToggleToAim());
    private final TimedInputHandler toggleSprint = new TimedInputHandler(this::isToggleSprintButtonDown, null, () -> !Options.isToggleToSprint());
    private final TimedInputHandler toggleRun = new TimedInputHandler(this::isToggleRunButtonDown, null, () -> !Options.isToggleToRun());
    ...
    public void updateToggleToSprint() {
        ...
        if (!this.shouldAllowForceRunOrSprint()) {
            this.setForceSprint(false);
            return;
        }
    ...
    public boolean isForwardKeyDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> {
                if (!this.isKeyboardSelectingAll() && CharacterInputKeyBinding.Forward.isKeyDownInWorld()) {
                    yield true;
                }
                yield false;
            }
            case CharacterInputMode.GAMEPAD -> {
                if (CharacterJoypadAxis2dBinding.Movement.getValueY(this.getJoypadBind()) > 0.0f) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isBackwardKeyDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> {
                if (!this.isKeyboardSelectingAll() && CharacterInputKeyBinding.Backward.isKeyDownInWorld()) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isLeftKeyDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> {
                if (!this.isKeyboardSelectingAll() && CharacterInputKeyBinding.Left.isKeyDownInWorld()) {
                    yield true;
                }
                yield false;
            }
            case CharacterInputMode.GAMEPAD -> {
                if (CharacterJoypadAxis2dBinding.Movement.getValueX(this.getJoypadBind()) < 0.0f) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isRightKeyDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> {
                if (!this.isKeyboardSelectingAll() && CharacterInputKeyBinding.Right.isKeyDownInWorld()) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isKeyboardSelectingAll() {
        return Core.getInstance().isSelectingAll() && UIManager.isMouseOverInventory() && this.isCtrlKeyDown() && (CharacterInputKeyBinding.Left.isKeyDown(30) || CharacterInputKeyBinding.Right.isKeyDown(30) || CharacterInputKeyBinding.Forward.isKeyDown(30) || CharacterInputKeyBinding.Backward.isKeyDown(30));
    }
    ...
    public boolean isToggleAimKeyDown() {
        ...
        block6: {
            block5: {
                if (!Options.isToggleToAim()) break block5;
                switch (this.getInputMode()) {
                    default: {
    ...
    public boolean isToggleSprintButtonDown() {
        return Options.isToggleToSprint() && this.isSprintButtonDown();
    }

    public boolean isToggleRunButtonDown() {
        return Options.isToggleToRun() && this.isRunButtonDown();
    }
    ...
    public boolean isAimKeyDownInternal() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.Aim.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadAimingAxisApplied() && CharacterJoypadButtonBinding.Aim.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isPrecisionAimKeyDownInternal() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> this.isAimKeyDownInternal();
            case CharacterInputMode.GAMEPAD -> {
                if (!this.isJoypadIgnoreAim() && !this.isJoypadIgnoreAimUntilCentered() && CharacterJoypadButtonBinding.PrecisionAim.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isMeleeButtonDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> this.melee.isKeyDown();
            case CharacterInputMode.GAMEPAD -> {
                if (this.melee.isKeyDown() && !this.melee.isKeyPressed()) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isMeleeButtonDownInternal() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.Melee.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> CharacterJoypadButtonBinding.Melee.isDown(this.getJoypadBind());
            default -> false;
        };
    }
    ...
    public boolean isAttackButtonDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.Attack.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> CharacterJoypadButtonBinding.Attack.isDown(this.getJoypadBind());
            default -> false;
        };
    }
    ...
    public boolean isBuildButtonDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.Attack.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> CharacterJoypadButtonBinding.Interact.isDown(this.getJoypadBind());
            default -> false;
        };
    }
    ...
    public boolean isRunButtonDown() {
        ...
        block7: {
            block6: {
                if (!this.isAllowRun()) break block6;
                switch (this.getInputMode()) {
                    case KEYBOARD: {
    ...
    public boolean isInteractButtonDownInternal() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.Interact.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> CharacterJoypadButtonBinding.Interact.isDown(this.getJoypadBind());
            default -> false;
        };
    }
    ...
    public boolean isCrouchButtonDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.Crouch.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadButtonsActive() && CharacterJoypadButtonBinding.Crouch.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isCrouchButtonPressed() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> this.toggleCrouch.isKeyPressed();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadButtonsActive() && this.toggleCrouch.isKeyClicked()) {
                    yield true;
                }
                yield false;
            }
    ...
    private boolean isReloadWeaponButtonDownInternal() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.ReloadWeapon.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadButtonsActive() && CharacterJoypadButtonBinding.ReloadWeapon.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
    ...
    private boolean isRackFirearmButtonDownInternal() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.RackFirearm.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadButtonsActive() && CharacterJoypadButtonBinding.RackFirearm.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isSprintButtonDown() {
        ...
        block7: {
            block6: {
                if (!this.isAllowSprint()) break block6;
                switch (this.getInputMode()) {
                    case KEYBOARD: {
    ...
    public boolean isCancelActionButtonDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.CancelAction.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadButtonsActive() && CharacterJoypadButtonBinding.CancelAction.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
    ...
    public boolean isManualFloorAtkButtonDown() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.ManualFloorAtk.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadButtonsActive() && CharacterJoypadButtonBinding.ManualFloorAtk.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
    ...
    private boolean isZoomInButtonDownInternal() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.ZoomIn.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadButtonsActive() && CharacterJoypadButtonBinding.ZoomIn.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
    ...
    private boolean isZoomOutButtonDownInternal() {
        return switch (this.getInputMode()) {
            case CharacterInputMode.KEYBOARD -> CharacterInputKeyBinding.ZoomOut.isKeyDownInWorld();
            case CharacterInputMode.GAMEPAD -> {
                if (this.isJoypadButtonsActive() && CharacterJoypadButtonBinding.ZoomOut.isDown(this.getJoypadBind())) {
                    yield true;
                }
                yield false;
            }
