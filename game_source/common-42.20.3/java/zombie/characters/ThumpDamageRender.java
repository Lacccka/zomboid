...
public final class ThumpDamageRender {
    ...
    private final Map<IsoMovingObject, Thumper> damageMap = new HashMap<IsoMovingObject, Thumper>();
    private final List<IsoMovingObject> removeList = new ArrayList<IsoMovingObject>();

    public void thump(IsoMovingObject chr, float damage, float health) {
        Thumper damage1 = this.damageMap.get(chr);
        if (damage1 == null) {
            damage1 = new Thumper(chr);
        ...
        Damage damage2 = new Damage(damage, health);
    ...
    public void render() {
        ...
        for (Map.Entry<IsoMovingObject, Thumper> entry : this.damageMap.entrySet()) {
            IsoMovingObject chr = entry.getKey();
            if (!IsoWorld.instance.currentCell.getObjectList().contains(chr)) {
                this.removeList.add(chr);
                continue;
            }
            Thumper thumper = entry.getValue();
            thumper.render(dt);
            if (!thumper.damageQueue.isEmpty()) continue;
            this.removeList.add(chr);
        }
    ...
    private static final class Thumper {
        IsoMovingObject chr;
        final List<Damage> damageQueue = new ArrayList<Damage>();
        ...
        void render(float dt) {
            ...
            sy -= 128.0f / (2.0f / (float)Core.tileScale) / zoom + (float)fontHgt;
            this.removeOldDamage();
            for (int i = 0; i < this.damageQueue.size(); ++i) {
                Damage damage = this.damageQueue.get(i);
                if (!GameTime.isGamePaused()) {
                    damage.time += dt;
                }
                float sy1 = sy - (float)(i * fontHgt);
                TextManager.instance.DrawStringCentre(UIFont.Medium, (double)sx, (double)sy1, String.format("%.1f / %.1f", Float.valueOf(damage.damage), Float.valueOf(damage.health)), 1.0, 0.0, 0.0, 1.0);
            }
        }
