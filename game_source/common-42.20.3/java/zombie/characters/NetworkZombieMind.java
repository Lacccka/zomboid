...
public class NetworkZombieMind {
    ...
    public void set(ZombiePacket packet) {
        ...
        if (pfb.getIsCancelled() || pfb.isGoalNone() || pfb.stopping || this.zombie.realState == null || NetworkVariables.ZombieState.Idle == this.zombie.realState) {
    ...
    public void doRestorePFBTarget() {
        if (!this.pfb.isCanceled()) {
            if (this.pfb.goal == PathFindBehavior2.Goal.Character && this.pfb.target.getPlayer() != null) {
                ...
                this.zombie.pathToSound(PZMath.fastfloor(this.pfb.position.getX()), PZMath.fastfloor(this.pfb.position.getY()), PZMath.fastfloor(this.pfb.position.getZ()));
                this.zombie.alerted = false;
                this.zombie.setLastHeardSound(PZMath.fastfloor(this.pfb.position.getX()), PZMath.fastfloor(this.pfb.position.getY()), PZMath.fastfloor(this.pfb.position.getZ()));
