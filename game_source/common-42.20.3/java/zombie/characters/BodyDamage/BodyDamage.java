...
public final class BodyDamage {
            ...
            if (!this.isInfected()) {
                if (Rand.Next(100) < illnessChance) {
                    ...
                    if (isoGameCharacter instanceof IsoPlayer) {
                        ...
                        DebugType.Objects.debugln(debugStr2);
    ...
    public void TriggerSneezeCough() {
        ...
        if (tissueConsumed) {
            ...
            WorldSoundManager.WorldSound sneeze = WorldSoundManager.instance.addSound(this.parentChar, PZMath.fastfloor(this.parentChar.getX()), PZMath.fastfloor(this.parentChar.getY()), PZMath.fastfloor(this.parentChar.getZ()), dist, vol2, false);
            sneeze.stressAnimals = false;
        }
        if (GameServer.server && (vol2 = this.parentChar) instanceof IsoPlayer) {
            IsoPlayer player = (IsoPlayer)vol2;
            int sneezeVar = 0;
    ...
    public boolean AddRandomDamageFromZombie(IsoZombie zombie, String hitReaction, int partIndex) {
        ...
        if (GameServer.server && this.parentChar instanceof IsoPlayer || Core.debug && this.parentChar instanceof IsoPlayer) {
            DebugType.DetailedInfo.trace("zombie did " + damage + " dmg to " + ((IsoPlayer)this.parentChar).getDisplayName() + " on body part " + BodyPartType.getDisplayName(BodyPartType.FromIndex(partIndex)));
        }
        ...
        if (Rand.Next(100) > baseChance) {
            ...
            if (zombie.scratch) {
                ...
                painType = 1;
                if (isoPlayer != null) {
                    DebugType.DetailedInfo.trace("zombie scratched %s in body location %s", new Object[]{isoPlayer.getUsername(), BloodBodyPartType.FromIndex(partIndex)});
                ...
                painType = 1;
                if (isoPlayer != null) {
                    DebugType.DetailedInfo.trace("zombie laceration %s in body location %s", new Object[]{isoPlayer.getUsername(), BloodBodyPartType.FromIndex(partIndex)});
                ...
                painType = 2;
                if (isoPlayer != null) {
                    DebugType.DetailedInfo.trace("zombie bite %s in body location %s", new Object[]{isoPlayer.getUsername(), BloodBodyPartType.FromIndex(partIndex)});
