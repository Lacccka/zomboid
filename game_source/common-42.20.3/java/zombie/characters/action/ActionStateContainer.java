...
public final class ActionStateContainer {
    ...
    private boolean tryInsertChildState(ActionContext actionContext, ActionState nextState) {
        ...
        if (!this.currentStateSupportsChildState(nextState)) {
            DebugType.ActionSystem.trace("%s> Transition's target state \"%s\" not supported by parent: \"%s\"", actionContext.getOwner().getUID(), nextState.getName(), this.currentState.getName());
            return false;
        }
        int insertAt = -1;
        ...
        for (int i = 0; i < this.childStates.size(); ++i) {
            ...
            if (upperState.canHaveSubState(nextState)) continue;
            DebugType.ActionSystem.trace("%s> Transition's target state \"%s\" not supported by parent: \"%s\"", actionContext.getOwner().getUID(), nextState.getName(), upperState.getName());
            return false;
        }
        if (insertAt > -1) {
