...
import zombie.characters.CharacterDiedListener;
import zombie.characters.CharacterGender;
import zombie.characters.CharacterInputComponentEntity;
...
CharacterInputComponentEntity {
    ...
    private final Map<CharacterDiedListener, Boolean> onDiedListeners = new LinkedHashMap<CharacterDiedListener, Boolean>();
    ...
    private void registerVariableCallbacks() {
        ...
        this.setVariable("aimingMode", AimingMode.class, this::getAimingMode, (IAnimationVariableSource owner) -> "Get the character's aiming mode. UNARMED/MELEE/RANGED means it's free-aiming, _TARGET_FOUND means it's spotted a target.");
        this.setVariable("hasTarget", () -> this.hasTarget, (IAnimationVariableSource owner) -> "Does this character have at least one target.");
        this.setVariable("isUnarmed", this::isUnarmed, (IAnimationVariableSource owner) -> "Is this character unarmed. Would have to fight with bare hands. No weapon equipped, neither melee or ranged.");
        this.setVariable("isMeleeWeaponEquipped", this::isMeleeWeaponEquipped, (IAnimationVariableSource owner) -> "Does this character have a melee weapon equipped.");
        this.setVariable("isRangedWeaponEquipped", this::isRangedWeaponEquipped, (IAnimationVariableSource owner) -> "Does this character have a ranged weapon equipped.");
    ...
    public String getAnimationDebug() {
        if (this.getAdvancedAnimator() != null) {
            return this.instancename + "\n" + this.getAdvancedAnimator().GetDebug();
        }
        return this.instancename + "\n - No Animator";
    }
    ...
    private void restoreAnimatorStateToActionContext() {
        if (this.getActionContext().getCurrentState() != null) {
            this.getAdvancedAnimator().setState(this.getActionContext().getCurrentStateName(), PZArrayUtil.listConvert(this.getActionContext().getChildStates(), state -> state.getName()));
        }
    }
    ...
    protected boolean TestIfSeen(int playerIndex, IsoPlayer player) {
        ...
        if (!canSee && couldSee) {
            boolean bl2 = canSee = dist < detectionRange;
        }
        if (!canSee) {
            return false;
        }
        ...
        if (dist < detectionRange && player.getCurrentSquare() != null && (lightInfo = player.getCurrentSquare().lighting[playerIndex].lightInfo()) != null) {
    ...
    public static boolean canDropCorpseInto(IsoGameCharacter chr, ItemContainer container) {
        ...
        return container.canHumanCorpseFit(chr);
    }
    ...
    public void setPrimaryHandItem(InventoryItem leftHandItem) {
        ...
        if ((isoGameCharacter = this) instanceof IsoPlayer) {
            IsoPlayer player = (IsoPlayer)isoGameCharacter;
    ...
    public void onWornItemsChanged() {
        ...
        if (this.wornClothingCanRagdoll != clothingCanRagdoll) {
            ...
            DebugType.General.debugln("%s worn items changed. %s.", this.getName(), clothingCanRagdoll ? "Character's clothes can now ragdoll." : "Character's clothing prevent ragdolling.");
        }
        if (GameServer.server) {
    ...
    public void Callout() {
        ...
        if (Core.getInstance().getGameMode().equals("Tutorial")) {
            ...
            if (shoutItem != null) {
                ...
                switch (Rand.Next(3)) {
                    case 0: {
                        String string = Translator.getText("IGUI_PlayerText_Callout1New", new Object[0]);
                        break;
                    }
                    case 1: {
                        String string = Translator.getText("IGUI_PlayerText_Callout2New", new Object[0]);
                        break;
                    }
                    case 2: {
                        String string = Translator.getText("IGUI_PlayerText_Callout3New", new Object[0]);
                        break;
                    }
                    default: {
    ...
    public boolean hasTrait(CharacterTrait ... characterTrait) {
        for (int i = 0; i < characterTrait.length; ++i) {
            if (!this.hasTrait(characterTrait[i])) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean isCurrentState(State state) {
        if (this.getStateMachine().isSubstate(state)) {
            return true;
        }
        return this.getStateMachine().getCurrent() == state;
    }
    ...
    public void DrawSneezeText() {
        if (this.getBodyDamage().IsSneezingCoughing() > 0) {
            ...
            sy -= 48.0f;
            if (sneezeText != null) {
                TextManager.instance.DrawStringCentre(UIFont.Dialogue, (double)((int)sx), (double)((int)sy), sneezeText, (double)this.speakColour.r, (double)this.speakColour.g, (double)this.speakColour.b, (double)this.speakColour.a);
            }
        }
    }
    ...
    public boolean isMaskClicked(int x, int y, boolean flip) {
        ...
        return this.legsSprite.isMaskClicked(this.getForwardIsoDirection(), x, y, flip);
    }
    ...
    protected void updateUserName() {
        block19: {
            if (this.userName == null || this.isoPlayer == null) break block19;
            ...
            if (showMyUsername && this.isoPlayer.role != null && this.isoPlayer.role.hasCapability(Capability.ToggleWriteRoleNameAbove) && this.isoPlayer.isShowAdminTag()) {
            ...
            if (this.isoPlayer.getVehicle() != null) {
                nameStr = this.isoPlayer.getVehicle().getNamePrefixForPlayer(this.isoPlayer) + (String)nameStr;
            }
    ...
    public static void getNameCoords(float x, float y, float z, float offX, float offY, float zoom, Vector2 coord) {
        ...
        sx /= zoom;
        sy /= zoom;
        coord.x = sx += IsoCamera.cameras[IsoCamera.frameState.playerIndex].fixJigglyModelsX;
        coord.y = sy += IsoCamera.cameras[IsoCamera.frameState.playerIndex].fixJigglyModelsY;
    }
    ...
    public void renderlast() {
        ...
        if (this.hasInitTextObjects && this.isoPlayer != null || this.chatElement.getHasChatToDisplay()) {
            ...
            if (vehicle == null || !vehicle.getNameCoordForPlayer(this, zoom, tempVector2_2)) {
                ...
                IsoGameCharacter.getNameCoords(renderX, renderY, renderZ, this.offsetX, this.offsetY + nameOffY, zoom, tempVector2_2);
            }
            float sx = IsoGameCharacter.tempVector2_2.x;
            float sy = IsoGameCharacter.tempVector2_2.y;
            ...
            if (this.isoPlayer != null && (this == IsoCamera.frameState.camCharacter || this.getCurrentSquare() != null && this.getCurrentSquare().getCanSee(playerIndex)) || IsoPlayer.getInstance().canSeeAll()) {
                ...
                if (GameClient.client && this.userName != null && !(this instanceof IsoAnimal)) {
                    ...
                    if (ServerOptions.getInstance().mouseOverToSeeDisplayName.getValue() && this != IsoPlayer.getInstance() && !IsoPlayer.getInstance().canSeeAll()) {
                        ...
                        if (object != null && object.tile != null) {
                            block0: for (int x = object.tile.square.getX() - 1; x < object.tile.square.getX() + 2; ++x) {
                ...
                if (!GameClient.client && this.isoPlayer != null && !this.isAnimal() && this.isoPlayer.getVehicle() == null) {
                    ...
                    if (vehicle == null && nearVehicle != null && nearVehicle.getPartById("Engine") != null && (this.isoPlayer.getInventory().haveThisKeyId(nearVehicle.getKeyId()) != null || nearVehicle.isHotwired() || SandboxOptions.getInstance().vehicleEasyUse.getValue()) && UIManager.visibleAllUi) {
                        Color newC = Color.HSBtoRGB(nearVehicle.colorHue, nearVehicle.colorSaturation * 0.5f, nearVehicle.colorValue, L_renderLast.color);
                        nameStr = " [img=media/ui/CarKey.png," + newC.getRedByte() + "," + newC.getGreenByte() + "," + newC.getBlueByte() + "]";
                    }
                    if (!((String)nameStr).isEmpty()) {
                        this.userName.ReadString((String)nameStr);
                        this.drawUserName = true;
                    }
                }
            ...
            IsoGameCharacter isoGameCharacter = IsoCamera.getCameraCharacter();
            boolean bl = bViewerIsAdmin = GameClient.client && isoGameCharacter instanceof IsoPlayer && (player = (IsoPlayer)isoGameCharacter).getRole() != null && player.getRole().hasCapability(Capability.CanSeePlayersStats);
    ...
    private void debugRenderLast() {
        ...
        if (DebugOptions.instance.character.debug.render.angle.getValue() && this.hasActiveModel()) {
            dir = tempo;
            ((Vector2)dir).set(this.getForwardIsoDirection().ToVector());
    ...
    public void drawDebugTextBelow(String text) {
        ...
        TextManager.instance.DrawStringCentre(UIFont.Small, (double)sx, (double)sy, text, 1.0, 1.0, 1.0, 1.0);
        ...
        if (this.ballisticsController != null && weapon.isRanged()) {
            ...
            for (int i = 0; i < this.hitInfoList.size(); ++i) {
                ...
                if (obj != null) {
                    ...
                    TextManager.instance.DrawStringCentre(font, (double)sx4, (double)sy4, String.valueOf(hitInfo2.dot), 1.0, 1.0, 1.0, 1.0);
                    TextManager.instance.DrawStringCentre(font, (double)sx4, (double)(sy4 + (float)TextManager.instance.getFontHeight(font)), hitInfo2.chance + "%", 1.0, 1.0, 1.0, 1.0);
                    r = 1.0f;
                    g = 1.0f;
                    float b = 1.0f;
                    ...
                    TextManager.instance.DrawStringCentre(font, (double)sx4, (double)(sy4 + (float)(TextManager.instance.getFontHeight(font) * 2)), "DIST: " + dist, (double)r, (double)g, (double)b, 1.0);
                }
    ...
    private void debugTestDotSide() {
        ...
        for (int i = 0; i < zombies.size(); ++i) {
            ...
            TextManager.instance.DrawStringCentre(font, (double)sx4, (double)(sy4 + (float)fontHgt), "SIDE: " + this.testDotSide(obj), 1.0, 1.0, 1.0, 1.0);
            ...
            TextManager.instance.DrawStringCentre(font, (double)sx4, (double)(sy4 + (float)(fontHgt * 2)), "ANGLE (0-360): " + PZMath.radToDeg(radians), 1.0, 1.0, 1.0, 1.0);
            radians = (float)Math.acos(this.getDotWithForwardDirection(obj.getX(), obj.getY()));
            TextManager.instance.DrawStringCentre(font, (double)sx4, (double)(sy4 + (float)(fontHgt * 3)), "ANGLE (0-180): " + PZMath.radToDeg(radians), 1.0, 1.0, 1.0, 1.0);
        }
    }
    ...
    public void Throw(HandWeapon weapon) {
        ...
        if ((physicsObject = weapon.getPhysicsObject()) != null) {
            if (physicsObject.equals(ItemKey.Normal.BALL.toString())) {
                IsoBall isoBall = new IsoBall(this.getCell(), this.getX(), this.getY(), this.getZ() + 0.6f, x * 0.4f, y * 0.4f, weapon, this);
            } else {
                IsoMolotovCocktail isoMolotovCocktail = new IsoMolotovCocktail(this.getCell(), this.getX(), this.getY(), this.getZ() + 0.6f, x * 0.4f, y * 0.4f, weapon, this);
            }
        }
        if (this instanceof IsoPlayer) {
    ...
    public boolean helmetFall(boolean hitHead) {
        ...
        boolean removed = zombie == null || zombie.isUsingWornItems() ? this.helmetFallFromWornItems(hitHead) : zombie.helmetFallFromVisuals(hitHead);
    ...
    protected IsoDirections getCardinalDirectionTo(IsoGridSquare square, boolean north) {
        if (north) {
            return this.getY() >= (float)square.getY() ? IsoDirections.N : IsoDirections.S;
        }
        return this.getX() >= (float)square.getX() ? IsoDirections.W : IsoDirections.E;
    }
    ...
    private void updateInternal() {
        ...
        if (!this.characterActions.isEmpty()) {
            ...
            IsoPlayer player;
            BaseAction act = (BaseAction)this.characterActions.get(0);
            IsoGameCharacter panicInc = this;
            if (panicInc instanceof IsoPlayer && act == (player = (IsoPlayer)panicInc).getTimedActionToRetrigger()) {
                player.setTimedActionToRetrigger(null);
            }
            if ((valid = act.valid()) && !act.started) {
                act.waitToStart();
        ...
        if (this.attachedAnimSprite != null) {
            ...
            for (int i2 = 0; i2 < n; ++i2) {
                IsoSpriteInstance s = (IsoSpriteInstance)this.attachedAnimSprite.get(i2);
        ...
        if ((i2 = this) instanceof IsoPlayer) {
            IsoPlayer player = (IsoPlayer)i2;
            if (!(this instanceof IsoAnimal)) {
    ...
    protected void calculateWalkSpeed() {
        ...
        float runSpeed = walkSpeed = this.calculateBaseSpeed();
        runSpeed -= 0.15f;
        runSpeed *= this.fullSpeedMod;
        runSpeed += (float)this.getPerkLevel(PerkFactory.Perks.Sprinting) / 20.0f;
    ...
    public final CharacterGender getCharacterGender() {
        ...
        return this.descriptor.getCharacterGender();
    }
    ...
    public void saveChange(IsoObjectChange change, KahluaTable tbl, ByteBufferWriter bb) {
        ...
        if (change == IsoObjectChange.ADD_ITEM) {
            DebugType.General.warn("The addItem change type in the IsoGameCharacter.saveChange function  was disabled. The server should create item and sent it using the sendAddItemToContainer function.");
        } else if (change == IsoObjectChange.ADD_ITEM_OF_TYPE) {
            DebugType.General.warn("The addItemOfType change type in the IsoGameCharacter.saveChange function  was disabled. The server should create item and sent it using the sendAddItemToContainer function.");
            ...
            if (change == IsoObjectChange.REMOVE_ITEM) {
                DebugType.General.warn("The removeItem change type in the IsoGameCharacter.saveChange function  was disabled. The server must use the sendRemoveItemFromContainer function.");
            } else if (change == IsoObjectChange.REMOVE_ITEM_ID) {
                DebugType.General.warn("The removeItemID change type in the IsoGameCharacter.saveChange function  was disabled. The server must use the sendRemoveItemFromContainer function.");
            } else if (change == IsoObjectChange.REMOVE_ITEM_TYPE) {
                DebugType.General.warn("The removeItemType change type in the IsoGameCharacter.saveChange function  was disabled. The server must use the sendRemoveItemFromContainer function.");
            } else if (change == IsoObjectChange.REMOVE_ONE_OF) {
                DebugType.General.warn("The removeOneOf change type in the IsoGameCharacter.saveChange function  was disabled. The server must use the sendRemoveItemFromContainer function.");
                ...
                } else if (change == IsoObjectChange.SET_HALO_NOTE && tbl != null && (object = tbl.rawget("note")) instanceof String) {
                    ...
                    if (object instanceof Double) {
                        ...
                        if (object instanceof Double) {
                            ...
                            if (object instanceof Double) {
                                ...
                                object = tbl.rawget("dispTime");
                                if (object instanceof Double) {
                                    Double dispTime = (Double)object;
                                    bb.putUTF(note);
    ...
    private void postUpdateAnimating() {
        ...
        if (this.isAnimationRecorderActive()) {
            ...
            for (int i = 0; i < animPlayer.getNumTwistBones(); ++i) {
                ...
                animationRecorder.logVariable("twistBone_" + i + "_Twist", 57.295776f * twistableBone.twist);
        ...
        for (int y = chunkMinY; y < chunkMaxY; ++y) {
            for (int x = chunkMinX; x < chunkMaxX; ++x) {
                ...
                for (int i = 0; i < chunk.vehicles.size(); ++i) {
                    ...
                    if (z < -0.5f || z > 0.3f || (v = vehicle.testCollisionWithCharacter(this, radius, vector2)) == null || v.x == -1.0f) continue;
    ...
    private boolean isFacingNorthWesterly() {
        return this.getForwardIsoDirection() == IsoDirections.W || this.getForwardIsoDirection() == IsoDirections.NW || this.getForwardIsoDirection() == IsoDirections.N || this.getForwardIsoDirection() == IsoDirections.NE;
    }
    ...
    public float Hit(BaseVehicle vehicle, float speed, boolean isHitFromBehind, float hitDirX, float hitDirY, boolean pushedBack, float collisionPosOnVehicleX, float collisionPosOnVehicleY) {
        ...
        try {
            float f = this.onHitByVehicle(vehicle, speed, this.getHitDir().set(hitDirX, hitDirY), impactPosOnVehicle);
            return f;
        }
        finally {
    ...
    private void setGodModCheat(boolean enabled) {
        ...
        if (GameServer.server && wasEnabled && !enabled && (isoGameCharacter = this) instanceof IsoPlayer) {
            IsoPlayer player = (IsoPlayer)isoGameCharacter;
    ...
    public ActionStateSnapshot playbackRecordCurrentStateSnapshot() {
        ...
        return this.getActionContext().getPlaybackStateSnapshot();
    }
    ...
    private float calculateDamageFromVehicleImpact(float impactSpeed) {
        ...
        return 10.0f * PZMath.lerpFunc_EaseOutQuad(impactDamageAlpha);
    }
    ...
    public boolean causesDamageToVehicleWhenHit(BaseVehicle impactingVehicle) {
        ...
        return CombatManager.checkPVP(this, impactingVehicle, false) && this.hasCurrentState() && this.getCurrentState().causesDamageToVehicleWhenHit(this, impactingVehicle);
    }
    ...
    public void startEvent(long eventInstance, GameSoundClip clip, boolean remote, BitSet parameterSet) {
        ...
        ArrayList<FMOD_STUDIO_PARAMETER_DESCRIPTION> eventParameters = clip.getEventDescription((boolean)remote).parameters;
    ...
    private void invokeOnDiedListeners(IsoDeadBody body) {
        LinkedHashMap<CharacterDiedListener, Boolean> onDiedListeners = new LinkedHashMap<CharacterDiedListener, Boolean>(this.onDiedListeners);
        for (Map.Entry listenerEntry : onDiedListeners.entrySet()) {
            CharacterDiedListener listener = (CharacterDiedListener)listenerEntry.getKey();
            boolean autoRemove = (Boolean)listenerEntry.getValue();
    ...
    public InventoryItem becomeCorpseItem(ItemContainer placeInContainer, IsoGameCharacter chr) {
        ...
        if (!placeInContainer.canItemFit(inventoryItem, chr)) {
    ...
    public Float getNextAnimationTranslationLength() {
        ...
        for (AnimNode node : nodes) {
            AnimationClip clip;
            if (StringUtils.isNullOrEmpty(node.animName) || !node.useDeferredMovement || (clip = skinningData.animationClips.get(node.animName)) == null) continue;
    ...
    public float getCorpseSicknessDefense(float rate, boolean drain) {
        ...
        for (int i = 0; i < this.getWornItems().size(); ++i) {
            ...
            if (item instanceof Clothing) {
                ...
                if (clothing.hasTag(ItemTag.GAS_MASK, ItemTag.RESPIRATOR, ItemTag.IMPROVISED_GAS_MASK) && clothing.hasFilter() && clothing.getUsedDelta() > 0.0f) {
                    defense = Math.max(defense, clothing.hasTag(ItemTag.IMPROVISED_GAS_MASK) ? ZomboidGlobals.improvisedGasMaskDefense : ZomboidGlobals.standardGasMaskDefense);
                    if (drain && rate > 0.0f) {
                        clothing.drainGasMask((float)((double)rate * ZomboidGlobals.corpseSicknessFilterDrainMultiplier));
                    }
                }
            }
            if (!(defense >= ZomboidGlobals.standardGasMaskDefense)) continue;
            return ZomboidGlobals.standardGasMaskDefense;
        }
        return defense;
    }
    ...
    public boolean isProtectedFromToxic(boolean drain) {
        for (int i = 0; i < this.getWornItems().size(); ++i) {
            ...
            if (!clothing.hasTag(ItemTag.GAS_MASK, ItemTag.RESPIRATOR, ItemTag.IMPROVISED_GAS_MASK) || !clothing.hasFilter() || !(clothing.getUsedDelta() > 0.0f)) continue;
            if (drain) {
    ...
    public float getDetectionRange() {
        ...
        float adjustableRange = Math.max(detectionRange -= this.characterTraits.get(CharacterTrait.HARD_OF_HEARING) ? 1.0f : 0.0f, 2.0f) - 2.0f;
    ...
    private void renderDebugData() {
        ...
        if (this.getNetworkCharacterAI().getBooleanDebugOptions().enable.getValue() && this.getNetworkCharacterAI().getBooleanDebugOptions().animation.getValue()) {
            ...
            if (this.getAdvancedAnimator().getRootLayer() != null) {
                stringDrawer.draw(font, sx, sy + (dy += 14.0f), "State: " + this.getAdvancedAnimator().getCurrentStateName(), c.r, c.g, c.b, c.a);
                AnimationPlayer animationPlayer = this.getAnimationPlayer();
                if (animationPlayer != null) {
    ...
    public AimingMode getAimingMode() {
        if (!this.isAiming()) {
            if (this.isSittingOnFurniture()) {
                return AimingMode.MELEE;
            }
    ...
    public boolean isMeleeWeaponEquipped() {
        ...
        InventoryItem inventoryItem;
        return !this.isUnarmed() && this.getUseHandWeapon() != null && !this.getUseHandWeapon().isRanged() || (inventoryItem = this.getPrimaryHandItem()) instanceof HandWeapon && !(handWeapon = (HandWeapon)inventoryItem).isRanged();
    }
    ...
    public boolean isAimingFirearmEquipped() {
        HandWeapon handWeapon;
        InventoryItem inventoryItem;
        return !this.isUnarmed() && HandWeapon.isAimedFirearm(this.getUseHandWeapon()) || (inventoryItem = this.getPrimaryHandItem()) instanceof HandWeapon && HandWeapon.isAimedFirearm(handWeapon = (HandWeapon)inventoryItem);
    }
    ...
    public boolean isClimbingRope() {
        return this.getCurrentState() != null && (this.getCurrentState().equals(ClimbSheetRopeState.instance()) || this.getCurrentState().equals(ClimbDownSheetRopeState.instance()));
    }
    ...
    public boolean isCurrentlyIdle() {
        ...
        if (isoGameCharacter instanceof IsoPlayer) {
            ...
            if (GameServer.server) {
                if (player.getNetworkCharacterAI().getState().getEnterState() != null && !IdleState.instance().equals(player.getNetworkCharacterAI().getState().getEnterState().getState())) {
                    return false;
                }
                if (player.getNetworkCharacterAI().getState().getExitState() != null && !IdleState.instance().equals(player.getNetworkCharacterAI().getState().getExitState().getState())) {
                    return false;
                }
            }
        } else {
    ...
    public void onFireLightSourceCheck() {
        ...
        if (this.onFireLightSource != null && (this.onFireLightSource.x != currentX || this.onFireLightSource.y != currentY || this.onFireLightSource.z != currentZ)) {
            this.removeOnFireLightSource();
        }
        if (this.onFireLightSource == null) {
    ...
    protected void createFallingItem(InventoryItem item) {
        ...
        if (GameServer.server) {
            ...
            INetworkPacket.sendToRelative(PacketTypes.PacketType.ZombieHelmetFalling, x, y, this, item, Float.valueOf(x), Float.valueOf(y), Float.valueOf(z));
        } else {
    ...
    public class XP {
        ...
        public void AddXP(PerkFactory.Perk type, float amount, boolean callLua, boolean doXPBoost, boolean remote, boolean haloText) {
            ...
            if (haloText && newXPTotal > oldXP) {
                ...
                if ((haloAward = (float)Math.round(haloAward * 10.0f) / 10.0f) > 0.0f) {
                    HaloTextHelper.addGoodText((IsoPlayer)this.chr, Translator.getText(((PerkFactory.Perk)perk).getName(), new Object[0]) + " XP: " + haloAward, "[br/]");
                }
            }
            if (!GameClient.client) {
        ...
        public void load(ByteBuffer input, int worldVersion) throws IOException {
            ...
            this.this$0.getNetworkCharacterAI().updateXpChecker();
        }
    ...
    public static class TorchInfo {
        private static final ObjectPool<TorchInfo> TorchInfoPool = new ObjectPool<TorchInfo>(TorchInfo::new, "TorchInfo.TorchInfoPool");
        ...
        private void update(IsoGameCharacter chr) {
            ...
            for (i = 0; i < BodyPartType.ToIndex(BodyPartType.MAX); ++i) {
                ...
                if (bodyPart.bitten() != bodyPartLastState.bitten()) {
                    if (bodyPart.bitten()) {
                        woundType = bodyPart.getType().getBiteWoundModel(chr.getCharacterGender());
                ...
                if (bodyPart.scratched() != bodyPartLastState.scratched()) {
                    if (bodyPart.scratched()) {
                        woundType = bodyPart.getType().getScratchWoundModel(chr.getCharacterGender());
                        if (StringUtils.isNullOrWhitespace(woundType)) continue;
                        this.addBandageModel(chr, woundType);
                    } else {
                ...
                if (bodyPart.isCut()) {
                    woundType = bodyPart.getType().getCutWoundModel(chr.getCharacterGender());
                    if (StringUtils.isNullOrWhitespace(woundType)) continue;
                    this.addBandageModel(chr, woundType);
                    continue;
                }
