...
import zombie.characters.animals.pathfind.OffMeshConnection;
...
public final class Mesh {
    ...
    static final ObjectPool<Mesh> pool = new ObjectPool<Mesh>(Mesh::new, "Mesh.pool"){
        ...
        public void release(Mesh obj) {
            AnimalPathfind.getInstance().vector2fObjectPool.releaseAll((List<Vector2f>)obj.triangles);
            obj.triangles.clear();
            obj.trianglesOnBoundaries.clear();
            obj.edgesOnBoundaries.clear();
            OffMeshConnection.pool.releaseAll((List<OffMeshConnection>)obj.offMeshConnections);
    ...
    void initFromZone(Zone zone) {
        ...
        for (int i = 0; i < zone.points.size(); i += 2) {
            this.polygon.add(new Vector2f((float)zone.points.get(i), zone.points.get(i + 1)));
        }
    ...
    void initAdjacentTriangles(int tri1) {
        ...
        this.adjacentTriangles.add(-1);
        this.adjacentTriangles.add(-1);
        block0: for (int tri2 = 0; tri2 < this.triangles.size(); tri2 += 3) {
            if (tri2 == tri1) continue;
    ...
    int getSharedEdge(int tri1, int edge1, int tri2) {
        ...
        for (int edge2 = 0; edge2 < 3; ++edge2) {
            ...
            if (!this.isSameEdge(p1, p2, p3, p4 = this.triangles.get(tri2 + (edge2 + 1) % 3))) continue;
            return edge2;
        }
        return -1;
    }
    ...
    boolean isEdgeOnBoundary(Vector2f v1, Vector2f v2) {
        for (int i = 0; i < this.polygon.size(); ++i) {
            ...
            if ((!v1.equals(v3) || !v2.equals(v4)) && (!v1.equals(v4) || !v2.equals(v3))) continue;
            return true;
        }
        return false;
    }
    ...
    int getTriangleAt(float x, float y) {
        for (int tri = 0; tri < this.triangles.size(); tri += 3) {
            ...
            if (!Mesh.testPointInTriangle(x, y, 0.0f, p1.x, p1.y, 0.0f, p2.x, p2.y, 0.0f, p3.x, p3.y, 0.0f)) continue;
            return tri;
        }
        return -1;
    }
    ...
    public Vector2f pickRandomPointInTriangle(int triangleIndex, Vector2f out) {
        ...
        boolean bl = inTriangle = s + (t = Rand.Next(0.01f, 0.99f)) <= 1.0f;
        if (inTriangle) {
    ...
    void gatherConnectedMeshes(Collection<Mesh> added) {
        for (OffMeshConnection omc : this.offMeshConnections) {
            if (added.contains(omc.meshTo)) continue;
    ...
    int getTriangleEdgeOnX(int tri, float x) {
        ...
        if (PZMath.equal(p2.x, p3.x, 0.001f) && PZMath.equal(p2.x, x, 0.001f)) {
            return 1;
        }
        if (PZMath.equal(p3.x, p1.x, 0.001f) && PZMath.equal(p3.x, x, 0.001f)) {
            return 2;
        }
        return -1;
    }
    ...
    int getTriangleEdgeOnY(int tri, float y) {
        ...
        if (PZMath.equal(p2.y, p3.y, 0.001f) && PZMath.equal(p2.y, y, 0.001f)) {
            return 1;
        }
        if (PZMath.equal(p3.y, p1.y, 0.001f) && PZMath.equal(p3.y, y, 0.001f)) {
            return 2;
        }
        return -1;
    }
    ...
    void renderOffMeshConnections() {
        ...
        for (int j = 0; j < this.offMeshConnections.size(); ++j) {
            ...
            if (omc.edgeFrom != -1) {
                p1 = this.triangles.get(omc.triFrom + omc.edgeFrom);
                p2 = this.triangles.get(omc.triFrom + (omc.edgeFrom + 1) % 3);
                LineDrawer.addLine(p1.x, p1.y, z, p2.x, p2.y, z, 1.0f, 0.0f, 0.0f, null, true);
            }
            p1 = this.triangles.get(omc.triFrom);
            p2 = this.triangles.get(omc.triFrom + 1);
