...
public class BaseAnimalBehavior {
    ...
    public void wanderIdle() {
        ...
        if (this.isDoingBehavior || this.blockMovement || this.parent.isAlerted() || this.parent.getData().goingToMom || this.parent.isAnimalEating() || this.parent.isAnimalMoving() || this.parent.getLastSoundRespondedTo() != null || this.parent.walkToCharLuring || this.parent.drinkFromTrough != null || this.parent.eatFromTrough != null || this.parent.eatFromGround != null) {
            return;
        }
    ...
    private void updateGoingToHutch() {
        if (this.isDoingBehavior && BehaviorAction.ENTERHUTCH == this.behaviorAction) {
            ...
            if (this.hutchPathTimer > -1.0f && hutchToEnter != null) {
                this.hutchPathTimer -= GameTime.getInstance().getMultiplier();
    ...
    public boolean callToHutch(IsoHutch hutch, boolean force) {
        if (this.enterHutchTimerAfterDestroy > 0.0f) {
            this.enterHutchTimerAfterDestroy -= GameTime.getInstance().getMultiplier();
            if (this.enterHutchTimerAfterDestroy < 0.0f) {
        ...
        if (hutch.getAnimalInside().size() < hutch.getMaxAnimals()) {
            ...
            if (hutch.isOpen()) {
                if (this.hutchPathTimer <= -1.0f) {
                    this.hutchPathTimer = this.parent.isBaby() ? Rand.Next(10.0f, 30.0f) : (this.parent.isFemale() ? Rand.Next(350.0f, 500.0f) : Rand.Next(950.0f, 1000.0f));
                }
                if (force) {
                    this.hutchPathTimer = 1.0f;
                }
    ...
    private void followChr() {
        ...
        if (this.parent.getData().getAttachedPlayer() != null) {
            if (this.followChrTimer > 0.0f) {
                this.followChrTimer -= GameTime.getInstance().getMultiplier();
                return;
            }
    ...
    private void fleeFromAttacker() {
        ...
        if (this.parent.adef.attackBack && (isoGameCharacter = this.parent.getAttackedBy()) instanceof IsoPlayer) {
            IsoPlayer isoPlayer = (IsoPlayer)isoGameCharacter;
            this.parent.fightingOpponent = this.parent.getAttackedBy().isInvisible() || isoPlayer.isGhostMode() ? null : this.parent.getAttackedBy();
            return;
        }
        BaseAnimalBehavior.tempVector2.x = this.parent.getX();
        BaseAnimalBehavior.tempVector2.y = this.parent.getY();
        BaseAnimalBehavior.tempVector2.x -= this.parent.getAttackedBy().getX();
        BaseAnimalBehavior.tempVector2.y -= this.parent.getAttackedBy().getY();
    ...
    public void spotted(IsoMovingObject other, boolean bForced, float dist) {
        ...
        if (otherCharacter instanceof IsoZombie && dist <= 10.0f) {
            spotted = true;
            this.parent.setDebugStress(this.parent.getStress() + Rand.Next(5.0E-5f, 9.0E-5f) * GameTime.getInstance().getMultiplier() / (14.0f - dist));
