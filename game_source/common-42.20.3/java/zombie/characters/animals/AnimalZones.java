...
public final class AnimalZones {
    ...
    private static void addTrack(AnimalZone zone, float t2, VirtualAnimal virtualAnimal, Vector2f pos, String type, long hourInMilli) {
        ...
        AnimalChunk chunk = AnimalManagerWorker.getInstance().getOrCreateAnimalChunk(track.x, track.y);
        if (chunk != null) {
    ...
    public static void updateVirtualAnimals() {
        ...
        for (AnimalChunk chunk : instance.chunksWithTracksSet) {
            ...
            if (!chunk.isEmpty() || chunk.cell == null) continue;
            chunk.cell.releaseChunkIfEmpty(chunk);
        }
    }
    ...
    private void renderTracks(UIWorldMap ui, List<AnimalTracks> tracks) {
        ...
        for (AnimalTracks track : tracks) {
            ...
            String txt = Translator.getText("IGUI_MigrationGroup_" + track.animalType, new Object[0]) + " " + AnimalTracks.getTrackStr(track.trackType);
            if (track.dir != null) {
                    ...
                    for (int chx = 0; chx < 32; ++chx) {
                        ...
                        if (bAnimals) {
                            for (int i = 0; i < chunk.animals.size(); ++i) {
                                ...
                                String txt = Translator.getText("IGUI_MigrationGroup_" + animal.migrationGroup, new Object[0]) + " x" + animal.animals.size() + " " + animal.state.getClass().getSimpleName() + (animal.moveForwardOnZone ? " F" : " B");
                                if (animal.state instanceof VirtualAnimalState.StateSleep) {
                                    txt = txt + " " + Math.max(0, (int)Math.floor((animal.wakeTime - GameTime.getInstance().getWorldAgeHours()) * 60.0) + 1) + " mins";
                                }
                                if (animal.state instanceof VirtualAnimalState.StateEat) {
                                    txt = txt + " " + ((int)Math.floor((animal.eatStartTime + (double)((float)animal.timeToEat / 60.0f) - GameTime.getInstance().getWorldAgeHours()) * 60.0) + 1) + " mins";
                                }
    ...
    static {
        ...
        clientTracks = new ArrayList<AnimalTracks>();
    }
}
