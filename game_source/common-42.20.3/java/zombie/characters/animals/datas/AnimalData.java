...
public class AnimalData {
    ...
    private void updateMilk() {
        if (this.parent.adef.udder) {
            if (this.canHaveMilk) {
                ...
                if (!this.isPregnant() && GameTime.getInstance().getCalender().getTimeInMillis() - this.lastMilkTimer > 1814400000L && (this.parent.getBabies() == null || this.parent.getBabies().isEmpty())) {
    ...
    public void eatItem(InventoryItem item, boolean onground) {
        ...
        if (item.getContainer() != null && (isoObject2 = item.getContainer().parent) instanceof IsoFeedingTrough) {
            IsoFeedingTrough isoFeedingTrough;
            trough = isoFeedingTrough = (IsoFeedingTrough)isoObject2;
        }
        Stats stats = this.parent.getStats();
        ...
        if (item instanceof Food) {
            food = (Food)item;
            hungReduce = stats.get(CharacterStat.HUNGER) / this.parent.adef.hungerBoost;
            hungOnFood = ((Food)food).getHungerChange();
            if (AnimalData.doesItemSatisfyHunger(hungOnFood, hungReduce)) {
                ...
                if (onground) {
                    ...
                    for (int j = 0; j < this.parent.getConnectedDZone().size(); ++j) {
                        zone = this.parent.getConnectedDZone().get(j);
                        ((DesignationZoneAnimal)zone).foodOnGround.remove(item.getWorldItem());
                ...
                ((Food)food).setHungChange(((Food)food).getHungChange() * ((hungOnFood + hungReduce) / hungOnFood));
                ...
                if ((double)((Food)food).getHungChange() > -0.01 && item.getContainer() != null) {
                    IsoPlayer player;
                    if (item.getContainer().getParent() != null && (isoObject = item.getContainer().getParent()) instanceof IsoPlayer && (player = (IsoPlayer)isoObject).getPrimaryHandItem() == item) {
                        player.setPrimaryHandItem(null);
                    }
                    if (item.getReplaceOnUse() != null) {
                        InventoryItem newItem = item.getContainer().AddItem(item.getReplaceOnUse());
                    ...
                    if (object2 instanceof IsoWorldInventoryObject) {
                        ...
                        if (object2 instanceof IsoAnimal) {
                            ...
                            if (object2 instanceof IsoHutch) {
                                ...
                                result = this.parent.getBehavior().hutchPathTimer > -1.0f ? result + "Animal is waiting to enter hutch in " + this.parent.getBehavior().hutchPathTimer + " at " + (isoHutch.getSquare().getX() + isoHutch.getEnterSpotX()) + "," + (isoHutch.getSquare().getY() + isoHutch.getEnterSpotY()) + "\r\n" : result + "Animal is going to enter hutch at " + (isoHutch.getSquare().getX() + isoHutch.getEnterSpotX()) + "," + (isoHutch.getSquare().getY() + isoHutch.getEnterSpotY()) + "\r\n";
                            }
                        }
                    }
                }
