...
implements IAnimalVisual {
    ...
    public IsoAnimal(IsoCell cell, int x, int y, int z, String type, String breedName) {
        ...
        AnimalBreed breed = !StringUtils.isNullOrEmpty(breedName) ? this.adef.getBreedByName(breedName) : this.adef.getRandomBreed();
        this.init(breed);
        ...
        for (int x = this.getSquare().getX() - 4; x < this.getSquare().getX() + 4; ++x) {
            for (int y = this.getSquare().getY() - 4; y < this.getSquare().getY() + 4; ++y) {
                ...
                for (int i = 0; i < animals.size(); ++i) {
                    ...
                    if (animalTest == this || animalTest.getAnimalID() != this.getAnimalID()) continue;
    ...
    public boolean testCollideWithVehicles(BaseVehicle vehicle, BaseVehicle.HitVars hitVars) {
        ...
        if (vehicle.shouldCollideWithCharacters()) {
            Vector2 collisionPos = (Vector2)Vector2ObjectPool.get().alloc();
    ...
    private void reattachBackToMom() {
        if (this.attachBackToMother > 0 && this.getVehicle() != null) {
            for (int i = 0; i < this.getVehicle().getAnimals().size(); ++i) {
                ...
                if (mom.getAnimalID() != this.attachBackToMother) continue;
                this.setMother(mom);
                break;
            }
        }
        ...
        if (this.attachBackToMother > 0 && this.getVehicle() == null) {
            ...
            for (int k = 0; k < this.connectedDZone.size(); ++k) {
                if (!this.findMotherAndAttach(this.connectedDZone.get(k).getAnimals())) continue;
                return;
            }
    ...
    private boolean findMotherAndAttach(List<IsoAnimal> animals) {
        for (int i = 0; i < animals.size(); ++i) {
            ...
            if (animal.animalId != this.attachBackToMother) continue;
    ...
    public DesignationZone getZone() {
        ...
        return this.connectedDZone.get(Rand.Next(0, this.connectedDZone.size()));
    }
    ...
    public void updateStress() {
        ...
        if (this.adef.stressUnderRain && RainManager.isRaining().booleanValue() && this.getSquare() != null && !this.getSquare().haveRoof) {
    ...
    private IsoObject getCanAttachAnimalObject(IsoGridSquare square) {
        ...
        for (int i = 0; i < square.getSpecialObjects().size(); ++i) {
            ...
            if (obj.getProperties() == null || !obj.getProperties().has(IsoPropertyType.CAN_ATTACH_ANIMAL)) continue;
            return obj;
        }
        return null;
    }
    ...
    private void checkTreeExists() {
        ...
        IsoGridSquare treeSq = this.getCell().getGridSquare(this.getData().getAttachedTreeX(), this.getData().getAttachedTreeY(), PZMath.fastfloor(this.getZ()));
        if (treeSq == null) {
            return;
        }
    ...
    public void respondToSound() {
        if (this.lastSoundRespondedTo != null) {
            ...
            if (this.timeSinceFleeFromSound <= 0.0f || !this.isAnimalMoving()) {
                Object object;
                this.setMoving(false);
                if (GameServer.server && (object = this.lastSoundRespondedTo.source) instanceof IsoPlayer) {
        ...
        if (object instanceof IsoPlayer) {
            IsoPlayer player = (IsoPlayer)object;
            if (player.callOut && this.getPlayerAcceptance(player) >= 40.0f) {
        ...
        this.timeSinceFleeFromSound = 2000.0f * (float)sound.radius;
        tempVector2.set(this.getX() - (float)sound.x, this.getY() - (float)sound.y);
        float fleeDistance = sound.radius + 10;
        if (this.isWild()) {
            fleeDistance = (float)sound.radius * 3.0f + 20.0f;
        ...
        this.changeStress((float)sound.radius * 0.05f);
    ...
    public void HitByAnimal(IsoAnimal animal, boolean bIgnoreDamage) {
        ...
        if (!bIgnoreDamage) {
            IsoGridSquare sq;
            this.setHealth(this.getHealth() - baseDmg * this.getData().getHealthLoss(Float.valueOf(0.04f)));
            if (this.getSquare() != null && (sq = IsoWorld.instance.currentCell.getGridSquare(this.getSquare().x, this.getSquare().y, this.getSquare().getZ())) != null) {
                for (int i = 0; i < 10; ++i) {
                    sq.getChunk().addBloodSplat((float)sq.x + Rand.Next(-0.8f, 0.8f), (float)sq.y + Rand.Next(-0.8f, 0.8f), sq.z, Rand.Next(8));
                }
            }
        }
        if (this.isDead()) {
            IsoGameCharacter isoGameCharacter = this.fightingOpponent;
    ...
    public void drawRope(IsoGameCharacter chr) {
        ...
        Model.boneToWorldCoords((IsoGameCharacter)this, bone, tempVector3);
        float r = 0.5f;
        float g = 0.37f;
        float b = 0.3f;
        ...
        float sx0 = IsoUtils.XToScreenExact(IsoAnimal.tempVector3.x, IsoAnimal.tempVector3.y, IsoAnimal.tempVector3.z, 0);
        sx0 -= this.getAnimalSize();
        float sy0 = IsoUtils.YToScreenExact(IsoAnimal.tempVector3.x, IsoAnimal.tempVector3.y, IsoAnimal.tempVector3.z, 0);
        ...
        float sx1 = IsoUtils.XToScreenExact(IsoAnimal.tempVector3.x, IsoAnimal.tempVector3.y, IsoAnimal.tempVector3.z, 0);
        float sy1 = IsoUtils.YToScreenExact(IsoAnimal.tempVector3.x, IsoAnimal.tempVector3.y, IsoAnimal.tempVector3.z, 0);
    ...
    public void renderlast() {
        IsoGridSquare treeSq;
        if (this.data == null) {
            return;
        }
        if (!this.isAlphaZero(IsoCamera.frameState.playerIndex) && this.data.getAttachedTree() != null && (treeSq = this.getCell().getGridSquare(this.getData().getAttachedTreeX(), this.getData().getAttachedTreeY(), PZMath.fastfloor(this.getZ()))) != null) {
    ...
    private void renderCustomName() {
        ...
        float renderZ = p3d == null ? 0.5f : p3d.z() * 1.5f;
        ...
        this.userName.AddBatchedDraw(sx, sy -= (float)this.userName.getHeight());
    }
    ...
    private void doDebugString() {
        ...
        sb.append(Translator.getText("IGUI_Name", new Object[0])).append(": ").append(this.getFullName()).append("\n");
        sb.append(Translator.getText("IGUI_StatsAndBody_Stress", new Object[0])).append(": ").append(Math.round(this.stressLevel)).append("\n");
        sb.append(Translator.getText("IGUI_HealthTooltip", new Object[0])).append(": ").append(this.getHealth() * 100.0f).append("\n");
        for (short id : this.playerAcceptanceList.keySet()) {
            if (id != IsoPlayer.getInstance().getOnlineID()) continue;
            sb.append(Translator.getText("IGUI_Acceptance", new Object[0])).append(": ").append(Math.round(this.playerAcceptanceList.get(id).floatValue())).append("\n");
        }
        if (this.getData().getMilkQuantity() > 0.0f) {
            sb.append(Translator.getText("ContextMenu_Milk", new Object[0])).append(": ").append(this.getData().getMilkQuantity()).append("\n");
        }
        IndieGL.enableBlend();
        IndieGL.glBlendFunc(770, 771);
        IndieGL.StartShader(0);
    ...
    public void renderShadow(float x, float y, float z) {
        ...
        if (this.getHook() != null) {
            ...
            if (PerformanceSettings.fboRenderChunk) {
                FBORenderShadows.getInstance().addShadow((float)this.getHook().getSquare().getX() + 0.35f, (float)this.getHook().getSquare().getY() + 0.9f, this.getHook().getSquare().getZ(), forward, 0.5f, 0.7f, 0.6f, lightInfo.r, lightInfo.g, lightInfo.b, alpha, true);
                return;
            }
            IsoDeadBody.renderShadow(this.getHook().getSquare().getX(), this.getHook().getSquare().getY(), this.getHook().getSquare().getZ(), forward, 0.7f, 0.8f, 0.8f, lightInfo, alpha, true);
            return;
        }
    ...
    public void hitConsequences(HandWeapon weapon, IsoGameCharacter wielder, boolean bIgnoreDamage, float damage, boolean bRemote) {
        if (!GameClient.client && !bIgnoreDamage) {
            this.setHealth(this.getHealth() - damage * this.getData().getHealthLoss(Float.valueOf(0.025f)));
        }
        if (this.isWild() && !bIgnoreDamage) {
            ...
            this.getBehavior().setWildDropDeadTimer(dropDeadTimer /= 1.0f + (float)wielder.getPerkLevel(PerkFactory.Perks.Aiming) / 10.0f);
        }
        if (!bIgnoreDamage) {
    ...
    public void sendExtraUpdateToClients() {
        ...
        for (int n = 0; n < GameServer.udpEngine.connections.size(); ++n) {
            ...
            if (!c.isRelevantTo(this.getX(), this.getY())) continue;
    ...
    public void killed(IsoPlayer chr) {
        if (this.health <= 0.0f && this.dZone != null) {
            ...
            for (int i = 0; i < animals.size(); ++i) {
                ...
                if (chr == null || !(animal.DistToProper(chr) < 10.0f)) continue;
    ...
    public Texture getInventoryIconTexture() {
        ...
        if (!StringUtils.isNullOrWhitespace(textureName)) {
            return Texture.getSharedTexture(textureName);
        }
        return null;
    }
    ...
    public void save(ByteBuffer output, boolean isDebugSave, boolean serialize) throws IOException {
        ...
        output.put(this.getDescriptor().isFemale() ? (byte)1 : 0);
        output.putInt(this.animalId);
        ArrayList<String> genes = new ArrayList<String>(this.fullGenome.keySet());
        ...
        output.put(this.getData().pregnant ? (byte)1 : 0);
        if (this.getData().pregnant) {
            output.putInt(this.getData().pregnantTime);
        }
        output.put(this.getData().canHaveMilk() ? (byte)1 : 0);
        ...
        output.put(this.getData().fertilized ? (byte)1 : 0);
        ...
        for (Short pId : this.playerAcceptanceList.keySet()) {
            output.putShort(pId);
            output.putFloat(this.playerAcceptanceList.get(pId).floatValue());
        }
        ...
        output.put(this.isWild() ? (byte)1 : 0);
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        this.animalZone = zoneID.getMostSignificantBits() == 0L && zoneID.getLeastSignificantBits() == 0L ? null : IsoWorld.instance.metaGrid.animalZoneHandler.getZone(zoneID);
        this.setX(this.setLastX(this.setNextX(input.getFloat() + (float)(IsoWorld.saveoffsetx * 300))));
        this.setY(this.setLastY(this.setNextY(input.getFloat() + (float)(IsoWorld.saveoffsety * 300))));
        ...
        boolean bl = this.getData().pregnant = input.get() != 0;
        if (this.getData().pregnant) {
        ...
        boolean bl2 = this.getData().fertilized = input.get() != 0;
        if (this.adef.eggsPerDay > 0) {
        ...
        for (int i = 0; i < size; ++i) {
            this.playerAcceptanceList.put(input.getShort(), Float.valueOf(input.getFloat()));
        }
    ...
    private void initStress() {
        ...
        if (!this.isBaby()) {
            base += (float)(GameTime.getInstance().getWorldAgeDaysSinceBegin() * (double)0.005f);
        }
    ...
    private void initTexture() {
        ...
        if (!StringUtils.isNullOrEmpty(this.adef.textureSkinned) && ((KahluaTableImpl)this.getModData()).rawgetBool("skinned")) {
    ...
    public boolean canGoThere(IsoGridSquare sq) {
        ...
        return this.getData().getAttachedTree() == null || !(IsoUtils.DistanceTo((float)(treeX = this.getData().getAttachedTreeX()) + 0.5f, (float)(treeY = this.getData().getAttachedTreeY()) + 0.5f, (float)sq.getX() + 0.5f, (float)sq.getY() + 0.5f) > 15.0f);
    }
    ...
    public IsoAnimal addBaby() {
        ...
        IsoAnimal baby = new IsoAnimal(this.getCell(), this.getXi(), this.getYi(), this.getZi(), this.adef.babyType, babyDef.getBreedByName(this.getBreed().getName()));
    ...
    public void debugAgeAway(int hour) {
        long hoursmilli = (long)hour * 3600000L;
        long timeSinceLastUpdate2 = GameTime.getInstance().getCalender().getTimeInMillis();
        long delta = GameTime.getInstance().getCalender().getTimeInMillis() - (timeSinceLastUpdate2 -= hoursmilli);
        int age = this.getData().getAge();
        int hours = (int)(delta / 3600000L);
        int deltaAge = hours / 24;
        int newage = age + (deltaAge *= (int)this.getData().getAgeGrowModifier());
        this.setHoursSurvived(this.getHoursSurvived() + (double)hours);
        ...
        for (int i = 0; i < hours; ++i) {
            ...
            cal.setTimeInMillis(timeSinceLastUpdate2 += 3600000L);
            int realHour = cal.get(11);
            if (!this.checkKilledByMetaPredator(realHour)) continue;
            return;
        }
    }
        ...
        if (this.adef.isInsideHutchTime(hour)) {
            ...
            if (Rand.NextBool(baseChance)) {
                if (this.hutch != null && !this.hutch.isDoorClosed()) {
                    ...
                    IsoGridSquare sq = IsoWorld.instance.currentCell.getGridSquare((double)(this.hutch.getSquare().x + this.hutch.getEnterSpotX()), (double)(this.hutch.getSquare().y + this.hutch.getEnterSpotY()), this.hutch.getZ());
                    if (sq != null) {
                        for (int i = 0; i < 20; ++i) {
                            sq.getChunk().addBloodSplat((float)sq.x + Rand.Next(-0.8f, 0.8f), (float)sq.y + Rand.Next(-0.8f, 0.8f), this.hutch.getZ(), Rand.Next(8));
                        }
                    }
                } else {
                    int nbr = 20;
                    if (this.getCurrentSquare() != null) {
                        for (int i = 0; i < 20; ++i) {
                            this.getCurrentSquare().getChunk().addBloodSplat((float)this.getCurrentSquare().x + Rand.Next(-0.8f, 0.8f), (float)this.getCurrentSquare().y + Rand.Next(-0.8f, 0.8f), this.getCurrentSquare().z, Rand.Next(8));
                        }
                    }
    ...
    public boolean shearAnimal(IsoGameCharacter chr, InventoryItem shear) {
        ...
        Object wool = InventoryItemFactory.CreateItem(this.getBreed().woolType);
        this.getCurrentSquare().AddWorldInventoryItem((InventoryItem)wool, Rand.Next(0.0f, 0.8f), Rand.Next(0.0f, 0.8f), 0.0f);
        ...
        if (chr.getPerkLevel(PerkFactory.Perks.Husbandry) <= 5 && Rand.NextBool(chr.getPerkLevel(PerkFactory.Perks.Husbandry) + 3)) {
            this.changeStress(Rand.Next(6.0f - (float)chr.getPerkLevel(PerkFactory.Perks.Husbandry) * 2.0f, (float)(6 - chr.getPerkLevel(PerkFactory.Perks.Husbandry) * 6)) / 8.0f);
        }
        return true;
    }
    ...
    public InventoryItem addDebugBucketOfMilk(IsoGameCharacter chr) {
        Object newBucket = InventoryItemFactory.CreateItem("Base.BucketEmpty");
        ((GameEntity)newBucket).getFluidContainer().addFluid(this.getBreed().getMilkType(), ((GameEntity)newBucket).getFluidContainer().getCapacity());
        return newBucket;
    }

    public InventoryItem milkAnimal(IsoGameCharacter chr, InventoryItem bucket) {
        float minQty = Math.min(0.1f, bucket.isEquipped() || chr.isEquippedClothing(bucket) ? chr.getFreeInventoryCapacity() / (float)ZomboidGlobals.equippedOrWornEncumbranceMultiplier : chr.getFreeInventoryCapacity());
    ...
    public Food createEgg() {
        Food egg = (Food)InventoryItemFactory.CreateItem(this.adef.eggType);
        if (egg == null) {
    ...
    public boolean isGeriatric() {
        ...
        float delta = this.getData().getMaxAgeGeriatric() - (float)this.adef.minAge;
        return (double)this.getData().getDaysSurvived() > (double)delta * 0.8;
    }
        ...
        if (this.isBaby()) {
            ...
            if (list != null && !list.isEmpty()) {
                for (int i = 0; i < list.size(); ++i) {
                    ...
                    if (!stageCheck.stage.equals(this.getAnimalType()) || StringUtils.isNullOrEmpty(stageCheck.nextStage)) continue;
                    stage = stageCheck;
                    break;
                }
            }
            if (stage != null) {
                if (this.getData().getDaysSurvived() < stage.getAgeToGrow(this) / 2) {
                    txt = Translator.getText("IGUI_Animal_Baby", new Object[0]);
                    dbgInfo = "/ " + stage.getAgeToGrow(this) / 2;
                } else {
                    txt = Translator.getText("IGUI_Animal_Juvenile", new Object[0]);
            ...
            if (this.getData().getDaysSurvived() < this.getMinAgeForBaby()) {
                txt = Translator.getText("IGUI_Animal_JuvenileWeaned", new Object[0]);
                dbgInfo = "/ " + this.getMinAgeForBaby();
            } else {
                txt = Translator.getText("IGUI_Animal_Adolescent", new Object[0]);
                dbgInfo = "/ " + this.adef.minAge * 2;
            }
            if (this.getData().getDaysSurvived() > this.adef.minAge * 2) {
                txt = this.isGeriatric() ? Translator.getText("IGUI_Animal_Geriatric", new Object[0]) : Translator.getText("IGUI_Animal_Adult", new Object[0]);
                dbgInfo = "/ " + Float.valueOf(delta * 0.8f).intValue();
            }
            if (skillLvl < 4 && !Core.getInstance().animalCheat) {
                txt = Translator.getText("IGUI_Animal_Adult", new Object[0]);
            }
        }
        dbgInfo = cheat ? " ( " + this.getData().getDaysSurvived() + (String)dbgInfo + ")" : "";
        return txt + (String)dbgInfo;
    }
    ...
    public String getHealthText(boolean cheat, int skillLvl) {
        ...
        if (cheat) {
            ...
            if (this.isGeriatric()) {
                dbgTxt = (String)dbgTxt + " losing health due to age.";
            }
        }
        String txt = (double)this.getHealth() > 0.8 ? Translator.getText("IGUI_Animal_Healthy", new Object[0]) : ((double)this.getHealth() > 0.55 ? Translator.getText("IGUI_Animal_OffColor", new Object[0]) : ((double)this.getHealth() > 0.3 ? Translator.getText("IGUI_Animal_Sickly", new Object[0]) : Translator.getText("IGUI_Animal_Dying", new Object[0])));
        if (skillLvl < 5) {
            txt = Translator.getText("IGUI_Animal_Healthy", new Object[0]);
            if ((double)this.getHealth() < 0.7) {
                txt = Translator.getText("IGUI_Animal_Sickly", new Object[0]);
            }
        }
        return txt + (String)dbgTxt;
    }
    ...
    public String getAppearanceText(boolean cheat) {
        ...
        String hunger = (double)this.getHunger() < 0.3 ? Translator.getText("IGUI_Animal_WellFed", new Object[0]) : ((double)this.getHunger() < 0.6 ? Translator.getText("IGUI_Animal_Underfed", new Object[0]) : Translator.getText("IGUI_Animal_Starving", new Object[0]));
        if (cheat) {
            dbgTxt = (String)dbgTxt + "," + df.format(this.getThirst()) + ")";
        }
        String thirst = (double)this.getThirst() < 0.3 ? Translator.getText("IGUI_Animal_FullyWatered", new Object[0]) : ((double)this.getThirst() < 0.6 ? Translator.getText("IGUI_Animal_Thirsty", new Object[0]) : Translator.getText("IGUI_Animal_DyingThirst", new Object[0]));
        return hunger + ", " + thirst + (String)dbgTxt;
    }
    ...
    public void fertilize(IsoAnimal male, boolean force) {
        if (!force) {
            float maleFertility = male.getUsedGene((String)"fertility").currentValue;
            ...
            float femaleFertility = this.getUsedGene((String)"fertility").currentValue;
            ...
            float randValue = maleFertility * (femaleFertility *= 1.0f - this.getData().getGeriatricPercentage()) * 100.0f;
            if (male != null) {
                male.getData().lastImpregnateTime = 24;
                male.getData().animalToInseminate = new ArrayList();
            }
            if ((float)Rand.Next(100) > randValue) {
                return;
            }
        }
        this.getData().maleGenome = male == null ? this.fullGenome : male.fullGenome;
        if (this.adef.eggsPerDay > 0) {
    ...
    public boolean isAnimalAttacking() {
        if (GameClient.client) {
            return this.isAttackingOnClient;
        }
    ...
    public String getFullName() {
        if (!StringUtils.isNullOrEmpty(this.customName)) {
            Object txt = "";
            if (this.isWild()) {
                txt = (String)txt + " (" + Translator.getText("IGUI_Animal_Wild", new Object[0]) + ")";
            }
            return this.customName + (String)txt;
        }
        Object name = Translator.getText("IGUI_AnimalType_" + this.getAnimalType(), new Object[0]);
        if (this.getData().getBreed() != null) {
            name = Translator.getText("IGUI_AnimalFullName", Translator.getText("IGUI_Breed_" + this.getData().getBreed().getName(), new Object[0]), name);
        }
        if (this.isWild()) {
            name = (String)name + " (" + Translator.getText("IGUI_Animal_Wild", new Object[0]) + ")";
        }
        return name;
    }
    ...
    public ArrayList<AnimalGene> getFullGenomeList() {
        ArrayList<AnimalGene> result = new ArrayList<AnimalGene>();
    ...
    public AnimalAllele getUsedGene(String name) {
        AnimalGene geneSet = this.fullGenome.get(name = name.toLowerCase());
        if (geneSet == null) {
            DebugType.Animal.debugln(name + " wasn't found in the full genome of the animal (" + this.getFullName() + ")");
            return null;
        }
        if (geneSet.allele1.used) {
            return geneSet.allele1;
        }
        return geneSet.allele2;
    }
    ...
    public void changeStress(float inc) {
        ...
        AnimalAllele allele = this.getUsedGene("stress");
        if (allele != null) {
            inc = inc > 0.0f ? (inc *= 1.0f + allele.currentValue) : (inc *= allele.currentValue);
        }
        if (this.geneticDisorder.contains("highstress")) {
            inc = inc < 0.0f ? (inc *= 10.0f) : (inc /= 10.0f);
        }
    ...
    public void setDebugAcceptance(IsoPlayer chr, float acceptance) {
        this.playerAcceptanceList.put(chr.getOnlineID(), Float.valueOf(acceptance));
    }
    ...
    public ArrayList<InventoryItem> getAllPossibleFoodFromInv(IsoGameCharacter chr) {
        ...
        for (int i = 0; i < foodInInv.size(); ++i) {
            ...
            if (foodInv instanceof Food && (food = (Food)foodInv).getMilkType() != null) {
                testType = food.getMilkType();
            }
            if (foodInv instanceof Food) {
                food = (Food)foodInv;
            ...
            if (!foodList.contains(testType) && !foodList.contains(testType2)) continue;
            result.add(foodInv);
        }
        return result;
    }
    ...
    public void addAcceptance(IsoPlayer chr, float acceptance) {
        ...
        if ((newAcceptance *= 1.0f + (float)chr.getPerkLevel(PerkFactory.Perks.Husbandry) / 10.0f) > 100.0f) {
            newAcceptance = 100.0f;
        }
        this.playerAcceptanceList.put(chr.getOnlineID(), Float.valueOf(newAcceptance));
    }
    ...
    public void feedFromHand(IsoPlayer chr, InventoryItem food) {
        ...
        this.changeStress((float)Rand.Next(-4, -1) - (float)chr.getPerkLevel(PerkFactory.Perks.Husbandry) / 5.0f);
        this.setDebugAcceptance(chr, Math.min(100.0f, this.getAcceptanceLevel(chr) + (float)Rand.Next(1, 4)));
    }
    ...
    public void petAnimal(IsoPlayer chr) {
        if (!GameClient.client && this.petTimer == 0.0f) {
            ...
            this.setDebugAcceptance(chr, Math.min(100.0f, this.getAcceptanceLevel(chr) + (float)Rand.Next(2, 7)));
    ...
    public String getStressTxt(boolean cheat, int skillLvl) {
        String txt = Translator.getText("IGUI_Animal_Calm", new Object[0]);
        Object dbgTxt = "";
        if (cheat) {
            dbgTxt = " (" + PZMath.roundFloat(this.stressLevel, 2) + ")";
        }
        if (this.stressLevel > 40.0f) {
            txt = Translator.getText("IGUI_Animal_Unnerved", new Object[0]);
        }
        if (this.stressLevel > 60.0f) {
            txt = Translator.getText("IGUI_Animal_Stressed", new Object[0]);
        }
        if (this.stressLevel > 80.0f) {
            txt = Translator.getText("IGUI_Animal_Agitated", new Object[0]);
        }
        if (skillLvl < 4) {
            txt = Translator.getText("IGUI_Animal_Calm", new Object[0]);
            if (this.stressLevel > 40.0f) {
                txt = Translator.getText("IGUI_Animal_Stressed", new Object[0]);
            }
        }
    ...
    public float getAcceptanceLevel(IsoPlayer chr) {
        if (!this.playerAcceptanceList.containsKey(chr.getOnlineID())) {
            this.playerAcceptanceList.put(chr.getOnlineID(), Float.valueOf(Rand.Next(0.0f, 20.0f)));
        }
        return this.playerAcceptanceList.get(chr.getOnlineID()).floatValue();
    }
    ...
    public void tryLure(IsoPlayer chr, InventoryItem item) {
        ...
        acceptance *= 1.0f + (float)chr.getPerkLevel(PerkFactory.Perks.Husbandry) / 10.0f;
        if ((float)Rand.Next(100) > acceptance) {
    ...
    public Position3D getAttachmentWorldPos(String attachmentName, Position3D pos) {
        ...
        pos.x = IsoAnimal.tempVector3f.x;
        pos.y = IsoAnimal.tempVector3f.z;
        pos.z = (float)PZMath.fastfloor(IsoAnimal.tempVector3f.y) + (IsoAnimal.tempVector3f.y + 32.0f) % 1.0f / 2.44949f;
        return pos;
    }
    ...
    public void carCrash(float delta, boolean front) {
        ...
        this.setHealth(this.getHealth() - delta * this.getData().getHealthLoss(Float.valueOf(divide)));
    }
    ...
    public void pathToCharacter(IsoGameCharacter target) {
        ...
        if (this.data.getAttachedTree() != null && IsoUtils.DistanceTo((float)(treeX = this.data.getAttachedTreeX()) + 0.5f, (float)(treeY = this.data.getAttachedTreeY()) + 0.5f, target.getX(), target.getY()) > 15.0f) {
            return;
        }
    ...
    public void pathToLocation(int x, int y, int z) {
        ...
        int treeX;
        int ropeLength = 15;
        if (this.data.getAttachedPlayer() != null && this.data.getAttachedPlayer().getCurrentSquare() != null && this.data.getAttachedPlayer().getCurrentSquare().DistToProper(x, y) > 15.0f) {
            return;
        }
        if (this.data.getAttachedTree() != null && IsoUtils.DistanceTo((float)(treeX = this.data.getAttachedTreeX()) + 0.5f, (float)(treeY = this.data.getAttachedTreeY()) + 0.5f, (float)x + 0.5f, (float)y + 0.5f) > 15.0f) {
            return;
        }
    ...
    public void pathToTrough(IsoFeedingTrough trough) {
        ...
        boolean bl = sq4Free = sq4.isFree(false) && !sq4.isWallTo(trough.square) && !sq4.isWindowTo(trough.square);
    ...
    public boolean isInMatingSeason() {
        ...
        return currentMonth < end || currentMonth >= start;
    }
    ...
    public void pathFailed() {
        ...
        if ((object = this.getBehavior().behaviorObject) instanceof IsoFeedingTrough) {
            IsoFeedingTrough isoFeedingTrough = (IsoFeedingTrough)object;
    ...
    public void playDeadSound() {
        if (this.isCloseKilled()) {
            ...
            if (attacker != null && (inventoryItem = attacker.getPrimaryHandItem()) instanceof HandWeapon) {
    ...
    private void chooseIdleSound() {
        ...
        if (this.data == null || this.data.getBreed() == null) {
            ass.setDesiredSoundName(null);
            ass.setDesiredSoundPriority(0);
            this.forceNextIdleSound = null;
            return;
        ...
        } else if (this.getStress() > 50.0f && (absStressed != null && ass.isPlaying(absStressed.soundName) || (float)Rand.Next(100) < this.getStress())) {
            desiredSound = "stressed";
        }
        ...
        boolean bl = bForced = this.forceNextIdleSound != null;
        if (bForced) {
        ...
        if (ass.isPlayingDesiredSound() && abs.isIntervalValid() && (expireTime = ass.getIntervalExpireTime(abs.soundName)) < System.currentTimeMillis()) {
    ...
    public void updateRunLoopingSound() {
        AnimalSoundState ass;
        boolean bListenerInRange;
        ...
        if (rootLayer != null) {
            ...
            for (LiveAnimNode animNode : animNodes) {
                if ("runPathfind".equalsIgnoreCase(animNode.getName())) {
                    isRunAnimPlaying = true;
                    break;
                }
                if (!"run".equalsIgnoreCase(animNode.getName())) continue;
                isRunAnimPlaying = true;
                break;
            }
        }
        if (!(bListenerInRange = SoundManager.instance.isListenerInRange(this.getX(), this.getY(), 10.0f))) {
            isRunAnimPlaying = false;
        }
        if ((ass = this.getAnimalSoundState(abs.slot)).isPlaying()) {
            if (!isRunAnimPlaying) {
                ass.stop();
            }
    ...
    public void updateWalkLoopingSound() {
        ...
        if (rootLayer != null) {
            ...
            for (int i = 0; i < animNodes.size(); ++i) {
                ...
                if (!"defaultWalk".equalsIgnoreCase(animNode.getName())) continue;
                isWalkAnimPlaying = true;
                break;
            }
        }
    ...
    public void setMother(IsoAnimal mom) {
        ...
        for (int i = 0; i < mom.getBabies().size(); ++i) {
            if (mom.getBabies().get(i).getAnimalID() != this.getAnimalID() && mom.getBabies().get(i) != this) continue;
    ...
    public boolean canBePicked(IsoGameCharacter chr) {
        if (chr.isUnlimitedCarry() || chr.isGodMod()) {
            return true;
        }
        return this.adef.canBePicked && this.getData().getWeight() < (float)(40 + chr.getPerkLevel(PerkFactory.Perks.Strength) * 7);
    }
    ...
    public String getNextStageAnimalType() {
        ...
        if (this.isFemale()) {
            return this.getData().currentStage.nextStage;
        }
        return this.getData().currentStage.nextStageMale;
    }
    ...
    public void alertOtherAnimals(IsoMovingObject chr, boolean alert) {
        for (int x = (int)this.getX() - 5; x < (int)this.getX() + 5; ++x) {
            for (int y = (int)this.getY() - 5; y < (int)this.getY() + 5; ++y) {
                IsoGridSquare sq = this.getSquare().getCell().getGridSquare((double)x, (double)y, this.getZ());
                if (sq == null || sq.getAnimals().isEmpty()) continue;
    ...
    public int getMinClutchSize() {
        ...
        return Float.valueOf((float)this.adef.minClutchSize * mod).intValue();
    }
    ...
    public int getMaxClutchSize() {
        ...
        return Float.valueOf((float)this.adef.maxClutchSize * mod).intValue();
    }
    ...
    public void climbOverFence(IsoDirections dir) {
        ...
        AnimalClimbOverFenceState.instance().setParams((IsoGameCharacter)this, dir);
        this.setVariable("ClimbFence", true);
        if (GameClient.client && this.isLocalPlayer()) {
            INetworkPacket.send(PacketTypes.PacketType.AnimalEvent, this, Float.valueOf(this.getX()), Float.valueOf(this.getY()), Float.valueOf(this.getZ()));
        }
    }
    ...
    public void remove() {
        ...
        if ((player = this.getData().getAttachedPlayer()) != null) {
    ...
    public InventoryItem canEatFromTrough(IsoFeedingTrough trough) {
        ...
        for (int i = 0; i < trough.getContainer().getItems().size(); ++i) {
            ...
            if (this.adef.eatTypeTrough.contains("All") || this.adef.eatTypeTrough.contains(item.getFullType()) || this.adef.eatTypeTrough.contains(item.getAnimalFeedType())) {
                return item;
            }
            if (!(item instanceof Food) || !this.adef.eatTypeTrough.contains((food = (Food)item).getFoodType())) continue;
            return item;
        }
        return null;
    }
    ...
    public int getFeatherNumber() {
        if (this.isBaby() || StringUtils.isNullOrEmpty(this.getBreed().featherItem) || this.getBreed().maxFeather <= 0) {
            return 0;
        }
        return (int)((float)this.getBreed().maxFeather * this.getData().getSize());
    }
    ...
    public float getPlayerAcceptance(IsoPlayer chr) {
        ...
        return Math.round(this.playerAcceptanceList.get(chr.getOnlineID()).floatValue());
    }

    public static void addAnimalPart(AnimalPart part, IsoPlayer player, IsoDeadBody carcass) {
        Object item;
        float minNb = (float)part.minNb * carcass.getAnimalSize();
        float maxNb = (float)part.maxNb * carcass.getAnimalSize();
        int nb = part.nb;
        float meatRatio = (float)((Double)carcass.getModData().rawget("meatRatio")).doubleValue();
        if (meatRatio <= 0.0f) {
            meatRatio = 1.0f;
        }
        if ((item = InventoryItemFactory.CreateItem(part.item)) instanceof Food) {
            minNb *= meatRatio;
            maxNb *= meatRatio;
        }
        if (nb == -1) {
        ...
        for (int i = 0; i < nb; ++i) {
            ...
            if (item instanceof Food) {
                Food food = (Food)item;
                IsoAnimal.modifyMeat(food, carcass.getAnimalSize(), meatRatio);
            }
            player.getInventory().AddItem((InventoryItem)item);
    ...
    public boolean shouldStartFollowWall() {
        return this.getData().getAttachedPlayer() != null || this.getStress() > 20.0f && this.getVariableBoolean(AnimationVariableHandles.animalRunning) || this.isWild();
    }
    ...
    public void reattachBackToHook() {
        ...
        for (int i = 0; i < hookSq.getObjects().size(); ++i) {
            IsoObject isoObject = hookSq.getObjects().get(i);
            if (!(isoObject instanceof IsoButcherHook)) continue;
            IsoButcherHook hook = (IsoButcherHook)isoObject;
    ...
    private void ensureCorrectSkin() {
        if (!StringUtils.isNullOrEmpty(AnimalDefinitions.getDef((String)this.getAnimalType()).textureSkinned) && ((KahluaTableImpl)this.getModData()).rawgetBool("skinned") && !this.getAnimalVisual().getSkinTexture().equalsIgnoreCase(AnimalDefinitions.getDef((String)this.getAnimalType()).textureSkinned)) {
            this.getAnimalVisual().setSkinTextureName(AnimalDefinitions.getDef((String)this.getAnimalType()).textureSkinned);
            this.resetModel();
    ...
    public static IsoAnimal createAnimalFromCorpse(IsoDeadBody body) {
        IsoAnimal animal = new IsoAnimal(body.getSquare().getCell(), body.getSquare().getX(), body.getSquare().getY(), body.getSquare().getZ(), ((KahluaTableImpl)body.getModData()).rawgetStr("AnimalType"), ((KahluaTableImpl)body.getModData()).rawgetStr("AnimalBreed"));
    ...
    public void updateLOS() {
        ...
        for (IsoMovingObject movingObject : this.getCell().getObjectList()) {
            IsoZombie zombie;
            if (movingObject instanceof IsoPhysicsObject || movingObject instanceof BaseVehicle || movingObject instanceof IsoZombie && (zombie = (IsoZombie)movingObject).isReanimatedForGrappleOnly()) continue;
            ...
            if (PZMath.abs(movingObjectZ - this.getZ()) > 1.0f) continue;
            ...
            if (chrCurrentSquare == null) continue;
            ...
            if (movingCharacter instanceof IsoAnimal || !(movingCharacter instanceof IsoPlayer)) continue;
            IsoPlayer player = (IsoPlayer)movingCharacter;
            if (movingCharacter.isInvisible() || player.isGhostMode()) continue;
            this.getBehavior().spotted(movingCharacter, false, distanceToMovingObject);
        }
    }

    public boolean canBePutInHutch(IsoHutch hutch) {
        if (this.adef != null && this.adef.hutches != null) {
            return this.adef.hutches.contains(hutch.type);
        }
        return false;
    }

    public boolean shouldCreateZone() {
        return this.getDZone() == null && !this.isWild() && this.getData().getAttachedPlayer() == null && this.getData().getAttachedTree() == null && !this.getVariableBoolean(AnimationVariableHandles.animalRunning);
    }
