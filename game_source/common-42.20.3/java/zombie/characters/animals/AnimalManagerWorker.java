...
public final class AnimalManagerWorker {
    ...
    void addAnimal(VirtualAnimal animal) {
        ...
        this.loadIfNeeded(cell);
        AnimalChunk chunk = cell.getOrCreateChunkFromSquarePos(PZMath.fastfloor(animal.getX()), PZMath.fastfloor(animal.getY()));
        if (chunk == null) {
            return;
        }
    ...
    public AnimalChunk getOrCreateAnimalChunk(float x, float y) {
        AnimalCell cell = this.getCellFromSquarePos(PZMath.fastfloor(x), PZMath.fastfloor(y));
        if (cell == null) {
            return null;
        }
        this.loadIfNeeded(cell);
        return cell.getOrCreateChunkFromSquarePos(PZMath.fastfloor(x), PZMath.fastfloor(y));
    }
    ...
    void removeFromChunk(VirtualAnimal animal) {
        ...
        cell.releaseChunkIfEmpty(chunk);
    }
