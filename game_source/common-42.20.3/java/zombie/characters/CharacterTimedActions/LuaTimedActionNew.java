...
implements IPathfinder {
    ...
    public LuaTimedActionNew(KahluaTable table, IsoGameCharacter chr) {
        ...
        if (caloriesModifier != null) {
            this.caloriesModifier = LuaManager.converterManager.fromLuaToJava(caloriesModifier, Float.class).floatValue();
        }
    ...
    public void update() {
        ...
        if (GameClient.client && !this.useCustomRemoteTimedActionSync) {
            ...
            if (this.getTime() == -1 && (duration = (float)ActionManager.getDuration(this.transactionId) / 20.0f) > 0.0f && !ActionManager.isLooped(this.transactionId)) {
                this.table.rawset("maxTime", (Object)duration);
    ...
    public boolean valid() {
        Object[] o = LuaManager.caller.pcall(LuaManager.thread, this.table.rawget("isValid"), (Object)this.table);
        return o.length > 1 && o[1] instanceof Boolean && (Boolean)o[1] != false;
    }

    @Override
    public void interruptWaitToStart() {
        DebugType.Action.trace("%s: interruptWaitToStart", this.table.rawget("Type"));
        LuaManager.caller.pcall(LuaManager.thread, this.table.rawget("interruptWaitToStart"), (Object)this.table);
    }
    ...
    public void start() {
        ...
        if (GameClient.client && DebugLog.isLogEnabled(DebugType.Action, LogSeverity.Trace)) {
            ChatManager.getInstance().showServerChatMessage(" -> " + String.valueOf(this.table.rawget("Type")));
        }
    }
    ...
    public void complete() {
        ...
        if (!GameClient.client) {
            LuaManager.caller.pcall(LuaManager.thread, this.table.rawget("complete"), (Object)this.table);
        }
    }
    ...
    public void Succeeded(Path path, Mover mover) {
        this.table.rawset("path", (Object)path);
    ...
    public void replaceObjectInTable(Object oldObj, Object newObj) {
        if (this.table != null) {
            ...
            synchronized (arrayList) {
                ...
                while (it.advance()) {
                    if (it.getValue() != oldObj) continue;
                    keys.add(it.getKey());
                }
                if (!keys.isEmpty()) {
                    for (Object key : keys) {
