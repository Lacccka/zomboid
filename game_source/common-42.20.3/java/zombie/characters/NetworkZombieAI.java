...
extends NetworkCharacterAI {
    ...
    private void setUsingPathFindState(ZombiePacket packet) {
        ...
        packet.y = this.getPfb2().pathNextY;
        packet.z = (byte)PZMath.fastfloor(this.zombie.getZ());
        float time = IsoUtils.DistanceTo(this.zombie.getX(), this.zombie.getY(), this.getPfb2().pathNextX, this.getPfb2().pathNextY) / 5.0E-4f * this.zombie.speedMod;
        packet.predictionType = (byte)2;
    }
    ...
    public void set(ZombiePacket packet) {
        ...
        Thumpable thumpable = this.zombie.target;
        if (thumpable instanceof IAnimatable) {
            IAnimatable animated = (IAnimatable)((Object)thumpable);
            n = animated.getOnlineID();
        } else {
            n = -1;
        }
        packet.target = (short)n;
        packet.speedMod = (short)(this.zombie.speedMod * 1000.0f);
        packet.timeSinceSeenFlesh = (short)this.zombie.timeSinceSeenFlesh;
        packet.smParamTargetAngle = (short)(this.zombie.get(ZombieTurnAlerted.TARGET_ANGLE).floatValue() * 1000.0f);
        ...
        packet.realState = this.zombie.realState = NetworkVariables.ZombieState.fromString(this.zombie.getAdvancedAnimator().getCurrentStateName());
        ...
        if (this.zombie.getCurrentState() == ThumpState.instance() && this.zombie.getThumpTarget() != null && !this.zombie.getThumpTarget().isDestroyed()) {
            IsoObject isoObject;
            thumpable = this.zombie.getThumpTarget();
            if (thumpable instanceof IsoObject && (isoObject = (IsoObject)thumpable).getSquare() != null) {
    ...
    private boolean checkZTeleport(IsoZombie zombie) {
        ...
        IsoGridSquare square = zombie.getCell().getGridSquare(zombie.realx, zombie.realy, (double)zombie.realz);
        if (square == null) {
            return true;
        }
        return !square.HasStairs() && !square.hasSlopedSurface();
    }
    ...
    public void parse(ZombiePacket packet) {
        ...
        if (this.zombie.isRemoteZombie()) {
            ...
            this.zombie.set(ZombieTurnAlerted.TARGET_ANGLE, Float.valueOf((float)packet.smParamTargetAngle / 1000.0f));
        ...
        this.zombie.setDirectionAngle(packet.dirAngleRads * 57.295776f);
        if ((IsoUtils.DistanceToSquared(this.zombie.getX(), this.zombie.getY(), this.zombie.realx, this.zombie.realy) > 9.0f || this.checkZTeleport(this.zombie)) && (this.zombie.isRemoteZombie() || IsoPlayer.getInstance() != null && IsoUtils.DistanceToSquared(this.zombie.getX(), this.zombie.getY(), IsoPlayer.getInstance().getX(), IsoPlayer.getInstance().getY()) > 2.0f)) {
            this.zombie.setCurrentSquare(null);
            this.zombie.teleportTo(this.zombie.realx, this.zombie.realy, (int)this.zombie.realz);
        }
    }
    ...
    public void preupdate() {
        if (GameClient.client) {
            ...
            if (this.lastDirection != (direction = (byte)this.zombie.getForwardIsoDirection().ordinal())) {
