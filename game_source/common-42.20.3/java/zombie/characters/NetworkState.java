...
public class NetworkState {
    ...
    private StatePacket getState(LinkedList<StatePacket> statePackets, State state) {
        for (StatePacket statePacket : statePackets) {
            if (!statePacket.getState().getName().equals(state.getName())) continue;
            return statePacket;
        }
        return null;
    }
    ...
    public void updateExitState(StatePacket packet) {
        if (NetworkState.tryCancelPendingEnterPair(this.enterStatePackets, packet)) {
            return;
        }
    ...
    public void updateExitSubState(StatePacket packet) {
        if (NetworkState.tryCancelPendingEnterPair(this.enterSubStatePackets, packet)) {
            return;
        }
    ...
    private static boolean tryCancelPendingEnterPair(LinkedList<StatePacket> enterQueue, StatePacket exitPacket) {
        ...
        for (StatePacket pending : enterQueue) {
            if (!pending.getState().getName().equals(exitState.getName())) continue;
