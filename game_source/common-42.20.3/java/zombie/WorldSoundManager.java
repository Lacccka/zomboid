...
public final class WorldSoundManager {
    ...
    public final List<WorldSound> soundList = new ArrayList<WorldSound>();
    private final ObjectPool<WorldSound> freeSounds = new ObjectPool<WorldSound>(WorldSound::new, "WorldSoundManager.freeSounds");
    ...
    public WorldSound release(WorldSound worldSound) {
    ...
    public WorldSound addSound(Object source2, int x, int y, int z, int radius, int volume, float zombieIgnoreDist, float stressMod, boolean sourceIsZombie, boolean doSend, boolean remote, boolean repeating, short flags) {
        ...
        List<WorldSound> list = this.soundList;
        synchronized (list) {
            ...
            if (!GameServer.server) {
                ...
                float animalHearingMultiplier = s.stresshumans || s.stressAnimals ? 3.0f : 1.0f;
                ...
                int radiusMax = (int)PZMath.ceil((float)radius * radiusMultiplier);
    ...
    public WorldSound getSoundZomb(IsoZombie zom) {
        ...
        List<WorldSound> soundList = chunk == null || GameServer.server ? this.soundList : chunk.soundList;
    ...
    public WorldSound getSoundAnimal(IsoAnimal animal) {
        ...
        for (int n = 0; n < soundList.size(); ++n) {
            ...
            float distSq;
            WorldSound sound = soundList.get(n);
            if (!sound.stresshumans && !sound.stressAnimals || (distSq = IsoUtils.DistanceToSquared(animal.getX(), animal.getY(), animal.getZ() * 3.0f, sound.x, sound.y, (float)sound.z * 3.0f)) > (radius = (float)sound.radius * (radiusBonus = animal.isWild() ? 3.0f : 1.0f)) * radius || !((volume = (float)sound.volume * (delta = 1.0f - distSq / (radius * radius))) > loudestVolume)) continue;
    ...
    public float getSoundAttract(WorldSound sound, IsoZombie zom) {
        ...
        if (distSq > (radius = (float)sound.radius * this.getHearingMultiplier(zom)) * radius) {
            return 0.0f;
        }
        if (distSq < sound.zombieIgnoreDist * sound.zombieIgnoreDist && zom.getZ() == (float)sound.z) {
            return 0.0f;
        }
        IsoGridSquare sq = IsoWorld.instance.currentCell.getGridSquare(sound.x, sound.y, sound.z);
    ...
    public float getSoundAttractAnimal(WorldSound sound, IsoAnimal animal) {
        ...
        if (distSq > (radius = (float)sound.radius * (radiusBonus = animal.isWild() ? 3.0f : 1.0f)) * radius) {
            return 0.0f;
        }
        if (distSq < sound.zombieIgnoreDist * sound.zombieIgnoreDist && animal.getZ() == (float)sound.z) {
            return 0.0f;
        }
        IsoGridSquare sq = IsoWorld.instance.currentCell.getGridSquare(sound.x, sound.y, sound.z);
    ...
    public static final class WorldSound {
        ...
        public WorldSound init(WorldSound other) {
