...
extends ErosionCategory {
    ...
    public boolean replaceExistingObject(IsoGridSquare square, ErosionData.Square sqErosionData, ErosionData.Chunk chunkData, boolean isExterior, boolean hasWall) {
        ...
        for (int i = objsSize - 1; i >= 1; --i) {
            ...
            for (int t = 0; t < this.trees.length; ++t) {
                ...
                if (!spr.getName().startsWith(this.trees[t].jumboXLTilesetName) && !spr.getName().startsWith(this.trees[t].jumboXXLTilesetName)) continue;
                CategoryData sqCategoryData = (CategoryData)this.setCatModData(sqErosionData);
                sqCategoryData.gameObj = t;
    ...
    public void init() {
        ...
        for (int id = 0; id < this.trees.length; ++id) {
            ...
            int TREES_NORMAL = 4;
            int TREES_JUMBO = 2;
            boolean TREES_JUMBOXL = true;
            boolean TREES_JUMBOXXL = true;
            for (int stage = 0; stage < 8; ++stage) {
                for (int season = 0; season < snames.length; ++season) {
                    ...
                    switch (stage) {
                        case 4: 
                        case 5: {
                            String string = sheet.replace("_1", "JUMBO_1");
                            break;
                        }
                        case 6: {
                            String string = sheet.replace("_1", "JUMBOXL_1");
                            break;
                        }
                        case 7: {
                            String string = sheet.replace("_1", "JUMBOXXL_1");
                            break;
                        }
                        default: {
                            String string = repl = sheet;
                        }
                    }
                    if (season == 0) {
