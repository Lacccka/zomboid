    ...
    public static class ScriptRegister<T extends BaseScriptObject> {
        ...
        public void saveScript(ByteBuffer output, T script) {
            ...
            if (info != null) {
                ...
                output.put((byte)0);
                if (Core.debug) {
                    DebugType.General.warn("[" + this.name + "] Unable to save script: '" + ((BaseScriptObject)script).getScriptObjectFullType() + "'");
                }
            }
        }
        ...
        public boolean isRegistered(T script) {
            ...
            if (WorldDictionary.isAllowScriptItemLoading()) {
                if (script != null && this.loadList.containsKey(((BaseScriptObject)script).getScriptObjectFullType())) {
                    ...
                    if (Core.debug) {
                        DebugType.General.warn("[" + this.name + "] A script with same full type is registered, but objects are not equal, script: '" + ((BaseScriptObject)script).getScriptObjectFullType() + "'");
                    }
                }
                return false;
            }
