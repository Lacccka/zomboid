...
public class WorldDictionary {
    ...
    private static final Map<String, ItemInfo> itemLoadListMain = new HashMap<String, ItemInfo>();
    private static final Map<String, EntityInfo> entityLoadListMain = new HashMap<String, EntityInfo>();
    private static final Map<String, ItemInfo> itemLoadListMP = new HashMap<String, ItemInfo>();
    private static final Map<String, EntityInfo> entityLoadListMP = new HashMap<String, EntityInfo>();
    ...
    public static void StartScriptLoading() {
        ...
        WorldDictionary.getItemLoadList().clear();
        WorldDictionary.getEntityLoadList().clear();
    ...
    private static void onLoadItem(Item item) {
        ...
        ItemInfo info = WorldDictionary.getItemLoadList().get(item.getFullName());
        if (info == null) {
            info = new ItemInfo();
            info.name = item.getName();
            info.moduleName = item.getModuleName();
            info.fullType = item.getFullName();
            WorldDictionary.getItemLoadList().put(item.getFullName(), info);
        }
    ...
    public static void onLoadEntity(GameEntityScript entityScript) {
        ...
        EntityInfo info = WorldDictionary.getEntityLoadList().get(entityScript.getFullName());
        if (info == null) {
            info = new EntityInfo();
            info.name = entityScript.getName();
            info.moduleName = entityScript.getModuleName();
            info.fullType = entityScript.getFullName();
            WorldDictionary.getEntityLoadList().put(entityScript.getFullName(), info);
        }
    ...
    public static void init() throws WorldDictionaryException {
        ...
        block13: {
            ...
            try {
                ...
                data.parseInfoLoadList(WorldDictionary.getItemLoadList());
                data.parseInfoLoadList(WorldDictionary.getEntityLoadList());
    ...
    private static Map<String, ItemInfo> getItemLoadList() {
        return GameClient.client || GameServer.server ? itemLoadListMP : itemLoadListMain;
    }

    private static Map<String, EntityInfo> getEntityLoadList() {
        return GameClient.client || GameServer.server ? entityLoadListMP : entityLoadListMain;
    }
