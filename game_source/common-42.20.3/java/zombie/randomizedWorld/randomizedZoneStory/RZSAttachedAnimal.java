...
@UsedFromLua
public class RZSAttachedAnimal
extends RandomizedZoneStoryBase {
    ...
    public void randomizeZoneStory(Zone zone) {
        ...
        ArrayList<String> breeds = RZSAttachedAnimal.getBreeds();
        String breed = breeds.get(Rand.Next(breeds.size()));
        String type = (String)breedFemale.get(breed);
        if (Rand.Next(5) == 0) {
            type = (String)breedMale.get(breed);
        }
        IsoGridSquare square = RZSAttachedAnimal.getRandomExtraFreeUnoccupiedSquare(this, zone);
        IsoGridSquare square2 = RZSAttachedAnimal.getRandomExtraFreeUnoccupiedSquare(this, zone);
