...
public final class CombatManager {
    ...
    public final ObjectPool<HitInfo> hitInfoPool = new ObjectPool<HitInfo>(HitInfo::new, "CombatManager.hitInfoPool");
    ...
    private final List<TracerInfo> tracers = new ArrayList<TracerInfo>();
    ...
    private boolean checkObjectHit(IsoGameCharacter owner, HandWeapon weapon, IsoGridSquare square, boolean north, boolean west) {
        ...
        for (int n = square.getSpecialObjects().size() - 1; n >= 0; --n) {
            ...
            if (thumpable != null) {
                if (thumpable.isDoor() || thumpable.isWindow() || thumpable.isWall() || !thumpable.isBlockAllTheSquare()) {
                    if (north && thumpable.north || west && !thumpable.north) {
                        thumpable1 = thumpable.getThumpableFor(owner, weapon);
            ...
            if (window != null && (north && window.isNorth() || west && !window.isNorth()) && (thumpable1 = window.getThumpableFor(owner, weapon)) != null && canHitWindow) {
                thumpable1.WeaponHit(owner, weapon);
                this.objHit = window;
                return true;
            }
    ...
    public void attackCollisionCheck(IsoGameCharacter owner, HandWeapon weapon, SwipeStatePlayer swipeStatePlayer, AttackType attackTypeModifier) {
        ...
        for (int i = 0; i < hitCount; ++i) {
            ...
            if (!this.processIsoBarricadeOpposite(owner, weapon, hitInfo) || hitObject == null) continue;
            ...
            if (!hit) {
                ...
                if (SandboxOptions.instance.firearmUseDamageChance.getValue() != 3 && (SandboxOptions.instance.firearmUseDamageChance.getValue() != 2 || hitZombie == null)) continue;
            ...
            if (owner.isAimAtFloor() && !weapon.isRanged() && owner.isNpc()) {
                ...
                if (ownerPlayer == null || ownerPlayer.isLocalPlayer()) {
                    if (!StringUtils.isNullOrEmpty(weapon.getHitFloorSound())) {
                        ...
                        if (GameClient.client) {
                            INetworkPacket.send(GameClient.connection, PacketTypes.PacketType.PlaySound, new Object[]{weapon.getHitFloorSound(), (byte)0, owner, material});
                        }
                        ...
                        if (GameClient.client) {
                            INetworkPacket.send(GameClient.connection, PacketTypes.PacketType.PlaySound, new Object[]{weapon.getZombieHitSound(), (byte)0, owner, material});
                        }
                    }
                }
            ...
            if (!(owner.getAttackVars().aimAtFloor || owner.getAttackVars().closeKill && owner.isCriticalHit() || ownerLiving.isDoShove() || !(hitObject instanceof IsoGameCharacter))) {
                ...
                if (ownerPlayer == null || ownerPlayer.isLocalPlayer()) {
                    if (weapon.isRanged()) {
                        ...
                        ParameterBulletHitSurface.Material material = hitHead > 0 ? ParameterBulletHitSurface.Material.Flesh_Hollow : ParameterBulletHitSurface.Material.Flesh;
                        isoGameCharacter.getEmitter().setParameterValueByName(zombieHitSound, "BulletHitSurface", material.label);
                        if (GameClient.client) {
                            INetworkPacket.send(GameClient.connection, PacketTypes.PacketType.PlaySound, new Object[]{weapon.getZombieHitSound(), (byte)0, isoGameCharacter, material});
                        }
                    } else {
                        owner.getEmitter().stopSoundByName(weapon.getSwingSound());
                        ParameterMeleeHitSurface.Material material = ParameterMeleeHitSurface.Material.Body;
            ...
            if (hitVehicle == null && hitObject.getSquare() != null && owner.getSquare() != null) {
                ...
                if (hitObject instanceof IsoGameCharacter) {
                    ...
                    GameClient.sendPlayerHit(owner, hitObject, weapon, hitDamage, ignoreHitCountDamage, rangeDel, isCriticalHit, this.tracers, helmetFall, hitHead > 0, hitLegs, removeKnife);
                }
                ...
                GameClient.sendPlayerHit(owner, hitVehicle, weapon, hitDamage, ignoreHitCountDamage, rangeDel, isCriticalHit, this.tracers, helmetFall, hitHead > 0, hitLegs, removeKnife);
                break;
            }
        ...
        if (!processConditionLoss) {
            ...
            if (weapon.getPhysicsObject() != null && (!GameClient.client || owner.isLocal()) && (sq = owner.getCurrentSquare()) != null) {
    ...
    private void calculateAttackVars(IsoLivingCharacter owner, AttackVars vars) {
        ...
        if (!(lowestDistSq >= vars.getWeapon(owner).getMinRange() * vars.getWeapon(owner).getMinRange() || bestStanding != null && this.isWindowBetween(null, null, owner, bestStanding.getObject()))) {
    ...
    public void calcValidTargets(IsoLivingCharacter owner, HandWeapon weapon, PZArrayList<HitInfo> targetsProne, PZArrayList<HitInfo> targetsStanding) {
        ...
        Set<IsoMovingObject> objects = IsoWorld.instance.currentCell.getObjectList();
        for (IsoMovingObject mov : objects) {
    ...
    private HitInfo calcValidTarget(IsoLivingCharacter owner, HandWeapon weapon, IsoMovingObject mov, float range) {
        ...
        if (!isoGameCharacter.isAnimal() && !CombatManager.checkPVP(owner, mov, weapon.isMelee())) {
            return null;
        }
    ...
    public static boolean checkPVP(IsoMovingObject owner, IsoMovingObject target, boolean isMelee) {
        IsoPlayer player = CombatManager.getPVPPlayerFromObject(owner);
        IsoPlayer targetPlayer = CombatManager.getPVPPlayerFromObject(target);
        if (PZOptional.ifPresent(targetPlayer, false, IsoGameCharacter::isGodMod).booleanValue()) {
            return false;
        }
    ...
    public static IsoPlayer getPVPPlayerFromObject(IsoMovingObject obj) {
        BaseVehicle vehicle;
        IsoPlayer driver;
        if (obj instanceof BaseVehicle && (driver = (vehicle = (BaseVehicle)obj).getPVPPlayerDriver()) != null) {
            return driver;
        }
    ...
    private boolean removeTargetObjects(IsoMovingObject obj, IsoGameCharacter owner, boolean isMelee) {
        ...
        return !CombatManager.checkPVP(owner, obj, isMelee);
    }
    ...
    private void calculateHitListWeapon(IsoGameCharacter owner, AttackVars attackVars, PZArrayList<HitInfo> hitList) {
        ...
        objects.removeIf(obj -> this.removeTargetObjects((IsoMovingObject)obj, owner, weapon.isMelee()));
        for (int n = 0; n < objects.size(); ++n) {
            ...
            if ((testResults = LosUtil.lineClear(owner.getCell(), PZMath.fastfloor(owner.getX()), PZMath.fastfloor(owner.getY()), PZMath.fastfloor(owner.getZ()), PZMath.fastfloor(obj2.getX()), PZMath.fastfloor(obj2.getY()), PZMath.fastfloor(obj2.getZ()), false)) == LosUtil.TestResults.Blocked || testResults == LosUtil.TestResults.ClearThroughClosedDoor || obj2.getSquare() != null && owner.getSquare() != null && obj2.getSquare().getTransparentWallTo(owner.getSquare()) != null && !weapon.canAttackPierceTransparentWall(owner, weapon) || (window = this.getWindowBetween(owner, weapon, owner, obj2)) != null && window.isBarricaded() && !window.canAttackBypassIsoBarricade(owner, weapon)) continue;
            float dot = posAndDot.w;
    ...
    private boolean isWindowInHitList(IsoWindow window, PZArrayList<HitInfo> hitList) {
        for (int i = 0; i < hitList.size(); ++i) {
            ...
            if (hitInfo1.getWindow() != window) continue;
            return true;
        }
        return false;
    }
    ...
    private void CalcHitListWindow(IsoGameCharacter owner, HandWeapon weapon, PZArrayList<HitInfo> hitList) {
        ...
        for (int i = 0; i < windows.size(); ++i) {
            ...
            if (PZMath.fastfloor(window.getZ()) != PZMath.fastfloor(owner.getZ()) || this.isWindowInHitList(window, hitList) || !this.windowVisitor.isHittable(window)) continue;
            ...
            if (!Line2D.linesIntersect(owner.getX(), owner.getY(), owner.getX() + lookVector.x, owner.getY() + lookVector.y, windowX1, windowY1, windowX2, windowY2) || (square = window.getAddSheetSquare(owner)) == null || LosUtil.lineClearCollide(owner.getXi(), owner.getYi(), owner.getZi(), square.x, square.y, square.z, false)) continue;
    ...
    private IsoWindow getWindowBetween(IsoGameCharacter owner, HandWeapon weapon, IsoMovingObject a, IsoMovingObject b) {
        return this.getWindowBetween(owner, weapon, PZMath.fastfloor(a.getX()), PZMath.fastfloor(a.getY()), PZMath.fastfloor(b.getX()), PZMath.fastfloor(b.getY()), PZMath.fastfloor(a.getZ()));
    }
        ...
        block40: {
            ...
            if (handWeapon != null && weaponType.isRanged()) {
                ...
                if (isoPlayer.isLocalPlayer() && handWeapon.getCurrentAmmoCount() == 0) {
                    INetworkPacket.send(PacketTypes.PacketType.PlayerEmptyShot, new Object[]{isoPlayer});
                }
            }
            ...
            if (!isoPlayer.isAttackFromBehind()) break block40;
    ...
    implements LOSVisitor {
        ...
        boolean isHittable(IsoWindow window) {
            ...
            if (!window.isDestroyed() && !window.IsOpen()) {
                return true;
            }
            if (window.isBarricaded()) {
                return this.owner == null || this.weapon == null || !window.canAttackBypassIsoBarricade(this.owner, this.weapon);
            }
            return false;
        }
    }
