...
extends GameState {
    ...
    public static void main(String[] args2) {
        ...
        for (Map.Entry entry : System.getProperties().entrySet()) {
            ...
            if (!(key.contains("user") || key.contains("path") || key.contains("dir"))) {
                DebugType.General.println(key + "=" + (String)val);
                continue;
            }
            DebugType.DetailedInfo.trace(key + "=" + (String)val);
        }
        System.out.println("-----");
        Bullet.init();
        ...
        while (var4_10 < args2.length) {
            if (args2[var4_10] != null) {
                ...
                } else if (args2[var4_10].equals("-anti-cheats")) {
        ...
        try {
            ...
            DebugType.General.printException(exception, "Exception thrown during MainScreenState.main.", LogSeverity.Error);
        }
    }

    private static void writeOutCurrentVersion() {
        String versionStr = "revision=%s pzbullet=%s".formatted("70207f62e0", Bullet.getPZBulletVersion());
    ...
    private boolean renderVideo() {
        ...
        float scale = Math.min((float)screenWidth / 16.0f, (float)screenHeight / 9.0f);
        int width = (int)(scale * 16.0f);
        int height = (int)(scale * 9.0f);
        int xPos = (screenWidth - width) / 2;
        int yPos = (screenHeight - height) / 2;
        MainScreenState.DrawTexture((Texture)this.videoTex, xPos, yPos, width, height, 1.0f);
        return true;
    }
    ...
    private static void printSpecs() {
        try {
            ...
            if (cacheDir.exists()) {
                System.out.printf("Disk info: %.2f Gb, free space: %.2f Gb, usable space: %.2f Gb%n", Float.valueOf((float)cacheDir.getTotalSpace() / 1.0737418E9f), Float.valueOf((float)cacheDir.getFreeSpace() / 1.0737418E9f), Float.valueOf((float)cacheDir.getUsableSpace() / 1.0737418E9f));
            }
