...
public final class BuildingDef {
    ...
    public int getMinLevel() {
        ...
        for (int i = 0; i < this.rooms.size(); ++i) {
            this.minLevel = Math.min(this.rooms.get((int)i).level, this.minLevel);
        }
        return this.minLevel;
    }
    ...
    public int getMaxLevel() {
        ...
        for (int x = 0; x < this.rooms.size(); ++x) {
            this.maxLevel = Math.max(this.rooms.get((int)x).level, this.maxLevel);
        }
        return this.maxLevel;
    }
    ...
    public RoomDef getRoom(String roomName, boolean noKids) {
        for (int i = 0; i < this.rooms.size(); ++i) {
            ...
            boolean bl = nope = noKids && room.isKidsRoom();
            if (nope || !room.getName().equalsIgnoreCase(roomName)) continue;
            return room;
        }
        return null;
    }
    ...
    public boolean isAllExplored() {
        for (int n = 0; n < this.rooms.size(); ++n) {
            if (this.rooms.get((int)n).explored) continue;
            return false;
        }
        return true;
    }
    ...
    public int getCellX() {
        return PZMath.fastfloor((float)this.getX() / 256.0f);
    }

    public int getCellY() {
        return PZMath.fastfloor((float)this.getY() / 256.0f);
    }

    public int getCellX2() {
        return PZMath.fastfloor((float)(this.getX2() - 1) / 256.0f);
    }

    public int getCellY2() {
        return PZMath.fastfloor((float)(this.getY2() - 1) / 256.0f);
    }

    public int getChunkX() {
        return PZMath.fastfloor((float)this.x / 8.0f);
    }

    public int getChunkY() {
        return PZMath.fastfloor((float)this.y / 8.0f);
    }
    ...
    public void CalculateBounds(ArrayList<RoomDef> tempRooms) {
        ...
        for (n = 0; n < this.rooms.size(); ++n) {
            ...
            for (nn = 0; nn < r.rects.size(); ++nn) {
                ...
                if (rect.y + rect.h <= this.y2) continue;
                this.y2 = rect.y + rect.h;
            }
        }
        for (n = 0; n < this.emptyoutside.size(); ++n) {
            r = this.emptyoutside.get(n);
            r.rects.trimToSize();
            for (nn = 0; nn < r.rects.size(); ++nn) {
                rect = r.rects.get(nn);
                if (rect.x < this.x) {
        ...
        int minX = PZMath.fastfloor((float)this.x / 8.0f);
        int minY = PZMath.fastfloor((float)this.y / 8.0f);
        int maxX = PZMath.fastfloor((float)this.x2 / 8.0f);
        int maxY = PZMath.fastfloor((float)this.y2 / 8.0f);
        ...
        for (int n2 = 0; n2 < tempRooms.size(); ++n2) {
            RoomDef r2 = tempRooms.get(n2);
            for (int nn2 = 0; nn2 < r2.rects.size(); ++nn2) {
                RoomDef.RoomRect rect2 = r2.rects.get(nn2);
                minX = (rect2.x - 1) / 8;
                minY = (rect2.y - 1) / 8;
                maxX = (rect2.x + rect2.w + 0) / 8;
                maxY = (rect2.y + rect2.h + 0) / 8;
                for (int wy = minY; wy <= maxY; ++wy) {
                    for (int wx = minX; wx <= maxX; ++wx) {
                        if (this.overlapsChunk(wx, wy)) continue;
    ...
    public long calculateMetaID(int cellX, int cellY) {
        ...
        for (int i = 0; i < roomDefs.size(); ++i) {
            ...
            if (roomDef.level > minZ) continue;
        ...
        return (long)minZ << 32 | (long)(minY -= cellY * 256) << 16 | (long)(minX -= cellX * 256);
    }
    ...
    public boolean overlapsChunk(int wx, int wy) {
        for (int i = 0; i < this.overlappedChunks.size(); i += 2) {
            if (wx != this.overlappedChunks.get(i) || wy != this.overlappedChunks.get(i + 1)) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean isFullyStreamedIn() {
        for (int i = 0; i < this.overlappedChunks.size(); i += 2) {
            IsoChunk chunk;
            short wx = this.overlappedChunks.get(i);
            short wy = this.overlappedChunks.get(i + 1);
    ...
    public RoomDef getRandomRoom(int minArea, boolean noKids) {
        ...
        for (int count = 0; count <= 20; ++count) {
            ...
            if (nope || room.area < minArea) continue;
            return room;
        }
        return room;
    }
    ...
    public boolean containsXYZ(int x, int y, int z) {
        if (x >= this.x && y >= this.y && x < this.x + this.getW() && y < this.y + this.getH()) {
            for (int i = 0; i < this.rooms.size(); ++i) {
                RoomDef roomDef = this.rooms.get(i);
                if (roomDef.level != z || !roomDef.intersects(x, y, 1, 1)) continue;
                return true;
            }
        }
        return false;
    ...
    public boolean intersects(int x, int y, int w, int h, int z) {
        for (RoomDef roomDef : this.rooms) {
            if (roomDef.level != z || !roomDef.intersects(x, y, w, h)) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean isAdjacent(int x, int y, int w, int h, int z) {
        for (RoomDef roomDef : this.rooms) {
            if (roomDef.level != z || !roomDef.isAdjacent(x, y, w, h)) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean isAdjacent(BuildingDef other, boolean bIgnoreZ) {
        for (int i = 0; i < this.rooms.size(); ++i) {
            ...
            for (int j = 0; j < other.rooms.size(); ++j) {
                ...
                if (!bIgnoreZ && roomDef.level != roomDef1.level || !roomDef.isAdjacent(roomDef1)) continue;
                return true;
            }
        }
        return false;
    ...
    public boolean overlaps(BuildingDef other, boolean bIgnoreZ) {
        for (int i = 0; i < this.rooms.size(); ++i) {
            ...
            for (int j = 0; j < other.rooms.size(); ++j) {
                RoomDef roomDef1 = other.rooms.get(j);
                if (!bIgnoreZ && roomDef.level != roomDef1.level || !roomDef.overlaps(roomDef1)) continue;
                return true;
            }
        }
        return false;
    ...
    public void addRoomsOf(BuildingDef sourceDef, ArrayList<RoomDef> tempRooms) {
        RoomDef roomDef;
        int i;
        for (i = 0; i < sourceDef.rooms.size(); ++i) {
            roomDef = sourceDef.rooms.get(i);
            roomDef.building = this;
            this.rooms.add(roomDef);
        }
        for (i = 0; i < sourceDef.emptyoutside.size(); ++i) {
            roomDef = sourceDef.emptyoutside.get(i);
            roomDef.building = this;
    ...
    public boolean isShop() {
        for (int i = 0; i < this.rooms.size(); ++i) {
            if (!this.rooms.get(i).isShop()) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean isBasement() {
        return !this.getRooms().isEmpty() && this.getRooms().get((int)0).level < 0;
    }
    ...
    public List<IsoGridSquare> getSquares() {
        ArrayList<IsoGridSquare> squareList = new ArrayList<IsoGridSquare>();
