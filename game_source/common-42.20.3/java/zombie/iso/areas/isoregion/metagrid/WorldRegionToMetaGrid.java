...
public final class WorldRegionToMetaGrid {
    ...
    private final List<IsoGameCharacter.Location> dirtyChunks = new ArrayList<IsoGameCharacter.Location>();
    private final List<IsoChunk> updateChunks = new ArrayList<IsoChunk>();
    ...
    public void clientProcessBuildings(ArrayList<IsoGameCharacter.Location> changedCells) {
        ...
        for (IsoWorldRegion isoWorldRegion : this.allWorldRegions) {
            this.markRegionChunksDirty(isoWorldRegion);
        }
        ...
        for (int i = 0; i < IsoPlayer.numPlayers; ++i) {
            ...
            for (int cy = 0; cy < IsoChunkMap.chunkGridWidth; ++cy) {
                for (int cx = 0; cx < IsoChunkMap.chunkGridWidth; ++cx) {
                    ...
                    if (chunk == null || !this.chunkIsDirty(chunk)) continue;
    ...
    private void markRegionChunksDirty(IsoWorldRegion region) {
        List<IsoChunkRegion> chunks = region.getChunkRegions();
        for (IsoChunkRegion chunk : chunks) {
            DataChunk dataChunk = chunk.getDataChunk();
            tempLocation.set(dataChunk.getChunkX(), dataChunk.getChunkY(), 0);
            if (this.dirtyChunks.contains(tempLocation)) continue;
    ...
    private boolean chunkIsDirty(IsoChunk chunk) {
        int chunkX = PZMath.fastfloor((float)(chunk.wx * 8) / 8.0f);
        int chunkY = PZMath.fastfloor((float)(chunk.wy * 8) / 8.0f);
