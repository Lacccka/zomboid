...
public final class NonPvpZone {
    ...
    public static void removeNonPvpZone(String title) {
        NonPvpZone zone = NonPvpZone.getZoneByTitle(title);
        if (zone != null) {
    ...
    public static NonPvpZone getZoneByTitle(String title) {
        for (int i = 0; i < nonPvpZoneList.size(); ++i) {
            ...
            if (!zone.getTitle().equals(title)) continue;
            return zone;
        }
        return null;
    }
    ...
    public static NonPvpZone getNonPvpZone(int x, int y) {
        for (int i = 0; i < nonPvpZoneList.size(); ++i) {
            NonPvpZone zone = nonPvpZoneList.get(i);
            if (x < zone.getX() || x >= zone.getX2() || y < zone.getY() || y >= zone.getY2()) continue;
            return zone;
        }
        return null;
    }
    ...
    public static boolean isInNonPvpZone(IsoPlayer player) {
        return NonPvpZone.getNonPvpZone(PZMath.fastfloor(player.getX()), PZMath.fastfloor(player.getY())) != null;
    }
