    ...
    public boolean isFacing(IsoPlayer player) {
        ...
        switch (this.facing) {
            case N: {
                ...
                return player.getForwardIsoDirection() == IsoDirections.SW || player.getForwardIsoDirection() == IsoDirections.S || player.getForwardIsoDirection() == IsoDirections.SE;
            }
            case S: {
                if (player.getY() < (float)(this.square.y + 1)) {
                    return false;
                }
                return player.getForwardIsoDirection() == IsoDirections.NW || player.getForwardIsoDirection() == IsoDirections.N || player.getForwardIsoDirection() == IsoDirections.NE;
            }
            case W: {
                if (player.getX() >= (float)this.square.x) {
                    return false;
                }
                return player.getForwardIsoDirection() == IsoDirections.SE || player.getForwardIsoDirection() == IsoDirections.E || player.getForwardIsoDirection() == IsoDirections.NE;
            }
            case E: {
                if (player.getX() < (float)(this.square.x + 1)) {
                    return false;
                }
                return player.getForwardIsoDirection() == IsoDirections.SW || player.getForwardIsoDirection() == IsoDirections.W || player.getForwardIsoDirection() == IsoDirections.NW;
            }
        }
        return false;
    }
