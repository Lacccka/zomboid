...
extends IsoPushableObject {
    ...
    public void update() {
        ...
        if (IsoPlayer.getInstance() != null && IsoPlayer.getInstance().getDragObject() != this) {
            if (this.velx != 0.0f && this.vely == 0.0f && (this.getForwardIsoDirection() == IsoDirections.E || this.getForwardIsoDirection() == IsoDirections.W)) {
                this.setNextX(this.getNextX() + this.velx * 0.65f * capacityDelta);
            }
            if (this.vely != 0.0f && this.velx == 0.0f && (this.getForwardIsoDirection() == IsoDirections.N || this.getForwardIsoDirection() == IsoDirections.S)) {
    ...
    public float getWeight(float x, float y) {
        ...
        if (this.getForwardIsoDirection() == IsoDirections.N || this.getForwardIsoDirection() == IsoDirections.S && x == 0.0f) {
            return weight / 2.0f;
        }
        return weight * 3.0f;
    }
