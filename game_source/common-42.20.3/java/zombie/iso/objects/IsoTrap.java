...
implements IItemProvider {
    ...
    private void updateVictimsInSensorRange() {
        ...
        for (IsoMovingObject movingObject : IsoWorld.instance.currentCell.getObjectList()) {
            ...
            if (GameServer.server) {
                ...
                INetworkPacket.sendToAll(PacketTypes.PacketType.AddExplosiveTrap, excluded, TRIGGER_EXPLOSION, this.getAttacker(), this.getSquare());
            }
    ...
    private boolean shouldProcess(IsoGameCharacter target) {
        ...
        if (GameClient.client) {
            IsoZombie zombie;
            return target instanceof IsoZombie && (zombie = (IsoZombie)target).isLocal();
        }
        if (target instanceof IsoAnimal || target instanceof IsoZombie) {
            return true;
        }
        if (target instanceof IsoPlayer) {
            IsoPlayer playerAttacker;
            IsoPlayer player = (IsoPlayer)target;
            IsoGameCharacter attacker = this.getAttacker();
            if (attacker != null && attacker == target) {
                return true;
            }
            return ServerOptions.getInstance().pvp.getValue() && (!player.getSafety().isEnabled() || attacker instanceof IsoPlayer && !(playerAttacker = (IsoPlayer)attacker).getSafety().isEnabled());
        }
        return false;
    }
    ...
    private void explosion(IsoGridSquare isoGridSquare) {
        ...
        for (int i = 0; i < isoGridSquare.getMovingObjects().size(); ++i) {
            ...
            if (!(moving instanceof IsoGameCharacter) || !this.shouldProcess(isoGameCharacter = (IsoGameCharacter)moving)) continue;
