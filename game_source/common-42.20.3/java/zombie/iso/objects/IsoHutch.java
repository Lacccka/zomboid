...
extends IsoObject {
    ...
    public void transmitCompleteItemToClients() {
        if (GameServer.server) {
            ...
            if (extraSprites != null) {
                HashSet<IsoGridSquare> checkedSquares = new HashSet<IsoGridSquare>();
                KahluaTableIterator iterator2 = extraSprites.iterator();
                while (iterator2.advance()) {
                    KahluaTableImpl spriteDef = (KahluaTableImpl)iterator2.getValue();
                    int xoffset = spriteDef.rawgetInt("xoffset");
                    ...
                    if (checkedSquares.contains(sq2)) continue;
    ...
    public void update() {
        ...
        for (Map.Entry entry : animalInside1.entrySet()) {
            ...
            if (animal2.getData().getHutchPosition() != hutchPosition.intValue()) {
                DebugType.Animal.warn("animal hutchPosition %d != animalInside index %d".formatted(animal2.getData().getHutchPosition(), hutchPosition));
    ...
    public void removeHutch() {
        List<IsoHutch> hutchObjects = this.getAllHutchObjects();
        for (IsoHutch hutch : hutchObjects) {
    ...
    public List<IsoHutch> getAllHutchObjects() {
        ArrayList<IsoHutch> hutchObjects = new ArrayList<IsoHutch>();
        for (int x = this.square.x - 2; x < this.square.x + 3; ++x) {
            for (int y = this.square.y - 2; y < this.square.y + 3; ++y) {
                ...
                if (sq == null) continue;
                ArrayList<IsoHutch> hutches = sq.getHutchTiles(this);
    ...
    public void handleBurning() {
        HashSet<IsoGridSquare> burnSquares = new HashSet<IsoGridSquare>();
        burnSquares.add(this.getSquare());
        List<IsoHutch> hutchObjects = this.getAllHutchObjects();
        for (IsoHutch hutch : hutchObjects) {
            if (hutch.getSquare() == null || burnSquares.contains(hutch.getSquare())) continue;
            burnSquares.add(hutch.getSquare());
            this.removeHutchObject(hutch);
        }
        for (IsoGridSquare square : burnSquares) {
            square.addAshes();
            if (square == this.getSquare()) continue;
