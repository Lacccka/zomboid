...
implements IHasHealth {
    ...
    private static final int[] LOGS_PER_SIZE;
    ...
    public void render(float x, float y, float z, ColorInfo col, boolean bDoAttached, boolean bWallLightingPass, Shader shader) {
        ...
        boolean transparent = this.sprite.name != null && this.sprite.name.contains("XL") && (mainTreeCanSee || (cutaway += this.countObscuredSeenSquaresOriginal(playerIndex, false)) >= 4) && IsoPlayer.getPlayer(playerIndex).isAnyAimKeyDown();
    ...
    private int countObscuredSeenSquaresOriginal(int playerIndex, boolean debugRender) {
        ...
        for (IsoDirections d : IsoDirections.values()) {
            ...
            if (debugRender) {
                LineDrawer.addRect((float)square1.x + 0.1f, (float)square1.y + 0.1f, square1.z, 0.8f, 0.8f, 0.0f, 0.0f, square1.lighting[playerIndex].bCanSee() ? 1.0f : 0.0f);
                continue;
            }
            if (!square1.lighting[playerIndex].bCanSee()) continue;
            ++count;
        }
        return count;
    }
    ...
    private int countObscuredSeenSquares(int playerIndex, int stopCount, boolean debugRender) {
        ...
        count = this.countObscuredSeenSquares(playerIndex, stopCount, debugRender, count, this.square.x, this.square.y - 1, this.square.z, HGT - 2);
        return count;
    }
    ...
    private int countObscuredSeenSquares(int playerIndex, int stopCount, boolean debugRender, int count, int x1, int y1, int z1, int num) {
        ...
        for (int dxy = 1; dxy >= -num; --dxy) {
            ...
            if (square1 == this.square || square1 == null) continue;
            if (debugRender) {
                LineDrawer.addRect((float)square1.x + 0.1f, (float)square1.y + 0.1f, square1.z, 0.8f, 0.8f, 0.0f, 0.0f, square1.lighting[playerIndex].bCanSee() ? 1.0f : 0.0f);
                continue;
            }
            if (!square1.lighting[playerIndex].bCanSee() || stopCount == -1 || ++count < stopCount) continue;
            return count;
        }
        return count;
    }
    ...
    private void render(float x, float y, float z, ColorInfo col, boolean bDoAttached, int playerIndex, boolean transparent) {
        ...
        if (PerformanceSettings.fboRenderChunk && FBORenderTrees.current != null) {
            ...
            float f = DebugOptions.instance.terrain.renderTiles.forceFullAlpha.getValue() ? 1.0f : (minAlpha = bPlayerInside ? 0.05f : 0.15f);
            if (bUseStencil) {
            ...
            if (transparent) {
                ...
                maxAlpha = this.getTargetAlpha(playerIndex);
                this.cutawayAlpha += alphaStep;
            ...
            if (transparent && this.cutawayAlpha != 1.0f && IsoTreeJumbo.Jumbos.get(this.sprite.name) != null) {
                ...
                IsoTreeJumbo.TreeDescription attachedDescription = this.attachedAnimSprite == null || this.attachedAnimSprite.isEmpty() ? null : IsoTreeJumbo.Jumbos.get(((IsoSpriteInstance)this.attachedAnimSprite.get((int)0)).parentSprite.name);
                ...
                Texture trunk2 = attachedDescription == null ? null : IsoSpriteManager.instance.getSprite(attachedDescription.trunk()).getTextureForCurrentFrame(this.getDir(), this);
                Texture treetop2 = attachedDescription == null ? null : IsoSpriteManager.instance.getSprite(attachedDescription.treetop()).getTextureForCurrentFrame(this.getDir(), this);
                renderTrees.addTree(trunk, trunk2, x, y, z, col.r, col.g, col.b, col.a, this.getObjectRenderEffectsToApply(), bUseStencil, this.fadeAlpha, false, 1.0f);
                renderTrees.addTree(treetop, treetop2, x, y, z, col.r, col.g, col.b, col.a, this.getObjectRenderEffectsToApply(), bUseStencil, this.fadeAlpha, true, this.cutawayAlpha);
            } else {
                ...
                renderTrees.addTree(baseTexture, foliageTexture, x, y, z, col.r, col.g, col.b, col.a, this.getObjectRenderEffectsToApply(), bUseStencil, this.fadeAlpha, this.cutawayAlpha != 1.0f, this.cutawayAlpha);
            }
        } else if (bUseStencil) {
            float maxAlpha;
            float minAlpha;
    ...
    private void renderInner(float x, float y, float z, ColorInfo col, boolean bDoAttached, boolean bShader) {
        ...
        this.offsetX = (float)FBORenderChunk.FLOOR_WIDTH / 2.0f;
        ...
        if (bShader && this.sprite != null && (texture = this.sprite.getTextureForCurrentFrame(this.getForwardIsoDirection(), this)) != null) {
        ...
        if (this.attachedAnimSprite != null) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                s.render(this, x, y, z, this.getForwardIsoDirection(), this.offsetX, this.offsetY, this.isHighlighted(playerIndex) ? this.getHighlightColor(playerIndex) : col);
    ...
    public boolean isMaskClicked(int x, int y, boolean flip) {
        ...
        for (int i = 0; i < this.attachedAnimSprite.size(); ++i) {
            if (!((IsoSpriteInstance)this.attachedAnimSprite.get((int)i)).parentSprite.isMaskClicked(this.getForwardIsoDirection(), x, y, flip)) continue;
            return true;
        }
        return false;
    }
    ...
    public void toppleTree(IsoGameCharacter owner) {
        ...
        if (stump != null) {
            ...
            IsoObject stumpObject = (IsoObject)this.square.getObjects().getLast();
    ...
    static {
        LOGS_PER_SIZE = new int[]{1, 1, 2, 3, 4, 5, 6, 8};
    }
