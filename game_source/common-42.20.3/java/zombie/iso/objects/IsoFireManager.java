...
public class IsoFireManager {
    ...
    public static void StartFire(IsoCell cell, IsoGridSquare gridSquare, boolean igniteOnAny, int fireStartingEnergy, int life) {
        ...
        if (GameServer.server) {
            INetworkPacket.sendToRelative(PacketTypes.PacketType.StartFire, gridSquare.getX(), (float)gridSquare.getY(), gridSquare, igniteOnAny, fireStartingEnergy, newFire.life, false, newFire.spreadDelay, newFire.numFlameParticles);
        }
    ...
    private static final class FireSounds {
        ...
        private final Comparator<IsoFire> comp = new Comparator<IsoFire>(this){
            ...
            public int compare(IsoFire a, IsoFire b) {
                float bScore;
                float aScore = FMODParameterUtils.getClosestListenerDistanceSquared((float)a.square.x + 0.5f, (float)a.square.y + 0.5f, a.square.z);
                if (aScore > (bScore = FMODParameterUtils.getClosestListenerDistanceSquared((float)b.square.x + 0.5f, (float)b.square.y + 0.5f, b.square.z))) {
                    return 1;
                }
                if (aScore < bScore) {
                    return -1;
