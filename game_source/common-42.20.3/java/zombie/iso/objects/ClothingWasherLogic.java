...
implements IClothingWasherDryerLogic {
    ...
    public void update() {
        ...
        for (int i = 0; i < this.getContainer().getItems().size(); ++i) {
            ...
            if (item.getModData().rawget("ItemAfterCleaning") != null) {
                ...
                if (cleaningProgress == null) {
                    ...
                    if (cleaningProgress >= 100.0) {
                        Object replacement = InventoryItemFactory.CreateItem(resultItem);
                        this.getContainer().getItems().remove(item);
                        this.getContainer().addItem((InventoryItem)replacement);
            ...
            if (item instanceof InventoryContainer && (invContainer = (InventoryContainer)item).getBloodLevel() > 0.0f) {
                invContainer.setBloodLevel(invContainer.getBloodLevel() - (float)(elapsedMinutes * 2) / 100.0f);
                containerBloodChanged = true;
            }
            if (!modDataChanged && !containerBloodChanged || !GameServer.server) continue;
