...
extends IsoObject {
    ...
    private static final DecimalFormat decimalFormatB;
    ...
    private static HashMap<String, String> getGeneratorSpriteToType() {
        if (generatorSpriteToType == null) {
            ...
            ArrayList<Item> items = ScriptManager.instance.getItemsTag(ItemTag.GENERATOR);
            for (Item item : items) {
                if (item.getWorldObjectSprite() == null) continue;
    ...
    public String getGeneratorItemType() {
        String type;
        if (this.getSprite() != null && this.getSprite().getName() != null && (type = IsoGenerator.getGeneratorSpriteToType().get(this.getSprite().getName())) != null) {
            return type;
        }
        return DEFAULT_GENERATOR_TYPE;
    }
            ...
            for (int dx = -generatorChunkRange; dx <= generatorChunkRange; ++dx) {
                ...
                for (int z = minZ; z <= maxZ; ++z) {
                    ...
                    for (int i = 0; i < objects.size(); ++i) {
                        float power;
                        IsoObject obj = (IsoObject)objects.get(i);
                        if (IsoUtils.DistanceToSquared((float)obj.getSquare().getX() + 0.5f, (float)obj.getSquare().getY() + 0.5f, (float)this.getSquare().getX() + 0.5f, (float)this.getSquare().getY() + 0.5f) > (float)(generatorRadius * generatorRadius)) continue;
                        obj.checkHaveElectricity();
                        if (ItemContainer.isObjectPowered(obj, false) || !allowExteriorGenerator && obj.getSquare().has(IsoFlagType.exterior) || !((power = obj.getGeneratorPowerConsumption()) > 0.0f)) continue;
    ...
    private void addPoweredItem(IsoObject obj, float powerConsumption) {
        ...
        String name = Translator.getText("IGUI_VehiclePartCatOther", new Object[0]);
        if (obj.getPipedFuelAmount() > 0) {
            name = Translator.getText("IGUI_GasPump", new Object[0]);
        }
        ...
        if (obj instanceof IsoLightSwitch) {
            name = Translator.getText("IGUI_Lights", new Object[0]);
        }
        if (obj instanceof IsoCarBatteryCharger) {
            name = Translator.getText("IGUI_VehiclePartCatOther", new Object[0]);
        }
        int nbr = 1;
    ...
    public void save(ByteBuffer output, boolean isDebugSave) throws IOException {
        this.getModData().rawset("totalPowerDraw", (Object)this.totalPowerUsing);
    ...
    static {
        decimalFormat = new DecimalFormat("#.#### L/h");
        decimalFormatB = new DecimalFormat(" (#.#### L/h)");
    }
}
