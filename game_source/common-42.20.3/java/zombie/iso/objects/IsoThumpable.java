...
ILockableDoor {
    ...
    public void setModData(KahluaTable modData) {
        if (modData != null) {
            ArrayList<String> needKeys = new ArrayList<String>();
            KahluaTableIterator it = modData.iterator();
            while (it.advance()) {
                String key;
                Object object = it.getKey();
                if (!(object instanceof String) || !(key = (String)object).startsWith(BUILD_MATERIAL_PREFIX)) continue;
                this.getBuildMaterials().rawset(key.substring(BUILD_MATERIAL_PREFIX.length()), it.getValue());
                needKeys.add(key);
            }
    ...
    public String getBreakSound() {
        if (this.breakSound == null || this.breakSound.equalsIgnoreCase(DEFAULT_BREAK_SOUND.getSoundName())) {
            return IsoThumpable.GetBreakFurnitureSound(this.getSprite());
        }
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        if (!bits.equals(0)) {
            ...
            if (bits.hasFlags(0x40000000000L)) {
                if (this.buildMaterials == null) {
                    this.buildMaterials = LuaManager.platform.newTable();
                }
    ...
    public void Thump(IsoMovingObject thumper, int thumpEventCount) {
        ...
        if (thumper instanceof IsoZombie) {
            ...
            if (totalThumpers >= maxThumpers) {
                ...
                float partialAmount = (float)(1 * totalThumpers) / (float)maxThumpers;
                this.partialThumpDmg += partialAmount * (float)thumpEventCount;
            ...
            WorldSoundManager.instance.addSound(thumper, this.square.getX(), this.square.getY(), this.square.getZ(), 20, 20, true, 4.0f, this.getStressModFromThumping());
            if (this.isDoor()) {
                this.setRenderEffect(RenderEffectType.Hit_Door, true);
            }
        ...
        if (this.health <= 0) {
            if (!bBreakableFence) {
                ...
                if (GameServer.server) {
                    GameServer.PlayWorldSoundServer((IsoGameCharacter)thumper, this.getBreakSound(), false, thumper.getCurrentSquare(), 0.2f, 70.0f, 1.1f, true);
                }
            }
    ...
    public boolean isLockedToCharacter(IsoGameCharacter chr) {
        IsoPlayer isoPlayer;
        if (GameClient.client && chr instanceof IsoPlayer && (isoPlayer = (IsoPlayer)chr).getRole() != null && isoPlayer.getRole().hasCapability(Capability.CanOpenLockedDoors)) {
            return false;
        }
        if (this.getLockedByCode() > 0) {
            return true;
    ...
    private ParameterMeleeHitSurface.Material getMeleeHitSurface() {
        String soundName;
        if (!this.isDoor() && this.hasProperty(IsoPropertyType.THUMP_SOUND) && !StringUtils.isNullOrWhitespace(soundName = this.getProperty(IsoPropertyType.THUMP_SOUND))) {
    ...
    public static String GetBreakFurnitureSound(IsoSprite sprite) {
        ...
        if (sinkTypeStr != null) {
            ...
            return switch (sinkType) {
                case ParameterSinkType.SinkType.CERAMIC -> SoundKey.BREAK_FURNITURE_CERAMIC.getSoundName();
                case ParameterSinkType.SinkType.METAL -> SoundKey.BREAK_FURNITURE_METAL.getSoundName();
        ...
        if (fenceTypeLow != null) {
            ...
            return switch (fenceType) {
                case ParameterFenceTypeLow.FenceType.BARBWIRE, ParameterFenceTypeLow.FenceType.METAL, ParameterFenceTypeLow.FenceType.METAL_GATE -> SoundKey.BREAK_FURNITURE_METAL.getSoundName();
                default -> SoundKey.BREAK_FURNITURE.getSoundName();
            };
        }
        String fenceTypeHigh = sprite.getProperties().get(IsoPropertyType.FENCE_TYPE_HIGH);
        if (fenceTypeHigh != null) {
            ParameterFenceTypeHigh.FenceType fenceType = ParameterFenceTypeHigh.FenceType.fromString(fenceTypeHigh, ParameterFenceTypeHigh.FenceType.WOOD);
            return switch (fenceType) {
                case ParameterFenceTypeHigh.FenceType.METAL, ParameterFenceTypeHigh.FenceType.METAL_GATE -> SoundKey.BREAK_FURNITURE_METAL.getSoundName();
                default -> SoundKey.BREAK_FURNITURE.getSoundName();
            };
        }
        ...
        return switch (materialType) {
            case MaterialType.Ceramic -> SoundKey.BREAK_FURNITURE_CERAMIC.getSoundName();
            case MaterialType.Metal, MaterialType.Metal_Large, MaterialType.Metal_Light, MaterialType.Metal_Solid -> SoundKey.BREAK_FURNITURE_METAL.getSoundName();
            default -> SoundKey.BREAK_FURNITURE.getSoundName();
        };
    }
