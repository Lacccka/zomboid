...
public final class WorldStreamer {
    ...
    private final ConcurrentLinkedQueue<ChunkRequest> mainThreadRequestQueue = new ConcurrentLinkedQueue();
    ...
    private boolean shouldSendChunk(IsoChunk chunk) {
        ...
        for (IsoPlayer player : players) {
            ...
            if (Math.abs(chunk.wx - pChunk.wx) >= 10 || Math.abs(chunk.wy - pChunk.wy) >= 10) continue;
            return true;
        }
        return false;
    }
    ...
    public void updateMain() {
        ...
        Object object = this.chunkHeadMainLock;
        synchronized (object) {
        ...
        ChunkRequest request = this.mainThreadRequestQueue.poll();
        while (request != null) {
            while (request != null) {
            ...
            request = this.mainThreadRequestQueue.poll();
        }
        ...
        for (int i = 0; i < this.pendingRequests1.size(); ++i) {
            ...
            if ((request.flagsUdp & 8) != 0) {
                if ((request.flagsWs & 1) == 0 && !request.chunk.refs.isEmpty()) {
                    DebugType.Multiplayer.warn("the server did not generate the chunk %d,%d in time, requesting it again", request.chunk.wx, request.chunk.wy);
            ...
            if (requestBB != null) {
                ...
                try {
                    ...
                    DebugType.General.error("WorldStreamer.loadReceivedChunks: Error while the chunk (" + request.chunk.wx + ", " + request.chunk.wy + ") was decompressing");
                    this.chunkRequests1.add(request.chunk);
                    continue;
                }
    ...
    public boolean isBusy() {
        if (!(!GameClient.client || this.chunkRequests0.isEmpty() && this.chunkRequests1.isEmpty() && this.chunkHeadMain == null && this.mainThreadRequestQueue.isEmpty() && this.waitingToCancelQ.isEmpty() && this.sentRequests.isEmpty() && this.pendingRequests.isEmpty() && this.pendingRequests1.isEmpty())) {
            return true;
        }
    ...
    public void receiveChunkPart(ByteBufferReader bb) {
        ...
        for (int i = 0; i < this.pendingRequests.size(); ++i) {
            ...
            if (this.networkFileDebug) {
                DebugType.NetworkFileDebug.debugln("received all parts for " + request2.chunk.wx + "," + request2.chunk.wy);
            }
    ...
    public void receiveNotRequired(ByteBufferReader bb) {
        ...
        block1: for (int n = 0; n < count; ++n) {
            ...
            for (int i = 0; i < this.pendingRequests.size(); ++i) {
                ...
                if (request2.requestNumber != requestNumber) continue;
                if (this.networkFileDebug) {
                    DebugType.NetworkFileDebug.debugln("NotRequiredInZip " + request2.chunk.wx + "," + request2.chunk.wy + " delete=" + !sameOnServer);
                }
                if (!sameOnServer) {
                    request2.flagsUdp |= 4;
                }
