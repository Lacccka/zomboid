...
public class WorldGenChunk {
    ...
    public void generateChunks(ChunksCache chunks) {
        ...
        for (int x = chunks.getMinChunkX(); x < chunks.getMinChunkX() + 2; ++x) {
            for (int y = chunks.getMinChunkY(); y < chunks.getMinChunkY() + 2; ++y) {
                IsoChunk ch = (IsoChunk)chunks.get(new ChunkCoord(x, y));
    ...
    private void genRandomSquare(IsoCell cell, ChunksCache chunks, IsoChunk ch, int x, int minTileX, int y, int minTileY, Set<Road> roads, EnumMap<FeatureType, String[]> toBeDone) {
        boolean z = false;
        ...
        if (tmpStaticModules.isEmpty()) {
            ...
            if (sm2.biome() != null) {
                this.genRandomTiles(cell, chunks, ch, x, y, sm2.biome(), toBeDone, ccTileY, ccTileX, square, 0, rnd, 0);
            } else if (sm2.prefab() != null) {
                IBiome biome = this.getBiome(x, y);
                PrefabStructure prefab = sm2.prefab();
                this.applyPrefab(prefab, biome, cell, chunks, ch, square, x, y, 0, ccTileX, ccTileY, 0, toBeDone, sm2.xmin(), sm2.xmax(), sm2.ymin(), sm2.ymax(), rnd);
            } else {
    ...
    private void genRandomTiles(IsoCell cell, ChunksCache chunks, IsoChunk ch, int x, int y, IBiome biome, EnumMap<FeatureType, String[]> toBeDone, int ccTileY, int ccTileX, IsoGridSquare square, int z, Random rnd, int tileZ) {
        HashMap<FeatureType, IBiome> subBiomes = new HashMap<FeatureType, IBiome>();
        this.doPending(cell, ch, x, y, toBeDone, ccTileY, ccTileX, square, biome, biome, subBiomes, z, rnd);
        for (FeatureType type : FeatureType.values()) {
            biome = subBiomes.isEmpty() || subBiomes.get((Object)type) == null ? biome : (IBiome)subBiomes.get((Object)type);
            TileReplacementRetValue retval = this.applyBiome(biome, type, cell, chunks, ch, square, x, y, z, ccTileX, ccTileY, tileZ, toBeDone, rnd);
            if (retval.retval() != TileReplacement.PENDING) continue;
            this.doPending(cell, ch, x, y, toBeDone, ccTileY, ccTileX, square, biome, biome, subBiomes, z, rnd);
            biome = subBiomes.isEmpty() || subBiomes.get((Object)type) == null ? biome : (IBiome)subBiomes.get((Object)type);
    ...
    private void genMapSquare(IsoCell cell, ChunksCache chunks, IsoChunk ch, int x, int minTileX, int y, int minTileY, BiomeMap map, int[] biomes, EnumMap<FeatureType, String[]> toBeDone) {
        ...
        if (Objects.equals(lookup.biome(), "$random")) {
            ...
            square = IsoGridSquare.getNew(cell, null, x, y, 0);
            ch.setSquare(tileX, tileY, 0, square);
            this.genRandomSquare(cell, chunks, ch, x, minTileX, y, minTileY, new HashSet<Road>(), toBeDone);
            return;
        }
        Random rnd = WorldGenParams.INSTANCE.getRandom(ch.wx * 8 + x, ch.wy * 8 + y);
        HashMap<FeatureType, IBiome> subBiomes = new HashMap<FeatureType, IBiome>();
        ...
        HashMap<FeatureType, IsoObject> currentTiles = new HashMap<FeatureType, IsoObject>();
        ...
        for (FeatureType type : FeatureType.values()) {
            IBiome biome;
            if (currentTiles.get((Object)type) == null || ((IsoObject)currentTiles.get((Object)((Object)type))).sprite == null) continue;
            IBiome iBiome = biome = subBiomes.isEmpty() || subBiomes.get((Object)type) == null ? this.getMapBiome(x, y, lookup.biome()) : (IBiome)subBiomes.get((Object)type);
            if (biome == null) {
                return;
            }
            if (this.isProtected(biome.protectedList(), ((IsoObject)currentTiles.get((Object)((Object)type))).sprite.name)) continue;
            if (WorldGenUtils.INSTANCE.canPlace(biome.placements().get((Object)type), floorName)) {
                IsoObject obj;
                int i;
                TileReplacementRetValue retval = this.applyBiome(biome, type, cell, chunks, ch, square, x, y, 0, ccTileX, ccTileY, 0, toBeDone, rnd);
                if (retval.retval() == TileReplacement.PENDING) {
                    this.doPending(cell, ch, x, y, toBeDone, ccTileY, ccTileX, square, lookupBiome, lookupOre, subBiomes, 0, rnd);
                    biome = subBiomes.isEmpty() || subBiomes.get((Object)type) == null ? this.getMapBiome(x, y, lookup.biome()) : (IBiome)subBiomes.get((Object)type);
                ...
                if (retval.retval() == TileReplacement.SUCCESS) {
                    for (i = 1; i < square.getObjects().size(); ++i) {
                        obj = square.getObjects().get(i);
                        if (obj.sprite == retval.placed() || !(obj instanceof IsoTree) && !obj.isBush() && !obj.isGrassLike()) continue;
                        square.DeleteTileObject(obj);
                    }
                }
                if (retval.retval() != TileReplacement.DELETE) continue;
                for (i = 1; i < square.getObjects().size(); ++i) {
                    obj = square.getObjects().get(i);
                    if (!(obj instanceof IsoTree) && !obj.isBush() && !obj.isOres() && !obj.isGrassLike()) continue;
                    square.DeleteTileObject(obj);
                }
                continue;
            }
            square.DeleteTileObject((IsoObject)currentTiles.get((Object)type));
        }
        List<IsoObject> grasses = square.getGrassLike();
        if (square.getObjects().size() - grasses.size() == 1) {
            IBiome biome;
            IBiome iBiome = biome = subBiomes.isEmpty() || subBiomes.get((Object)FeatureType.ORE) == null ? this.getMapBiome(x, y, lookup.ore()) : (IBiome)subBiomes.get((Object)FeatureType.ORE);
            if (biome == null) {
                return;
            }
            ...
            if (rocks) {
                TileReplacementRetValue retval = this.applyBiome(biome, FeatureType.ORE, cell, chunks, ch, square, x, y, 0, ccTileX, ccTileY, 0, toBeDone, rnd);
                if (retval.retval() == TileReplacement.PENDING) {
                    this.doPending(cell, ch, x, y, toBeDone, ccTileY, ccTileX, square, lookupBiome, lookupOre, subBiomes, 0, rnd);
                    biome = subBiomes.isEmpty() || subBiomes.get((Object)FeatureType.ORE) == null ? this.getMapBiome(x, y, lookup.biome()) : (IBiome)subBiomes.get((Object)FeatureType.ORE);
    ...
    private void doPending(IsoCell cell, IsoChunk ch, int x, int y, EnumMap<FeatureType, String[]> toBeDone, int ccTileY, int ccTileX, IsoGridSquare square, IBiome lookupBiome, IBiome lookupOre, Map<FeatureType, IBiome> subBiomes, int z, Random rnd) {
        block12: for (FeatureType featureType : FeatureType.values()) {
            String tile = toBeDone.get((Object)featureType)[ccTileY * 16 + ccTileX];
            if (tile == null || tile.equals("$any")) {
                toBeDone.get((Object)((Object)featureType))[ccTileY * 16 + ccTileX] = null;
                continue;
            }
            HashMap<FeatureType, IsoObject> currentTiles = new HashMap<FeatureType, IsoObject>();
            currentTiles.put(FeatureType.TREE, square.getTree());
            ...
            switch (tile) {
                case "$subbiome": {
                    ...
                    Map<FeatureType, Map<FeatureType, List<IBiome>>> tmpBiomes = featureType != FeatureType.ORE && lookupBiome != null ? lookupBiome.subBiomes() : (featureType == FeatureType.ORE && lookupOre != null ? lookupOre.subBiomes() : null);
                    if (tmpBiomes == null || tmpBiomes.isEmpty()) continue block12;
                    for (FeatureType subType : FeatureType.values()) {
                        if (tmpBiomes.get((Object)subType) == null) continue;
                        subBiomes.put(subType, tmpBiomes.get((Object)subType).get((Object)featureType).get(rnd2.nextInt(0, tmpBiomes.size())));
                        continue block12;
                    }
                    continue block12;
                }
                case "$no_tree": {
                    square.DeleteTileObject((IsoObject)currentTiles.get((Object)FeatureType.TREE));
                    continue block12;
                }
                case "$no_bush": {
                    square.DeleteTileObject((IsoObject)currentTiles.get((Object)FeatureType.BUSH));
                    continue block12;
                }
                case "$no_grass": {
                    square.DeleteTileObject((IsoObject)currentTiles.get((Object)FeatureType.PLANT));
                    continue block12;
                }
                default: {
                    square.DeleteTileObject((IsoObject)currentTiles.get((Object)FeatureType.TREE));
    ...
    private TileReplacementRetValue applyBiome(IBiome biome, FeatureType type, IsoCell cell, ChunksCache chunks, IsoChunk ch, IsoGridSquare square, int x, int y, int z, int tileX, int tileY, int tileZ, EnumMap<FeatureType, String[]> done, Random rnd) {
    ...
    private void applyPrefab(PrefabStructure prefab, IBiome biome, IsoCell cell, ChunksCache chunks, IsoChunk ch, IsoGridSquare square, int x, int y, int z, int tileX, int tileY, int tileZ, EnumMap<FeatureType, String[]> done, int xmin, int xmax, int ymin, int ymax, Random rnd) {
