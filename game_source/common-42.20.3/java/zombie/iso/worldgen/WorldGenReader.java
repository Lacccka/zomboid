...
public class WorldGenReader {
    public Map<String, Biome> loadBiomes(KahluaTable worldgenTable, String tableKey) {
        ...
        biomes.forEach((k, v) -> {
            ...
            Map<FeatureType, Map<FeatureType, List<IBiome>>> subBiomes = ((Biome)biomeList.get(0)).subBiomes();
            ...
            for (Biome biome2 : biomeList) {
                ...
                if (biome2.subBiomes() != null && !biome2.subBiomes().isEmpty()) {
                    subBiomes = biome2.subBiomes();
                }
            ...
            biomes.put((String)k, new Biome((String)k, v.parent(), v.generate(), (Map<FeatureType, List<Feature>>)features, (Map<String, List<Feature>>)replacements, landscape, plant, bush, temperature, hygrometry, oreLevel, zombies, placements, subBiomes, protectedBiomes, grass));
        });
        return biomes;
    }
    ...
    public Biome loadBiome(String biomeName, KahluaTable biomeTable) {
        ...
        return new Biome(biomeName, parent, generate, features, replacements, landscapes, plants, bushes, temperatures, hygrometries, oreLevels, zombies.floatValue(), placements, subBiomes, protectedBiomes, grass);
    }
            ...
            while (iterTypeTable.advance()) {
                ...
                if (featuresObj == null || probaObj == null) {
                    try {
                        ...
                        throw new RuntimeException(String.format("%s{%s:%s/%s} => Features not found or probability absents | %s | %s", name, typeName, curKey, keyIndex, featuresObj, probaObj));
                    }
    ...
    public Map<FeatureType, Map<FeatureType, List<IBiome>>> loadSubBiomes(KahluaTable subBiomesTable) {
        if (subBiomesTable == null) {
            return new HashMap<FeatureType, Map<FeatureType, List<IBiome>>>();
        }
        KahluaTableIterator iter = subBiomesTable.iterator();
        HashMap<FeatureType, Map<FeatureType, List<IBiome>>> subBiomes = new HashMap<FeatureType, Map<FeatureType, List<IBiome>>>();
        while (iter.advance()) {
            String typeName = (String)iter.getKey();
            ...
            while (subTypeIter.advance()) {
                ...
                ArrayList<Biome> innerBiomes = new ArrayList<Biome>();
