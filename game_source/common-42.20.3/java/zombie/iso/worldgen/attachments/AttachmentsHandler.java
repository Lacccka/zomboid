...
public class AttachmentsHandler {
    ...
    private void attach(IsoCell cell, int x, int y, WorldGenChunk wgChunk, boolean doPriority) {
        ...
        if (materialsFound.size() > 1) {
            for (Direction dir : Direction.diagonals()) {
                ...
                this.applyAttachment(dir, wgChunk, doPriority, true, rnd, floor, floorMaterial, (String)materials.get((Object)dir));
            }
        }
        for (Direction dir : Direction.cardinals()) {
            ...
            validDirs.remove((Object)dir);
            this.applyAttachment(dir.opposite(), wgChunk, doPriority, false, rnd, floorRemote, (String)materials.get((Object)dir), floorMaterial);
        }
    }
    ...
    private void applyAttachment(Direction dir, WorldGenChunk wgChunk, boolean doPriority, boolean priorityOrder, Random rnd, IsoObject floor, String material, String materialRemote) {
        boolean priorityValid;
        boolean bl = priorityValid = priorityOrder ? wgChunk.priority(material, materialRemote) : wgChunk.priority(materialRemote, material);
