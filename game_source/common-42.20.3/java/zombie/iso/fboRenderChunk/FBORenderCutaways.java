...
public final class FBORenderCutaways {
    ...
    private final ObjectPool<PointOfInterest> pointOfInterestStore = new ObjectPool<PointOfInterest>(PointOfInterest::new, "FBORenderCutaways.pointOfInterestStore");
        ...
        for (int i = 0; i < onScreenChunks.size(); ++i) {
            ...
            if (chunkLevelData.adjacentChunkLoadedCounter != chunk.adjacentChunkLoadedCounter) {
                ...
                for (z1 = chunk.getMinLevel(); z1 <= chunk.getMaxLevel(); ++z1) {
                    if (!renderLevels.isOnScreen(z1) || !renderLevels.isDirty(z1, 192L, zoom)) continue;
                    chunk.getCutawayData().recreateLevel(z1);
                    bForceCutawayUpdate = true;
                }
            }
    ...
    private boolean IsCollapsibleBuildingSquare(CutawayWall wall, IsoGridSquare square) {
        ...
        if (IsoCamera.frameState.camCharacterSquare != null) {
            ...
            for (int i = 0; i < size; ++i) {
                BuildingDef buildingDef = btc.buildingsToCollapse.get(i);
                if (!wall.isPartOfBuilding(buildingDef)) continue;
    ...
    static {
        s_cutawayWallPool = new ObjectPool<CutawayWall>(CutawayWall::new, "FBORenderCutaways.s_cutawayWallPool", 8192);
        s_slopedSurfacePool = new ObjectPool<SlopedSurface>(SlopedSurface::new, "FBORenderCutaways.s_slopedSurfacePool", 1024);
    }
