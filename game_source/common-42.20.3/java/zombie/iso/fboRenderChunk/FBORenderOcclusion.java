...
public final class FBORenderOcclusion {
        ...
        for (int level2 = 30; level2 >= -32; --level2) {
            ...
            for (int wy2 = minY; wy2 <= maxY; ++wy2) {
                for (int wx2 = minX; wx2 <= maxX; ++wx2) {
                    ...
                    if (!this.polygonsOverlap(this.floatArrayList1, this.floatArrayList2)) continue;
                    ints.add(wx2);
                    ints.add(wy2);
                }
            }
    ...
    float XToIso(float screenX, float screenY, float level) {
        float tx = (screenX + 2.0f * screenY) / (64.0f * (float)Core.tileScale);
        return tx += 3.0f * level;
    }

    float YToIso(float screenX, float screenY, float level) {
        float ty = (screenX - 2.0f * screenY) / (-64.0f * (float)Core.tileScale);
    ...
    public void invalidateOverlappedChunkLevels(int playerIndex, IsoChunk chunk, int level) {
        ...
        for (int level2 = level - 1; level2 >= chunkMap.minHeight; --level2) {
            ...
            for (int i = 0; i < occludingChunkLevels1.length; i += 2) {
                ...
                if (chunk2 == null || level2 < chunk2.getMinLevel() || level2 > chunk2.getMaxLevel()) continue;
    ...
    boolean polygonsOverlap(TFloatArrayList polygon1, TFloatArrayList polygon2) {
        ...
        for (int i = 0; i < numPolygons; ++i) {
            ...
            short numPoints = this.clipperBuffer.getShort();
    ...
    public boolean isOccluded(int x, int y, int z) {
        ...
        if (!this.isOccludedAux(x - 1, y - 1, z)) {
            return false;
        }
        if (!this.isOccludedAux(x - 2, y - 2, z)) {
            return false;
        }
        if (!this.isOccludedAux(x - 3, y - 3, z)) {
            return false;
        }
        if (!this.isOccludedAux(x - 1, y - 0, z)) {
            return false;
        }
        if (!this.isOccludedAux(x - 0, y - 1, z)) {
            return false;
        }
        if (!this.isOccludedAux(x - 2, y - 1, z)) {
            return false;
        }
        if (!this.isOccludedAux(x - 1, y - 2, z)) {
            return false;
        }
        if (!this.isOccludedAux(x - 3, y - 2, z)) {
            return false;
        }
        return this.isOccludedAux(x - 2, y - 3, z);
    }
