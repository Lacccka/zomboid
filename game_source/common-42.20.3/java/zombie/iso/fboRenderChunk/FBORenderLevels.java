...
public final class FBORenderLevels {
    ...
    void calculateRenderBounds(int minLevel, int maxLevel) {
        ...
        if (minLevel == 0) {
            ...
            y1 += (float)FBORenderChunk.FLOOR_HEIGHT;
            float dh1 = y - (y1 -= (float)FBORenderChunk.JUMBO_XXL_HEIGHT);
            if (dh1 > 0.0f) {
                height += dh1;
                y -= dh1;
            }
    ...
    public boolean calculateInStencilRect(int level, IsoCell cell) {
        ...
        for (IsoCell.StencilArea stencilArea : cell.getStencilAreas()) {
            ...
            float px = (float)stencilArea.x() * zoom - (float)stencilArea.offX();
            float py = (float)stencilArea.y() * zoom - (float)stencilArea.offY();
            ...
            if (!(this.renderX + this.renderW > stencilX1) || !(this.renderX < stencilX2) || !(this.renderY + this.renderH > stencilY1) || !(this.renderY < stencilY2)) continue;
            return true;
        }
        return false;
    }
    ...
    public List<IsoGridSquare> getCachedSquares_CutawayWindowFrames(int level) {
        return this.getNLevels((int)level).cutawayWindowFrames;
    }
    ...
    public static int extraHeightForJumboTrees(int minLevel, int maxLevel) {
        ...
        if (maxLevel == 1) {
            return FBORenderChunk.JUMBO_L_HEIGHT - FBORenderChunk.PIXELS_PER_LEVEL * 2 - FBORenderChunk.FLOOR_HEIGHT / 2;
        }
        return FBORenderChunk.JUMBO_L_HEIGHT - FBORenderChunk.PIXELS_PER_LEVEL - FBORenderChunk.FLOOR_HEIGHT / 2;
    }
    ...
    private static final class NLevels {
        ...
        final List<IsoGridSquare> animatedAttachments = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> corpseSquares = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> cutawayWindowFrames = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> fliesSquares = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> itemSquares = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> puddleSquares = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> translucentFloor = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> translucentNonFloor = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> waterSquares = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> waterShoreSquares = new ArrayList<IsoGridSquare>();
        final List<IsoGridSquare> waterAttachSquares = new ArrayList<IsoGridSquare>();
