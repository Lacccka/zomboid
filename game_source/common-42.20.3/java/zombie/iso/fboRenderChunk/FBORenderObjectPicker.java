...
public final class FBORenderObjectPicker {
    ...
    private final ObjectPool<IsoObjectPicker.ClickObject> clickObjectPool = new ObjectPool<IsoObjectPicker.ClickObject>(IsoObjectPicker.ClickObject::new, "FBORenderObjectPicker.clickObjectPool");
    ...
    static int renderLayerIndex(IsoObjectPicker.ClickObject o) {
        return switch (o.tile.getRenderInfo((int)0).layer) {
            ...
            case ObjectRenderLayer.MinusFloorSE, ObjectRenderLayer.TranslucentSE -> 5;
        };
    }
    ...
    void getObjectsAt(int screenX, int screenY, ArrayList<IsoObject> objects) {
        ...
        if (player.getZi() < 0) {
            for (int z = -32; z < 0; ++z) {
                ...
                boolean bRightOfSquare = worldX % 1.0f > (worldY = IsoUtils.YToIso(0, screenX, screenY, z)) % 1.0f;
    ...
    public IsoObject PickDoor(int screenX, int screenY, boolean bTransparent) {
        return this.getFirst(screenX, screenY, (obj, x, y) -> {
            ...
            if (obj.contains(x, y, IsoPlayer.players[0].getZi()) && obj.tile.isMaskClicked(lx = PZMath.fastfloor(x - (float)obj.x), ly = PZMath.fastfloor(y - (float)obj.y), obj.flip)) {
                return 1;
            }
            return 0;
        });
    ...
    public IsoObject PickHoppable(int screenX, int screenY) {
        return this.getFirst(screenX, screenY, (obj, x, y) -> {
            ...
            if (obj.contains(x, y, IsoPlayer.players[0].getZi()) && obj.tile.isMaskClicked(lx = (int)(x - (float)obj.x), ly = (int)(y - (float)obj.y), obj.flip)) {
                return 1;
            }
            return 0;
        });
