...
public final class FBORenderObjectHighlight {
    ...
    private boolean isRenderedToChunkTexture(IsoObject object) {
        ...
        if (object instanceof IsoMannequin) {
            ...
            return !mannequin.shouldRenderEachFrame();
        }
        return false;
    }
    ...
    private boolean isRenderedEveryFrame(int playerIndex, IsoObject object) {
        ...
        if (renderInfo.layer == ObjectRenderLayer.Translucent || renderInfo.layer == ObjectRenderLayer.TranslucentFloor || renderInfo.layer == ObjectRenderLayer.TranslucentSE) {
            return true;
        }
    ...
    public void render(int playerIndex) {
        ...
        for (int i = 0; i < this.objectList.size(); ++i) {
            ...
            if (this.isRenderedToChunkTexture(object)) continue;
            ...
            if (!object.isHighlighted(playerIndex)) continue;
            byte playerFlags = this.objectHighlightRendered.getOrDefault(object, (byte)0);
            if (object.isHighlightRenderOnce(playerIndex) && (playerFlags & 1 << playerIndex) == 0) continue;
    ...
    public void clearHighlightOnceFlag() {
        ...
        for (int i = 0; i < this.objectList.size(); ++i) {
            byte playerFlags;
            IsoObject object = this.objectList.get(i);
            if (!object.isHighlightRenderOnce() || (playerFlags = this.objectHighlightRendered.getOrDefault(object, (byte)0).byteValue()) != 0) continue;
    ...
    private void renderObject(int playerIndex, IsoObject object) {
        ...
        for (int i = object.getObjectIndex(); i < objectCount; ++i) {
            ...
            if (this.isRenderedToChunkTexture(object1) || this.isRenderedEveryFrame(playerIndex, object1) || object != object1 && this.objectSet.contains(object1) || this.objectSet2.contains(object1) || object1 instanceof IsoWorldInventoryObject && DebugOptions.instance.fboRenderChunk.itemsInChunkTexture.getValue()) continue;
            renderInfo = object1.getRenderInfo(playerIndex);
            if (renderInfo.layer == ObjectRenderLayer.None || renderInfo.targetAlpha <= 0.0f) continue;
    ...
    public boolean shouldRenderObjectHighlight(IsoObject object) {
        ...
        if (object.isHighlighted(playerIndex)) {
            if (PerformanceSettings.fboRenderChunk) {
                return this.isRendering() || this.isRenderedEveryFrame(playerIndex, object);
            }
            return true;
        }
        return false;
