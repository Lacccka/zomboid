...
import zombie.iso.fboRenderChunk.DepthStagger;
...
extends TextureDraw.GenericDrawer {
    static final ObjectPool<FBORenderTrees> s_pool = new ObjectPool<FBORenderTrees>(FBORenderTrees::new, "FBORenderTrees.s_pool");
    static final ObjectPool<Tree> s_treePool = new ObjectPool<Tree>(Tree::new, "FBORenderTrees.s_treePool", 2048);
    ...
    static final DepthStagger depthStagger;
    static final TIntIntHashMap depthDone;
    ...
    private void renderTreeTextures(Tree tree, boolean bUseShader) {
        ...
        if (tree.texture != null) {
            ...
            y2 = PZMath.min((float)tree.texture.getHeightOrig() - dyFloor, tree.texture.getOffsetY() + (float)tree.texture.getHeight());
    ...
    private float calculateDepth(Tree tree, boolean bTexture2, boolean bUseShader, boolean bFloorHack) {
        ...
        targetDepth += (float)tree.depthOffset * 2.0E-4f;
        if (bFloorHack) {
            targetDepth -= 0.0015f;
        }
        return targetDepth;
    ...
    static {
        ...
        depthStagger = new DepthStagger();
        depthDone = new TIntIntHashMap();
