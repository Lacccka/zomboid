...
public final class FBORenderChunk {
    ...
    public void renderInWorldMainThread() {
        ...
        if (SceneShaderStore.chunkRenderShader != null) {
            ...
            IsoDepthHelper.Results result = IsoDepthHelper.getChunkDepthData(PZMath.fastfloor(IsoCamera.frameState.camCharacterX / 8.0f), PZMath.fastfloor(IsoCamera.frameState.camCharacterY / 8.0f), this.chunk.wx, this.chunk.wy, this.getMinLevel());
            ...
            if (!DebugOptions.instance.fboRenderChunk.combinedFbo.getValue()) {
                ...
                float depthStart = ((float)(result.indexX + result.indexY) - dx - dy) / 8.0f / 40.0f;
                depthStart *= 0.46187335f;
                texd.chunkDepth = depthStart -= (float)FBORenderLevels.calculateMinLevel(this.getMinLevel()) * 0.0028867084f;
            }
        }
        ...
        y -= (float)(PIXELS_PER_LEVEL * (this.getTopLevel() - this.getMinLevel() + 1));
        y -= (float)FBORenderLevels.extraHeightForJumboTrees(this.getMinLevel(), this.getTopLevel());
        x -= IsoCamera.getOffX();
        y -= IsoCamera.getOffY();
        if (!DebugOptions.instance.fboRenderChunk.combinedFbo.getValue()) {
