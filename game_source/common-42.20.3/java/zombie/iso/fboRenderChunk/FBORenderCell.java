...
public final class FBORenderCell {
    ...
    private final ObjectPool<IsoGameCharacter.Location> locationPool = new ObjectPool<IsoGameCharacter.Location>(IsoGameCharacter.Location::new, "FBORenderCell.locationPool");
    ...
    public void renderInternal() {
        ...
        for (int i = 0; i < IsoPlayer.numPlayers; ++i) {
            ...
            if (player2 == null || (isoBuilding = player2.getCurrentBuilding()) != null) continue;
            IsoDirections playerDir = IsoDirections.fromAngle(player2.getForwardDirection());
            IsoBuilding isoBuilding2 = this.cell.GetPeekedInBuilding(player2.getCurrentSquare(), playerDir);
            if (isoBuilding2 == null) continue;
        ...
        try (GameProfiler.ProfileArea profileArea = GameProfiler.getInstance().profile("renderLast");){
            for (IsoMovingObject isoMovingObject : this.cell.getObjectList()) {
                isoMovingObject.renderlast();
            }
            for (int i = 0; i < this.cell.getStaticUpdaterObjectList().size(); ++i) {
                IsoObject isoObject = this.cell.getStaticUpdaterObjectList().get(i);
    ...
    private boolean isSquareObscuringPlayer(int playerIndex, IsoGridSquare square) {
        ...
        for (int i = 0; i < square.getObjects().size(); ++i) {
            Texture texture;
            IsoObject object = square.getObjects().get(i);
            if (!this.isPotentiallyObscuringObject(object) || (texture = object.sprite.getTextureForCurrentFrame(object.getForwardIsoDirection(), object)) == null || !perPlayerData1.isObjectObscuringPlayer(square, texture, object.offsetX, object.offsetY + object.getRenderYOffset() * (float)Core.tileScale)) continue;
            return true;
        }
        return false;
    }
    ...
    private boolean checkTreeTranslucency(int playerIndex, FBORenderLevels renderLevels) {
        ...
        for (int i = 0; i < squares.size(); ++i) {
            ...
            if (square.chunk == null || (tree = square.getTree()) == null || this.isTreeRenderedEveryFrame(tree)) continue;
    ...
    private void renderOneLevel(IsoChunk c, int level, IsoCell.PerPlayerRender perPlayerRender, int playerIndex, long currentTimeMillis, Shader floorRenderShader, Shader wallRenderShader) {
        ...
        if (!isDirty || !canRender) {
            ...
            if (renderLevels.getCachedSquares_Items(level).size() + renderLevels.getCachedSquares_TranslucentNonFloor(level).size() + renderLevels.getCachedSquares_CutawayWindowFrames(level).size() > 0) {
    ...
    private void calculateObjectRenderInfo(int playerIndex, IsoGridSquare square, IsoObject object) {
        ...
        if ((renderInfo.layer == ObjectRenderLayer.Translucent || renderInfo.layer == ObjectRenderLayer.TranslucentSE) && square.getLightLevel(playerIndex) == 0.0f) {
    ...
    private ObjectRenderLayer calculateObjectRenderLayer(IsoObject object) {
        ...
        if (this.isObjectRenderLayer_MinusFloorSE(object)) {
            if (object.getSprite() != null && (object.getSprite().depthFlags & 2) != 0) {
                return ObjectRenderLayer.TranslucentSE;
            }
    ...
    public boolean isTranslucentTree(IsoObject object) {
        ...
        IsoGridSquare square = object.square;
        square.IsOnScreen();
        if (!(isAiming || square.x >= PZMath.fastfloor(IsoCamera.frameState.camCharacterX) && square.y >= PZMath.fastfloor(IsoCamera.frameState.camCharacterY) && IsoCamera.frameState.camCharacterSquare != null)) {
            return false;
        }
...
lbl15:
    ...
    private void renderMinusFloor_DoorOrWall(IsoObject object) {
        ...
        if (renderInfo.cutawayOutline && !(renderLevels = square.getChunk().getRenderLevels(playerIndex)).getCachedSquares_CutawayWindowFrames(square.getZ()).contains(square)) {
    ...
    private void renderTranslucent(IsoGridSquare square, ObjectRenderLayer renderLayer) {
        ...
        for (int i = 0; i < numObjects; ++i) {
            IsoObject object = objects[i];
            ObjectRenderInfo renderInfo = object.getRenderInfo(playerIndex);
            if (renderInfo.layer != renderLayer) continue;
    ...
    private void renderCutawayOutline(IsoGridSquare square, boolean bAttachedSE) {
        ...
        for (int i = 0; i < numObjects; ++i) {
            ...
            ObjectRenderInfo renderInfo = object.getRenderInfo(playerIndex);
            if (!renderInfo.cutawayOutline) continue;
            this.renderTranslucent(object);
        }
    }
    ...
    private void renderAnimatedAttachments(IsoGridSquare square) {
        ...
        for (int i = 0; i < numObjects; ++i) {
            IsoObject object = objects[i];
            ObjectRenderLayer renderLayer = object.getRenderInfo((int)playerIndex).layer;
            if (renderLayer == ObjectRenderLayer.None || renderLayer == ObjectRenderLayer.Translucent || renderLayer == ObjectRenderLayer.TranslucentSE || !object.hasAnimatedAttachments()) continue;
            this.renderAnimatedAttachments(object);
        }
        this.renderAnimatedAttachments = false;
    }
    ...
    private void renderOneLevel_Translucent(int playerIndex, IsoChunk c, int level) {
        ...
        try (GameProfiler.ProfileArea profileArea = GameProfiler.getInstance().profile("Sort");){
            ...
            for (i = 0; i < squaresCutawayOutlines.size(); ++i) {
                square = squaresCutawayOutlines.get(i);
                if (sorted2.contains(square)) continue;
                sorted2.add(square);
            }
            for (i = 0; i < squaresObjects.size(); ++i) {
                square = squaresObjects.get(i);
                if (sorted2.contains(square)) continue;
                sorted2.add(square);
            }
        }
        ...
        for (int z = 0; z <= maxZ; ++z) {
            ...
            for (i = 0; i < perPlayerData1.onScreenChunks.size(); ++i) {
                ...
                for (j = 0; j < n; ++j) {
                    ...
                    if (!square.IsOnScreen() || square.getZ() != z || (floor = square.getFloor()) != null && floor.getRenderInfo((int)playerIndex).layer == ObjectRenderLayer.TranslucentFloor) continue;
            ...
            for (i = 0; i < this.waterAttachSquares.size(); ++i) {
                ...
                for (j = 0; j < numObjects; ++j) {
                    ...
                    this.renderTranslucentOnly = true;
                    object.renderAttachedAndOverlaySprites(object.getForwardIsoDirection(), square.x, square.y, square.z, square.getLightInfo(playerIndex), true, false, null, null);
                    this.renderTranslucentOnly = false;
                }
            }
    ...
    private void renderWaterFlow(List<IsoGridSquare> waterSquares) {
        for (int i = 0; i < waterSquares.size(); ++i) {
            ...
            float flow = (float)Math.PI - (water.getFlow() - 0.7853982f);
            float speed = water.getSpeed();
            LineDrawer.DrawIsoLine((float)square.x + 0.5f, (float)square.y + 0.5f, square.z, (float)square.x + 0.5f + (float)Math.cos(flow) * speed, (float)square.y + 0.5f + (float)Math.sin(flow) * speed, square.z, 1.0f, 1.0f, 1.0f, 1.0f, 1);
        }
    }
    ...
    private void renderWaterShore(int playerIndex) {
        ...
        for (int z = 0; z <= maxZ; ++z) {
            ...
            for (i = 0; i < perPlayerData1.onScreenChunks.size(); ++i) {
                ...
                for (j = 0; j < n; ++j) {
                    IsoGridSquare square = squares.get(j);
                    if (square.getZ() != z || !square.IsOnScreen() || square.getWater() == null || !square.getWater().isbShore()) continue;
                    this.waterSquares.add(square);
                    this.waterAttachSquares.add(square);
                }
            }
    ...
    public void renderSeamFix1_Floor(IsoObject object, float x, float y, float z, ColorInfo stCol, Consumer<TextureDraw> texdModifier) {
        if (PerformanceSettings.fboRenderChunk && DebugOptions.instance.fboRenderChunk.seamFix1.getValue()) {
            ...
            if (PZMath.coordmodulo(square.y, 8) == 7 && (s = square.getAdjacentSquare(IsoDirections.S)) != null && s.getFloor() != null) {
                sprite.render(object, x, y, z, object.getForwardIsoDirection(), object.offsetX + 5.0f, object.offsetY + object.getRenderYOffset() * (float)Core.tileScale - 5.0f, stCol, !object.isBlink(), texdModifier);
            }
    ...
    public void renderSeamFix2_Floor(IsoObject object, float x, float y, float z, ColorInfo stCol, Consumer<TextureDraw> texdModifier) {
        ...
        if (!(PZMath.coordmodulo(square.y, 8) != 7 && !bShoreS || (s = square.getAdjacentSquare(IsoDirections.S)) == null || s.getFloor() == null || !bShoreS && s.has(IsoFlagType.water) && PerformanceSettings.waterQuality != 2)) {
            ...
            sprite.render(object, x, y, z, object.getForwardIsoDirection(), object.offsetX + 6.0f, object.offsetY + object.getRenderYOffset() * (float)Core.tileScale - 3.0f, stCol, !object.isBlink(), texdModifier);
            object.sx = 0.0f;
        ...
        if (bShoreN) {
            ...
            object.renderDepthAdjust = -0.001f;
            sprite.render(object, x, y - 1.0f, z, object.getForwardIsoDirection(), object.offsetX, object.offsetY + object.getRenderYOffset() * (float)Core.tileScale, stCol, !object.isBlink(), texdModifier);
            object.sx = 0.0f;
        ...
        if (bShoreW) {
            ...
            object.renderDepthAdjust = -0.001f;
            sprite.render(object, x - 1.0f, y, z, object.getForwardIsoDirection(), object.offsetX - 2.0f, object.offsetY - 1.0f + object.getRenderYOffset() * (float)Core.tileScale, stCol, !object.isBlink(), texdModifier);
            object.sx = 0.0f;
            object.renderSquareOverride2 = null;
            object.renderDepthAdjust = 0.0f;
            IsoSprite.seamFix2 = null;
    ...
    public void renderSeamFix1_Wall(IsoObject object, float x, float y, float z, ColorInfo stCol, Consumer<TextureDraw> texdModifier) {
        if (PerformanceSettings.fboRenderChunk && DebugOptions.instance.fboRenderChunk.seamFix1.getValue()) {
            ...
            if (sprite.getProperties().has(IsoFlagType.WallW) && PZMath.coordmodulo(square.y, 8) == 7 && (s = square.getAdjacentSquare(IsoDirections.S)) != null && ((s.getWallType() & 4) != 0 || s.getWindowFrame(false) != null || s.has(IsoFlagType.DoorWallW))) {
                sprite.renderWallSliceW(object, x, y, z, object.getForwardIsoDirection(), object.offsetX, object.offsetY + object.getRenderYOffset() * (float)Core.tileScale, stCol, !object.isBlink(), texdModifier);
            }
            if (sprite.getProperties().has(IsoFlagType.WallN) && PZMath.coordmodulo(square.x, 8) == 7 && (e = square.getAdjacentSquare(IsoDirections.E)) != null && ((e.getWallType() & 1) != 0 || e.getWindowFrame(true) != null || e.has(IsoFlagType.DoorWallN))) {
                sprite.renderWallSliceN(object, x, y, z, object.getForwardIsoDirection(), object.offsetX, object.offsetY + object.getRenderYOffset() * (float)Core.tileScale, stCol, !object.isBlink(), texdModifier);
            }
        }
    }
    ...
    private void callBeforeWorldRender(int playerIndex) {
        ...
        if (JoypadManager.instance.getFromPlayer(playerIndex) == null) {
            ...
            if (xJoypadObj == null) {
                buildingObject.rawset("xJoypad", (Object)BoxedStaticValues.toDouble(buildX));
                buildingObject.rawset("yJoypad", (Object)BoxedStaticValues.toDouble(buildY));
            }
            buildingObject.rawset("zJoypad", (Object)BoxedStaticValues.toDouble(buildZ));
            buildX = ((KahluaTableImpl)buildingObject).rawgetInt("xJoypad");
            buildY = ((KahluaTableImpl)buildingObject).rawgetInt("yJoypad");
        }
        Object[] args2 = this.L_callBeforeWorldRender;
        args2[0] = buildingObject;
        args2[1] = buildX;
        args2[2] = buildY;
        args2[3] = buildZ;
        LuaManager.caller.pcallvoid(LuaManager.thread, functionObj, args2);
    }
    ...
    static final class FadingRoom {
        ...
        private static final ObjectPool<FadingRoom> pool = new ObjectPool<FadingRoom>(FadingRoom::new, "FadingRoom.pool");
