...
public final class WaterFlowLookup {
    ...
    private WaterFlowObject nearest(WaterFlowObject node, WaterFlowObject point, WaterFlowObject closest, boolean compareX) {
        ...
        float f = distFromPartition = compareX ? (float)(point.x - node.x) : (float)(point.y - node.y);
        if (distFromPartition < 0.0f) {
            if ((closest = this.nearest(node.kdLeft, point, closest, !compareX)).distSq(point) >= distFromPartition * distFromPartition) {
                closest = this.nearest(node.kdRight, point, closest, !compareX);
            }
        } else if ((closest = this.nearest(node.kdRight, point, closest, !compareX)).distSq(point) >= distFromPartition * distFromPartition) {
    ...
    private void nearestMultiple(WaterFlowObject current, WaterFlowObject query, int max, boolean checkX, PriorityQueue<WaterFlowObject> queue) {
        ...
        double planeDiff = checkX ? (double)(query.x - current.x) : (double)(query.y - current.y);
