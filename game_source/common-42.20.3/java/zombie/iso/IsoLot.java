...
public class IsoLot {
    ...
    public static final ObjectPool<IsoLot> pool = new ObjectPool<IsoLot>(IsoLot::new, "IsoLot.pool");
    public int maxLevel;
    public int minLevel;
    ...
    public static synchronized IsoLot get(MapFiles mapFiles, int cX, int cY, int wX, int wY, IsoChunk ch) {
        return IsoLot.get(mapFiles, (Integer)cX, (Integer)cY, (Integer)wX, (Integer)wY, ch);
    }
    ...
    public void load(MapFiles mapFiles, Integer cX, Integer cY, Integer wX, Integer wY, IsoChunk ch) {
        Object filenameheader = ChunkMapFilenames.instance.getHeader(cX, cY);
        ...
        try {
            ...
            if (this.in == null || !this.lastUsedPath.equals(fo.getAbsolutePath())) {
                ...
                if (bHasMagic) {
                    this.version = IsoLot.readInt(this.in);
            ...
            this.in.seek((long)((this.version >= 1 ? 8 : 0) + 4) + (long)index * 8L);
            int pos = IsoLot.readInt(this.in);
            this.in.seek(pos);
            ...
            for (int z = minZ; z <= maxZ; ++z) {
                for (int x = 0; x < 8; ++x) {
                    for (int y = 0; y < 8; ++y) {
                        ...
                        int count = IsoLot.readInt(this.in);
                        if (count == -1 && (skip = IsoLot.readInt(this.in)) > 0) {
                            --skip;
                            continue;
                        }
                        if (count <= 1) continue;
                        ...
                        int room = IsoLot.readInt(this.in);
                        for (int n = 1; n < count; ++n) {
                            int d = IsoLot.readInt(this.in);
                            this.data.add(d);
                        }
                    }
                }
