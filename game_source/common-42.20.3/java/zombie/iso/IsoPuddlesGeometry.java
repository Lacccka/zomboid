...
public final class IsoPuddlesGeometry {
    ...
    public static final ObjectPool<IsoPuddlesGeometry> pool = new ObjectPool<IsoPuddlesGeometry>(IsoPuddlesGeometry::new, "IsoPuddlesGeometry.pool", 24576);
    ...
    public IsoPuddlesGeometry init(IsoGridSquare square) {
        ...
        boolean bl = bShore = IsoWater.getInstance().getShaderEnable() && square.getWater() != null && square.getWater().isbShore();
        if (!(!PerformanceSettings.fboRenderChunk || PerformanceSettings.puddlesQuality != 2 || bShore || floor != null && floor.getProperties() != null && floor.getProperties().has(IsoFlagType.transparentFloor))) {
            ...
            this.x[0] = this.x[0] - 1.0f;
            this.x[1] = this.x[1] - 1.0f;
            this.x[2] = this.x[2] - 1.0f;
            this.x[3] = this.x[3] - 1.0f;
        }
        this.square = square;
    ...
    public boolean shouldRender() {
        ...
        this.recalcIfNeeded();
        for (i = 0; i < 4; ++i) {
            if (!(this.pdne[i] + this.pdnw[i] + this.pda[i] + this.pnon[i] > 0.0f)) continue;
            return true;
        }
        ...
        if (IsoPuddles.leakingPuddlesInTheRoom && !this.interiorCalc && this.square != null) {
            ...
            if (gsW == null || gsN == null || gsS == null || gsE == null || gsNW == null || gsNE == null || gsSW == null || gsSE == null || !gsW.getProperties().has(IsoFlagType.exterior) && !gsN.getProperties().has(IsoFlagType.exterior) && !gsS.getProperties().has(IsoFlagType.exterior) && !gsE.getProperties().has(IsoFlagType.exterior)) {
                return false;
            }
            ...
            for (i2 = 0; i2 < 4; ++i2) {
                if (!(this.pdne[i2] + this.pdnw[i2] + this.pda[i2] + this.pnon[i2] > 0.0f)) continue;
                return true;
            }
        }
        return false;
