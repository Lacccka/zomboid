...
public final class LightingJNI {
    ...
    private static void checkLights() {
        ...
        for (i = 0; i < lights.size(); ++i) {
            lightSource = (IsoLightSource)lights.get(i);
        ...
        for (i = 0; i < JNILights.size(); ++i) {
            lightSource = JNILights.get(i);
        ...
        for (int i2 = 0; i2 < roomLights.size(); ++i2) {
            IsoRoomLight roomLight = roomLights.get(i2);
        ...
        if (GameClient.client) {
            IsoPlayer player;
            IsoGameCharacter id3 = IsoCamera.getCameraCharacter();
            boolean playerCanSeeInvisiblePlayers = id3 instanceof IsoPlayer && (player = (IsoPlayer)id3).getRole() != null && player.getRole().hasCapability(Capability.SeesInvisiblePlayers);
            ArrayList<IsoPlayer> players = GameClient.instance.getPlayers();
            for (int i3 = 0; i3 < players.size(); ++i3) {
                IsoPlayer p = players.get(i3);
                if (p.isInvisible() && !p.isLocalPlayer() && !playerCanSeeInvisiblePlayers) continue;
        ...
        for (BaseVehicle vehicle : IsoWorld.instance.currentCell.getVehicles()) {
            ...
            for (int j = 0; j < vehicle.getLightCount(); ++j) {
                ...
                LightingJNI.checkTorch(part, 4096 + vehicle.vehicleId * 10 + j);
            }
        }
        for (int i4 = 0; i4 < torches.size(); ++i4) {
            IsoGameCharacter.TorchInfo torchInfo = torches.get(i4);
    ...
    public static void updatePlayer(int playerIndex) {
        ...
        LightingJNI.playerSet(player.getX(), player.getY(), player.getZ() + 32.0f, lookVector.x, lookVector.y, false, player.reanimatedCorpse != null, player.isGhostMode(), player.hasTrait(CharacterTrait.SHORT_SIGHTED), effectiveFatigue, perceptionDistance, cone);
    }
                        ...
                        for (int x = 0; x < 8; ++x) {
                            ...
                            if (sq != null) {
                                ...
                                try {
                                    ...
                                    LightingJNI.squareSet(visionUnblocked, sq.testVisionAdjacent(0, 0, 1, true, false) != LosUtil.TestResults.Blocked, sq.testVisionAdjacent(0, 0, -1, true, false) != LosUtil.TestResults.Blocked, hasElevatedFloor, visionMatrix, buildingDef == null ? -1L : buildingDef.getID(), sq.getRoom() == null ? -1L : sq.getRoomID(), sq.lightLevel, isOpenAir);
                                    ...
                                    for (int i = 0; i < sq.getSpecialObjects().size(); ++i) {
                                        ...
                                        if (object instanceof IsoDoor) {
                                            ...
                                            trans = door.isOpen() ? true : trans && (door.HasCurtains() == null || door.isCurtainOpen());
                                            ...
                                            LightingJNI.squareAddDoor(door.north, door.isOpen(), trans);
                                            ...
                                            if (door.isOpen() || door.HasCurtains() == null || door.isCurtainOpen()) continue;
                                            float curtainR = 0.0f;
                                            float curtainG = 0.0f;
                                            float curtainB = 0.0f;
                                            curtainIntensity = 0.33f;
    ...
    public static final class VisibleRoom {
        ...
        public int cellY;
        public long metaId;
        private static final ObjectPool<VisibleRoom> pool = new ObjectPool<VisibleRoom>(VisibleRoom::new, "VisibleRoom.pool");

        private VisibleRoom set(int cellX, int cellY, long metaID) {
            this.cellX = cellX;
            this.cellY = cellY;
            this.metaId = metaID;
            return this;
                ...
                if (player != null) {
                    ...
                    if ((this.vis & 2) == 0 && (this.vis & 4) != 0) {
                        ...
                        if (player.getForwardIsoDirection() != null && Math.abs(dx) <= 2 && Math.abs(dy) <= 2) {
                            int[] fv = ForcedVis[player.getForwardIsoDirection().ordinal()];
