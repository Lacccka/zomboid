...
extends BaseAmbientStreamManager {
    ...
    private final FMOD_STUDIO_EVENT_CALLBACK electricityShutOffEventCallback = new FMOD_STUDIO_EVENT_CALLBACK(this){
        final /* synthetic */ AmbientStreamManager this$0;
        {
            AmbientStreamManager ambientStreamManager = this$0;
            Objects.requireNonNull(ambientStreamManager);
            this.this$0 = ambientStreamManager;
        }
    ...
    public void update() {
        ...
        for (int n2 = 0; n2 < this.worldEmitters.size(); ++n2) {
            IsoGridSquare sq;
            WorldSoundEmitter worldSoundEmitter = this.worldEmitters.get(n2);
            if (worldSoundEmitter.daytime != null) {
                sq = IsoWorld.instance.currentCell.getGridSquare(worldSoundEmitter.x, worldSoundEmitter.y, worldSoundEmitter.z);
                if (sq == null) {
            ...
            if (player != null && ((IsoGameCharacter)player).hasTrait(CharacterTrait.DEAF)) {
                worldSoundEmitter.fmodEmitter.stopAll();
                SoundManager.instance.unregisterEmitter(worldSoundEmitter.fmodEmitter);
                this.worldEmitters.remove(worldSoundEmitter);
                this.freeEmitters.add(worldSoundEmitter);
        ...
        for (n = 0; n < this.nightAmbient.size(); ++n) {
            this.nightAmbient.get((int)n).targVol *= night;
        }
        for (n = 0; n < this.dayAmbient.size(); ++n) {
            this.dayAmbient.get((int)n).targVol *= 1.0f - night;
        }
        for (n = 0; n < this.indoorAmbient.size(); ++n) {
            this.indoorAmbient.get((int)n).targVol = this.indoorAmbient.get((int)n).targVol * (bIndoors ? 0.8f : 0.0f);
        }
        for (n = 0; n < this.outdoorAmbient.size(); ++n) {
            this.outdoorAmbient.get((int)n).targVol = this.outdoorAmbient.get((int)n).targVol * (bIndoors ? 0.15f : 0.8f);
        }
        for (n = 0; n < this.rainAmbient.size(); ++n) {
            this.rainAmbient.get((int)n).targVol = this.rainAmbient.get((int)n).targVol * (bRaining ? 1.0f : 0.0f);
    ...
    public void addRandomAmbient(boolean force) {
        boolean beforeWildlifeInCities;
        ...
        IsoPlayer player = (IsoPlayer)players.get(Rand.Next(players.size()));
        String sound = "";
        boolean bl = beforeWildlifeInCities = player.getSquare() != null && !player.getSquare().isRuralExtraFussy() && GameTime.instance.getWorldAgeDaysSinceBegin() < 29.0;
        if (GameTime.instance.getHour() > 7 && GameTime.instance.getHour() < 21) {
            switch (Rand.Next(3)) {
                case 0: {
                    if (!force && Rand.Next(10) >= 2) break;
                    sound = "MetaDogBark";
                    break;
                }
                case 1: {
                    if (!force && Rand.Next(10) >= 3) break;
                    sound = "MetaScream";
                }
            }
        } else {
            switch (Rand.Next(5)) {
                ...
                case 2: {
                    ...
                    if (!beforeWildlifeInCities) break;
        ...
        if (!GameClient.client) {
            ...
            Ambient a = new Ambient(sound, x, y, 50.0f, Rand.Next(0.2f, 0.5f));
            this.ambient.add(a);
        }
    }
    ...
    public void addBlend(String name, float vol, boolean bIndoors, boolean bRain, boolean bNight, boolean bDay) {
        AmbientLoop loop = new AmbientLoop(0.0f, name, vol);
    ...
    public void doGunEvent() {
        ...
        WorldSoundManager.instance.addSound(null, PZMath.fastfloor(x += this.tempo.x), PZMath.fastfloor(y += this.tempo.y), 0, 600, 600);
        float gain = 1.0f;
        Ambient a = new Ambient(weaponFire, x, y, 700.0f, 1.0f);
        this.ambient.add(a);
    }
    ...
    public void doAlarm(RoomDef room) {
        if (room != null && room.building != null && room.building.alarmed) {
            ...
            DebugType.Sound.debugln("nights survived = " + (float)(GameTime.getInstance().getWorldAgeHours() / 24.0 + (double)((SandboxOptions.instance.timeSinceApo.getValue() - 1) * 30)));
    ...
    public void handleThunderEvent(int x, int y) {
        ...
        if (dist <= 5000) {
            ...
            Ambient a = new Ambient("", x, y, 5100.0f, 1.0f);
            this.ambient.add(a);
        }
    }
    ...
    @Override
    public void stop() {
        for (AmbientLoop l : this.allAmbient) {
            l.stop();
        }
    ...
    public void addAmbient(String name, int x, int y, int radius, float volume) {
        ...
        Ambient a = new Ambient(name, x, y, radius, volume, true);
        this.ambient.add(a);
    }

    @Override
    public void addAmbientEmitter(float x, float y, int z, String name) {
        WorldSoundEmitter e = this.freeEmitters.isEmpty() ? new WorldSoundEmitter() : this.freeEmitters.pop();
        e.x = x;
        e.y = y;
        e.z = z;
        e.daytime = null;
    ...
    private void updatePowerSupply() {
        ...
        if (this.electricityShutOffState == 1 && !bPowerOn) {
            ...
            if (!this.electricityShutOffEmitter.isPlaying(this.electricityShutOffEvent)) {
                ...
                BuildingDef buildingDef = AmbientStreamManager.getNearestBuilding(pos.x, pos.y);
                if (buildingDef == null) {
        ...
        int n = this.electricityShutOffState = bPowerOn ? 1 : 0;
        if (this.electricityShutOffEmitter != null) {
        ...
        for (int cy = cellY - 1; cy <= cellY + 1; ++cy) {
            for (int cx = cellX - 1; cx <= cellX + 1; ++cx) {
                ...
                for (BuildingDef buildingDef : metaCell.buildings) {
                    float dx = Math.abs(px - (float)buildingDef.x);
                    dx = Math.min(dx, Math.abs(px - (float)buildingDef.x2));
                    float dy = Math.abs(py - (float)buildingDef.y);
                    float dist = org.joml.Math.sqrt(dx * dx + (dy = Math.min(dy, Math.abs(py - (float)buildingDef.y2))) * dy);
    ...
    public static final class Ambient {
        ...
        public void update() {
            ...
            if (this.trackMouse && IsoPlayer.getInstance() != null) {
                float x = Mouse.getXA();
                float y = Mouse.getYA();
                x -= (float)IsoCamera.getScreenLeft(IsoPlayer.getPlayerIndex());
                y -= (float)IsoCamera.getScreenTop(IsoPlayer.getPlayerIndex());
                int z = IsoPlayer.getInstance().getZi();
                this.emitter.x = PZMath.fastfloor(IsoUtils.XToIso(x *= Core.getInstance().getZoom(IsoPlayer.getPlayerIndex()), y *= Core.getInstance().getZoom(IsoPlayer.getPlayerIndex()), z));
