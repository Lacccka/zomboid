...
public final class MapCollisionData {
    ...
    public void init(IsoMetaGrid metaGrid) {
        ...
        MapCollisionData.n_setGameState("Core.bLastStand", Core.lastStand);
        this.noSave = Core.getInstance().isNoSave();
        MapCollisionData.n_setGameState("Core.noSave", this.noSave);
        ...
        MapCollisionData.n_setGameState("SavefileNaming.SUBDIR_CHUNKDATA", "chunkdata");
        MapCollisionData.n_setGameState("SavefileNaming.SUBDIR_ZPOP", "zpop");
        ...
        MapCollisionData.n_initMetaGrid(minX, minY, width, height);
        for (int cy = minY; cy < minY + height; ++cy) {
            for (int cx = minX; cx < minX + width; ++cx) {
                ...
                for (int i = 0; i < IsoLot.MapFiles.size(); ++i) {
                    ...
                    MapCollisionData.n_initMetaCell(cx, cy, mapFiles.infoFileNames.get("chunkdata_" + cx + "_" + cy + ".bin"));
                }
                ...
                for (int wy = 0; wy < 32; ++wy) {
                    for (int wx = 0; wx < 32; ++wx) {
                        ...
                        MapCollisionData.n_initMetaChunk(cx, cy, wx, wy, Math.max(intensity, 0));
                    }
                }
            }
        }
        MapCollisionData.n_init(minX, minY, width, height);
    }
    ...
    public void updateMain() {
        ...
        PathTask task = this.pathResultQueue.poll();
        while (task != null) {
            task.result.finished(task.status, task.curX, task.curY);
            task.release();
            task = this.pathResultQueue.poll();
        }
    ...
    public void updateGameState() {
        ...
        if (paused != this.paused) {
            this.paused = paused;
            MapCollisionData.n_setGameState("PAUSED", this.paused);
        }
    }
    ...
    public void addChunkToWorld(IsoChunk chunk) {
        ...
        MapCollisionData.n_chunkUpdateTask(chunk.wx, chunk.wy, this.squares);
    }
    ...
    private boolean isSolid(IsoGridSquare sq) {
        boolean solid;
        boolean bl = solid = sq.isSolid() || sq.isSolidTrans();
        if (sq.HasStairs()) {
            solid = true;
        }
    ...
    extends Thread {
        ...
        public final Queue<PathTask> pathTasks;
        final /* synthetic */ MapCollisionData this$0;

        private MCDThread(MapCollisionData mapCollisionData) {
            MapCollisionData mapCollisionData2 = mapCollisionData;
            Objects.requireNonNull(mapCollisionData2);
            this.this$0 = mapCollisionData2;
        ...
        private void runInner() {
            ...
            synchronized (object) {
                PathTask task = this.this$0.pathTaskQueue.poll();
                while (task != null) {
                    task.execute();
                    task.release();
                    task = this.this$0.pathTaskQueue.poll();
                }
                if (this.save) {
                    MapCollisionData.n_save();
                    ZombiePopulationManager.instance.save();
        ...
        private boolean shouldWait() {
            ...
            return this.this$0.pathTaskQueue.isEmpty() && this.pathTasks.isEmpty();
        }
    }
    ...
    private final class PathTask {
        ...
        private PathTask(MapCollisionData mapCollisionData) {
            MapCollisionData mapCollisionData2 = mapCollisionData;
            Objects.requireNonNull(mapCollisionData2);
            this.this$0 = mapCollisionData2;
        }

        public void init(int sx, int sy, int ex, int ey, IPathResult result) {
            this.startX = sx;
            this.startY = sy;
            this.endX = ex;
            this.endY = ey;
